...
class _MessageScreenState extends State<MessageScreen> with WidgetsBindingObserver {
  ...
  Future<void> _loadUserData() async {
    try {
      ...
      setState(() {
        _users = usersJson.map((json) => UserData.fromJson(json)).toList();
      });
    } catch (e) {
      print('加载用户数据失败: $e');
    }
  ...
  /// 获取用户最后一条消息
  Future<ChatMessageData?> _getLastMessage(int userId) async {
    try {
      ...
      if (await file.exists()) {
        ...
        if (jsonList.isNotEmpty) {
          ...
          return ChatMessageData.fromJson(lastMessageJson);
        }
      }
    } catch (e) {
  ...
  /// 获取消息预览文本
  String _getMessagePreview(ChatMessageData message) {
    switch (message.type) {
      case 'text':
        return message.text ?? '';
      case 'image':
