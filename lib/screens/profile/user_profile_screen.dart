...
import '../message/video_call_screen.dart';
...
class _UserProfileScreenState extends State<UserProfileScreen> {
  ...
  Future<void> _checkUserStatus() async {
    final isBlocked = await UserManagementService.isUserBlocked(widget.user.id.toString());
    final isReported = await UserManagementService.isUserReported(widget.user.id.toString());
  ...
  Future<void> _confirmBlockUser() async {
    try {
      final success = await UserManagementService.blockUser(widget.user.id.toString());
      if (success && mounted) {
        setState(() {
          _isBlocked = true;
        });
  ...
  Future<void> _unblockUser() async {
    try {
      final success = await UserManagementService.unblockUser(widget.user.id.toString());
      if (success && mounted) {
        setState(() {
          _isBlocked = false;
        });
  ...
  Future<void> _confirmReportUser() async {
    try {
      final success = await UserManagementService.reportUser(widget.user.id.toString());
      if (success && mounted) {
        setState(() {
          _isReported = true;
        });
      ...
      child: Row(
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: Image.asset(
              'assets/images/foka_user_video_call.webp',
              width: 147,
              height: 43,
              fit: BoxFit.contain,
            ),
  ...
  Widget _buildPostTitle() {
    return Container(
      child: Image.asset(
        'assets/images/foka_user_post.webp',
        width: 100,
        height: 30,
        fit: BoxFit.contain,
      ),
