...
class _ConversationPageState extends State<ConversationPage> {
  List<_ConversationEntity> _messages = [];
  ...
  Future<void> _loadMessages() async {
    ...
    if (await file.exists()) {
      try {
        ...
        setState(() {
          _messages = jsonList.map((e) => _ConversationEntity.fromJson(e)).toList();
        });
  ...
  void _sendMessage() {
    ...
    setState(() {
      _messages.add(_ConversationEntity(
        text: text,
        isMe: true,
        time: _getTime(),
        type: _ConversationEntityType.text,
      ));
    });
    _controller.clear();
    _scrollToBottom();
    ...
    try {
      ...
      setState(() {
        _messages.add(_ConversationEntity(
          ...
          isMe: true,
          time: _getTime(),
          type: _ConversationEntityType.image,
        ));
      });
      _scrollToBottom();
      _saveMessages();
...
enum _ConversationEntityType { text, image }
...
class _ConversationEntity {
  ...
  final bool isMe;
  final String time;
  final _ConversationEntityType type;
  ...
  static _ConversationEntity fromJson(Map<String, dynamic> json) => _ConversationEntity(
    text: json['text'],
    imagePath: json['imagePath'],
    isMe: json['isMe'] ?? true,
    time: json['time'] ?? '',
    type: _ConversationEntityType.values.firstWhere(
      (e) => e.name == json['type'],
      orElse: () => _ConversationEntityType.text,
    ),
  );
}

class _ChatBubble extends StatefulWidget {
  final _ConversationEntity message;
...
class _ChatBubbleState extends State<_ChatBubble> {
  ...
  Widget build(BuildContext context) {
    ...
    if (msg.type == _ConversationEntityType.image && msg.imagePath != null) {
      return Align(
