...
/*: "%@ reply %@：%@" :*/
fileprivate let data_ovalFormat:[Character] = ["%","@"," ","r","e","p","l","y"]
fileprivate let dataUntilId:String = " %@：%@"
...
struct ThereReplyModel: HandyJSON {
    //: var cid: String?
...
extension ThereReplyModel {
    ...
    mutating func of() {
        ...
        if replyUser.count > 0 {
            ...
            tempContent = String(format: (String(data_ovalFormat) + dataUntilId.capitalized), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(EchoReactiveCompatible.commonTo(text: tempContent, size: CGSize(width: dataShowDetailPopMessage - 82, height: CGFloat(MAXFLOAT)), font: UIFont.opPoint(type: .Regular, fontSize: 14), lineNumber: 0).height)
