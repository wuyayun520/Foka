...
/*: "reply" :*/
fileprivate let k_performShapeKey:String = "replcoordinator"
...
struct TakeMeasurable: HandyJSON {
    //: var cid: String?
...
extension TakeMeasurable {
    //: mutating func caculateCommentItemHeight() {
    mutating func flatSolid() {
        //: var tempContent = ""
        var tempContent = ""
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if LieLanguageManager.shared.direction == .leftToRight {
                ...
                tempContent = (k_performShapeKey.replacingOccurrences(of: "coordinator", with: "y")).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (k_performShapeKey.replacingOccurrences(of: "coordinator", with: "y")).localized
            }
        ...
        let sizeheight = Int(EchoReactiveCompatible.commonTo(text: tempContent, size: CGSize(width: dataShowDetailPopMessage - 70, height: CGFloat(MAXFLOAT)), font: UIFont.opPoint(type: .Regular, fontSize: 16), lineNumber: 0).height)
