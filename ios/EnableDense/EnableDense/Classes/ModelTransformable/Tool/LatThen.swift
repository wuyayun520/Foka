...
/*: "/TXCache" :*/
fileprivate let show_postContent:String = "/"
fileprivate let userScrambleValue:String = "TXCachemedium read"
...
protocol PersonateThen: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func plusEnable(player: LatThen, status: HangPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func example(player: LatThen, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func rate(player: LatThen, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class LatThen: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: PersonateThen?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: HangPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = LatThen()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (show_postContent.capitalized + String(userScrambleValue.prefix(7)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(run), userInfo: nil, repeats: true)
...
extension LatThen {
    ...
    func clearView(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: dataAddressText.dramaticComposition(), with: notiNetworkTitle)
    ...
    @objc func run() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.rate(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func quote(renderMode: GroundPlusRenderMode) {
    ...
    func top(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isPage == true {
        if HemSocketManager.shared.isPage == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func solar() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.plusEnable(player: self, status: self.playStatus!)
        }
    }
...
extension LatThen: TXVodPlayListener {
