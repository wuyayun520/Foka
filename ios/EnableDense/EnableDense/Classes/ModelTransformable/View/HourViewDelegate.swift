...
fileprivate let appAgainData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class HourViewDelegate: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appAgainData.reversed(), encoding: .utf8)!)
    }

    //: func customUI() {
    func on() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(calendar))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(selectCity))
...
extension HourViewDelegate: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func calendar(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func selectCity(tap: UIGestureRecognizer) {
    ...
    func flag() {
        ...
        scrollView.setZoomScale(scrollView.minimumZoomScale, animated: false)
        //: resizeSubviews()
        quantityeractionSourceSystem()
    }

    //: func resizeSubviews() {
    func quantityeractionSourceSystem() {
    ...
    func oldIn(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.underStack(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func ex(image: UIImage) {
