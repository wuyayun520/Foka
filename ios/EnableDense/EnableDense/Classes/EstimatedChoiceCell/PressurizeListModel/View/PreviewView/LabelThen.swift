...
fileprivate let constWindowId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_male_default" :*/
fileprivate let noti_localData:[UInt8] = [0xd8,0xd2,0xde,0xdd,0xce,0xdc,0xd0,0xdb,0xd4,0xce,0xd3,0xd4,0xd5,0xd0,0xe4,0xdb,0xe3]

fileprivate func canopicJarLetter(thanks num: UInt8) -> UInt8 {
...
/*: "icon_female_default" :*/
fileprivate let constDestinationFormat:[Character] = ["i","c","o"]
fileprivate let appHelpVisualId:String = "jar comment scale neverthelessn_fem"
fileprivate let userStackTitle:String = "fmanlt"

/*: "level_%@" :*/
fileprivate let noti_toneUrl:String = "identity"
fileprivate let app_seaValue:String = "vel_%@between compute"
...
class LabelThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constWindowId.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .opPoint(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .opPoint(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension LabelThen {
    ...
    func compute(model: SpendHandyJSON) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.allSquare())
        ...
        let imgStr = model.sex == WalkMilitaryOutputStreamable.male.rawValue ? String(bytes: noti_localData.map{canopicJarLetter(thanks: $0)}, encoding: .utf8)! : (String(constDestinationFormat) + String(appHelpVisualId.suffix(5)) + "ale_de" + userStackTitle.replacingOccurrences(of: "man", with: "au"))
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (noti_toneUrl.replacingOccurrences(of: "identity", with: "le") + String(app_seaValue.prefix(6))), model.level)
