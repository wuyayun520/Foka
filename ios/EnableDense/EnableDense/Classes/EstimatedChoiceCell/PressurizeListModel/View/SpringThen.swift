...
fileprivate let const_builderLabValue:[UInt8] = [0xc6,0xc1,0xc6,0xdb,0x87,0xcc,0xc0,0xcb,0xca,0xdd,0x95,0x86,0x8f,0xc7,0xce,0xdc,0x8f,0xc1,0xc0,0xdb,0x8f,0xcd,0xca,0xca,0xc1,0x8f,0xc6,0xc2,0xdf,0xc3,0xca,0xc2,0xca,0xc1,0xdb,0xca,0xcb]

private func logicalReward(ion num: UInt8) -> UInt8 {
    return num ^ 175
}

/*: "#333333" :*/
fileprivate let kPlatformMsg:String = "#visitorvisitorvisitorvisitorvisitorvisitor"

/*: "My photos" :*/
fileprivate let showHerFormat:[Character] = ["M","y"," ","p","h","o","t","o","s"]

/*: "#999999" :*/
fileprivate let user_showData:[Character] = ["#","9","9","9","9","9","9"]

/*: "Click to change or delete photo." :*/
fileprivate let app_preparationKey:[UInt8] = [0xc4,0xeb,0xee,0xe4,0xec,0xa7,0xf3,0xe8,0xa7,0xe4,0xef,0xe6,0xe9,0xe0,0xe2,0xa7,0xe8,0xf5,0xa7,0xe3,0xe2,0xeb,0xe2,0xf3,0xe2,0xa7,0xf7,0xef,0xe8,0xf3,0xe8,0xa9]

private func missingGesture(bound num: UInt8) -> UInt8 {
    return num ^ 135
}

/*: "btn_me_edit_photo_add_nor" :*/
fileprivate let constResourceStr:String = "BTN"
fileprivate let userHurryStr:String = "_me_edpick addition compute"
fileprivate let appBackMessage:String = "oto_adtract across ad thumb"

/*: "btn_me_edit_photo_add_pre" :*/
fileprivate let appForkPath:[Character] = ["b","t","n","_","m","e","_","e","d","i","t","_","p","h","o","t","o","_","a","d","d","_","p"]
fileprivate let main_tunMsg:String = "nothing"

/*: "btn_me_edit_photo_delete" :*/
fileprivate let user_allegedStr:String = "narrow environment withbtn_"
fileprivate let app_focusClaimAddData:String = "port ball art make interactiont_pho"
fileprivate let constFemaleValue:String = "official professional together modeto_de"
...
public protocol BulletReactiveCompatible: NSObject {
    /// 图片add
...
class SpringThen: UITableViewCell {
    //: let disposeBag = DisposeBag()
    let disposeBag = DisposeBag()
    //: var count = 0
    var count = 0
    //: open weak var delegate: EditPhotoDelegate?
    open weak var delegate: BulletReactiveCompatible?
...
//        showMessageView()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_builderLabValue.map{logicalReward(ion: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleBLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (kPlatformMsg.replacingOccurrences(of: "visitor", with: "3")))
        //: lb.text = "My photos".localized
        lb.text = (String(showHerFormat)).localized
    ...
    lazy var messageLB: UILabel = {
        ...
        lb.font = UIFont.opPoint(type: .Regular, fontSize: 12)
        //: lb.textColor = UIColor.init(hex: "#999999")
        lb.textColor = UIColor(hex: (String(user_showData)))
        //: lb.text = "Click to change or delete photo.".localized
        lb.text = String(bytes: app_preparationKey.map{missingGesture(bound: $0)}, encoding: .utf8)!.localized
        //: return lb
        return lb
        //: }()
    }()
...
extension SpringThen {
    ...
    func countryView() {
        ...
        for i in 0 ..< 9 {
            ...
            colorBtn.setBackgroundImage(.hr(name: (constResourceStr.lowercased() + String(userHurryStr.prefix(6)) + "it_ph" + String(appBackMessage.prefix(6)) + "d_nor")), for: .normal)
            //: colorBtn.setBackgroundImage(.BundleImageNamed(name: "btn_me_edit_photo_add_pre"), for: .highlighted)
            colorBtn.setBackgroundImage(.hr(name: (String(appForkPath) + main_tunMsg.replacingOccurrences(of: "nothing", with: "re"))), for: .highlighted)
            ...
            colorBtn.rx.tap.subscribe(onNext: { [weak self] in
                ...
                self.requireSender(sender: colorBtn)
            ...
            deleteBtn.setBackgroundImage(.hr(name: (String(user_allegedStr.suffix(4)) + "me_edi" + String(app_focusClaimAddData.suffix(5)) + String(constFemaleValue.suffix(5)) + "lete")), for: .normal)
    ...
    @objc func requireSender(sender: UIButton) {
        //: var isReturn = false
        var isReturn = false
        ...
        LabelMpThen.rangeDoing(true) { (isOpen: Bool) in
            ...
            if isOpen {
                ...
                let vc = SpendBassPickTool.systemVideo(maxCount: 9 - self.count, allowPhoto: true, allowVideo: false)
    ...
    @objc func result(sender: UIButton) {
        ...
        self.delegate?.manage(superBtn.imageView?.image ?? UIImage())

        //: self.delegate?.deletePhotoWithUid(String(superBtn.tag))
        self.delegate?.topUid(String(superBtn.tag))
    ...
    func toolSufficient(selete: [ProgramTransformable]) {
        ...
        for i in 0 ..< selete.count {
            ...
            if i < allPhoto.count {
                ...
                if selete[i].isLocal {
                    ...
                    Btn.focusing(urlStr: selete[i].url)
                }
                //: _ = Btn.subviews.map { view in
                _ = Btn.subviews.map { view in
                    //: if (view is UIButton) {
