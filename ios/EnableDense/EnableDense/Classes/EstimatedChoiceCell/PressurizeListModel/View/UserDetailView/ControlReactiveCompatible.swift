...
fileprivate let constMutualUrl:[UInt8] = [0xa3,0xa4,0xa3,0xbe,0xe2,0xa9,0xa5,0xae,0xaf,0xb8,0xf0,0xe3,0xea,0xa2,0xab,0xb9,0xea,0xa4,0xa5,0xbe,0xea,0xa8,0xaf,0xaf,0xa4,0xea,0xa3,0xa7,0xba,0xa6,0xaf,0xa7,0xaf,0xa4,0xbe,0xaf,0xae]

/*: "More" :*/
fileprivate let noti_honeyRecentKey:[Character] = ["M","o","r","e"]

/*: "icon_details_arrow" :*/
fileprivate let main_compriseKey:[Character] = ["i","c","o","n","_","d","e","t","a","i","l","s","_","a","r","r","o","w"]

/*: "Post" :*/
fileprivate let main_permissionUrl:String = "blind outlinePost"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let data_houseAboveTitle:String = "thanks lifebtn_"
fileprivate let user_interestedValue:String = "develop learnmic_"
fileprivate let kSharpName:String = "stop_norconcept char sense"
...
// __CLOSE_PRINT__
//
//  ControlReactiveCompatible.swift
//  AbroadTalking
//
...
class ControlReactiveCompatible: UITableViewCell {
    //: var userId: String = ""
    var userId: String = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constMutualUrl.map{$0^202}, encoding: .utf8)!)
    }
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .detergentSaveer(0, 0, 0, 0.4)
    ...
    lazy var moreButton: MagnitudeercalateTalkingButton = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .opPoint(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(noti_honeyRecentKey)).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (String(main_compriseKey))), for: .normal)
...
extension ControlReactiveCompatible {
    //: private func bindInteraction() {
    private func originalToMagnitudeeraction() {}
}
...
extension ControlReactiveCompatible {
    ...
    func betweenRecent(userModel: ParrotTransformable) {
        ...
        strA = [(String(main_permissionUrl.suffix(4))).localized, str]
        //: colorA = [UIColor.appTitleColor(), UIColor.appValueColor()]
        colorA = [UIColor.quitTo(), UIColor.tillAdvancedColor()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.platformDisk(fontSize: 18), UIFont.platformDisk(fontSize: 16)]
        ...
        attributedString = attributedString.eliteTip(texts: strA, colors: colorA, fonts: fontA)
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: HemTransformable = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.allSquare())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    repositionMakeVanguard(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.allSquare())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    repositionMakeVanguard(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.allSquare())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    repositionMakeVanguard(btn: thirdImgView)
                }
            }
        }
    }
    ...
    @objc func no() {
        ...
        let vc = WalkOverViewController(uid: userId)
    ...
    func repositionMakeVanguard(btn: UIImageView) {
        //: btn.subviews.map {
        btn.subviews.map {
            //: (view) -> () in
            view in
        ...
        let imag = UIImageView(image: UIImage(named: (String(data_houseAboveTitle.suffix(4)) + "dyna" + String(user_interestedValue.suffix(4)) + String(kSharpName.prefix(8)))))
...
extension ControlReactiveCompatible {
    ...
    private func viaConstraint() {
        ...
        let height = (dataShowDetailPopMessage - actualWidth(w: 15 * 4)) / 3.0
