...
fileprivate let k_clearlyTabId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Video Cover" :*/
fileprivate let mainDuringUrl:[Character] = ["V","i","d","e","o"," ","C","o","v","e","r"]

/*: "Adding a video cover to show the charm can get more good feelings." :*/
fileprivate let constDecreaseMsg:[UInt8] = [0xb7,0x92,0x92,0x9f,0x98,0x91,0xd6,0x97,0xd6,0x80,0x9f,0x92,0x93,0x99,0xd6,0x95,0x99,0x80,0x93,0x84,0xd6,0x82,0x99,0xd6,0x85,0x9e,0x99,0x81,0xd6,0x82,0x9e,0x93,0xd6,0x95,0x9e,0x97,0x84,0x9b,0xd6,0x95,0x97,0x98,0xd6,0x91,0x93,0x82,0xd6,0x9b,0x99,0x84,0x93,0xd6,0x91,0x99,0x99,0x92,0xd6,0x90,0x93,0x93,0x9a,0x9f,0x98,0x91,0x85,0xd8]

private func computerPopular(spot num: UInt8) -> UInt8 {
    return num ^ 246
}

/*: "img_me_edit_video_default" :*/
fileprivate let mainAgendaPath:String = "woman whichimg_me"
fileprivate let constModelEffectFormat:String = "_videofunction demonstrate alter"
fileprivate let user_heMsg:String = "_defframework below food dramatic"

/*: "btn_me_edit_photo_change" :*/
fileprivate let userTingFormat:[Character] = ["b","t","n","_","m","e","_","e","d","i","t","_","p","h","o","t","o","_","c","h"]
fileprivate let notiAtUrl:String = "aforke"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let user_putKey:String = "outlet three wantbtn_"
fileprivate let show_accessId:String = "c_stopbox birthday immediate include"
fileprivate let mainForthName:String = "_nornow equal or layer"

/*: "#FF506D" :*/
fileprivate let constMixPath:[Character] = ["#","F","F","5","0","6","D"]

/*: "Reviewing" :*/
fileprivate let show_randomTitle:String = "ban simply now step installationRevie"
fileprivate let app_quantityFormat:[Character] = ["w","i","n","g"]

/*: "icon_edit_Sample_videocover" :*/
fileprivate let app_psychologicalMsg:String = "icobelow"
fileprivate let constNetworkUnderName:[Character] = ["a","m","p","l","e","_","v","i","d"]
fileprivate let constWeContent:[Character] = ["e","o","c","o","v","e","r"]

/*: "Sample" :*/
fileprivate let show_wantStr:[Character] = ["S","a","m","p","l","e"]

/*: "edit_Sample_cover" :*/
fileprivate let showUsId:String = "edit_Sampreminder resource"
fileprivate let app_attributeContent:String = "positive absle_cover"

/*: "Shoot video at least 5 seconds" :*/
fileprivate let constMouthThatFormat:[Character] = ["S","h","o","o","t"," ","v","i","d","e","o"," ","a","t"]
fileprivate let showCoalContent:[Character] = [" ","l","e","a","s","t"," ","5"," ","s"]
fileprivate let appQuickItemPath:[Character] = ["e","c","o","n","d","s"]
...
class BodyThen: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_clearlyTabId.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var tipsLB: UILabel = {
        ...
        lb.text = (String(mainDuringUrl)).localized
        ...
        if SteppingMotorThen.share.loginUserMode.sex == WalkMilitaryOutputStreamable.female.rawValue {
            ...
            let attributes = [.foregroundColor: UIColor.red, .font: UIFont.opPoint(type: .Semibold, fontSize: 18)] as [NSAttributedString.Key: Any]
    ...
    lazy var messageLB: UILabel = {
        ...
        lb.text = String(bytes: constDecreaseMsg.map{computerPopular(spot: $0)}, encoding: .utf8)!.localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.setImage(UIImage.hr(name: (String(mainAgendaPath.suffix(6)) + "_edit" + String(constModelEffectFormat.prefix(6)) + String(user_heMsg.prefix(4)) + "ault")), for: .normal)
        //: btn.imageView?.contentMode = .scaleAspectFill
        btn.imageView?.contentMode = .scaleAspectFill
        //: btn.addTarget(self, action: #selector(iconbtnclick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(model), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var changeBtn: UIButton = {
        ...
        btn.setImage(UIImage.hr(name: (String(userTingFormat) + notiAtUrl.replacingOccurrences(of: "fork", with: "ng"))), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(changeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(tableAvailable), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var playBtn: UIImageView = {
        ...
        icon.image = UIImage.hr(name: (String(user_putKey.suffix(4)) + "dynami" + String(show_accessId.prefix(6)) + String(mainForthName.prefix(4))))
        //: icon.isHidden = true
        icon.isHidden = true
    ...
    private lazy var statusLB: UILabel = {
        ...
        lb.backgroundColor = UIColor(hex: (String(constMixPath)))
        //: lb.text = "Reviewing".localized
        lb.text = (String(show_randomTitle.suffix(5)) + String(app_quantityFormat)).localized
    ...
    lazy var sampleBtn: UIButton = {
        ...
        btn.setImage(UIImage.hr(name: (app_psychologicalMsg.replacingOccurrences(of: "below", with: "n") + "_edit_S" + String(constNetworkUnderName) + String(constWeContent))), for: .normal)
        //: btn.addTarget(self, action: #selector(sampleBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(homeClick), for: .touchUpInside)
        ...
        lb.text = (String(show_wantStr)).localized
        //: view.addSubview(lb)
        view.addSubview(lb)
...
extension BodyThen {
    ...
    func field(videoPlayPath: String) {
    ...
    func moment(iconPath: String) {
        //: iconBtn.setUrlImage(urlStr: iconPath)
        iconBtn.focusing(urlStr: iconPath)
    }

    //: func setVideoStatusLB(status: Int) {
    func stageTar(status: Int) {
...
extension BodyThen {
    ...
    @objc private func homeClick() {
        //: let filePath = SVGAEffectTool.default.getVideoPath(name: "edit_Sample_cover")
        let filePath = EpisodeThen.default.nameOner(name: (String(showUsId.prefix(9)) + String(app_attributeContent.suffix(8))))
        //: let vc = TalkingMomentVideoVC.init(videoPath: filePath)
        let vc = DialogCoordinatorViewController(videoPath: filePath)
    ...
    @objc private func tableAvailable() {
        //: seleteVideoTool()
        quitTool()
    }
    ...
    @objc private func model() {
        ...
        if self.videoStatus == -3, self.videoPlayPath.isEmptyString {
            ...
            let vc = DialogCoordinatorViewController(videoPath: self.videoPlayPath)
            //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
            self.strokeController()?.navigationController?.pushViewController(vc, animated: true)
        }
    }
    ...
    private func quitTool() {
        ...
        LabelMpThen.rangeDoing(true) { (isOpen: Bool) in
            ...
            if isOpen {
                ...
                let vc = SpendBassPickTool.systemVideo(maxCount: 1, allowPhoto: false, allowVideo: true)
                ...
                vc.didFinishPickingVideoHandle = { [weak self] (_: UIImage?,
                        ...
                        SpendBassPickTool.low(asset: assets) { filePath in
                            ...
                            if seconds >= 5 {
                                ...
                                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) { // 视频压缩取消
                                    ...
                                    self.revenueMsg(showMsg: (String(constMouthThatFormat) + String(showCoalContent) + String(appQuickItemPath)).localized)
                                }
                            }
                        }
                }
    ...
    private func still(url: URL) {
        //: let videoVC = TalkingVideoEditorController(maxDuration: 30.0, videoURL: url) { [weak self] videoInfo in
        let videoVC = PocketViewController(maxDuration: 30.0, videoURL: url) { [weak self] videoInfo in
            //: guard let self = self else {return}
            guard let self = self else { return }
        ...
        self.strokeController()?.navigationController?.pushViewController(videoVC, animated: true)
    }

    //: private func deleteFile() {
    private func skinFor() {
