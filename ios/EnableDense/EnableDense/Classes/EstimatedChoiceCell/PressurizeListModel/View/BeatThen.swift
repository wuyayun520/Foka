...
fileprivate let app_buttUrl:[UInt8] = [0x1a,0x1f,0x1a,0x25,0xd9,0x14,0x20,0x15,0x16,0x23,0xeb,0xda,0xd1,0x19,0x12,0x24,0xd1,0x1f,0x20,0x25,0xd1,0x13,0x16,0x16,0x1f,0xd1,0x1a,0x1e,0x21,0x1d,0x16,0x1e,0x16,0x1f,0x25,0x16,0x15]

fileprivate func clubElementListener(plus num: UInt8) -> UInt8 {
...
/*: "Birthday" :*/
fileprivate let notiCapitalFormat:String = "Birthdaycalculate dynamics"

/*: "btn_me_edit" :*/
fileprivate let data_retainText:[Character] = ["b","t","n","_","m","e","_","e"]
fileprivate let k_inmateReplyPath:[Character] = ["d","i","t"]
...
class BeatThen: UITableViewCell, OvateObjectProtocol {
    //: var myDateView: TalkingDateView!
    var myDateView: FairnessDataSource!
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_buttUrl.map{clubElementListener(plus: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLB: UILabel = {
        ...
        lb.text = (String(notiCapitalFormat.prefix(8))).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var editImag: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.hr(name: (String(data_retainText) + String(k_inmateReplyPath))))
        //: return imag
        return imag
        //: }()
    }()
    ...
    lazy var dataBtn: UIButton = {
        ...
        //: dataBtn.setTitle(SteppingMotorThen.share.loginUserMode.birthday, for: .normal)
        dataBtn.setTitle(SteppingMotorThen.share.loginUserMode.birthday, for: .normal)
        //: dataBtn.setTitleColor(UIColor.appTitleColor(), for: .normal)
        dataBtn.setTitleColor(UIColor.quitTo(), for: .normal)
        //: dataBtn.titleLabel?.font = UIFont.pingfangFont(type: .Medium, fontSize: 16)
        dataBtn.titleLabel?.font = UIFont.opPoint(type: .Medium, fontSize: 16)
        //: dataBtn.addTarget(self, action: #selector(dataBtnClick), for: .touchUpInside)
        dataBtn.addTarget(self, action: #selector(modify), for: .touchUpInside)
        //: return dataBtn
        return dataBtn
        //: }()
    }()
...
extension BeatThen {
    ...
    @objc func modify() {
        //: LatPushManager.share.func__getCurrentActivityVC()?.view.endEditing(true)
        LatPushManager.share.topical()?.view.endEditing(true)
        //: myDateView = TalkingDateView.init()
        myDateView = FairnessDataSource()
        ...
        if parts?.count ?? 0 >= 3 {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
                ...
                self.myDateView.fresh(year: year, month: month, day: day)
            }
        }
    }
