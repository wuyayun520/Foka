...
fileprivate let notiLipMessage:[UInt8] = [0x79,0x72,0x74,0x65,0x72,0x20,0x64,0x6e,0x61,0x20,0x74,0x69,0x78,0x65,0x20,0x65,0x73,0x61,0x65,0x6c,0x70,0x20,0x2c,0x64,0x65,0x67,0x6e,0x61,0x68,0x63,0x20,0x73,0x75,0x74,0x61,0x74,0x53]

/*: "GET" :*/
fileprivate let showAwayText:[Character] = ["G","E","T"]
...
protocol AnalysisReactiveCompatible: NSObject {
    /// 播放完毕的回调
    //: func playerStatusChange(status: AudioPlayerStatus)
    func spectrum(status: SpecialValueConvertible)
}

//: class TalkingAudioPlayTool: NSObject {
class AbbreviationThen: NSObject {
    //: var player: AVAudioPlayer?
    var player: AVAudioPlayer?
    //: open weak var delegate: TalkingAudioPlayToolDelegate?
    open weak var delegate: AnalysisReactiveCompatible?

    // singleton
    //: static let shared = TalkingAudioPlayTool()
    static let shared = AbbreviationThen()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(home(notification:)), name: AVAudioSession.interruptionNotification, object: nil)
    }

    //: func play(urlStr: String) {
    func recover(urlStr: String) {
        ...
        askCompletion(audioUrl: urlStr) { data in
            //: do {
            do {
                ...
                self.revenueMsg(showMsg: String(bytes: notiLipMessage.reversed(), encoding: .utf8)!.localized)
                //: self.delegate?.playerStatusChange(status: .PlayerFinish)
                self.delegate?.spectrum(status: .PlayerFinish)
...
extension AbbreviationThen {
    //: func downloadAudioWith(audioUrl: String, completion: @escaping AudioCompletionblock) {
    func askCompletion(audioUrl: String, completion: @escaping AudioCompletionblock) {
        //: var urlOk = true
        var urlOk = true
        ...
        request.httpMethod = (String(showAwayText))
...
extension AbbreviationThen: AVAudioPlayerDelegate {
    /// 播放过程中中断
    ...
    @objc func home(notification: NSNotification) {
        ...
        if changeReason?.intValue ?? 0 == AVAudioSession.InterruptionType.began.rawValue {
            ...
            self.delegate?.spectrum(status: .PlayerInterruption)
        }
    }
