...
class EchoRecognizerDelegate: BodyLatViewController {
    //: var type = LoginType.PhoneLogin
    var type = BulletTextConvertible.PhoneLogin // 修改类型
    //: var content = ""                // 手机号
    var content = "" // 手机号
    //: var isNewPwd = false
    ...
    private lazy var pwdView: CookiePwdView = {
        //: let v = TalkingSetNewPwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
        let v = CookiePwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
...
extension EchoRecognizerDelegate {
    ...
    private func should(_ pwd: String, _ vcode: String?) {
        //: ProgressHUD.show()
        ...
        CigaretteThen.agentDay(pwd: pwd, vcode: vcode) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
    ...
    private func m() {
        ...
        CigaretteThen.animal { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            DirectProgressHUD.thatDismiss()
            //: guard let self = self else { return }
            guard let self = self else { return }
