...
// __CLOSE_PRINT__
//
//  CommentAdvertisingManager.swift
//  AbroadTalking
//
...
class CommentAdvertisingManager: NSObject {

    //: var windownsPostion: CGPoint = CGPoint(x: ScreenWidth-4, y: ScreenHeight-TabBarViewHeight-66-38)   
    var windownsPostion: CGPoint = CGPoint(x: dataShowDetailPopMessage-4, y: dataViewPath-userAgentMsg-66-38)   // 窗口默认位置
    //: var advSuperView: UIView?                                                       
    var advSuperView: UIView?                                                       // 窗口容器
    // singleton
    //: static let shared = TalkingAdvertisingManager()
    static let shared = CommentAdvertisingManager()
...
extension CommentAdvertisingManager {
    /// 创建窗口最小化容器
    ...
    func recentView() {
        ...
        let windowView = OrganizerContainerView()
        //: let keywindow = DetermineJoinReactiveCompatible.getWindow()
        let keywindow = DetermineJoinReactiveCompatible.existent()
