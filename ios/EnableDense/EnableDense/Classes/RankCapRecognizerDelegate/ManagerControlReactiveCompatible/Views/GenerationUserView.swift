...
/*: "icon_randow_user_bg" :*/
fileprivate let const_tarName:String = "icon_spectrum deal reader behavior"
fileprivate let mainBubblePath:[Character] = ["u","s","e","r","_","b","g"]
...
class GenerationUserView: UIView {
    // MARK: - UI Components

    //: private let leftAvatarView = AvatarView()
    private let leftAvatarView = RedAvatarView()
    //: private let centerAvatarView = AvatarView()
    private let centerAvatarView = RedAvatarView()
    //: private let rightAvatarView = AvatarView()
    private let rightAvatarView = RedAvatarView()
    ...
    private func avatarEvent() {
        ...
        centerBgView.image = UIImage.hr(name: (String(const_tarName.prefix(5)) + "randow_" + String(mainBubblePath)))
    }

    //: private func updateAvatarDisplay() {
    private func mandible() {
...
extension GenerationUserView {
    // MARK: - Public Methods

    //: func configure(avatars: [String]) {
    func secureBroad(avatars: [String]) {
...
class RedAvatarView: UIView {
    ...
    private func cuttingAngleRes() {
        //: imageView.setUrlImage(urlStr: self.imageurl ?? "")
        imageView.underStack(urlStr: self.imageurl ?? "")
    }
}
