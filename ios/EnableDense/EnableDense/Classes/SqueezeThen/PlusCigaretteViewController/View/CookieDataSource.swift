...
fileprivate let k_connectId:[UInt8] = [0xf7,0xf0,0xf7,0xea,0xb6,0xfd,0xf1,0xfa,0xfb,0xec,0xa4,0xb7,0xbe,0xf6,0xff,0xed,0xbe,0xf0,0xf1,0xea,0xbe,0xfc,0xfb,0xfb,0xf0,0xbe,0xf7,0xf3,0xee,0xf2,0xfb,0xf3,0xfb,0xf0,0xea,0xfb,0xfa]

private func scienceLogical(choice num: UInt8) -> UInt8 {
    return num ^ 158
}

/*: "JXBannerCellID" :*/
fileprivate let constRelieveId:String = "prepare agentJXBann"
fileprivate let show_docMessage:String = "llIDselected secure"

/*: "#FFFFFF" :*/
fileprivate let appPackagePrettyData:String = "unknown birthday permission base#FFFFFF"

/*: "#BDB8B5" :*/
fileprivate let appHurryMessage:String = "strength reduce#BDB8B"
fileprivate let app_padMightMessage:String = "5"
...
protocol YachtingCapViewProtocol: NSObjectProtocol {
    // 加载更多数据
...
class CookieDataSource: UICollectionReusableView {
    //: weak var delegate: SocialPopularListFooterViewProtocol?
    weak var delegate: YachtingCapViewProtocol?
    //: var listData: [PremiumStarListModel]?
    var listData: [SpecialFoodHandyJSON]? // 巨星计划数据
    //: var adBannderListData: [SocialAdBannerModel]?
    var adBannderListData: [MpTransformable]? // 广告banner数据

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: k_connectId.map{scienceLogical(choice: $0)}, encoding: .utf8)!)
    }
...
extension CookieDataSource {
    /// 获取当前视图高度
    /// - Returns: 高度
    //: func getFooterViewHeight() -> CGFloat {
    func behindHeight() -> CGFloat {
    ...
    func notUniversal(data: [SpecialFoodHandyJSON]) {
        //: self.listData = data
        self.listData = data
    }
    ...
    func cuttingEdgeRemind(data: [MpTransformable]) {
...
extension CookieDataSource: JXBannerDataSource, JXBannerDelegate {
    //: func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        //: return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: "JXBannerCellID")
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(constRelieveId.suffix(6)) + "erCe" + String(show_docMessage.prefix(4))))
    }
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(appPackagePrettyData.suffix(7))))!
        //: pageControl.inactiveSize = CGSize(width: 6, height: 6)
        pageControl.inactiveSize = CGSize(width: 6, height: 6)
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(appHurryMessage.suffix(6)) + app_padMightMessage.capitalized))!
                         ...
                         didSelectItemAt index: Int)
    {
        ...
        delegate?.diagnosticTechnique(model: model)
    }
}
...
extension CookieDataSource {
    ...
    private func setObserveRestriction() {
        ...
        bannerView.snp.makeConstraints { make in
            ...
            make.size.equalTo(CGSize(width: dataShowDetailPopMessage - 30, height: 84))
        }
    }
}
