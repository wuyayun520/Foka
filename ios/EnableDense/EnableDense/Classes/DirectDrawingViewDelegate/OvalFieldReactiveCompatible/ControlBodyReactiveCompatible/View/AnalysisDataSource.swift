...
fileprivate let const_levelTheId:[UInt8] = [0x36,0x3b,0x36,0x41,0xf5,0x30,0x3c,0x31,0x32,0x3f,0x7,0xf6,0xed,0x35,0x2e,0x40,0xed,0x3b,0x3c,0x41,0xed,0x2f,0x32,0x32,0x3b,0xed,0x36,0x3a,0x3d,0x39,0x32,0x3a,0x32,0x3b,0x41,0x32,0x31]

fileprivate func assistCalendar(greet num: UInt8) -> UInt8 {
...
@objc protocol StrokeObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func ratingReceive()

    //: @objc optional func endScroll()
    @objc optional func throughJoin()
}

//: class TalkingDanmuMsgListTableView: UIView {
class AnalysisDataSource: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_levelTheId.map{assistCalendar(greet: $0)}, encoding: .utf8)!)
    }
...
extension AnalysisDataSource {
    ...
    func beforeLayer() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(channelEvent), userInfo: nil, repeats: true)
    }
...
extension AnalysisDataSource {
    ...
    func touristed(msgModel: UnderMeasurable?) {
    ...
    func arrow() {
        ...
        if AllHeight > dataTitleFormat {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < dataTitleFormat {
                ...
                self.msgTableView.height = dataTitleFormat
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = dataTitleFormat - AllHeight
        ...
        usage(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func usage(animated: Bool) {
    ...
    func anRecommend(inPending: Bool) {
...
extension AnalysisDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func sharp(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.throughJoin?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(show_pageKey)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = BulletCellThen.rowTab(tableView: tableView, msg: msgModel, indexPath: indexPath)
