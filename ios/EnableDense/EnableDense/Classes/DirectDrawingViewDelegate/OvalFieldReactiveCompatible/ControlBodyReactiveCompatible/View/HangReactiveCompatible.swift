...
fileprivate let mainObjectData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Select mute duration" :*/
fileprivate let userSignRequireMessage:String = "Selectreplacement stream jump"
fileprivate let appErrorHeelButtUrl:String = "duanothert"
fileprivate let kProOptionTitle:[Character] = ["i","o","n"]

/*: "Cirform" :*/
fileprivate let const_replySpectrumValue:[Character] = ["C"]
fileprivate let show_distributionText:String = "iprocedurefoprocedurem"

/*: "Mute success" :*/
fileprivate let userQueryName:String = "greet drag container thenMute "
fileprivate let show_cancelKey:String = "suconversationess"

/*: "day" :*/
fileprivate let constPadQuantityValue:[Character] = ["d","a","y"]

/*: "days" :*/
fileprivate let dataDetailedId:String = "DAYS"
...
//: enum MuteViewType {
enum ConversionViewType {
    //: case live
    case live
}

//: class TalkingLiveRoomMuteView: UIView {
class HangReactiveCompatible: UIView {
    ...
    var viewType: ConversionViewType = .live
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainObjectData.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(coverEnd), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(userSignRequireMessage.prefix(6)) + " mute " + appErrorHeelButtUrl.replacingOccurrences(of: "another", with: "ra") + String(kProOptionTitle)).localized
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
        //: return lb
        return lb
    ...
    lazy var commitBtn: UIButton = {
        ...
        btn.setTitle((String(const_replySpectrumValue) + show_distributionText.replacingOccurrences(of: "procedure", with: "r")).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(commitMp), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension HangReactiveCompatible {
    /// 消除视图按钮
    //: @objc private func bgDismissButtonClick() {
    @objc private func coverEnd() {
        //: dismiss()
        accountForDismiss()
    }
    ...
    func immediate() {
        ...
        self.contentView.frame = CGRect(x: (dataShowDetailPopMessage - contentV_Size.width) / 2, y: dataViewPath, width: contentV_Size.width, height: contentV_Size.height)

        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y = (ScreenHeight-self.contentV_Size.height)/2
            self.contentView.frame.origin.y = (dataViewPath - self.contentV_Size.height) / 2
        }
    }

    /// 隐藏视图
    //: func dismiss() {
    func accountForDismiss() {
        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y = ScreenHeight
            self.contentView.frame.origin.y = dataViewPath
...
extension HangReactiveCompatible {
    ...
    @objc private func beforeRecommend(btn: UIButton) {
    ...
    @objc private func commitMp() {
        ...
        if self.viewType == .live {
            ...
            ControlBodyReactiveCompatible().photoAlways(targetUid: targetUid, duration: index) { succeed, _, _ in
                //: ProgressHUD.dismiss()
                DirectProgressHUD.thatDismiss()
                //: if succeed {
                if succeed {
                    //: self.func__showStatusBarSuccessMsg(showMsg: "Mute success".localized)
                    self.pointRate(showMsg: (String(userQueryName.suffix(5)) + show_cancelKey.replacingOccurrences(of: "conversation", with: "cc")).localized)
                }
                //: self.dismiss()
                self.accountForDismiss()
            }
        }
    }
}
...
extension HangReactiveCompatible {
    ...
    private func toCapacityAvailable() {
        ...
        for i in 0 ..< 3 {
            ...
            btn.micState(color: UIColor.nameure(), forState: .selected)
            ...
            let dayStr = (title == "1") ? (String(constPadQuantityValue)).localized : (dataDetailedId.lowercased()).localized
            //: btn.setTitle("\(title) " + dayStr, for: .normal)
            btn.setTitle("\(title) " + dayStr, for: .normal)
            //: btn.tag = 100+i
            btn.tag = 100 + i
            //: btn.addTarget(self, action: #selector(muteBtnClick), for: .touchUpInside)
            btn.addTarget(self, action: #selector(beforeRecommend), for: .touchUpInside)
