...
/*: "num" :*/
fileprivate let noti_supportUrl:String = "conversationum"
...
@objc public enum ResourceTotalLiteral: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct RepresentModelType: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// ResourceTotalLiteral
    ...
    mutating func sameNum(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct InThatLocationTransformable: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (noti_supportUrl.replacingOccurrences(of: "conversation", with: "n"))
    }
}
