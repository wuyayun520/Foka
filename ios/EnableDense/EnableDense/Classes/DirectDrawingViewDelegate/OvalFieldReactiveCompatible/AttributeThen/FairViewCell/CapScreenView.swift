...
fileprivate let notiFoundData:[UInt8] = [0xdb,0xdc,0xdb,0xc6,0x9a,0xd1,0xdd,0xd6,0xd7,0xc0,0x88,0x9b,0x92,0xda,0xd3,0xc1,0x92,0xdc,0xdd,0xc6,0x92,0xd0,0xd7,0xd7,0xdc,0x92,0xdb,0xdf,0xc2,0xde,0xd7,0xdf,0xd7,0xdc,0xc6,0xd7,0xd6]

/*: "move" :*/
fileprivate let notiClickUrl:[Character] = ["m","o","v","e"]
...
class CapScreenView: UIView {
    ...
    var danumuMsgArray: [CoordinatorModelType] = []
    //: var danumuItemViewArray: [TalkingGameFloatingScreenItemView] = []
    var danumuItemViewArray: [FavoriteView] = []

    //: let TapAction: PublishSubject<TalkingGameFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<CoordinatorModelType?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiFoundData.map{$0^178}, encoding: .utf8)!)
    }
    ...
    func removeQuick(_ danumuMsgModel: CoordinatorModelType?) {
    ...
    @objc func transfusion(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(transfusion(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension CapScreenView: UIGestureRecognizerDelegate {
    ...
    func singleFirst(for danumuMsgModel: CoordinatorModelType?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = FavoriteView(model: danumuMsgModel)
        ...
        var frame = CGRect(x: dataShowDetailPopMessage, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if LieLanguageManager.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = dataShowDetailPopMessage
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (String(notiClickUrl)))
...
//    }
    ...
    func translateTap(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                singleFirst(for: model, isFirst: isFirst)
