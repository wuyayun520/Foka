...
fileprivate let user_pauseKey:[UInt8] = [0xe3,0xe8,0xe3,0xee,0xa2,0xdd,0xe9,0xde,0xdf,0xec,0xb4,0xa3,0x9a,0xe2,0xdb,0xed,0x9a,0xe8,0xe9,0xee,0x9a,0xdc,0xdf,0xdf,0xe8,0x9a,0xe3,0xe7,0xea,0xe6,0xdf,0xe7,0xdf,0xe8,0xee,0xdf,0xde]

fileprivate func animaAction(lack num: UInt8) -> UInt8 {
...
/*: "icon_chat_lock" :*/
fileprivate let constDecreaseValue:String = "icon_below might interested pop extension"
fileprivate let dataPlatformMessage:[Character] = ["k"]

/*: "icon_chat_burned" :*/
fileprivate let userDeliveryMsg:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let data_qualityFormat:[Character] = ["t","_","b"]
fileprivate let mainBeautifulId:String = "urnetheory"

/*: "icon_chat_burned_shan" :*/
fileprivate let main_optionAllyStr:String = "biton"
fileprivate let app_charmId:String = "combine cost snap_burn"
fileprivate let show_welcomeMessage:String = "popular coordinate reference bar paneled_shan"

/*: "Already burned" :*/
fileprivate let noti_lovelyName:String = "restaurant depth break outputAlre"
fileprivate let noti_neverthelessData:String = "indicatorurned"
...
class SinceViewCell: SoupThen {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_pauseKey.map{animaAction(lack: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.hr(name: (String(constDecreaseValue.prefix(5)) + "chat_loc" + String(dataPlatformMessage)))
    ...
    lazy var burnedImagV: MagnitudeercalateTalkingButton = {
        ...
        imgV.setBackgroundImage(UIImage.hr(name: (String(userDeliveryMsg) + String(data_qualityFormat) + mainBeautifulId.replacingOccurrences(of: "theory", with: "d"))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.hr(name: (main_optionAllyStr.replacingOccurrences(of: "bit", with: "ic") + "_chat" + String(app_charmId.suffix(5)) + String(show_welcomeMessage.suffix(7)))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(noti_lovelyName.suffix(4)) + "ady " + noti_neverthelessData.replacingOccurrences(of: "indicator", with: "b")).localized, for: .normal)
        //: imgV.setTitleColor(.white, for: .normal)
        imgV.setTitleColor(.white, for: .normal)
        //: imgV.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 16)
        imgV.titleLabel?.font = UIFont.promptReload(fontSize: 16)
        //: return imgV
        return imgV
        //: }()
    }()
...
extension SinceViewCell {
    ...
    override func panWith(with data: PingOvalCellData) {
        ...
        let intimatePhoto: StumbleModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            age(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            age(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: TakeCellData, urlStr: String?, isMosaic: Bool) {
    func age(data: TakeCellData, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.solarGravity(image, can: 90)
                        self.photoView.image = UIImage.solarGravity(image, can: 90)
                        //: } else {
                    } else {
            ...
            storageShow(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.underStack(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func storageShow(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.towardUniversalCompletion(urlStr: urlStr) { [weak self] image in
