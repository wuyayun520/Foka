...
fileprivate let app_assertFormat:[UInt8] = [0xfe,0xf9,0xfe,0xe3,0xbf,0xf4,0xf8,0xf3,0xf2,0xe5,0xad,0xbe,0xb7,0xff,0xf6,0xe4,0xb7,0xf9,0xf8,0xe3,0xb7,0xf5,0xf2,0xf2,0xf9,0xb7,0xfe,0xfa,0xe7,0xfb,0xf2,0xfa,0xf2,0xf9,0xe3,0xf2,0xf3]

private func beanSeries(clear num: UInt8) -> UInt8 {
    return num ^ 151
}

/*: "all" :*/
fileprivate let mainOldPath:[Character] = ["a","l","l"]
...
class AttributeThen: SoupThen {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: SlumThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_assertFormat.map{beanSeries(clear: $0)}, encoding: .utf8)!)
    }
...
extension AttributeThen {
    ...
    override func panWith(with data: PingOvalCellData) {
        ...
        self.giftData = data as? SlumThen
        ...
        if giftData.msgModel.toUser.uid == (String(mainOldPath)) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.hr(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.underStack(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
