...
fileprivate let noti_drownData:[UInt8] = [0x67,0x60,0x67,0x7a,0x26,0x6d,0x61,0x6a,0x6b,0x7c,0x34,0x27,0x2e,0x66,0x6f,0x7d,0x2e,0x60,0x61,0x7a,0x2e,0x6c,0x6b,0x6b,0x60,0x2e,0x67,0x63,0x7e,0x62,0x6b,0x63,0x6b,0x60,0x7a,0x6b,0x6a]

private func ampleName(since num: UInt8) -> UInt8 {
    return num ^ 14
}

/*: "#AAAAAA" :*/
fileprivate let notiMagnitudeMessage:[Character] = ["#","A","A","A","A","A","A"]

/*: "btn_chatsettings_unchoice" :*/
fileprivate let main_backgroundValue:[Character] = ["b","t","n","_","c","h","a","t"]
fileprivate let dataWipeStr:String = "settiindicator"
fileprivate let dataAboveStr:String = "gs_unbridge example laboratory"

/*: "btn_chatsettings_choiced" :*/
fileprivate let userPlatformBucketStr:String = "btn_chcompare particle news general"
fileprivate let show_playerName:String = "tiadjust"
fileprivate let userGovernData:String = "document"

/*: "Top" :*/
fileprivate let noti_overKey:[Character] = ["T","o","p"]

/*: "Save" :*/
fileprivate let user_houseProPath:[Character] = ["S","a","v","e"]

/*: "Cancel" :*/
fileprivate let userMirrorValue:[Character] = ["C","a","n","c","e","l"]
...
class WhoThereNotesView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_drownData.map{ampleName(since: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + k_depthUrl
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: dataViewPath - height, width: dataShowDetailPopMessage, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.vernalEquinox(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        ...
        v.backgroundColor = UIColor.originalAlways()
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(notiMagnitudeMessage)))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: MagnitudeercalateTalkingButton = {
        ...
        btn.setImage(UIImage.hr(name: (String(main_backgroundValue) + dataWipeStr.replacingOccurrences(of: "indicator", with: "n") + String(dataAboveStr.prefix(5)) + "choice")), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.hr(name: (String(userPlatformBucketStr.prefix(6)) + "atset" + show_playerName.replacingOccurrences(of: "adjust", with: "ng") + "s_choic" + userGovernData.replacingOccurrences(of: "document", with: "ed"))), for: .selected)
        ...
        btn.setTitle((String(noti_overKey)).localized, for: .normal)
        //: btn.spaceBetweenTitleAndImage = 6
        btn.spaceBetweenTitleAndImage = 6
        //: btn.addTarget(self, action: #selector(topButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(topButtonMove), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(user_houseProPath)).localized, for: .normal)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(userMirrorValue)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(arrayShare), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension WhoThereNotesView: YYTextViewDelegate {
...
extension WhoThereNotesView {
    ...
    @objc private func write() {
        ...
        TimeTestedRequestTool.aggregationPic(content: textView.text, isTop: isTop, toUid: targetId) {
            //: succeed, _, _ in
            succeed, _, _ in
    ...
    @objc private func topButtonMove() {
    ...
    func hostLess(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
