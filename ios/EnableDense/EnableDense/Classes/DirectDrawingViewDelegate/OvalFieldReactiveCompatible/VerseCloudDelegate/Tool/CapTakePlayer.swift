...
/*: "call_bgm" :*/
fileprivate let dataPlyRefName:String = "cagamegame"

/*: "call_response_bgm" :*/
fileprivate let showNothingUrl:[Character] = ["c","a","l","l","_"]
fileprivate let const_residentName:String = "sustainsp"

/*: "invitePop_call_bgm" :*/
fileprivate let app_itsContent:String = "inloopt"
fileprivate let constProductionId:String = "comply approximately chemical distinguishePop"
fileprivate let const_logMessage:String = "l_bgmnearby face tip basic"
...
enum MilitaryCoatSwitchsetTarget: Int {
    //: case Call
    case Call // 音视频拨打音乐
    //: case Response
...
class CapTakePlayer: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = MilitaryCoatSwitchsetTarget.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func quantityPlay(type: MilitaryCoatSwitchsetTarget) -> CapTakePlayer {
        //: let player = TalkingCallBGMPlayer()
        let player = CapTakePlayer()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.takePackage()
...
extension CapTakePlayer {
    ...
    private func replay(notify: Notification) {
    ...
    private func creationBroad(notify: Notification) {
    ...
    private func takePackage() {
        //: var fileName = "call_bgm"
        var fileName = (dataPlyRefName.replacingOccurrences(of: "game", with: "l") + "_bgm")
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(showNothingUrl) + const_residentName.replacingOccurrences(of: "sustain", with: "re") + "onse_bgm")
            //: } else if self.type == .invitePop {
        } else if self.type == .invitePop {
            //: fileName = "invitePop_call_bgm"
            fileName = (app_itsContent.replacingOccurrences(of: "loop", with: "vi") + String(constProductionId.suffix(4)) + "_cal" + String(const_logMessage.prefix(5)))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = EpisodeThen.default.databaseAlways(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension CapTakePlayer: TXVodPlayListener {
