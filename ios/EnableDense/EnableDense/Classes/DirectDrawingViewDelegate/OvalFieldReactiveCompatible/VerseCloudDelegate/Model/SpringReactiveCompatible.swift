...
/*: "#BAFBFF" :*/
fileprivate let mainDragContent:String = "#BAFBFproperty supply"
fileprivate let dataControlContent:[Character] = ["F"]
...
class SpringReactiveCompatible: NSObject {
    ...
    func transformModel(model: MpDanmuModel) -> MpDanmuModel {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func changeStateComment() {
        ...
        let name = satisfyBasic(text: firstStr, font: UIFont.platformDisk(fontSize: 14), color: UIColor(hex: (String(mainDragContent.prefix(6)) + String(dataControlContent)))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = satisfyBasic(text: self.model.content, font: UIFont.platformDisk(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        stint(attribText: self.model.msgAttribText)
    }
    ...
    func shareDay() {
        ...
        let content = satisfyBasic(text: self.model.transContent, font: UIFont.platformDisk(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func labIn() -> NSMutableParagraphStyle {
    ...
    func satisfyBasic(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = labIn()
        //: return attribute
        return attribute
    }
    ...
    func stint(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = app_buildPath - 36
