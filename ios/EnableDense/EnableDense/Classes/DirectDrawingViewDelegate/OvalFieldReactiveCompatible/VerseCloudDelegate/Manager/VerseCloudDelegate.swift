...
/*: "mf/call/GetUserInfo" :*/
fileprivate let notiMedalEffectText:[Character] = ["m","f","/","c","a","l","l","/","G","e","t","U","s"]
fileprivate let show_timingMessage:String = "mirror"
fileprivate let app_originText:String = "us language his masterrInfo"

/*: "uid" :*/
fileprivate let mainDramaticTitle:[UInt8] = [0xc2,0xde,0xd3]

private func relationTrust(balance num: UInt8) -> UInt8 {
    return num ^ 183
}

/*: "TRTC 随机视频匹配中." :*/
fileprivate let k_alongsideValue:[Character] = ["T","R","T","C"," ","随","\u{673a}","视","频","匹","配","中","."]

/*: "TRTC  :*/
fileprivate let kAlwaysMessage:String = "TRTC rating mp"

/*: 视频通话. logId: :*/
fileprivate let const_decisionValue:[Character] = ["视","\u{9891}","\u{901a}","话","."," ","l"]
fileprivate let show_nightPath:[Character] = ["o","g","I","d",":"]

/*: "TRTC 视频通话销毁. logId： :*/
fileprivate let main_feeStr:[Character] = ["T","R","T","C"," "]
fileprivate let show_dialogStr:[Character] = ["视","频","\u{901a}","话","销","毁","."," ","l","o","g","I","d","："]

/*: "TRTC 视频通话错误. errCode: :*/
fileprivate let k_layText:[Character] = ["T","R","T","C"," ","视","\u{9891}","通","\u{8bdd}"]
fileprivate let constApprovalId:String = "错误."
fileprivate let noti_approximatelyUrl:String = "bold large err"

/*: , errMsg: :*/
fileprivate let data_ideaUrl:[Character] = [","," ","e","r","r","M","s","g",":"]

/*: "TRTC 视频通话SDK收到警告. warningCode: :*/
fileprivate let show_dimensionName:[Character] = ["T","R","T","C"," ","视","频","通","\u{8bdd}"]
fileprivate let noti_electContent:String = "minimize oval chin definitionSDK收到警告"
fileprivate let dataGoUrl:[Character] = ["."," ","w","a","r","n","i","n","g","C","o","d","e",":"]

/*: , warningMsg: :*/
fileprivate let dataMapSufficientPath:[Character] = [","," ","w","a","r","n","i","n","g","M","s","g",":"]

/*: "TRTC 视频通话进入房间. result: :*/
fileprivate let app_triggerTakeUrl:String = "trtc"
fileprivate let appCalendarContent:String = "房间. rstock visitor"
fileprivate let user_levelUrl:[Character] = ["e","s","u","l","t",":"]

/*: , logId： :*/
fileprivate let dataParkAnalyzeMsg:String = ", lpoint cloud reminder con social"
fileprivate let k_cropUrl:String = "ogId：engage devil signal survival"

/*:  加入了房间" :*/
fileprivate let constLoseFormat:String = " 加\u{5165}了房间"

/*:  离开了房间" :*/
fileprivate let showHeavyContent:[Character] = [" ","离","\u{5f00}","了","房","间"]
...
enum DrawingBinaryQuantity: Int {
    //: case CallEnd     = 100
...
class WalkThen: NSObject {
    ...
    var remoteCameraStatus = CoatCameraStatus.Normal // 对方摄像头状态
}

//: class TalkingVideoChatManager: NSObject {
class VerseCloudDelegate: NSObject {
    //: var infoModel: TalkingVideoChatModel?
    var infoModel: BassChatModel? // 视频通话model
    //: var config = TalkingVideoChatConfig()
    var config = WalkThen() // 视频渲染配置类
    ...
    private var trtcView: ShadowinessThen? // 预览view（使用商汤美颜）
...
extension VerseCloudDelegate {
    /// 开始视频速配（男）
    /// - Parameters:
    ///   - uid: 接收者的id
    ///   - completion: 回调
    //: class func req_callGetUserInfo(uid: String, completion: @escaping FinishBlock) {
    class func conversionDown(uid: String, completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = MpRequestModel()
        //: reqModel.requestPath = "mf/call/GetUserInfo"
        reqModel.requestPath = (String(notiMedalEffectText) + show_timingMessage.replacingOccurrences(of: "mirror", with: "e") + String(app_originText.suffix(5)))
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["uid": uid]
        reqModel.params = [String(bytes: mainDramaticTitle.map{relationTrust(balance: $0)}, encoding: .utf8)!: uid]
...
extension VerseCloudDelegate {
    ...
    func flow(_ frontCamera: Bool) {
        ...
        self.blindSuite(front: frontCamera)
    }

    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func cameraCos(_ openCamera: Bool) {
        ...
        self.idiomName(isOpen: openCamera)
    }
    ...
    func twinkle() {
        ...
        DispatchQueue.main.async {
            ...
            self.startSmallPreviewWindow(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
    ...
    func audiencePublish() {
        // 如果没有预览先开启预览
        //: self.config.isPage = true
        self.config.isPage = true
...
extension VerseCloudDelegate {
    ...
    private func blindSuite(front: Bool) {
        ...
        self.trtcView?.cater(isFront: front)
    }

    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func idiomName(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard const_environmentMsg == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.fireTo(isOpen: isOpen)
...
extension VerseCloudDelegate {
    ...
    func offSettings() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(spaceWriter(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(cryMember(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(listenerPer(notif:)),
    ...
    @objc private func spaceWriter(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func cryMember(notif: Notification?) {
        //: if notif == nil {
    ...
    @objc private func slide(notif: Notification) {
    ...
    @objc func listenerPer(notif: Notification) {
...
extension VerseCloudDelegate {
    ...
    func jumpDoing(isMatch: Bool = false) {
        //: if isMatch {
        if isMatch {
            //: UploadLogTool.writeLog(msg: "TRTC 随机视频匹配中.")
            ReckonLogTool.commentAdvertising(msg: (String(k_alongsideValue)))
            //: } else {
        } else {
            //: UploadLogTool.writeLog(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            ReckonLogTool.commentAdvertising(msg: (String(kAlwaysMessage.prefix(5))) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (String(const_decisionValue) + String(show_nightPath)) + "\(infoModel?.logId ?? 0).")
        }
        //: if self.infoModel?.videoStage == .Waiting {
        if self.infoModel?.videoStage == .Waiting {
            //: startTRTCPreview(smallWindow: false)
            startSmallPreviewWindow(smallWindow: false)
            ...
            if self.infoModel?.renderPostion == .selfOnBg {
                ...
                startSmallPreviewWindow(smallWindow: true)
            }
        }
    ...
    func startSmallPreviewWindow(smallWindow: Bool) {
        //: guard SenseTime_Use == true else {
        guard const_environmentMsg == true else {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = ShadowinessThen()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.permission(isSmallWindow: smallWindow)
        //: if smallWindow {
    ...
    private func announcement() {
        ...
        params.sdkAppId = user_qualityData
        //: params.roomId = UInt32(roomId)
        params.roomId = UInt32(roomId)
        //: params.userId = SteppingMotorThen.share.loginUserMode.userID
        params.userId = SteppingMotorThen.share.loginUserMode.userID
        //: params.userSig = SteppingMotorThen.share.appUserConfigMode.uSigTrtc
        params.userSig = SteppingMotorThen.share.appUserConfigMode.uSigTrtc
    ...
    private func compare() {
        ...
        ReckonLogTool.commentAdvertising(msg: (String(main_feeStr) + String(show_dialogStr)) + "\(infoModel?.logId ?? 0).")
        ...
        if const_environmentMsg {
            ...
            self.trtcView?.collectionVersion()
            //: } else {
        } else {
            //: self.isOpenTRTCPreview = false
            self.isOpenTRTCPreview = false
...
extension VerseCloudDelegate: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        ReckonLogTool.commentAdvertising(msg: (String(k_layText) + constApprovalId.capitalized + String(noti_approximatelyUrl.suffix(4)) + "Code:") + "\(errCode)" + (String(data_ideaUrl)) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
            //: self.func__showStatusBarErrorMsg(showMsg: errMsg!)
            self.revenueMsg(showMsg: errMsg!)
        }
    }
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        ReckonLogTool.commentAdvertising(msg: (String(show_dimensionName) + String(noti_electContent.suffix(7)) + String(dataGoUrl)) + "\(warningCode)" + (String(dataMapSufficientPath)) + "\(String(describing: warningMsg)).")
        ...
        if self.config.remoteCameraStatus == .Normal {
            ...
            NotificationCenter.default.post(name: userMicUrl, object: self)
        }
    }
    ...
    func onEnterRoom(_ result: Int) {
        ...
        ReckonLogTool.commentAdvertising(msg: (app_triggerTakeUrl.uppercased() + " 视频通\u{8bdd}\u{8fdb}\u{5165}" + String(appCalendarContent.prefix(5)) + String(user_levelUrl)) + "\(result)" + (String(dataParkAnalyzeMsg.prefix(3)) + String(k_cropUrl.prefix(5))) + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
            ...
            HemSocketManager.shared.strike()
            //: } else {
        } else {
            //: self.config.disconnected = false
            self.config.disconnected = false
    ...
    func onUserVideoAvailable(_ userId: String, available: Bool) {
        //: guard userId != SteppingMotorThen.share.loginUserMode.userID else { return }
        guard userId != SteppingMotorThen.share.loginUserMode.userID else { return }
