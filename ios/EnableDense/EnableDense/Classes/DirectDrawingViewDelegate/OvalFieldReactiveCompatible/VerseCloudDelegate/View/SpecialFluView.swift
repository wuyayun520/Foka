...
fileprivate let mainRecommendationName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class SpecialFluView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainRecommendationName.reversed(), encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview _: UIView?) {
        ...
        HemSocketManager.shared.containerView?.isHidden = false
    }
    ...
    func deformation() {
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= dataShowDetailPopMessage * 0.5) ? -self.width : dataShowDetailPopMessage + self.width
            ...
            HemSocketManager.shared.containerView?.isHidden = true
        }
    }
}
...
extension SpecialFluView {
    /// 添加点击手势
    //: @discardableResult
    @discardableResult
    //: func addTapGesture() -> UITapGestureRecognizer {
    func duringSeekGesture() -> UITapGestureRecognizer {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureEvent(sender:)))
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.repudiateSender(sender:)))
    ...
    func thinSignal() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.aboveSender(sender:)))
        //: let tap = self.addTapGesture()
        let tap = self.duringSeekGesture()
    ...
    @objc func aboveSender(sender: UIPanGestureRecognizer) {
        ...
        if sender.state == .ended {
            ...
            HemSocketManager.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func repudiateSender(sender _: UIPanGestureRecognizer) {
