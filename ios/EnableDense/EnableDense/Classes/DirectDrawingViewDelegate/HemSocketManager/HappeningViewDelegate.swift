...
fileprivate let kTriggerThreadKey:[UInt8] = [0x82,0x87,0x82,0x8d,0x41,0x7c,0x88,0x7d,0x7e,0x8b,0x53,0x42,0x39,0x81,0x7a,0x8c,0x39,0x87,0x88,0x8d,0x39,0x7b,0x7e,0x7e,0x87,0x39,0x82,0x86,0x89,0x85,0x7e,0x86,0x7e,0x87,0x8d,0x7e,0x7d]

fileprivate func communicateSustain(sight num: UInt8) -> UInt8 {
...
/*: "DialogReactiveCompatible" :*/
fileprivate let notiFormatPath:[Character] = ["D","r","o","p","M","e"]
fileprivate let show_springName:[Character] = ["m","b","e","r","M","e","n","u","C","e","l","l"]

/*: "roomId" :*/
fileprivate let const_thinName:String = "roomIdyes available"

/*: "type" :*/
fileprivate let show_panelWashId:String = "tpossiblepe"

/*: "page" :*/
fileprivate let mainConferPath:String = "PAGE"

/*: "icon_chats_pcr" :*/
fileprivate let constAllId:String = "icoloose"
fileprivate let data_missingUrl:String = "little scale guidance social varts_pcr"

/*: "All Numbers" :*/
fileprivate let app_spreadSkipProjectMsg:[Character] = ["A","l","l"," ","N","u","m","b","e","r","s"]

/*: "all" :*/
fileprivate let user_identicalPath:String = "apersonalpersonal"

/*: "Recent Gifts" :*/
fileprivate let dataPreparationGiveValue:String = "Recentsecure sign mp"
...
class HappeningViewDelegate: UIView {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var roomID = ""
    var roomID = ""
    ...
    public var triangleHeight = Config.WordsConfig.triangleHeight
    /**
     *  调整使下拉优先 当向下偏转屏幕距离足够，优先向下偏转
     */
    //: public var adjustPullDown = Config.DropMenuConfig.adjustPullDown
    public var adjustPullDown = Config.WordsConfig.adjustPullDown
    ...
    fileprivate lazy var contentView: UIView = self.aggregation()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.loadLeaveCrush()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = HappeningViewDelegate(frame: .zero)
    ...
    init(frame: CGRect, anchorView: UIView, roomID: String) {
        ...
        super.init(frame: frame)
        //: let window = DetermineJoinReactiveCompatible.getWindow()
        let window = DetermineJoinReactiveCompatible.existent()
        //: self.roomID = roomID
        self.roomID = roomID
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kTriggerThreadKey.map{communicateSustain(sight: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func aggregation() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func loadLeaveCrush() -> UITableView {
    ...
    private func parentBehind() {
        ...
        mTable.register(DialogReactiveCompatible.self, forCellReuseIdentifier: (String(notiFormatPath) + String(show_springName)))
    ...
    func platformFrame() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(const_thinName.prefix(6)))] = roomID
        //: dict["type"] = "3"
        dict[(show_panelWashId.replacingOccurrences(of: "possible", with: "y"))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(mainConferPath.lowercased())] = String(pageIndex)
        ...
        HimGiftManager.share.conversionCompletion(params: dict, completion: { array in
            ...
            self.mTable.culminateHaveRestore()
            ...
            if self.pageIndex == 0 {
                ...
                let model = StatusStrokeModelType()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (constAllId.replacingOccurrences(of: "loose", with: "n") + "_cha" + String(data_missingUrl.suffix(6)))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(app_spreadSkipProjectMsg)).localized
                //: model.uid = "all"
                model.uid = (user_identicalPath.replacingOccurrences(of: "personal", with: "l"))
...
extension HappeningViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(notiFormatPath) + String(show_springName)), for: indexPath) as! DialogReactiveCompatible
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! StatusStrokeModelType
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! StatusStrokeModelType
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.openicial(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.intervalernationality(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: dataShowDetailPopMessage, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.originalAlways()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .opPoint(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .uniformColor()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(dataPreparationGiveValue.prefix(6)) + " Gifts").localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(app_spreadSkipProjectMsg)).localized
        }
