...
/*: "rYMsgType" :*/
fileprivate let main_whyStr:[Character] = ["r","Y","M","s","g","T","y","p","e"]

/*: "GJ:CallCustom" :*/
fileprivate let user_lightUrl:String = "resource organize exhibit opGJ:C"
fileprivate let app_materialGoodPath:[Character] = ["u","s","t","o","m"]
...
// __CLOSE_PRINT__
//
//  PacketReactiveCompatible.swift
//  AbroadTalking
//
...
class PacketReactiveCompatible: NSObject {
    //: private var showView: TalkingMessageNotificationView?
    private var showView: PressView? // 当前View
    //: private var preView: TalkingMessageNotificationView?
    private var preView: PressView? // 旧的View
    ...
    static var shared = PacketReactiveCompatible()
    ...
    @objc func collection() {
        ...
        if self.preView != nil {
            ...
            self.preView?.scorePermission()
        }
        //: if self.showView != nil {
        if self.showView != nil {
            //: self.showView?.dismiss()
            self.showView?.scorePermission()
        }
    ...
    private lazy var msgArr: [SpendConversationModel] = //: return [SpendConversationModel]()
        .init()
    //: }()
}
...
extension PacketReactiveCompatible {
    /// 处理接收到的TX消息，展示消息通知
    /// - Parameters:
    ///   - msg: V2TIMMessage
    ///   - dataDict: 消息字典
    //: func onRecvNewMessage(msg: V2TIMMessage, dataDict: [String: Any]) {
    func faraway(msg: V2TIMMessage, dataDict: [String: Any]) {
        ...
        guard hairDisplayable() == true || include() == true else { return }
        //: guard msg.userID != DetermineJoinReactiveCompatible.getCustomerServiceID() && msg.userID != DetermineJoinReactiveCompatible.getXiaoMiID() else {
        guard msg.userID != DetermineJoinReactiveCompatible.facilityRank(), msg.userID != DetermineJoinReactiveCompatible.anthologist() else {
            //: return
            return
        }
        //: let contenType = TUISocialChatManager.messageCellType(dataDict)
        let contenType = CookieChatManager.engineering(dataDict)
        // 提示消息
        ...
        if contenType == .TXMessageContentType_text, dataDict[(String(main_whyStr))] as? String == (String(user_lightUrl.suffix(4)) + "allC" + String(app_materialGoodPath)) {
            //: return
            return
        }

        // 2. V2TIMMessage => SpendConversationModel
        ...
        //: if let userInfo: ComprehendReactiveCompatible = DBUserInfoManager.cache_getCachedUserInfo(targetId: model.targetId) {
        if let userInfo: ComprehendReactiveCompatible = CookieInfoManager.dealMp(targetId: model.targetId) {
    ...
    private func verse() {
        ...
        let notifView = PressView(frame: .zero)
        //: notifView.show(firstModel)
        notifView.guideActivity(firstModel)
    ...
    private func hairDisplayable() -> Bool {
        //: guard let currentVC = currentViewController() else { return false }
        guard let currentVC = strokeController() else { return false }
        //: if currentVC.isKind(of: TalkingVideoChatViewController.self) ||
        if currentVC.isKind(of: MotorViewDelegate.self) ||
            //: currentVC.isKind(of: TalkingVoiceChatViewController.self) {
            currentVC.isKind(of: StatusCallerDelegate.self)
        {
            //: return true
            return true
        }
    ...
    private func include() -> Bool {
        //: guard let currentVC = currentViewController() else { return false }
        guard let currentVC = strokeController() else { return false }
        //: if currentVC.isKind(of: TalkingSocialViewController.self) ||
        if currentVC.isKind(of: PlusCigaretteViewController.self) ||
            //: currentVC.isKind(of: SocialPopularViewController.self) ||
            currentVC.isKind(of: FoodViewDelegate.self) ||
            //: currentVC.isKind(of: TalkingMomentViewController.self) ||
            currentVC.isKind(of: AtThatPlaceReactiveCompatible.self) ||
            //: currentVC.isKind(of: TalkingLiveExplainViewController.self) ||
            currentVC.isKind(of: SqueezeReactiveCompatible.self) ||
            //: currentVC.isKind(of: TalkingMeViewController.self) {
            currentVC.isKind(of: KeepSoupViewDelegate.self)
        {
