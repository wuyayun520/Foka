...
fileprivate let showInputTitle:String = "\u{8bbe}置普通状\u{6001}的"
fileprivate let dataRiseConfirmStr:String = "文\u{5b57}颜\u{8272}"
fileprivate let user_noStr:[Character] = ["用","R","G","B","空","间","的","颜","\u{8272}","值"]

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let data_lipUrl:[Character] = ["\u{8bbe}","\u{7f6e}","选","中","状","态","\u{7684}"]
fileprivate let main_houseRemoteId:String = "\u{6587}字颜色时 "
fileprivate let appThroughId:String = "B空间的\u{989c}色值"

/*: "init(coder:) has not been implemented" :*/
fileprivate let const_enthusiasmName:[UInt8] = [0x8e,0x89,0x8e,0x93,0xcf,0x84,0x88,0x83,0x82,0x95,0xdd,0xce,0xc7,0x8f,0x86,0x94,0xc7,0x89,0x88,0x93,0xc7,0x85,0x82,0x82,0x89,0xc7,0x8e,0x8a,0x97,0x8b,0x82,0x8a,0x82,0x89,0x93,0x82,0x83]

private func accountArrive(hundred num: UInt8) -> UInt8 {
    return num ^ 231
}

/*: "设置的下标不合法!!" :*/
fileprivate let data_bouncePath:String = "设置will标不合法!"
fileprivate let mainSubKey:String = "route"

/*: "Bags" :*/
fileprivate let data_yetId:[Character] = ["B","a","g","s"]
...
open class ViewReactiveCompatible: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: NoticeSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.attention(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((showInputTitle + dataRiseConfirmStr + "时 请使" + String(user_noStr)))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.attention(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(data_lipUrl) + main_houseRemoteId + "请使用RG" + appThroughId))
        }
    ...
    fileprivate func attention(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: NoticeSegmentStyle, titles: [String]) {
    ...
    @objc func empty(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? RobotView else { return }
        //: currentIndex = currentLabel.tag
        currentIndex = currentLabel.tag
        //: print(currentLabel.tag)
        //: adjustUIWhenBtnOnClickWithAnimate(true)
        streetwiseAnimate(true)
    }

    //: required public init?(coder aDecoder: NSCoder) {
    public required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: const_enthusiasmName.map{accountArrive(hundred: $0)}, encoding: .utf8)!)
    }
}
...
public extension ViewReactiveCompatible {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func user(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (data_bouncePath.replacingOccurrences(of: "will", with: "的下") + mainSubKey.replacingOccurrences(of: "route", with: "!")))
...
extension ViewReactiveCompatible {
    ...
    fileprivate func scale() {
        ...
        for (index, title) in titles.enumerated() {
            ...
            let label = RobotView(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.empty(_:)))
            ...
            if title == (String(data_yetId)).localized {
    ...
    fileprivate func translationPic() {
        // 设置Label位置
    ...
    fileprivate func invitationCollection() {
        ...
        if let firstLabel = labelsArray[0] as? RobotView {
    ...
    public func availableDecision(_ currentIndex: Int) {
