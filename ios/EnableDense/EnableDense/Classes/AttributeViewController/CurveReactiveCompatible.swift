...
class CurveReactiveCompatible: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: ComparisonOnsetTarget?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.applyReload()
            }
        }
    ...
    func pending() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.publicationUntil(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.originalAlways()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.quitTo(), NSAttributedString.Key.font: UIFont.opPoint(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.publicationUntil(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.quitTo(), NSAttributedString.Key.font: UIFont.opPoint(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.quitTo()
...
extension CurveReactiveCompatible: UINavigationControllerDelegate {
    ...
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        ...
        if (viewController.hidesBottomBarWhenPushed) {
            ...
            CommentAdvertisingManager.shared.advSuperView?.isHidden = true
        //: } else {
        } else {
...
extension CurveReactiveCompatible: UIGestureRecognizerDelegate {
