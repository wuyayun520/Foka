...
fileprivate let show_jumpFingerId:String = "btn_shincome quote"
fileprivate let data_forthPeopleUrl:String = "ow_relation domain take"
fileprivate let user_featureValue:[Character] = ["a","l","e","r","t","B","a","c","k","g","r","o","u","n","d"]

/*: "btn_alertBackground_false" :*/
fileprivate let data_flagGivingAttachName:String = "btn_aanimal execute bear confirm"
fileprivate let showDemonstrateTitle:String = "ckprofilerou"
fileprivate let constDocActiveContent:String = "nd_falsehearing work normal"
...
public class EnhanceAlertShow: NSObject {
    /// 通用回调
...
public extension EnhanceAlertShow {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: WhiteAlertConfig) -> Void
    ...
    internal class func valueNog(title: String? = nil,
                                 //: message: String?  = nil,
                                 message: String? = nil,
                                 ...
                                 config: WhiteAlertConfig? = nil)
    {
        //: customAlert(title: title,
        columnPress(title: title,
                    //: message: message,
                    message: message,
    ...
    @objc internal class func burnBlock(title: String? = nil,
    ...
    class func columnPress(title: String? = nil,
                           ...
                           config: WhiteAlertConfig? = nil)
    {
        ...
        let alertView = MotorAlertView(title: title,
        ...
        //: DetermineJoinReactiveCompatible.getWindow().addSubview(alertView)
        DetermineJoinReactiveCompatible.existent().addSubview(alertView)
    ...
    class func stripAlert() {
        //: DetermineJoinReactiveCompatible.getWindow().subviews.forEach { (view) in
        DetermineJoinReactiveCompatible.existent().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: MotorAlertView.self) {
    ...
    class func spaceName(title: String, isTouch: Bool) {
        //: DetermineJoinReactiveCompatible.getWindow().subviews.forEach { (view) in
        DetermineJoinReactiveCompatible.existent().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: MotorAlertView.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: MotorAlertView = view as! MotorAlertView
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.doBuild(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class WhiteAlertConfig {
    ...
    public var maskType: EnhanceMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .opPoint(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.quitTo()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .opPoint(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.quitTo()
    ...
    public var buttonFont: UIFont = .opPoint(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.nameure()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(show_jumpFingerId.prefix(6)) + String(data_forthPeopleUrl.prefix(3)) + String(user_featureValue))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(data_flagGivingAttachName.prefix(5)) + "lertBa" + showDemonstrateTitle.replacingOccurrences(of: "profile", with: "g") + String(constDocActiveContent.prefix(8)))
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .opPoint(type: .Medium, fontSize: 16)
