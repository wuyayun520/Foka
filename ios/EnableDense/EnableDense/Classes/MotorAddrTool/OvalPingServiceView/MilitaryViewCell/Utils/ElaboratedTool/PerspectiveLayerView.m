...
+ (Byte *)TakeDataToCache:(Byte *)data {
    ...
    int styleEye = data[2];
    for (int i = styleEye; i < styleEye + another; i++) {
    ...
    data[styleEye + another] = 0;
    return data + styleEye;
}
...
//: STDisplayShader
+ (NSString *)const_blindData {
    /* static */ NSString *const_blindData = nil;
    if (!const_blindData) {
		NSArray<NSString *> *origin = @[@"15", @"39", @"7", @"150", @"242", @"188", @"226", @"44", @"45", @"29", @"66", @"76", @"73", @"69", @"58", @"82", @"44", @"65", @"58", @"61", @"62", @"75", @"172"];
		NSData *data = [TakeData TakeDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        const_blindData = [self StringFromTakeData:value];
    }
    return const_blindData;
}
...
@interface PerspectiveLayerView ()
{
...
@implementation PerspectiveLayerView
...
- (instancetype)initWithPost:(CGRect)frame strain:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self shape:[TakeData const_blindData]
                //: fragmentShader:@"STDisplayShader"
                motionWith:[TakeData const_blindData]
                    //: forProgram:&stDisplayProgram];
                    link:&stDisplayProgram];
...
- (BOOL)shape:(NSString *)vertexShaderName
          //: fragmentShader:(NSString *)fragmentShaderName
          motionWith:(NSString *)fragmentShaderName
              //: forProgram:(GLuint *)programPointer;
              link:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self app:&vertexShader cry:0x8B31 center:main_lastUrl]) {
    ...
    if (![self app:&fragShader cry:0x8B30 center:notiInfoValue]) {
    ...
    if (![self linkExpected:*programPointer]) {
...
- (BOOL)app:(GLuint *)shader cry:(GLenum)type center:(const char *)str
{
    //: GLint status;
    GLint status;
    //: const GLchar *source;
