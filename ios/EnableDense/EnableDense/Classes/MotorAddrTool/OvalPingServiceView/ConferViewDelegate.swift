...
class ConferViewDelegate: UIView {
    // MARK: - 懒加载属性
    ...
    convenience init(placeholder: String?, placeholderColor: UIColor?, frame: CGRect) {
        ...
        personUp(placeholder: placeholder, placeholderColor: placeholderColor)
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        personUp(placeholder: nil, placeholderColor: nil)
    }
}
...
extension ConferViewDelegate {
    /// placeholder Label Setup
    //: private func setup(placeholder: String?, placeholderColor: UIColor?) {
    private func personUp(placeholder: String?, placeholderColor: UIColor?) {
        ...
        plaleLabel.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(CaptureLabelOrigin().x)
            //: make.top.equalTo(PlaceholderLabelOrigin().y)
            make.top.equalTo(CaptureLabelOrigin().y)
            //: make.trailing.equalTo(-PlaceholderLabelOrigin().x)
            make.trailing.equalTo(-CaptureLabelOrigin().x)
        }
        ...
        placeholdertextView.textContainerInset = UIEdgeInsets(top: PowPlusContainerInset().top, left: PowPlusContainerInset().left, bottom: PowPlusContainerInset().bottom, right: PowPlusContainerInset().right)
...
extension ConferViewDelegate: UITextViewDelegate {
