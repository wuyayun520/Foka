...
fileprivate let noti_layMessage:[UInt8] = [0x11,0x16,0x11,0xc,0x50,0x1b,0x17,0x1c,0x1d,0xa,0x42,0x51,0x58,0x10,0x19,0xb,0x58,0x16,0x17,0xc,0x58,0x1a,0x1d,0x1d,0x16,0x58,0x11,0x15,0x8,0x14,0x1d,0x15,0x1d,0x16,0xc,0x1d,0x1c]

private func lateMinute(absolute num: UInt8) -> UInt8 {
    return num ^ 120
}
...
typealias SlidingHandler = (_ direction: SpecialTalkingSlideDirection) -> Void
...
class CoordinatorThen: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (appMeName - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_layMessage.map{lateMinute(absolute: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.nowText(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: WhiteThen = {
        ...
        let overlay = WhiteThen(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension CoordinatorThen {
    ...
    func modulation() {
        ...
        self.overlay.beanX(startX: appMeName - x)
    }
    ...
    private func nowText(point: CGFloat) -> UIImage {
...
extension CoordinatorThen: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = appMeName - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.keyFor(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.keyFor(isShow: true)
    }
...
extension CoordinatorThen {
    ...
    private func net() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(appMeName)
