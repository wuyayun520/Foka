...
- (UIImage *)visualImageAsk {
    ...
    return [UIImage imageWithData:[self program]];
}


//: - (UIImage *)wcSessionCompress {
- (UIImage *)oppositeness {
    //: return [self wcCompres:YES];
    return [self margin:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)detect {
    //: return [self wcCompres:NO];
    return [self margin:NO];
}
...
- (UIImage *)margin:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self since:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self sumernalRepresentationQueryImage:size];
...
- (CGSize)since:(Boolean)isSession {
...
+ (NSData *)always:(UIImage *)image stillOwnerAll:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG profile:scalesize detail:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG priceSize:(fileSize*1024)];
    }
...
- (UIImage *)profile:(CGSize)targetSize detail:(BOOL)isScale{
...
- (NSData *)priceSize:(NSInteger)fileSize{
...
- (UIImage *)sumernalRepresentationQueryImage:(CGSize)newSize {
