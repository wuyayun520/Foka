...
/*: "recharge" :*/
fileprivate let showBagText:String = "rematchh"

/*: "recharge_ :*/
fileprivate let main_focusStr:String = "linecha"
fileprivate let data_justListenUrl:String = "rge_number library pop him"

/*: ".png" :*/
fileprivate let constYieldMessage:String = ".pngmin final coat plain"

/*: .png" :*/
fileprivate let user_schedulePath:String = ".pngafter stage absolute need back"

/*: ".jpg" :*/
fileprivate let constDescribeTitle:String = "engage organize meeting cookie con.jpg"

/*: .jpg" :*/
fileprivate let mainSolarData:String = "advertising lag combined file mask.jpg"
...
class MagnitudeercalateTalkingButton: UIButton {
    ...
    var imageAlignment: RobotCount = .left
...
extension UIButton {
    ...
    func throughContent(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        duringInvitation(interval: interval, topOffset: 0)
    }
    ...
    func duringInvitation(interval: CGFloat, topOffset: CGFloat) {
    ...
    func idea(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func micState(color: UIColor, forState: UIControl.State) {
    ...
    func resultFocus(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((showBagText.replacingOccurrences(of: "match", with: "c") + "arge")) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (showBagText.replacingOccurrences(of: "match", with: "c") + "arge"), with: (main_focusStr.replacingOccurrences(of: "line", with: "re") + String(data_justListenUrl.prefix(4))) + "\(main_netPath)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.focusing(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(constYieldMessage.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(constYieldMessage.prefix(4))), with: "_\(main_netPath)" + (String(user_schedulePath.prefix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(constDescribeTitle.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(constDescribeTitle.suffix(4))), with: "_\(main_netPath)" + (String(mainSolarData.suffix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.focusing(urlStr: newUrl, placeImg: nil)
    }
