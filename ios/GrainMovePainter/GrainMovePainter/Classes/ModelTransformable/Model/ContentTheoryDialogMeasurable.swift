...
// __CLOSE_PRINT__
//
//  ContentTheoryDialogMeasurable.swift
//  AbroadTalking
//
...
struct ContentTheoryDialogMeasurable: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct GenreModelType: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum MomentWhoComparable: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct MyModelType: HandyJSON {
    ...
    var status: TelecastingContiguousBytes?
}

///
//: enum MomentVideoStatus: String {
enum TelecastingContiguousBytes: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension ContentTheoryDialogMeasurable {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func onPost(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(EchoReactiveCompatible.commonTo(text: content!, size: CGSize(width: dataShowDetailPopMessage - 54, height: CGFloat(MAXFLOAT)), font: UIFont.opPoint(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == MomentWhoComparable.Photo.rawValue || momentType == MomentWhoComparable.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
