...
fileprivate let mainLaterStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#4FAAFF" :*/
fileprivate let noti_operateUrl:[Character] = ["#","4","F","A","A","F"]
fileprivate let app_branchEnjoyRecommendUrl:[Character] = ["F"]

/*: "reply" :*/
fileprivate let kCapableName:[Character] = ["r","e","p","l","y"]

/*: "F5F5F5" :*/
fileprivate let constQuickMessage:[Character] = ["F","5","F","5","F","5"]
...
class OviformItemCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainLaterStr.reversed(), encoding: .utf8)!)
    }
...
extension OviformItemCell {
    //: @objc func onLongTapCell(sender: UIGestureRecognizer) {
    @objc func longSender(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension OviformItemCell {
    //: func configCell(model: TalkingCommentReplyModel) {
    func like(model: ThereReplyModel) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(noti_operateUrl) + String(app_branchEnjoyRecommendUrl)))
            ...
            let font = UIFont.promptReload(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (String(kCapableName)).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.appValueDetailColor(), UIColor.appTitleColor(), UIColor.appValueDetailColor(), UIColor.appTitleColor()]
                colorA = [UIColor.uniformColor(), UIColor.quitTo(), UIColor.uniformColor(), UIColor.quitTo()]
                ...
                colorA = [UIColor.uniformColor(), UIColor.quitTo()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func episode(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height _: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: dataShowDetailPopMessage - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (String(constQuickMessage)))?.cgColor
...
extension OviformItemCell {
    ...
    private func perCapitaIncome() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(longSender(sender:)))
