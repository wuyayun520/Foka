...
/*: "Cancel" :*/
fileprivate let kHereStageUrl:String = "double session grantCancel"

/*: "Post" :*/
fileprivate let k_readingStr:[Character] = ["P","o","s","t"]

/*: "Upload failed" :*/
fileprivate let noti_scaleForceTitle:String = "Upload dial ya report"
fileprivate let userSkipUrl:[Character] = ["f","a","i","l","e","d"]

/*: "imgUrl" :*/
fileprivate let userHoldTitle:[Character] = ["i","m","g","U","r"]
fileprivate let dataAvailableKey:[Character] = ["l"]

/*: "videoUrl" :*/
fileprivate let show_equityTitle:[Character] = ["v","i","d","e","o","U","r","l"]

/*: "content" :*/
fileprivate let data_beginTitle:[Character] = ["c","o","n","t","e","n","t"]

/*: "location" :*/
fileprivate let userConStr:[Character] = ["l","o","c"]
fileprivate let kConditionFormat:String = "atthinkn"

/*: "Video review in progress" :*/
fileprivate let show_modifyPath:String = "Video suspend harassment group just"
fileprivate let main_providerName:String = "w inapp effect filter"
fileprivate let user_platTitle:String = "search slim file holder pro"

/*: "Shoot video at least 5 seconds" :*/
fileprivate let k_prepareId:String = "average write suite agendaSho"
fileprivate let mainCreateFormatStr:[Character] = ["o","t"," ","v","i","d","e","o"," ","a","t"," ","l","e","a","s","t"," ","5"," ","s","e","c","o","n","d","s"]

/*: "Camera" :*/
fileprivate let appAddTitle:String = "customer lock launch favorite survivalCamera"

/*: "Photo" :*/
fileprivate let noti_nearContent:[Character] = ["P","h","o","t","o"]

/*: "Video" :*/
fileprivate let kComputeId:String = "Videoply guide good contact center"
...
protocol WhiteControllerDelegate: NSObjectProtocol {
    // 发布成功
    //: func ShareSucceed()
    func service()
}
...
class EpisodeObjectProtocol: BodyLatViewController {
    //: weak var delegate: TalkingFreeViewControllerDelegate?
    weak var delegate: WhiteControllerDelegate?
    ...
    private lazy var reqManager: DrawingRequestTool = //: return DrawingRequestTool()
        .init()
    //: }()
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(kHereStageUrl.suffix(6))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(holderObserve), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var postBtn: UIButton = {
        ...
        btn.setTitle((String(k_readingStr)).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.originInstance(colors: UIColor.o(), size: CGSize(width: 70, height: 30)), for: .normal)
    ...
    private lazy var collectionView: UnremarkablyCollectionView = {
        //: let collectionView = FreeCollectionView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        let collectionView = UnremarkablyCollectionView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        ...
        collectionView.deleteActionBlock = { [weak self] in
            ...
            if self.collectionView.imageArray.count == 0 {
                ...
                self.collectionView.contentType = MomentWhoComparable.nones
                //: self.videoPath = ""
                self.videoPath = ""
                //: self.coverPath = ""
                self.coverPath = ""
...
extension EpisodeObjectProtocol {
    /// 发布动态
    ...
    func remote() {
        ...
        reqManager.transfer(photos: self.collectionView.imageArray, content: content) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            ...
            NotificationCenter.default.post(name: userAccuracyToId, object: self, userInfo: nil)
        }
    }
    ...
    func listen() {
        ...
        ConferInsertListener.shared.whitehead(type: 1, coverPath: self.coverPath, videoPath: self.videoPath) { coverUrl, videoUrl, succeed in
            //: guard succeed else {
            guard succeed else {
                //: ProgressHUD.dismiss()
                DirectProgressHUD.thatDismiss()
                //: self.func__showStatusBarErrorMsg(showMsg: "Upload failed".localized)
                self.revenueMsg(showMsg: (String(noti_scaleForceTitle.prefix(7)) + String(userSkipUrl)).localized)
                //: return
                return
            }
            ...
            dict[(String(userHoldTitle) + String(dataAvailableKey))] = coverUrl
            //: dict["videoUrl"] = videoUrl
            dict[(String(show_equityTitle))] = videoUrl
            //: dict["content"] = content
            dict[(String(data_beginTitle))] = content
            //: dict["location"] = ""
            dict[(String(userConStr) + kConditionFormat.replacingOccurrences(of: "think", with: "io"))] = ""

            //: TalkingMomentRequestTool.releaseVideoMoment(params: dict) {[weak self] succeed, result, errorModel in
            DrawingRequestTool.document(params: dict) { [weak self] succeed, _, _ in
                //: ProgressHUD.dismiss()
                DirectProgressHUD.thatDismiss()
                //: guard let self = self else { return }
                guard let self = self else { return }
                ...
                self.pointRate(showMsg: (String(show_modifyPath.prefix(6)) + "revie" + String(main_providerName.prefix(4)) + String(user_platTitle.suffix(4)) + "gress").localized)
                //: self.navigationController?.popViewController(animated: true)
                self.navigationController?.popViewController(animated: true)
...
extension EpisodeObjectProtocol {
    ...
    func price() {
        ...
        LabelMpThen.commitBlock(true) { (isOpen: Bool) in
    ...
    func extendOneselfAlbum() {
        ...
        LabelMpThen.rangeDoing(true) { (isOpen: Bool) in
            ...
            let vc = SpendBassPickTool.systemVideo(maxCount: count, allowPhoto: true, allowVideo: false)
            ...
            vc.didFinishPickingVideoHandle = { [weak self] (_: UIImage?,
                    ...
                    SpendBassPickTool.low(asset: assets) { filePath in
                        ...
                        if seconds >= 5 {
                            ...
                            self.databaseRed(url: filePath!)
                            //: } else {
                        } else {
                            //: DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                                //: self.func__showStatusBarErrorMsg(showMsg: "Shoot video at least 5 seconds".localized)
                                self.revenueMsg(showMsg: (String(k_prepareId.suffix(3)) + String(mainCreateFormatStr)).localized)
                            }
                        }
                    }
            }
    ...
    func databaseRed(url: URL) {
        //: let videoVC = TalkingVideoEditorController(maxDuration: 60.0, videoURL: url) { videoInfo in
        let videoVC = PocketViewController(maxDuration: 60.0, videoURL: url) { videoInfo in
...
extension EpisodeObjectProtocol: UIImagePickerControllerDelegate & UINavigationControllerDelegate {
        ...
        self.dismiss(animated: true) {
            ...
            if mediaType == kUTTypeMovie as String {
                ...
                if seconds >= 5 {
                    ...
                    self.databaseRed(url: info[.mediaURL]! as! URL)
...
// MARK: - DrownViewDelegate

//: extension TalkingFreeViewController: FreeCollectionViewDelegate {
extension EpisodeObjectProtocol: DrownViewDelegate {
    /// 选择图片
    ...
    func provoke() {
        ...
        if self.collectionView.contentType == .Photo {
            ...
            vc.organiseList(cellTitleList: [(String(appAddTitle.suffix(6))).localized, (String(noti_nearContent)).localized])
            //: }else {
        } else {
            //: vc.initwithList(cellTitleList: ["Camera".localized, "Photo".localized, "Video".localized])
            vc.organiseList(cellTitleList: [(String(appAddTitle.suffix(6))).localized, (String(noti_nearContent)).localized, (String(kComputeId.prefix(5))).localized])
        }
    ...
    func scutWork(index: IndexPath) {
        //: let vc = TalkingMomentPhotosVC.init(images: self.collectionView.imageArray, index: index.row, type: .publish)
        let vc = ControlDataSource(images: self.collectionView.imageArray, index: index.row, type: .publish)
        // 删除按钮block
    ...
    func visualCommunication() {
        ...
        let vc = DialogCoordinatorViewController(videoPath: self.videoPath)
        //: vc.modalPresentationStyle = .fullScreen
        vc.modalPresentationStyle = .fullScreen
        //: self.present(vc, animated: true)
        self.present(vc, animated: true)
...
extension EpisodeObjectProtocol {
    ...
    func prompt() {
        ...
        cancelBtn.snp.makeConstraints { make in
            ...
            make.top.equalTo(dataEnvironmentId + 7)
        ...
        collectionView.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(show_turnStr)
