...
fileprivate let mainInstallationTitle:[UInt8] = [0x16,0x11,0x16,0xb,0x57,0x1c,0x10,0x1b,0x1a,0xd,0x45,0x56,0x5f,0x17,0x1e,0xc,0x5f,0x11,0x10,0xb,0x5f,0x1d,0x1a,0x1a,0x11,0x5f,0x16,0x12,0xf,0x13,0x1a,0x12,0x1a,0x11,0xb,0x1a,0x1b]

private func middleFemale(merely num: UInt8) -> UInt8 {
    return num ^ 127
}

/*: "Save successfully" :*/
fileprivate let user_providerName:[Character] = ["S","a","v","e"," ","s","u","c","c","e","s"]
fileprivate let main_starId:[Character] = ["s","f","u","l","l","y"]
...
class ControlDataSource: FluRecognizerDelegate {
    ...
    init(momentModel: ContentTheoryDialogMeasurable, index: Int, type: FieldPhotoType) {
        ...
        if LieLanguageManager.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(dataShowDetailPopMessage), y: 0)
    ...
    init(images: [UIImage], index: Int, type: FieldPhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainInstallationTitle.map{middleFemale(merely: $0)}, encoding: .utf8)!)
    }
...
//        }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: dataShowDetailPopMessage, height: dataViewPath)
        ...
        view.frame = CGRect(x: 0, y: 0, width: dataShowDetailPopMessage, height: dataViewPath)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(dataShowDetailPopMessage), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(dataShowDetailPopMessage), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(dataShowDetailPopMessage), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(HourViewDelegate.self, forCellWithReuseIdentifier: HourViewDelegate.className())
...
//    lazy var downloadBtn: MagnitudeercalateTalkingButton = {
//        let btn = MagnitudeercalateTalkingButton.init()
...
//        LabelMpThen.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: GenreModelType = self.currenModel.pic[self.pageIndex]
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.pointRate(showMsg: (String(user_providerName) + String(main_starId)).localized)
                }
                //: } else {
            } else {
...
extension ControlDataSource: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func doingeHalf() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func micView() {
        //: if  currenModel.sex==SteppingMotorThen.share.loginUserMode.sex {
        if currenModel.sex == SteppingMotorThen.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: HourViewDelegate = collectionView.dequeueReusableCell(withReuseIdentifier: HourViewDelegate.className(), for: indexPath) as! HourViewDelegate
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: GenreModelType = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.oldIn(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.ex(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.ex(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: HourViewDelegate = cell as! HourViewDelegate
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / dataShowDetailPopMessage)
        //: if myType == .normal {
        if myType == .normal {
