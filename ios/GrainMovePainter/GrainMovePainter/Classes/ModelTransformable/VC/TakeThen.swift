...
fileprivate let noti_normalContent:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "momentId" :*/
fileprivate let user_devoteName:[UInt8] = [0x9,0xb,0x9,0x1,0xa,0x10,0xe5,0x0]

fileprivate func clickFailure(relation num: UInt8) -> UInt8 {
...
/*: "lastId" :*/
fileprivate let notiAgentTitle:String = "small snap light temp soonlastId"

/*: "limit" :*/
fileprivate let app_carStr:[Character] = ["l","i","m","i","t"]

/*: "list" :*/
fileprivate let show_cookieName:String = "lijoint"

/*: "more>>" :*/
fileprivate let noti_additionKey:[Character] = ["m","o","r","e",">",">"]

/*: "Comment on success" :*/
fileprivate let k_countryContent:String = "mist correlation governCommen"
fileprivate let main_rocketId:String = "state storage portrait need fit success"

/*: "replyId" :*/
fileprivate let k_currencyContent:[Character] = ["r","e","p","l","y","I","d"]

/*: "type" :*/
fileprivate let const_cancelDoingeMsg:[UInt8] = [0xfd,0x2,0xf9,0xee]

fileprivate func grayEveryone(customer num: UInt8) -> UInt8 {
...
/*: "comment" :*/
fileprivate let main_recentId:[Character] = ["c","o","m","m","e","n","t"]

/*: "number" :*/
fileprivate let mainResolutionName:[UInt8] = [0x72,0x65,0x62,0x6d,0x75,0x6e]

/*: "Delete Post" :*/
fileprivate let appReplyPhaseFormat:String = "height enjoy comment beautifulDelete"

/*: "model" :*/
fileprivate let user_actionValue:[UInt8] = [0x6c,0x65,0x64,0x6f,0x6d]

/*: "The content cannot be empty!" :*/
fileprivate let userIllegalPath:String = "submit coat darkThe co"
fileprivate let app_partnerText:String = "disappear tendert ca"
fileprivate let appExtentData:String = "peer not space center drawing be "
fileprivate let user_readName:[Character] = ["e","m","p","t","y","!"]

/*: "content" :*/
fileprivate let main_browValue:[Character] = ["c","o","n","t","e"]
fileprivate let mainPackageId:[Character] = ["n","t"]

/*: "Delete" :*/
fileprivate let user_buttonFollowPath:String = "follow jarDelete"

/*: "Delete Comment?" :*/
fileprivate let showMixPath:[Character] = ["D","e","l","e","t","e"]
fileprivate let mainExampleId:[Character] = [" ","C","o","m","m","e","n","t","?"]

/*: "Cancel" :*/
fileprivate let const_dotFrameworkStr:String = "age perspective space fade instanceCancel"

/*: "Reply" :*/
fileprivate let const_ageName:[Character] = ["R","e","p","l","y"]

/*: "Details" :*/
fileprivate let notiHundredNumberryContent:String = "stage translationDetails"

/*: "icon_moment_nor_report" :*/
fileprivate let k_appropriateMessage:String = "spot front hi facility focusicon_m"
fileprivate let main_commentOurPath:String = "maybe see interested sound_nor_re"
fileprivate let showGoldLeastRawKey:String = "telephonert"

/*: "Comment" :*/
fileprivate let showActionKey:[Character] = ["C","o","m","m","e","n","t"]
...
class TakeThen: BodyLatViewController {
    ...
    init(model: ContentTheoryDialogMeasurable) {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_normalContent.reversed(), encoding: .utf8)!)
    }
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        (VersePowKingfisherCompatible.shared as! VersePowKingfisherCompatible).chatterbox(selectorString: notiBeginData)
    }

    //: lazy var mainTableView: UITableView = {
    lazy var mainTableView: UITableView = {
        //: let table = UITableView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight), style: UITableView.Style.grouped)
        let table = UITableView(frame: CGRect(x: 0, y: 0, width: dataShowDetailPopMessage, height: dataViewPath), style: UITableView.Style.grouped)
        ...
        table.register(OviformItemCell.self, forCellReuseIdentifier: OviformItemCell.className())
    ...
    lazy var comInputView: SatelliteShadowThen = {
        //: let text = TalkingCommentInputView.init()
        let text = SatelliteShadowThen()
        //: text.delegate = self
        text.delegate = self
        //: return text
        return text
    ...
    lazy var backBtn: UIButton = {
        ...
        btn.micState(color: .clear, forState: .normal)
        //: btn.addTarget( self, action: #selector(dismissClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(applyStar), for: .touchUpInside)
...
extension TakeThen {
    ...
    func first() {
        ...
        dict[String(bytes: user_devoteName.map{clickFailure(relation: $0)}, encoding: .utf8)!] = momentModel.mid

        //: TalkingMomentRequestTool.req_MomentDetail(params: dict) { succeed, result, errorModel in
        DrawingRequestTool.language(params: dict) { succeed, result, errorModel in

            //: if succeed {
            if succeed {
                //: if var model = JSONDeserializer<TalkingMomentModel>.deserializeFrom(dict: result as? Dictionary<String, Any>, designatedPath: nil) {
                if var model = JSONDeserializer<ContentTheoryDialogMeasurable>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                    ...
                    (VersePowKingfisherCompatible.shared as! VersePowKingfisherCompatible).provostMarshal(uid: model.uid ?? "", videoUrl: model.videoUrl, headPic: model.headPic ?? "")
                }
                //: } else {
            } else {
    ...
    func bitList() {
        ...
        dict[String(bytes: user_devoteName.map{clickFailure(relation: $0)}, encoding: .utf8)!] = momentModel.mid
        //: dict["lastId"] = lastId
        dict[(String(notiAgentTitle.suffix(6)))] = lastId
        //: dict["limit"] = limit
        dict[(String(app_carStr))] = limit
        //: ProgressHUD.show()
        DirectProgressHUD.discountShow()

        //: TalkingMomentRequestTool.req_CommentList(params: dict) { succeed, result, errorModel in
        DrawingRequestTool.completion(params: dict) { succeed, result, errorModel in
            //: ProgressHUD.dismiss()
            ...
            if succeed {
                ...
                let array: Array = dict[(show_cookieName.replacingOccurrences(of: "join", with: "s"))] as! [Any]
                ...
                for i in 0 ..< array.count {
                    ...
                    if var model = JSONDeserializer<TakeMeasurable>.deserializeFrom(dict: array[i] as? [String: Any], designatedPath: nil) {
                        //: model.caculateCommentItemHeight()
                        model.flatSolid()
                        //: var replyArray: [TalkingCommentReplyModel] = []
                        var replyArray: [ThereReplyModel] = []
                        ...
                        if model.totalReply > 3 {
                            ...
                            var replymodel = ThereReplyModel()
                            //: replymodel.allCountContent = String(format: "more>>")
                            replymodel.allCountContent = String(format: (String(noti_additionKey)))
                            //: replymodel.caculateReplyItemHeight()
                            replymodel.of()
                            //: replyArray.append(replymodel)
                            replyArray.append(replymodel)
    ...
    func country(params: [String: Any], index: Int) {
        //: ProgressHUD.show()
        DirectProgressHUD.discountShow()

        //: TalkingMomentRequestTool.req_SendComment(params: params) { [self] succeed, result, errorModel in
        DrawingRequestTool.misreckoning(params: params) { [self] succeed, result, _ in
            ...
            if succeed {
                ...
                self.pointRate(showMsg: (String(k_countryContent.suffix(6)) + "t on" + String(main_rocketId.suffix(8))).localized)
                //: if params["replyId"] != nil {
                if params[(String(k_currencyContent))] != nil {
                    //: var model = TalkingCommentModel()
                    var model = TakeMeasurable()
                    ...
                    if model.uid!.count > 0 {
                        ...
                        if var replyModel = JSONDeserializer<ThereReplyModel>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                            //: model.totalReply += 1
                            model.totalReply += 1
...
//                            if array.count>3 {
//                                array.removeLast()
//                                var tempModel = ThereReplyModel.init()
...
//                            }
                    ...
                    if var model = JSONDeserializer<TakeMeasurable>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                        //: model.caculateCommentItemHeight()
                        model.flatSolid()
                        //: if model.uid == SteppingMotorThen.share.loginUserMode.userID {
                        if model.uid == SteppingMotorThen.share.loginUserMode.userID {
                            //: model.headPicFrame = SteppingMotorThen.share.loginUserMode.headPicFrame
                            model.headPicFrame = SteppingMotorThen.share.loginUserMode.headPicFrame
                        }
                ...
                NotificationCenter.default.post(name: appWorkName, object: self, userInfo: [String(bytes: const_cancelDoingeMsg.map{grayEveryone(customer: $0)}, encoding: .utf8)!: (String(main_recentId)).localized, String(bytes: mainResolutionName.reversed(), encoding: .utf8)!: (self.momentModel.replyNum) ?? 0, String(bytes: user_devoteName.map{clickFailure(relation: $0)}, encoding: .utf8)!: self.momentModel.mid ?? ""])
            }
        }
    }
    ...
    func icon(cid: String, lastId: String, limit: Int, index: IndexPath) {
        //: ProgressHUD.show()
        DirectProgressHUD.discountShow()
        //: ProgressHUD.dismiss()
        DirectProgressHUD.thatDismiss()
        ...
        var dict = [String: Any]()
        //: dict["replyId"] = cid
        dict[(String(k_currencyContent))] = cid
        //: dict["lastId"] = lastId
        dict[(String(notiAgentTitle.suffix(6)))] = lastId
        //: dict["limit"] = limit
        dict[(String(app_carStr))] = limit
        ...
        DrawingRequestTool.logical(params: dict) { succeed, result, _ in
            //: ProgressHUD.dismiss()
            DirectProgressHUD.thatDismiss()
            //: if succeed {
            if succeed {
                ...
                for i in 0 ..< array.count {
                    ...
                    if var model = JSONDeserializer<ThereReplyModel>.deserializeFrom(dict: array[i] as? [String: Any], designatedPath: nil) {
...
extension TakeThen {
    ...
    func dayDraft() {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
            ...
            self.mainTableView.scrollRectToVisible(CGRect(x: 0, y: self.momentModel.itemHeight ?? 56, width: dataShowDetailPopMessage, height: dataViewPath - CGFloat(self.inputViewHeight) - k_depthUrl - show_turnStr), animated: true)
        }
    }

    //: @objc func BlockBtnClick() {
    @objc func brightIt() {
        //: if self.momentModel.uid == SteppingMotorThen.share.loginUserMode.userID {
        if self.momentModel.uid == SteppingMotorThen.share.loginUserMode.userID {
            //: let vc = TalkingMunuPopView.init(frame: self.view.frame)
            let vc = AttributePopView(frame: self.view.frame)
            //: vc.initwithList(cellTitleList: ["Delete Post".localized])
            vc.organiseList(cellTitleList: [(String(appReplyPhaseFormat.suffix(6)) + " Post").localized])
            ...
            vc.munuBlock = { [weak self] _, _ in
                ...
                DrawingRequestTool.res(params: dict) { succeed, _, _ in
                    //: ProgressHUD.dismiss()
                    DirectProgressHUD.thatDismiss()
                    //: if succeed {
                    if succeed {
                        ...
                        NotificationCenter.default.post(name: mainSucceedKey, object: self, userInfo: [String(bytes: user_actionValue.reversed(), encoding: .utf8)!: self.momentModel])
                    }
                }
            }
            ...
            let reportView = UnderViewDelegate(frame: UIScreen.main.bounds, type: .reportUserType, rId: momentModel.uid ?? "")
...
// MARK: - AllyStraddleThen

//: extension TalkingMomentDetailVC: CommentInputViewDelegate {
extension TakeThen: AllyStraddleThen {
    ...
    func gameRow(msgStr: String, row: IndexPath) {
        ...
        if msgStr.count == 0 {
            ...
            self.revenueMsg(showMsg: (String(userIllegalPath.suffix(6)) + "nten" + String(app_partnerText.suffix(4)) + "nnot" + String(appExtentData.suffix(4)) + String(user_readName)).localized)
        }
        ...
        dict[String(bytes: user_devoteName.map{clickFailure(relation: $0)}, encoding: .utf8)!] = momentModel.mid
        //: dict["content"] = msgStr
        dict[(String(main_browValue) + String(mainPackageId))] = msgStr
        ...
        dict[(String(k_currencyContent))] = replymodel.cid
        //: requestCommentWithParams(params: dict, index: row.section)
        country(params: dict, index: row.section)
    }
    ...
    func changed(heightToBottom: CGFloat) {
        ...
        if heightToBottom > 100 {
            ...
            backBtn.frame = CGRect(x: 0, y: 0, width: dataShowDetailPopMessage, height: self.view.frame.size.height - heightToBottom - inputViewHeight)
    ...
    func smart(height: CGFloat) {
        ...
        backBtn.frame = CGRect(x: 0, y: 0, width: dataShowDetailPopMessage, height: backBtn.frame.size.height - height)
        //: backBtn.isHidden = false
        backBtn.isHidden = false
    ...
    func two(cid: String) {
        ...
        if self.momentModel.uid == SteppingMotorThen.share.loginUserMode.userID {
            ...
            vc.organiseList(cellTitleList: [(String(user_buttonFollowPath.suffix(6))).localized])
            ...
            vc.munuBlock = { [weak self] _, _ in
                ...
                EnhanceAlertShow.burnBlock(title: (String(showMixPath) + String(mainExampleId)).localized, attributedMessage: NSMutableAttributedString(), leftBtnTitle: (String(const_dotFrameworkStr.suffix(6))).localized, rightBtnTitle: (String(user_buttonFollowPath.suffix(6))).localized) {
                    ...
                    DrawingRequestTool.reason(params: dict) { succeed, _, _ in
                        //: ProgressHUD.dismiss()
                        DirectProgressHUD.thatDismiss()
                        //: if succeed {
                        if succeed {
...
extension TakeThen: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = OviformItemCell.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingCommentReplyItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? OviformItemCell
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingCommentReplyItemCell(style: .default, reuseIdentifier: identifier)
            cell = OviformItemCell(style: .default, reuseIdentifier: identifier)
        }
        //: let model = self.commentList[indexPath.section]
        let model = self.commentList[indexPath.section]
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        let tcell: OviformItemCell = cell as! OviformItemCell
        //: tcell.shearTableViewSection(cell: cell, tableView: tableView, indexPath: indexPath as NSIndexPath, radius: 6, height: height)
        tcell.episode(cell: cell, tableView: tableView, indexPath: indexPath as NSIndexPath, radius: 6, height: height)
    }
    ...
    func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if replymodel.allCountContent == (String(noti_additionKey)) {
            ...
            icon(cid: model.cid!, lastId: lastmodel.cid ?? "", limit: 5, index: indexPath)
            //: } else {
        } else {
            //: self.comInputView.updatePlaceholder(holder: "Reply".localized + "\(replymodel.nickname)" + ":", index: 0, row: indexPath, type: 2)
            self.comInputView.reportType(holder: (String(const_ageName)).localized + "\(replymodel.nickname)" + ":", index: 0, row: indexPath, type: 2)
        }
    }
    ...
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let headeView = DrawingThen(style: .default, reuseIdentifier: nil)
        //: let model = self.commentList[section]
        let model = self.commentList[section]
        ...
        headeView.commentReplyBlock = { [weak self] nickname in
            ...
            self.comInputView.reportType(holder: (String(const_ageName)).localized + " \(nickname)" + ":", index: section, row: IndexPath(), type: 1)
        }
...
extension TakeThen {
    ...
    private func sameAcross() {
        //: self.view.backgroundColor = UIColor.white
        self.view.backgroundColor = UIColor.white
        //: self.title = "Details".localized
        self.title = (String(notiHundredNumberryContent.suffix(7))).localized
        ...
        if self.momentModel.uid == SteppingMotorThen.share.loginUserMode.userID {
            ...
            btn.setTitle((String(user_buttonFollowPath.suffix(6))).localized, for: .normal)
            ...
            btn.setImage(UIImage.hr(name: (String(k_appropriateMessage.suffix(6)) + "oment" + String(main_commentOurPath.suffix(7)) + showGoldLeastRawKey.replacingOccurrences(of: "telephone", with: "po"))), for: .normal)
        }
        //: btn.addTarget(self, action: #selector(BlockBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(brightIt), for: .touchUpInside)
    ...
    func rewardGroup() {
        ...
        let cell = DialogueViewCell(style: .default, reuseIdentifier: nil)
        //: cell.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: momentModel.itemHeight ?? 56 - 12 )
        cell.frame = CGRect(x: 0, y: 0, width: dataShowDetailPopMessage, height: momentModel.itemHeight ?? 56 - 12)
        //: cell.initwith(isListTableCell: false)
        cell.change(isListTableCell: false)
        //: cell.configCell(model: momentModel)
        cell.configShadeSimulationBlack(model: momentModel)
        //: cell.playVideo()
        ...
        if momentModel.replyNum! > 0 {
            ...
            commentNumberLabel.text = String(format: (String(showActionKey)).localized + " \(momentModel.replyNum!)")
            ...
            commentNumberLabel.snp.makeConstraints { make in
                ...
                make.size.equalTo(CGSize(width: dataShowDetailPopMessage, height: 22))
            }
