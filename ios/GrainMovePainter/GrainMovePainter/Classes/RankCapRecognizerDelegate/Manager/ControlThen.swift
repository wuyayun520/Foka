...
fileprivate let k_clothingPath:[UInt8] = [0x35,0x36,0x3a,0x38,0x2d,0x30,0x36,0x37,0x79,0x38,0x2c,0x2d,0x31,0x36,0x2b,0x30,0x23,0x38,0x2d,0x30,0x36,0x37,0xa,0x2d,0x38,0x2d,0x2c,0x2a,0x79,0x30,0x2a,0x79]

private func dayCandid(match num: UInt8) -> UInt8 {
    return num ^ 89
}

/*: "get location failed. error: :*/
fileprivate let noti_alreadyAssertMessage:String = "discount firm whether idea relationget l"
fileprivate let mainOldFormat:[Character] = ["f","a"]
fileprivate let mainTextMessage:String = "series goiled. "
...
enum WalkMagnitudeervalLiteral: Int {
    //: case firstEntry = 0
    case firstEntry = 0
...
class ControlThen: NSObject {
    ...
    var locationType: WalkMagnitudeervalLiteral?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: ControlThen?
    //: class func shared() -> TalkingLocationManager {
    class func outputAfter() -> ControlThen {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = ControlThen()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    func aboutTop(block: @escaping Locationblock) {
    ...
    func slab(block: @escaping Authblock) {
    ...
    func locationStackServiceCamera() -> Bool {
    ...
    func afterPhoneGift() -> WalkMagnitudeervalLiteral {
    ...
    func engage() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: k_clothingPath.map{dayCandid(match: $0)}, encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension ControlThen: CLLocationManagerDelegate {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(noti_alreadyAssertMessage.suffix(5)) + "ocation " + String(mainOldFormat) + String(mainTextMessage.suffix(6)) + "error:") + "\(error.localizedDescription)")
    }
}
