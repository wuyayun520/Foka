...
/*: "🌛 🌛 enter  :*/
fileprivate let kThinText:[Character] = ["🌛"," ","🌛"," ","e","n","t","e","r"," "]

/*: "🌛 🌛 deinit  :*/
fileprivate let app_theSpanName:String = "🌛 🌛 dpackage cut"
fileprivate let app_pageLowText:[Character] = ["e","i","n","i","t"," "]

/*: "nav_back_black_nor" :*/
fileprivate let main_restaurantPauseStr:[Character] = ["n","a","v","_","b","a","c","k"]
fileprivate let const_shrinkUrl:String = "_blagiving screen property reduce shall"
...
class BodyLatViewController: UIViewController {
    //: var hideNavi = false
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        printLog(message: (String(kThinText)) + "\(self)")
    }
    ...
    deinit {
        ...
        printLog(message: (String(app_theSpanName.prefix(5)) + String(app_pageLowText)) + "\(self)")
    }
}

//: extension TalkingBaseViewController: UIGestureRecognizerDelegate {
extension BodyLatViewController: UIGestureRecognizerDelegate {
    //: func addTapGestureRecognizer() {
    func yellowConvert() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(func__TapGestureRecognizer))
        let tap = UITapGestureRecognizer(target: self, action: #selector(gestureDark))
        //: tap.delegate = self
        tap.delegate = self
    ...
    @objc func gestureDark(_: UITapGestureRecognizer) {
...
extension BodyLatViewController {
    /// 侧滑返回手势开关【直播、音视频通话及后续页面不支持返回手势】
    /// - Parameter isOpen: 是否开启
    //: func popGesture(isOpen: Bool) {
    func blueBlindHome(isOpen: Bool) {
        //: var isOpen = isOpen
        var isOpen = isOpen
...
extension UIViewController {
    ...
    func applyReload() -> UIBarButtonItem {
        ...
        let item = UIBarButtonItem(image: UIImage.hr(name: (String(main_restaurantPauseStr) + String(const_shrinkUrl.prefix(4)) + "ck_nor")), style: .plain, target: self, action: #selector(myPlace))
