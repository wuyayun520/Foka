...
fileprivate let user_inputMsg:[UInt8] = [0xb6,0xb1,0xb6,0xab,0xf7,0xbc,0xb0,0xbb,0xba,0xad,0xe5,0xf6,0xff,0xb7,0xbe,0xac,0xff,0xb1,0xb0,0xab,0xff,0xbd,0xba,0xba,0xb1,0xff,0xb6,0xb2,0xaf,0xb3,0xba,0xb2,0xba,0xb1,0xab,0xba,0xbb]
...
enum ConferSwipeDirection: CaseIterable {
    //: case Left
    case Left
    //: case Right
    case Right
...
protocol KeepCapObjectProtocol: NSObjectProtocol {
    ///
...
class MixViewCell: UIView {
    ...
    var cell_delegate: KeepCapObjectProtocol?

    //: var model: TalkingMatchResultModel?
    var model: DrawingHandyJSON?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_inputMsg.map{$0^223}, encoding: .utf8)!)
    }
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(jabErase(pan:)))
        //: v.delegate = self
        v.delegate = self
        //: return v
        return v
...
extension MixViewCell: UIGestureRecognizerDelegate {
    ...
    @objc func jabErase(pan: UIPanGestureRecognizer) {
        //: switch pan.state {
        switch pan.state {
        //: case .began:
        case .began:
        ...
        case .changed:
            ...
            if LieLanguageManager.shared.direction == .rightToLeft {
                ...
                if movePoint.x > dataShowDetailPopMessage / 4 {
                    ...
                    self.cell_delegate?.pastView(cell: self, forMovePoint: self.currentPoint)
    ...
    func ditSuperview() {
        ...
        self.cell_delegate?.subjugate(cell: self)
    }

    /// 还原卡片位置
    //: func restoreCellLocation() {
    func locationRefreshCell() {
    ...
    func labelTranslation(direction: ConferSwipeDirection) {
        //: switch (direction) {
        switch direction {
        //: case .Left:
        case .Left:
