...
fileprivate let app_distanceStr:[UInt8] = [0x12,0x15,0x12,0xf,0x53,0x18,0x14,0x1f,0x1e,0x9,0x41,0x52,0x5b,0x13,0x1a,0x8,0x5b,0x15,0x14,0xf,0x5b,0x19,0x1e,0x1e,0x15,0x5b,0x12,0x16,0xb,0x17,0x1e,0x16,0x1e,0x15,0xf,0x1e,0x1f]

private func complyDisc(first num: UInt8) -> UInt8 {
    return num ^ 123
}

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let app_tradeName:[Character] = ["b","t","n","J","Q","b","E","A","B","_","e","t"]
fileprivate let user_styleName:String = "ihw_answer patron system board confirm"
fileprivate let noti_answerIconUrl:String = "SHADOW"

/*: "Edit" :*/
fileprivate let kExtentValue:[Character] = ["E","d","i","t"]

/*: "Finish" :*/
fileprivate let noti_monthDefinitionMsg:String = "Finishroll finite develop record"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let user_secondTitle:String = "former ok device short limitedbtnYD"
fileprivate let show_refStr:String = "outer entityo2AS_o"
fileprivate let mainPauseFormat:String = "_playbusiness restaurant yellow provision"

/*: "Cancel" :*/
fileprivate let app_combinedStr:[Character] = ["C","a","n","c","e","l"]

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let app_writtenName:[Character] = ["b","g","Q","k","R","T","b","F","_","o","e","d","i","v","_"]
fileprivate let kOrangeName:String = "TOAST"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let main_stretchFormat:[UInt8] = [0x9d,0xa1,0xac,0xe9,0xbf,0xa0,0xad,0xac,0xa6,0xe9,0xa5,0xac,0xa7,0xae,0xbd,0xa1,0xe9,0xba,0xa1,0xa6,0xbc,0xa5,0xad,0xe9,0xab,0xac,0xe9,0xec,0x89,0xe4,0xec,0x89,0xe9,0xba,0xac,0xaa,0xa6,0xa7,0xad,0xba,0xe7,0xe9,0x99,0xa5,0xac,0xa8,0xba,0xac,0xe9,0xa0,0xa7,0xbd,0xac,0xbb,0xaa,0xac,0xb9,0xbd,0xe9,0xa0,0xbd]

private func triggerTare(now num: UInt8) -> UInt8 {
    return num ^ 201
}
...
typealias VideoEditBlock = (_ videoInfo: ExpressionMeasurable?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let constResolutionThirdData: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class PocketViewController: BodyLatViewController {
    ...
    private var slider: CoordinatorThen?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = ExpressionMeasurable() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = GenEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.isPage == true {
        if HemSocketManager.shared.isPage == true { // 正在音视频通话，静音播放
    ...
    init(style: GenEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (dataShowDetailPopMessage - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_distanceStr.map{complyDisc(first: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.hr(name: (String(app_tradeName) + String(user_styleName.prefix(4)) + "kcab_" + noti_answerIconUrl.lowercased())), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = LieLanguageManager.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(tick), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(kExtentValue)).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.nameure(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.promptReload(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = LieLanguageManager.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(noti_monthDefinitionMsg.prefix(6))).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.platformDisk(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.nameure()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(streamAction), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.hr(name: (String(user_secondTitle.suffix(5)) + String(show_refStr.suffix(6)) + "ediv" + String(mainPauseFormat.prefix(5)))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(capriole(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(app_combinedStr)).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(fruitage), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.hr(name: (String(app_writtenName) + kOrangeName.lowercased())))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: main_stretchFormat.map{triggerTare(now: $0)}, encoding: .utf8)!.publicTransport(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: HemReactiveCompatible = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = HemReactiveCompatible(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension PocketViewController {
    ...
    @objc func tick() {
        ...
        ConferInsertListener.shared.tutorialBorder()
    ...
    private func enablece(animated _: Bool = true) {
    ...
    @objc func qualityInmate() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: dataShowDetailPopMessage - 2 * 26, height: dataViewPath - 46 - 127 - k_depthUrl)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = dataShowDetailPopMessage / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: dataViewPath, width: dataShowDetailPopMessage, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = CoordinatorThen(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: constResolutionThirdData)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - k_depthUrl)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: SpecialTalkingSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func streamAction() {
        ...
        if self.videoEditHandler != nil {
            ...
            ConferInsertListener.shared.active(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.enablece()
    ...
    @objc func capriole(sender: UIButton) {
...
extension PocketViewController {
    ...
    @objc private func packageToTalking() {
        //: self.player?.isMuted = true
        self.player?.isMuted = true
    }
}
...
extension PocketViewController {
    ...
    private func pursuing() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(smoulderTranslate), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.incident(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func postOf() {
        ...
        self.topBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(show_turnStr)
        }
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + k_depthUrl)
        }
    ...
    private func childMenu() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(packageToTalking),
