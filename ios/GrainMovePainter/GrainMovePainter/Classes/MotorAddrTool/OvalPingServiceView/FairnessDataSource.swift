...
/*: "Select a date" :*/
fileprivate let show_sweetData:String = "strength scene new later incomeSelect a"
fileprivate let notiFailureValue:String = " datealong sharp"
...
protocol OvateObjectProtocol: NSObjectProtocol {
...
class FairnessDataSource: TotalernationaliseReactiveCompatible, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: OvateObjectProtocol?
    ...
    override func atWrite() {
        ...
        titleString = (String(show_sweetData.suffix(8)) + String(notiFailureValue.prefix(5))).localized
        ...
        fresh(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func fresh(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func re(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.vidDay(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.vidDay(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return decideMonth(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func decideMonth(year: Int, month: Int) -> Int {
