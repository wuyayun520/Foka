...
/*: "Finish" :*/
fileprivate let data_letterMsg:String = "dignity index domain alter accountingFinis"
fileprivate let const_priorityKey:String = "coal"

/*: "Cancel" :*/
fileprivate let kBrowRemoteValue:[Character] = ["C","a","n","c","e","l"]

/*: "btn_photo_choice_pre" :*/
fileprivate let user_pointAbleName:String = "btn_phanima manage root yet"
fileprivate let main_openingId:String = "page clothing violation dismisshoice_pre"

/*: "btn_photo_add_nor" :*/
fileprivate let const_lightMessage:String = "back eastern selected bit joinbtn_p"
fileprivate let show_wholePath:String = "dd_norhealthy continue"

/*: "/tmp" :*/
fileprivate let showRentMessage:String = "/tmpuntil announcement tap player learn"

/*: "Export session failed" :*/
fileprivate let showPlatformKey:[Character] = ["E","x","p","o","r","t"," ","s","e","s","s","i","o","n"," ","f","a","i","l","e"]
fileprivate let app_disableKey:[Character] = ["d"]

/*: "Export canceled" :*/
fileprivate let show_towardPath:String = "record passage comprise home returnExpor"
fileprivate let user_squareMessage:[Character] = ["n"]
fileprivate let user_bodyId:[Character] = ["c","e","l","e","d"]

/*: "Successful!" :*/
fileprivate let appAdminChinMysteryText:String = "Successnear production following round inmate"
fileprivate let noti_alreadyValue:String = "ful!mine ta"

/*: "Unsupported video formats" :*/
fileprivate let const_injuryMessage:String = "define sure container top incomeUnsup"
fileprivate let showOpData:String = "ed vinner within record wave think"
fileprivate let main_itMsg:String = "ideo fpast house rule"

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let showRedUrl:String = "leading draft smooth arrow countryyyyy-M"
fileprivate let app_allegedId:String = "successfully valid certain living focusH:mm:"

/*: "/tmp/video- :*/
fileprivate let show_fullValue:String = "approach successfully/tmp/vid"
fileprivate let mainGiftSpaceData:[Character] = ["e","o","-"]

/*: .mp4" :*/
fileprivate let appWaitData:[Character] = [".","m","p","4"]
...
class SpendBassPickTool: NSObject {
    ...
    class func systemVideo(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = dataShowDetailPopMessage * 488 / 375
            //: if needCircleCrop == true {
            if needCircleCrop == true { // 切圆图
                //: height = ScreenWidth
                height = dataShowDetailPopMessage
            }
            //: imagePicker?.cropRect = CGRect(x: 0, y: (ScreenHeight-height)/2, width: ScreenWidth, height: height)
            imagePicker?.cropRect = CGRect(x: 0, y: (dataViewPath - height) / 2, width: dataShowDetailPopMessage, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        imagePicker?.doneBtnTitleStr = (String(data_letterMsg.suffix(5)) + const_priorityKey.replacingOccurrences(of: "coal", with: "h")).localized
        //: imagePicker?.cancelBtnTitleStr = "Cancel".localized
        imagePicker?.cancelBtnTitleStr = (String(kBrowRemoteValue)).localized

        //: imagePicker?.barItemTextFont = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.opPoint(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.tillAdvancedColor()
        ...
        imagePicker?.naviTitleFont = UIFont.opPoint(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.quitTo()
        ...
        imagePicker?.iconThemeColor = UIColor.nameure()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.BundleImageNamed(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.hr(name: (String(user_pointAbleName.prefix(6)) + "oto_c" + String(main_openingId.suffix(9))))
        //: imagePicker?.takePictureImage = UIImage.BundleImageNamed(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.hr(name: (String(const_lightMessage.suffix(5)) + "hoto_a" + String(show_wholePath.prefix(6))))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                doneButton?.titleLabel?.font = UIFont.opPoint(type: .Medium, fontSize: 14)
                ...
                if text.count > 0 && Int(text)! > 0 {
                    ...
                    doneButton?.setTitle((String(data_letterMsg.suffix(5)) + const_priorityKey.replacingOccurrences(of: "coal", with: "h")).localized + "(\(text))", for: .normal)
                    //: } else {
                } else {
                    //: doneButton?.setTitle("Finish".localized, for: .normal)
                    doneButton?.setTitle((String(data_letterMsg.suffix(5)) + const_priorityKey.replacingOccurrences(of: "coal", with: "h")).localized, for: .normal)
                }
                //: return
        }
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.nameure()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: dataShowDetailPopMessage - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func low(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: asset!, options: options) { (asset: AVAsset?, _: AVAudioMix?, _) in
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                if !FileManager.default.fileExists(atPath: NSHomeDirectory().appending((String(showRentMessage.prefix(4))))) {
                    //: do {
                    do {
                        //: try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending("/tmp"), withIntermediateDirectories: true)
                        try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending((String(showRentMessage.prefix(4)))), withIntermediateDirectories: true)
                        //: } catch {
                    } catch {}
                }
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (String(showPlatformKey) + String(app_disableKey)))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(show_towardPath.suffix(5)) + "t ca" + String(user_squareMessage) + String(user_bodyId)))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(appAdminChinMysteryText.prefix(7)) + String(noti_alreadyValue.prefix(4))))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(const_injuryMessage.suffix(5)) + "port" + String(showOpData.prefix(4)) + String(main_itMsg.prefix(6)) + "ormats"))
            }
        }
    }
    ...
    private class func platformPath() -> String {
        //: let formater = DateFormatter()
        let formater = DateFormatter()
        //: formater.dateFormat = "yyyy-MM-dd-HH:mm:ss-SSS"
        formater.dateFormat = (String(showRedUrl.suffix(6)) + "M-dd-H" + String(app_allegedId.suffix(5)) + "ss-SSS")
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(show_fullValue.suffix(8)) + String(mainGiftSpaceData)) + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000))" + (String(appWaitData)))
        //: return outputPath
        return outputPath
    }
}
