...
fileprivate let dataUponMustUrl:[UInt8] = [0xad,0xaa,0xad,0xb0,0xec,0xa7,0xab,0xa0,0xa1,0xb6,0xfe,0xed,0xe4,0xac,0xa5,0xb7,0xe4,0xaa,0xab,0xb0,0xe4,0xa6,0xa1,0xa1,0xaa,0xe4,0xad,0xa9,0xb4,0xa8,0xa1,0xa9,0xa1,0xaa,0xb0,0xa1,0xa0]

private func restoreMeasurement(calculation num: UInt8) -> UInt8 {
    return num ^ 196
}

/*: "FFD855" :*/
fileprivate let appSpeedAverageId:[Character] = ["F","F","D","8","5","5"]
...
class WhiteThen: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - main_closedReasonMsg
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: main_closedReasonMsg,
                ...
                self.beanX(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataUponMustUrl.map{restoreMeasurement(calculation: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (String(appSpeedAverageId)))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: appMeName, y: 0, width: main_closedReasonMsg, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - main_closedReasonMsg, y: 0, width: main_closedReasonMsg, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.detergentSaveer(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: appMeName, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.detergentSaveer(0, 0, 0, 0.6)
...
extension WhiteThen {
    ...
    func pollex() {
        ...
        self.leftSilder.frame = CGRect(x: appMeName, y: 0, width: main_closedReasonMsg, height: self.frame.size.height)
        //: self.rightSlider.frame = CGRect(x: self.frame.size.width - kSliderWidth, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.rightSlider.frame = CGRect(x: self.frame.size.width - main_closedReasonMsg, y: 0, width: main_closedReasonMsg, height: self.frame.size.height)
        //: resetIndicator()
        closeSort()
    }

    //: func needShowIndicator(isShow: Bool) {
    func keyFor(isShow: Bool) {
        //: self.indicatorView.isHidden = !isShow
        self.indicatorView.isHidden = !isShow
    }

    //: func refreshSliderValidScope(startX: CGFloat) {
    func beanX(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - main_closedReasonMsg,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - main_closedReasonMsg)
    }
}
...
extension WhiteThen {
    ...
    @objc func decideGesture(gesture: UIPanGestureRecognizer) {
    ...
    @objc func casementWindow(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension WhiteThen {
    ...
    private func setProduct() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(decideGesture(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(casementWindow(gesture:)))
    ...
    func incident(_ xposition: CGFloat) {
