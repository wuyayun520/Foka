...
/*: "http://stat. :*/
fileprivate let constCostPath:String = "hshouldp"
fileprivate let k_passageMessage:String = "be former foundation representationtat."

/*: .com" :*/
fileprivate let showQuickData:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let user_maxData:String = "import minute minhttp://"

/*: "https://" :*/
fileprivate let appSpeakerUrl:[Character] = ["h","t","t","p","s",":"]
fileprivate let appWeeFormat:String = "positposit"

/*: "http://m. :*/
fileprivate let userEngageTitle:String = "mighttt"
fileprivate let kScienceMessage:String = "person package carry try hep://m."

/*: "http:// :*/
fileprivate let mainPicTurnData:[Character] = ["h","t","t","p",":"]
fileprivate let noti_secretPath:String = "//"

/*: "3,90" :*/
fileprivate let constAppMessage:String = "3,exactly0"

/*: "1990-01-01" :*/
fileprivate let userHappyNowUrl:String = "1990"
fileprivate let k_braceStr:String = "1"

/*: "MM-dd-yyyy" :*/
fileprivate let appJudgeName:String = "identical obtain channelMM-dd"
...
class MpRequestModel: NSObject {
    ...
    var requestType: UnderRequestType?
    ...
    override init() {
        ...
        self.requestServer = MotorAddrTool.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func noticeJust() -> Bool {
...
struct FireguardHandyJSON: HandyJSON {
    //: var errno: Int!
...
public struct RedErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct ConversionMeasurable: HandyJSON {
    ...
    var CSConfig = DrownHandyJSON() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (constCostPath.replacingOccurrences(of: "should", with: "tt") + "://s" + String(k_passageMessage.suffix(4))) + "\(notiNetworkTitle)" + (String(showQuickData))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(user_maxData.suffix(7)))) && !statDomain.hasPrefix((String(appSpeakerUrl) + appWeeFormat.replacingOccurrences(of: "posit", with: "/"))) {
                //: return "http://".appending(statDomain)
                return (String(user_maxData.suffix(7))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (userEngageTitle.replacingOccurrences(of: "might", with: "h") + String(kScienceMessage.suffix(6))) + "\(notiNetworkTitle)" + (String(showQuickData))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(mainPicTurnData) + noti_secretPath.capitalized) + "\(urlDomain)"
        }
    }
...
public class BotModelType: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [NoticeSinceMeasurable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [NoticeSinceMeasurable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [NoticeSinceMeasurable] = [] // 语音收费设置
    ...
    var vipSetting = [ElectricMotorSettingModel]() // vip皮肤
    //: var gameShowBit = 0
    ...
    var tabPageBanner = [MpTransformable]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = HugeModelType() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class EchoModelType: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [LieMeasurable]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [LieMeasurable]?
    ...
    var gallery: [ControlGalleryModel]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (constAppMessage.replacingOccurrences(of: "exactly", with: "9")) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.lifeStory(self.birthday ?? (userHappyNowUrl.capitalized + "-01-0" + k_braceStr.capitalized), dateFormat: (String(appJudgeName.suffix(5)) + "-yyyy"))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.speakerComponents().year else { return "0" }
...
struct ControlGalleryModel: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct LieMeasurable: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func noctilucent(compareTo: LieMeasurable) -> Bool {
        //: return
        return
...
struct PocketHandyJSON: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [DrownTransformable]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [DrownTransformable]!
}

//: struct UserTagTypeModel: HandyJSON {
struct DrownTransformable: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [LieMeasurable]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct DrownHandyJSON: HandyJSON {
...
struct ElectricMotorSettingModel: HandyJSON {
...
struct HugeModelType: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
