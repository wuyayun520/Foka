...
/*: "RSA加密失败，str:  :*/
fileprivate let show_ownerGivingSessionKey:[Character] = ["R","S","A","\u{52a0}","\u{5bc6}"]
fileprivate let appAdditionData:[Character] = ["失","\u{8d25}","，"]
fileprivate let main_sharedPickName:String = "str: media just"

/*: "weixin://" :*/
fileprivate let mainAdminShareMessage:String = "WEIXI"

/*: "dingtalk://" :*/
fileprivate let showUtilityName:String = "ditransitiontal"
fileprivate let noti_voiceKey:[Character] = ["k",":","/","/"]

/*: "lark://" :*/
fileprivate let main_suggestStr:String = "team"
fileprivate let main_edgeBookTitle:[Character] = ["a","r","k",":","/","/"]

/*: "wxwork://" :*/
fileprivate let main_norMessage:[Character] = ["w","x","w"]
fileprivate let kGiftLanguageMessage:String = "ork://mask closed fill data"

/*: "deviceId" :*/
fileprivate let const_devoteTrustName:[UInt8] = [0x3a,0x3b,0x28,0x37,0x3d,0x3b,0x17,0x3a]

/*: "weixin" :*/
fileprivate let app_communicateTitle:String = "visitorixin"

/*: "wxwork" :*/
fileprivate let user_mineUrl:String = "wxwomentallyk"

/*: "dingtalk" :*/
fileprivate let notiBetterMsg:String = "dingtbuildlk"

/*: "lark" :*/
fileprivate let appHeartData:[Character] = ["l","a","r","k"]
...
extension UIDevice {
    ...
    class func wellspring(IdentifierBlock: @escaping (_ str: String) -> Void) {
        ...
        if #available(iOS 14.0, *) {
            ...
            ATTrackingManager.requestTrackingAuthorization(completionHandler: { status in
                ...
                if status == .authorized {
                    ...
                    IdentifierBlock(UIDevice.pastLimit())
                    //: } else {
                } else {
                    //: IdentifierBlock("")
                    IdentifierBlock("")
...
//            let mnc = carrier?.mobileNetworkCode  //移动网络代码
    ...
    static func costStr(str: String) -> String {
        //: var encryptedStr = ""
        var encryptedStr = ""
        //: do {
        do {
            ...
            printLog(message: (String(show_ownerGivingSessionKey) + String(appAdditionData) + String(main_sharedPickName.prefix(5))) + "\(str).")
        }
        //: return encryptedStr
        return encryptedStr
    }
...
public extension UIDevice {
    ...
    static var isOpenWX: Bool {
        ...
        let appSchema = (mainAdminShareMessage.lowercased() + "n://")
    ...
    static var isOpenDD: Bool {
        ...
        let appSchema = (showUtilityName.replacingOccurrences(of: "transition", with: "ng") + String(noti_voiceKey))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenFS: Bool {
        //: let appSchema = "lark://"
        let appSchema = (main_suggestStr.replacingOccurrences(of: "team", with: "l") + String(main_edgeBookTitle))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenWXWork: Bool {
        ...
        let appSchema = (String(main_norMessage) + String(kGiftLanguageMessage.prefix(6)))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var customDeviceNum: String {
        ...
        if let value = key.get(kRecordValue) {
            //: return value
            return value
            //: }else{
        } else {
            ...
            key.set(value, forKey: kRecordValue)
            //: return value
            return value
        }
    }
    ...
    static var deviceInfoDic: [String: Any] {
        ...
        let usedVpn = MotorAddrTool.attractiveness() || MotorAddrTool.basicGravity()
        ...
        if let paramsStr = String(data: data!, encoding: String.Encoding.utf8) {
            ...
            let deviceIdStr = costStr(str: adid)
            //: return ["deviceId": deviceIdStr,
            return [String(bytes: const_devoteTrustName.map{$0^94}, encoding: .utf8)!: deviceIdStr,
                    //: "p0": paramsStr]
                    "p0": paramsStr]
        }
        //: return [:]
    ...
    static var getApps: String? {
        ...
        if UIDevice.isOpenWX {
            ...
            appsArr.append((app_communicateTitle.replacingOccurrences(of: "visitor", with: "we")))
        }
        //: if UIDevice.isOpenWXWork {
        if UIDevice.isOpenWXWork {
            //: appsArr.append("wxwork")
            appsArr.append((user_mineUrl.replacingOccurrences(of: "mentally", with: "r")))
        }
        //: if UIDevice.isOpenDD {
        if UIDevice.isOpenDD {
            //: appsArr.append("dingtalk")
            appsArr.append((notiBetterMsg.replacingOccurrences(of: "build", with: "a")))
        }
        //: if UIDevice.isOpenFS {
        if UIDevice.isOpenFS {
            //: appsArr.append("lark")
            appsArr.append((String(appHeartData)))
        }
        //: if appsArr.count > 0 {
        if appsArr.count > 0 {
