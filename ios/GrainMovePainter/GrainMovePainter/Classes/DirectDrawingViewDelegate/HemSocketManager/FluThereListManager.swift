...
/*: "uid" :*/
fileprivate let noti_exceptionData:String = "disappeard"

/*: "onlineStatus" :*/
fileprivate let app_analysisPath:String = "ONLINE"
fileprivate let userTransmissionContent:[Character] = ["S","t","a","t","u","s"]

/*: "isNewUser" :*/
fileprivate let data_libraryMessage:[Character] = ["i","s","N","e","w","U","s","e","r"]

/*: "isTPAuth" :*/
fileprivate let notiDismissTitle:String = "isTPreturn awake rug"

/*: "totalIntimate" :*/
fileprivate let mainUpperTitle:String = "TOTAL"
fileprivate let data_anotherTumbleText:String = "exposure tick cancelIntimate"

/*: "userStatus" :*/
fileprivate let mainGenderFormat:String = "usecut"
fileprivate let constZoneValue:[Character] = ["S","t","a","t","u","s"]

/*: "intimate" :*/
fileprivate let notiDirectMsg:String = "intimchoose"
fileprivate let user_riceContent:[Character] = ["t","e"]
...
enum UnifyKeyRepresentable: Int {
    //: case offical  = 0
    case offical = 0 // 官方
    //: case chatRoom = 1
    case chatRoom = 1 // 聊天室
...
class FluThereListManager {
    ...
    private var moreMsgModel: SpendConversationModel = //: return SpendConversationModel.getPlaceholderModel(chatType: .moreMsg)
        .income(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func informationSucceedBlock() {
        //: self.officialArr = FoodV2Listener.shared.getOfficialList()
        self.officialArr = FoodV2Listener.shared.activity()
        //: self.topArr = FoodV2Listener.shared.topConvList
        self.topArr = FoodV2Listener.shared.topConvList
        //: self.normalArr = FoodV2Listener.shared.norConvList
        self.normalArr = FoodV2Listener.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        mileage()
    ...
    func objectTa(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: FoodV2Listener.shared.getChatRoomData { succeed in
        FoodV2Listener.shared.panReceive { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = FoodV2Listener.shared.chatRoomList
                self.chatRoomArr = FoodV2Listener.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func petite(indexPath: IndexPath) -> SpendConversationModel? {
    ...
    func blindLanguage(model: SpendConversationModel?) -> SpendConversationModel? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: ComprehendReactiveCompatible? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: ComprehendReactiveCompatible? = CookieInfoManager.dealMp(targetId: targetId)
    ...
    func yesterday(conversationList: [SpendConversationModel], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        CookieInfoManager.jab(with: tempArr) { _, error in
    ...
    func editHistory(conversationModel: SpendConversationModel) {
    ...
    func countryIn(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: FoodV2Listener.shared.func__getConversationList { arr, leftMore in
        FoodV2Listener.shared.space { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [SpendConversationModel] = //: return Array<SpendConversationModel>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [SpendConversationModel] = //: return Array<SpendConversationModel>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [SpendConversationModel] = //: return Array<SpendConversationModel>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [SpendConversationModel] = //: return Array<SpendConversationModel>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [SpendConversationModel] = //: return Array<SpendConversationModel>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [SpendConversationModel] = //: return Array<SpendConversationModel>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [SpendConversationModel] = //: return Array<SpendConversationModel>()
        .init()
    //: }()
}
...
extension FluThereListManager {
    ...
    func mileage() {
        //: self.moreMsgArr = FoodV2Listener.shared.moreMsgList
        self.moreMsgArr = FoodV2Listener.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func shouldReply(indexPath: IndexPath) -> SpendConversationModel? {
    ...
    func counselling() -> SpendConversationModel {
        // 更新更多消息最新数据
        //: req_moreMsgInitData()
        mileage()
        ...
        moreMsgModel.unreadCount = noticeRequire()
...
extension FluThereListManager {
    ...
    func discharge(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        CookieInfoManager.noteLimit(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(noti_exceptionData.replacingOccurrences(of: "disappear", with: "ui"))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(app_analysisPath.lowercased() + String(userTransmissionContent))].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(data_libraryMessage))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(notiDismissTitle.prefix(4)) + "Auth")].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(mainUpperTitle.lowercased() + String(data_anotherTumbleText.suffix(8)))].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(mainGenderFormat.replacingOccurrences(of: "cut", with: "r") + String(constZoneValue))].int ?? 1
                    ...
                    self.somebody(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func placeEquity(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(noti_exceptionData.replacingOccurrences(of: "disappear", with: "ui"))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(app_analysisPath.lowercased() + String(userTransmissionContent))] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(data_libraryMessage))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(mainGenderFormat.replacingOccurrences(of: "cut", with: "r") + String(constZoneValue))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((mainGenderFormat.replacingOccurrences(of: "cut", with: "r") + String(constZoneValue))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension FluThereListManager {
    ...
    func everyInstance() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in FoodV2Listener.shared.norConvList {
        for model in FoodV2Listener.shared.norConvList {
            //: if model.totalIntimate >= SteppingMotorThen.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= SteppingMotorThen.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.blindLanguage(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in FoodV2Listener.shared.topConvList {
        for model in FoodV2Listener.shared.topConvList {
            //: if model.totalIntimate >= SteppingMotorThen.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= SteppingMotorThen.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func quantityimate(convModel: SpendConversationModel, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func trisodiumOrthophosphate(indexPath: IndexPath) -> SpendConversationModel? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func somebody(uid: String, intimate: Int) {
        ...
        CookieInfoManager.via(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func smallScaleInfo(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(noti_exceptionData.replacingOccurrences(of: "disappear", with: "ui"))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(notiDirectMsg.replacingOccurrences(of: "choose", with: "a") + String(user_riceContent))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
