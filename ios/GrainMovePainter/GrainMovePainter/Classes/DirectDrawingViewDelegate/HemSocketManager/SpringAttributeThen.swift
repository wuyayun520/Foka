...
fileprivate let app_denseAspectId:[UInt8] = [0x3e,0x39,0x3e,0x23,0x7f,0x34,0x38,0x33,0x32,0x25,0x6d,0x7e,0x77,0x3f,0x36,0x24,0x77,0x39,0x38,0x23,0x77,0x35,0x32,0x32,0x39,0x77,0x3e,0x3a,0x27,0x3b,0x32,0x3a,0x32,0x39,0x23,0x32,0x33]

private func weekMaybe(go num: UInt8) -> UInt8 {
    return num ^ 87
}

/*: "SHBDropMenuCell" :*/
fileprivate let k_advancedFormat:[Character] = ["S","H","B","D","r","o","p","M","e","n","u","C"]
fileprivate let data_hearingId:String = "esupportive"

/*: "title" :*/
fileprivate let mainSecretCurrentTitle:[Character] = ["t","i","t","l","e"]
...
protocol FluAlongsideViewDelegate {
...
public class SpringAttributeThen: UIView {
    //: var menuDelegate: DropDownMenuViewDelegate?
    var menuDelegate: FluAlongsideViewDelegate?
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.WordsConfig.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.CommentCap.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.CommentCap.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.CommentCap.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? VerseMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
    ...
    public class func quoteTheme(anchorView: UIView, titleArray: [String], imageArray: [UIImage]) -> SpringAttributeThen {
        ...
        let menuView = pullThroughSpectrum(anchorView: anchorView, menuArray: menuArray)
        //: return menuView
        return menuView
    }

    //: public class func pullDropDrownMenu(anchorView: UIView, menuArray: [DropMenuModel]) -> DropDownMenuView {
    public class func pullThroughSpectrum(anchorView: UIView, menuArray: [AppearAddedMenuModel]) -> SpringAttributeThen {
        ...
        let menuView = SpringAttributeThen()
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_denseAspectId.map{weekMaybe(go: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func everyCount() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func viewMenu() -> UITableView {
    ...
    private func anSelectedStop() {
        //: addSubview(contentView)
        addSubview(contentView)
        //: mTable.register(DropMenuCell.self, forCellReuseIdentifier: "SHBDropMenuCell")
        mTable.register(LatReactiveCompatible.self, forCellReuseIdentifier: (String(k_advancedFormat) + data_hearingId.replacingOccurrences(of: "supportive", with: "ll")))
        //: contentView.addSubview(mTable)
        contentView.addSubview(mTable)
    }

    //: private func refreshUI() {
    private func belowLessForce() {
        //: drawmTableFrame()
        shadowgraph()
        //: mTable.reloadData()
        mTable.reloadData()
    ...
    private func animateUponForefront() {
    ...
    private func errorServer() -> CGFloat {
    ...
    override public func touchesBegan(_: Set<UITouch>, with _: UIEvent?) {
        //: animateRemoveView()
        animateUponForefront()
    }
}

//: extension DropDownMenuView: UITableViewDelegate, UITableViewDataSource {
extension SpringAttributeThen: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(k_advancedFormat) + data_hearingId.replacingOccurrences(of: "supportive", with: "ll")), for: indexPath) as! LatReactiveCompatible
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (String(mainSecretCurrentTitle)))
        //: animateRemoveView()
        animateUponForefront()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.clickHeadText(index: indexPath.row, title: cellModel.title ?? "")
    }
}
