...
/*: "gifts" :*/
fileprivate let mainEffPath:String = "giftrepresentative"

/*: "vipGifts" :*/
fileprivate let app_boundKey:[Character] = ["v","i","p","G","i","f","t","s"]

/*: "packGifts" :*/
fileprivate let main_bitAgencyId:String = "pstrikek"
fileprivate let show_installValue:[Character] = ["G","i","f","t","s"]

/*: "giftSendItem" :*/
fileprivate let app_wasteMessage:[Character] = ["g","i","f","t","S","e","n","d","I","t"]
fileprivate let app_hourGroupDevelopValue:[Character] = ["e","m"]
...
class HimGiftManager: NSObject {
    ...
    private var cacheDict = [String: [DoweryTermConvertible: [Any]]]()
    // 数据锁
    ...
    private var giftType = ThereQuickLookable.normal

    //: static let share = TalkingChatGiftManager()
    static let share = HimGiftManager()
...
extension HimGiftManager {
    ...
    func publishReplace(type: ThereQuickLookable = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        HimGiftManager.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[DoweryTermConvertible.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            HimGiftManager.share.captureData(completion: { isSucceed in
            ...
            HimGiftManager.share.strengthCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            HimGiftManager.share.historyFrame(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func historyFrame(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = HimGiftManager.share.giftType
        //: TalkingChatRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        TimeTestedRequestTool.observeCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [DoweryTermConvertible: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(mainEffPath.replacingOccurrences(of: "representative", with: "s"))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<MyGiftModel>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[DoweryTermConvertible.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(app_boundKey))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<MyGiftModel>.deserialize(from: array as? Array) {
                    ...
                    dataDict[DoweryTermConvertible.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(main_bitAgencyId.replacingOccurrences(of: "strike", with: "ac") + String(show_installValue))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<MyGiftModel>.deserialize(from: array as? Array) {
                    ...
                    dataDict[DoweryTermConvertible.Bags] = bagsArr
                }
                ...
                let sendItem: Array = data[(String(app_wasteMessage) + String(app_hourGroupDevelopValue))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<HrArrModel>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func captureData(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = HimGiftManager.share.giftType
        //: TalkingChatRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        TimeTestedRequestTool.smallScale(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func conversionCompletion(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TalkingChatRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        TimeTestedRequestTool.pieceOfLand(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<StatusStrokeModelType>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func strengthCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = ThereQuickLookable.live
        //: TalkingChatRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        TimeTestedRequestTool.collectionCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func recoupTerms(perPageCount: Int, dataType: DoweryTermConvertible) -> Int {
    ...
    func pulchritude(dataType: DoweryTermConvertible) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func people() -> [Any] {
        ...
        if HimGiftManager.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live {
        } else if HimGiftManager.share.giftType == .live {
    ...
    func carryThroughMutualCollectionArr(dataType: DoweryTermConvertible, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension HimGiftManager {
    ...
    private func bindPartner() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(indoorsJudge),
    ...
    @objc private func indoorsJudge() {
...
extension HimGiftManager {
    ...
    func invest(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        HimGiftManager.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
