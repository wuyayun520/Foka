...
fileprivate let data_rankFormat:String = "TSystemMcloud letter"
fileprivate let appSlideName:[Character] = ["e","s","s","a"]
fileprivate let kSearchData:[Character] = ["g","e","C","e","l","l","_","R","e","u","s","e","I","d"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let k_arcFormat:[UInt8] = [0xdb,0xdc,0xdb,0xc6,0x9a,0xd1,0xdd,0xd6,0xd7,0xc0,0x88,0x9b,0x92,0xda,0xd3,0xc1,0x92,0xdc,0xdd,0xc6,0x92,0xd0,0xd7,0xd7,0xdc,0x92,0xdb,0xdf,0xc2,0xde,0xd7,0xdf,0xd7,0xdc,0xc6,0xd7,0xd6]

private func yetStreet(ok num: UInt8) -> UInt8 {
    return num ^ 178
}

/*: "icon_talk_notic" :*/
fileprivate let mainAccelerateData:[Character] = ["i","c","o","n","_"]
fileprivate let show_holdId:[Character] = ["t","a","l","k","_","n","o","t","i","c"]
...
@objc public protocol MessageDelegate: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: RequestView)
    func forkCell(cell: RequestView)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: RequestView)
    func alongWithCompartment(cell: RequestView)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: RequestView)
    func apply(cell: RequestView)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: RequestView)
    func revenue(cell: RequestView)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: RequestView)
    func across(cell: RequestView)
}

//: @objcMembers public class RequestView: TCommonTableViewCell {
@objcMembers public class RequestView: ArrayWhiteReactiveCompatible {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\PhoneMessageThen.h
     */
    //: public var messageData: PhoneMessageThen?
    public var messageData: PhoneMessageThen?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(data_rankFormat.prefix(8)) + String(appSlideName) + String(kSearchData))

    /**
     *  协议委托
     *  负责实现 MessageDelegate 协议中的功能。
     */
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_arcFormat.map{yetStreet(ok: $0)}, encoding: .utf8)!)
    }
    ...
    override public func panWith(with data: PingOvalCellData) {
        //: super.fill(with: data)
        super.panWith(with: data)
        //: let newData: PhoneMessageThen = data as! PhoneMessageThen
        let newData: PhoneMessageThen = data as! PhoneMessageThen
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.largeInner()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(tract(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.platformDisk(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(resolution(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(member(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.hr(name: (String(mainAccelerateData) + String(show_holdId)))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(collideWith(recognizer:)))
...
extension RequestView {
    ...
    @objc func tract(recognizer _: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.revenue(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func member(recognizer: UIGestureRecognizer) {
    ...
    @objc func collideWith(recognizer _: UIGestureRecognizer) {
        //: if self.messageData?.status == .Msg_Status_Fail {
        if self.messageData?.status == .Msg_Status_Fail {
            //: self.delegate?.onRetryMessage(cell: self)
            self.delegate?.alongWithCompartment(cell: self)
        }
    }

    //: @objc func onSelectMessage(recognizer: UIGestureRecognizer) {
    @objc func resolution(recognizer _: UIGestureRecognizer) {
