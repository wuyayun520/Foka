 ...
 *
 *  WirePlaceUnderlyingDelegate 协议和 DetermineViewController 类。
 *  DetermineViewController 是聊天界面的核心类，负责聊天界面大部分核心业务逻辑的处理与运算。
 *  该类的详细属性与功能，请参展本文件中 DetermineViewController 的具体声明。
 *
 ******************************************************************************/
...
@class DetermineViewController;
//: @class RequestView;
@class RequestView;
//: @class PhoneMessageThen;
@class PhoneMessageThen;
//: @class V2TIMMessage;
@class V2TIMMessage;
...
@protocol WirePlaceUnderlyingDelegate <NSObject>
...
- (void)programThin:(DetermineViewController *)controller;
...
- (void)messaged:(DetermineViewController *)controller;
...
- (BOOL)gestureCell:(DetermineViewController *)controller noAt:(UIView *)view;
 ...
 */
//: - (PhoneMessageThen *)messageController:(TUIMessageController *)controller onNewMessage:(V2TIMMessage *)data;
- (PhoneMessageThen *)hair:(DetermineViewController *)controller reDelete:(V2TIMMessage *)data;
 ...
 */
//: - (RequestView *)messageController:(TUIMessageController *)controller onShowMessageData:(PhoneMessageThen *)data;
- (RequestView *)with:(DetermineViewController *)controller display:(PhoneMessageThen *)data;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageAvatar:(RequestView *)cell;
- (void)destroy:(DetermineViewController *)controller description:(RequestView *)cell;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageContent:(RequestView *)cell;
- (void)available:(DetermineViewController *)controller full:(RequestView *)cell;
 ...
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageMenu:(NSInteger)menuType withData:(PhoneMessageThen *)data;
- (void)draw:(DetermineViewController *)controller will:(NSInteger)menuType upward:(PhoneMessageThen *)data;
/**
...
/*重发*/
//: - (void)messageController:(TUIMessageController *)controller onRetryMessageContent:(RequestView *)cell;
- (void)failure:(DetermineViewController *)controller margin:(RequestView *)cell;

/*引用*/
//: - (void)messageController:(TUIMessageController *)controller onReplyMessage:(PhoneMessageThen *)cellData;
- (void)inscriptionDate:(DetermineViewController *)controller handle:(PhoneMessageThen *)cellData;

/*点击引用*/

//: - (void)messageController:(TUIMessageController *)controller onClickReplyMessage:(PhoneMessageThen *)cellData;
- (void)linkPost:(DetermineViewController *)controller gesture:(PhoneMessageThen *)cellData;
...
/////////////////////////////////////////////////////////////////////////////////
//
//                         DetermineViewController
//
/////////////////////////////////////////////////////////////////////////////////

/**
 * 腾讯云 TUIKit
 *  【模块名称】DetermineViewController
...
@interface DetermineViewController : UITableViewController

/**
 *  执行 WirePlaceUnderlyingDelegate 协议的委托
 */
//: @property (nonatomic, weak) id<TMessageControllerDelegate> delegate;
@property (nonatomic, weak) id<WirePlaceUnderlyingDelegate> delegate;
 ...
 * @param uiMsgs 待删除的数据
 */
//- (void)deleteMessages:(NSArray<PhoneMessageThen *> *)uiMsgs;
...
/**
 * 批量将V2TIMessage转换成PhoneMessageThen
 */
