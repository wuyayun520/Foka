...
fileprivate let user_attachText:[UInt8] = [0xd4,0xd9,0xd4,0xdf,0x93,0xce,0xda,0xcf,0xd0,0xdd,0xa5,0x94,0x8b,0xd3,0xcc,0xde,0x8b,0xd9,0xda,0xdf,0x8b,0xcd,0xd0,0xd0,0xd9,0x8b,0xd4,0xd8,0xdb,0xd7,0xd0,0xd8,0xd0,0xd9,0xdf,0xd0,0xcf]

fileprivate func remindPrivacy(violation num: UInt8) -> UInt8 {
...
/*: "[未知消息]" :*/
fileprivate let constWeeklyWithinName:String = "[未知消息]"

/*: "extra" :*/
fileprivate let userCleanStr:String = "elistenra"

/*: "#0084FF" :*/
fileprivate let constFuturePath:String = "half highlight#0084FF"

/*: "#FFEE4B" :*/
fileprivate let data_attentionSignPinContent:[Character] = ["#","F","F","E","E","4","B"]

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let dataClearlyLoadUrl:[Character] = ["<","a","t",">","@","[","\\"]
fileprivate let constWhoMsg:[Character] = ["S","\\","s","]","+","?","<","/","a","t",">"]

/*: "jumpKey" :*/
fileprivate let noti_computerValue:String = "jumpKeyplayer file"

/*: "user" :*/
fileprivate let k_sMsg:String = "ufounder"

/*: "uid" :*/
fileprivate let appDevelopValue:[UInt8] = [0xcc,0xd0,0xdd]

private func scienceBall(ticket num: UInt8) -> UInt8 {
    return num ^ 185
}

/*: "#16D073" :*/
fileprivate let userReplyValue:[Character] = ["#"]
fileprivate let show_countId:[Character] = ["1","6","D","0","7","3"]

/*: "#F95644" :*/
fileprivate let userDoingMessage:[Character] = ["#"]
fileprivate let app_zoneSaveValue:String = "narrow5644"
...
//: @objcMembers public class GroupThen: TalkingChatMsgBaseCellData {
@objcMembers public class GroupThen: MpCapCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_attachText.map{remindPrivacy(violation: $0)}, encoding: .utf8)!)
    }
    ...
    override public var msgModel: TareModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (constWeeklyWithinName.capitalized)
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = liveFor(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func liveFor(msgInfoModel: DetailedModel) -> NSMutableAttributedString {
        ...
        guard dic[(userCleanStr.replacingOccurrences(of: "listen", with: "xt"))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.quitTo() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(constFuturePath.suffix(7))))! : UIColor(hex: (String(data_attentionSignPinContent)))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(dataClearlyLoadUrl) + String(constWhoMsg)), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(noti_computerValue.prefix(7)))].stringValue == (k_sMsg.replacingOccurrences(of: "found", with: "s")) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: appDevelopValue.map{scienceBall(ticket: $0)}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: appDevelopValue.map{scienceBall(ticket: $0)}, encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func resign() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(userReplyValue) + String(show_countId)))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(userDoingMessage) + app_zoneSaveValue.replacingOccurrences(of: "narrow", with: "F9")))!
        }
