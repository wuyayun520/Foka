...
fileprivate let appLipName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_miss" :*/
fileprivate let app_remTitle:[Character] = ["i","c","o","n","_"]
fileprivate let main_castFormat:String = "success scan handchat_miss"

/*: "icon_chat_yes" :*/
fileprivate let user_flexibleValue:String = "icoquote"
fileprivate let app_alterValue:String = "scramble clean ort_yes"
...
class WalkMessageCell: SoupThen {
//    var transBlock: (() -> Void)?

    //: var textData: ChatCellData?
    var textData: ChatCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appLipName.reversed(), encoding: .utf8)!)
    }
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != DetermineJoinReactiveCompatible.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.present() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.hr(name: (String(app_remTitle) + String(main_castFormat.suffix(9))))
                ...
                self.transStatusImagV.image = UIImage.hr(name: (user_flexibleValue.replacingOccurrences(of: "quote", with: "n") + "_cha" + String(app_alterValue.suffix(5))))
            }
    ...
    func anyMic() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(cypher(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == DetermineJoinReactiveCompatible.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == DetermineJoinReactiveCompatible.anthologist())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            addVisitorWritten()
...
//            }
    ...
    @objc func answerCompartment(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
...
//    }
    ...
    @objc func previousMe() {
        //: self.delegate?.onReplyMessage(cell: self)
        self.delegate?.across(cell: self)
    }
}
