...
/*: "#ffffff" :*/
fileprivate let dataOutletContent:String = "#fffffrecess opportunity"
fileprivate let noti_tellMessage:String = "F"

/*: "earned" :*/
fileprivate let const_hideUrl:String = "eamerelyed"

/*: "Coins in" :*/
fileprivate let noti_welcomeUrl:String = "Coins iout connection red"
fileprivate let data_provokeKey:String = "heart"

/*: "Error:  :*/
fileprivate let notiConferPath:String = "Errorcheck edit"
fileprivate let dataExtendClaimTitle:String = ": "

/*: "#C946D2" :*/
fileprivate let const_walkSuiteData:[Character] = ["#","C","9","4","6","D","2"]

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let k_announcementData:String = "btn_within front minimum"
fileprivate let k_existUrl:String = "floatSbecome task host resistance dramatic"
fileprivate let notiFingerStr:String = "please seekBK_nor"
...
class FavoriteView: UIView {
    ...
    private let TextColor = (String(dataOutletContent.prefix(6)) + noti_tellMessage.lowercased())

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: CoordinatorModelType?) {
        //: self.init()
        self.init()
    ...
    private func modelConfig() {
        ...
        if danumuMsgModel.headPic.count > 0 {
            ...
            headAnimatedImageView.underStack(urlStr: danumuMsgModel.headPic)
        }
        //: if danumuMsgModel.bgPic.count > 0 {
        if danumuMsgModel.bgPic.count > 0 {
            //: updateBackground(withUrl: danumuMsgModel.bgPic)
            detect(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (const_hideUrl.replacingOccurrences(of: "merely", with: "rn"))) {
                ...
                if let range = attrStr.string.range(of: (String(noti_welcomeUrl.prefix(7)) + data_provokeKey.replacingOccurrences(of: "heart", with: "n"))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.opPoint(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (String(notiConferPath.prefix(5)) + dataExtendClaimTitle.capitalized) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func detect(withUrl url: String) {
        //: self.backgroundImageView.setUrlImage(urlStr: url) {[weak self] result, image in
        self.backgroundImageView.salute(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.hideAcrossMonth(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: CoordinatorModelType? {
        //: didSet {
        didSet {
            //: configModel()
            modelConfig()
    ...
    var totalTime: CGFloat {
        ...
        return (dataShowDetailPopMessage + widthValue) * SpendTime / dataShowDetailPopMessage
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * LineSpendTime / ScreenWidth
        return widthValue * LineSpendTime / dataShowDetailPopMessage
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * SpendTime / ScreenWidth
        return widthValue * SpendTime / dataShowDetailPopMessage
    }
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(const_walkSuiteData)))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.hr(name: (String(k_announcementData.prefix(4)) + "chat_" + String(k_existUrl.prefix(6)) + "_user" + String(notiFingerStr.suffix(6))))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(phaseSpaceAction(_:)))
        //: return t
        return t
        //: }()
    }()
    ...
    lazy var contentLabel: UILabel = {
        ...
        l.font = UIFont.platformDisk(fontSize: 12)
        //: l.textColor = .white
        l.textColor = .white
        //: l.numberOfLines = 1
        l.numberOfLines = 1
...
extension FavoriteView {
    ...
    @objc func phaseSpaceAction(_: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = SteppingMotorThen.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = SteppingMotorThen.share.appConfigMode.urlH5Domain + gameUrl
        ...
        config.clearBgColor = true
        //: LatPushManager.share.func__pushToWebVC(urlStr: httpsUrl, webConfig: config)
        LatPushManager.share.beforeRemarkConfig(urlStr: httpsUrl, webConfig: config)
    }
}
