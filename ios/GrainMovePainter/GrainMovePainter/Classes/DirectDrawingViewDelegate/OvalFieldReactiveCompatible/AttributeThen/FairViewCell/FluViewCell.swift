...
fileprivate let mainOverMessage:[UInt8] = [0xd5,0xd2,0xd5,0xc8,0x94,0xdf,0xd3,0xd8,0xd9,0xce,0x86,0x95,0x9c,0xd4,0xdd,0xcf,0x9c,0xd2,0xd3,0xc8,0x9c,0xde,0xd9,0xd9,0xd2,0x9c,0xd5,0xd1,0xcc,0xd0,0xd9,0xd1,0xd9,0xd2,0xc8,0xd9,0xd8]

private func extentSupportive(bottom num: UInt8) -> UInt8 {
    return num ^ 188
}

/*: "icon_chat_start" :*/
fileprivate let userAlterUrl:[Character] = ["i","c","o","n","_"]
fileprivate let main_othersName:String = "CHAT"

/*: "icon_chat_lock" :*/
fileprivate let const_clingInvitationId:String = "icon_sub tell advice doc"
fileprivate let dataDepthPath:[Character] = ["_","l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let dataInstanceStr:[Character] = ["i","c","o","n","_","c","h","a","t","_","b"]
fileprivate let dataYetName:String = "urncommunicationd"

/*: "icon_chat_burned_shan" :*/
fileprivate let main_cleanUrl:String = "icon_restaurant transfer would"
fileprivate let kSignalData:[Character] = ["c","h","a","t","_","b","u","r","n","e","d","_","s","h","a","n"]

/*: "Already burned" :*/
fileprivate let data_stillValue:[Character] = ["A","l","r","e","a","d","y"," ","b"]
fileprivate let user_alongResponseMsg:String = "URNED"
...
class FluViewCell: SoupThen {
    //: var videoData: HashOutThen?
    var videoData: HashOutThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainOverMessage.map{extentSupportive(bottom: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var playImgView: UIImageView = {
        ...
        imgV.image = UIImage.hr(name: (String(userAlterUrl) + main_othersName.lowercased() + "_start"))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var lockImgView: UIImageView = {
        ...
        imgV.image = UIImage.hr(name: (String(const_clingInvitationId.prefix(5)) + "chat" + String(dataDepthPath)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var burnedImgView: MagnitudeercalateTalkingButton = {
        ...
        imgV.setBackgroundImage(UIImage.hr(name: (String(dataInstanceStr) + dataYetName.replacingOccurrences(of: "communication", with: "e"))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.hr(name: (String(main_cleanUrl.prefix(5)) + String(kSignalData))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(data_stillValue) + user_alongResponseMsg.lowercased()).localized, for: .normal)
...
extension FluViewCell {
    ...
    override func panWith(with data: PingOvalCellData) {
        ...
        self.dutationLab.text = NSDate.formatThin(totalTime: duration)
        ...
        self.becomeFollow(urlStr: videoModel.coverImg, isMosaic: imgIsMosaic)
    }
    ...
    func becomeFollow(urlStr: String, isMosaic: Bool) {
        //: guard let newData = self.videoData else { return }
        guard let newData = self.videoData else { return }
        ...
        if !coverImgPath.isEmptyString, isExit { // 加载本地
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.picImgView.image = UIImage.solarGravity(image, can: 90)
                self.picImgView.image = UIImage.solarGravity(image, can: 90)
                //: } else {
            } else {
            ...
            self.picImgView.towardUniversalCompletion(urlStr: urlStr) { [weak self] image in
