...
fileprivate let show_incidentVisibleCorrelationText:[UInt8] = [0xb2,0xb7,0xb2,0xbd,0x71,0xac,0xb8,0xad,0xae,0xbb,0x83,0x72,0x69,0xb1,0xaa,0xbc,0x69,0xb7,0xb8,0xbd,0x69,0xab,0xae,0xae,0xb7,0x69,0xb2,0xb6,0xb9,0xb5,0xae,0xb6,0xae,0xb7,0xbd,0xae,0xad]

fileprivate func chinCount(course num: UInt8) -> UInt8 {
...
/*: "icon_lounge" :*/
fileprivate let show_enoughName:String = "from weighticon_lou"
fileprivate let constGiftMessage:String = "nganalyze"

/*: "icon_talk_points" :*/
fileprivate let showOndOwnFormat:String = "icon_tbutt walk tun administer"
fileprivate let notiRidStr:[Character] = ["t","s"]

/*: "C6BDFF" :*/
fileprivate let app_systemData:[Character] = ["C","6","B","D","F","F"]

/*: "#BBBBBB" :*/
fileprivate let mainThereMustMessage:String = "electronic guide#BBBBBB"

/*: "Reply to get points~" :*/
fileprivate let constManualArrayStr:String = "input arrowReply "
fileprivate let showBraceMessage:String = "generate cut exposure heto get p"
fileprivate let kDecisionTarId:String = "perform dialoints~"

/*: "bth_unread_nor" :*/
fileprivate let const_strainName:String = "bound behindbth_un"
fileprivate let k_choicePath:String = "_norlock future storage"

/*: "icon_male_default" :*/
fileprivate let notiUniversalValue:[UInt8] = [0x0,0xfa,0x6,0x5,0xf6,0x4,0xf8,0x3,0xfc,0xf6,0xfb,0xfc,0xfd,0xf8,0xc,0x3,0xb]

fileprivate func branchShirt(accounting num: UInt8) -> UInt8 {
...
/*: "icon_female_default" :*/
fileprivate let data_lieFormat:[Character] = ["i","c","o","n","_","f","e","m","a","l","e","_","d"]
fileprivate let data_yieldPath:String = "EFAULT"

/*: "+%@ points" :*/
fileprivate let show_shapeData:String = "why%@ "

/*: "bth_read_pre" :*/
fileprivate let mainReduceDevelopId:String = "bth_blast outer dialog accept"
...
//: class TalkingChatBaseMsgCell: RequestView {
class SoupThen: RequestView {
    //: var cellData: TalkingChatMsgBaseCellData?
    var cellData: MpCapCellData?
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        super.init(style: style, reuseIdentifier: reuseIdentifier)
        //: self.setupSubviews()
        self.magnitudeerpret()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: show_incidentVisibleCorrelationText.map{chinCount(course: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.hr(name: (String(show_enoughName.suffix(8)) + constGiftMessage.replacingOccurrences(of: "analyze", with: "e")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var ageLab: UILabel = {
        ...
        label.font = UIFont.platformDisk(fontSize: 10)
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var coinIconImg: UIImageView = {
        ...
        imgV.image = UIImage.hr(name: (String(showOndOwnFormat.prefix(6)) + "alk_poin" + String(notiRidStr)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    public lazy var coinLabel: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(app_systemData)))
        //: label.isHidden = true
        label.isHidden = true
        //: label.font = UIFont.pingfangRugularFont(fontSize: 14)
        label.font = UIFont.promptReload(fontSize: 14)
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var replyTipLab: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(mainThereMustMessage.suffix(7))))
        //: label.isHidden = true
        label.isHidden = true
        //: label.font = UIFont.pingfangRugularFont(fontSize: 14)
        label.font = UIFont.promptReload(fontSize: 14)
        //: label.text = "Reply to get points~".localized
        label.text = (String(constManualArrayStr.suffix(6)) + String(showBraceMessage.suffix(8)) + String(kDecisionTarId.suffix(6))).localized
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var isReadImg: UIImageView = {
        ...
        imgV.image = UIImage.hr(name: (String(const_strainName.suffix(6)) + "read" + String(k_choicePath.prefix(4))))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension SoupThen {
    ...
    override func panWith(with data: PingOvalCellData) {
        ...
        self.cellData = data as? MpCapCellData
        ...
        let userModel = newData.msgModel.user
        //: let loungePlus = (SteppingMotorThen.share.loginUid == String(userModel.uid)) ? SteppingMotorThen.share.loginUserMode.loungePlus : userModel.loungePlus
        let loungePlus = (SteppingMotorThen.share.loginUid == String(userModel.uid)) ? SteppingMotorThen.share.loginUserMode.loungePlus : userModel.loungePlus

        //: self.nameLabel.text = userModel.nickname
        self.nameLabel.text = userModel.nickname
        //: self.nameLabel.textColor = loungePlus ? .userVipColor() : .appValueColor()
        self.nameLabel.textColor = loungePlus ? .colorObserver() : .tillAdvancedColor()
        //: self.nameLabel.font = UIFont.pingfangMediumFont(fontSize: 13)
        self.nameLabel.font = UIFont.platformDisk(fontSize: 13)
        //: let imgStr = userModel.sex == Int(Gender.male.rawValue) ? "icon_male_default" : "icon_female_default"
        let imgStr = userModel.sex == Int(WalkMilitaryOutputStreamable.male.rawValue) ? String(bytes: notiUniversalValue.map{branchShirt(accounting: $0)}, encoding: .utf8)! : (String(data_lieFormat) + data_yieldPath.lowercased())
        //: sexIconBtn.setBackgroundImage(UIImage.BundleImageNamed(name: imgStr), for: .normal)
        sexIconBtn.setBackgroundImage(UIImage.hr(name: imgStr), for: .normal)
        ...
        self.avatarView.underStack(urlStr: userModel.headPic)
        ...
        self.iconBorder.resultFocus(urlStr: userModel.headPicFrame)

        //: let isShowCoin = newData.msgIncome > 0
        let isShowCoin = newData.msgIncome > 0
        //: self.coinIconImg.isHidden = !(isShowCoin && SteppingMotorThen.share.appStatus == AppSkinStatus.normal.rawValue)
        self.coinIconImg.isHidden = !(isShowCoin && SteppingMotorThen.share.appStatus == EchoTermConvertible.normal.rawValue)
        ...
        self.coinLabel.text = (show_shapeData.replacingOccurrences(of: "why", with: "+") + "points").publicTransport(NSNumber(value: Float(newData.msgIncome)))
        // 女性逻辑, 未回复付费消息提示
        //: if TalkingPrivateChatManager.needShowReplyTipsMessage(newData.direction,
        if GenChatManager.written(newData.direction,
        ...
        if GenChatManager.eye(cellData: newData) {
            //: self.isReadImg.isHidden = false
            self.isReadImg.isHidden = false
            //: var readImgStr = "bth_unread_nor"
            var readImgStr = (String(const_strainName.suffix(6)) + "read" + String(k_choicePath.prefix(4)))
            // 消息对端是否已读
            //: if TalkingPrivateChatManager.msgIsRead(cellData: newData) {
            if GenChatManager.cellHead(cellData: newData) {
                //: readImgStr = "bth_read_pre"
                readImgStr = (String(mainReduceDevelopId.prefix(4)) + "read_pre")
            }
            //: self.isReadImg.image = UIImage.BundleImageNamed(name: readImgStr)
            self.isReadImg.image = UIImage.hr(name: readImgStr)
    ...
    override public func updateConstraints() {
        ...
        if let temCellData = self.cellData {
            //: loungePlus = (SteppingMotorThen.share.loginUid == String(temCellData.msgModel.user.uid)) ? SteppingMotorThen.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
            loungePlus = (SteppingMotorThen.share.loginUid == String(temCellData.msgModel.user.uid)) ? SteppingMotorThen.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
        }
