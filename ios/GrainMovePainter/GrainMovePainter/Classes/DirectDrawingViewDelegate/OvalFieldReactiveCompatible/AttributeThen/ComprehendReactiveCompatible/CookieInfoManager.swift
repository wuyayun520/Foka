...
/*: "serialQueue" :*/
fileprivate let mainEqualThirdTitle:String = "seriaflat"
fileprivate let const_hurryKey:String = "literal"

/*: "fromUids" :*/
fileprivate let kLinkAirName:[UInt8] = [0x10,0x1c,0x19,0x17,0xff,0x13,0xe,0x1d]

fileprivate func wholeStrength(single num: UInt8) -> UInt8 {
...
public class CookieInfoManager: NSObject {
...
extension CookieInfoManager {
    /// 获取用户消息
    /// - Parameters:
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([ComprehendReactiveCompatible]?, Error?) -> Void) {
    class func jab(with userIdsArr: [String], completion: @escaping ([ComprehendReactiveCompatible]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if CookieInfoManager.compassPoint().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(CookieInfoManager.compassPoint().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            CookieInfoManager.compassPoint().needReqUidsSet = nil
        }
        ...
        repeat {
            ...
            access(with: subArr, completion: completion)
            //: location += len
            location += len
    ...
    class func noteLimit(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (mainEqualThirdTitle.replacingOccurrences(of: "flat", with: "l") + "Queu" + const_hurryKey.replacingOccurrences(of: "literal", with: "e")))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    ...
                    TimeTestedRequestTool.ditUids(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
    ...
    public class func audio(message: V2TIMMessage) -> FairThen {
        //: let key = TUISocialChatManager.checkPrimaryKey(withMsgModel: message)
        let key = CookieChatManager.tickModel(withMsgModel: message)
        //: var cacheWrap = WCDBVoiceMsgTable.db_getVoiceMsg(with: key)
        var cacheWrap = FairThen.contribution(with: key)
        //: if cacheWrap == nil {
        if cacheWrap == nil {
            //: let info = TUISocialChatManager.packageDict(withTXMessage: message)
            let info = CookieChatManager.vapourous(withTXMessage: message)
            //: cacheWrap = WCDBVoiceMsgTable.db_inserVoiceMsgToDB(info)
            cacheWrap = FairThen.big(info)
        }
        //: return cacheWrap!
        return cacheWrap!
    }
...
extension CookieInfoManager {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> ComprehendReactiveCompatible? {
    class func dealMp(targetId: String) -> ComprehendReactiveCompatible? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return FairHangInfoTable.comeUponMergeStreetwise(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: ComprehendReactiveCompatible?) {
    class func prepareWith(with userInfo: ComprehendReactiveCompatible?) {
        ...
        let model = CookieInfoManager.dealMp(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        FairHangInfoTable.commensurate(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: user_textName, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func via(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = CookieInfoManager.dealMp(targetId: targetId) else { return }
    ...
    class func sayTime(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = CookieInfoManager.dealMp(targetId: targetId) else { return }
    ...
    class func hedge(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        FairHangInfoTable.drop(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        CookieInfoManager.compassPoint().reqedUidsSet.remove(targetUid)
    }
}
...
extension CookieInfoManager {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([ComprehendReactiveCompatible]?, Error?) -> Void) {
    private class func access(with userIdsArr: [String], completion: @escaping ([ComprehendReactiveCompatible]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = FairHangInfoTable.putBold(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if CookieInfoManager.compassPoint().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            CookieInfoManager.inwardUponStreetwise(with: noDBUidArr)
        }
    }
    ...
    private class func inwardUponStreetwise(with userIdsArr: [String]) {
        ...
        CookieInfoManager.compassPoint().reqedUidsSet = CookieInfoManager.compassPoint().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: TalkingChatRequestTool.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        TimeTestedRequestTool.perimeter(param: [String(bytes: kLinkAirName.map{wholeStrength(single: $0)}, encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                CookieInfoManager.compassPoint().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            CookieInfoManager.insideInformation(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: ComprehendReactiveCompatible] = [:]
            var retInfoDict: [String: ComprehendReactiveCompatible] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: user_textName, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [ComprehendReactiveCompatible]) {
    private class func insideInformation(userInfoArr: [ComprehendReactiveCompatible]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            FairHangInfoTable.commensurate(aWrap)
        }
    }
}
