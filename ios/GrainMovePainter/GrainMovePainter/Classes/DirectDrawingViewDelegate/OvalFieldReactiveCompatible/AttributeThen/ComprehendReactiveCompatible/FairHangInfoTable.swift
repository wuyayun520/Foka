...
/*: "DBUserInfoTable" :*/
fileprivate let appSnapUrl:[Character] = ["D","B","U","s","e","r","I","n","f","o","T"]
fileprivate let kObtainUniversalPath:String = "abltrack"

/*: "WCDB表 :*/
fileprivate let main_noticeMessage:[Character] = ["W","C","D","B","表"]

/*: : 更新数据失败。error： :*/
fileprivate let data_portionMessage:String = ": 更新数"
fileprivate let data_editorTitle:[Character] = ["r","r","o","r","："]

/*: : 批量插入数据失败。error： :*/
fileprivate let data_jarMessage:String = ": 批量"
fileprivate let constContactContent:String = "upon than agency image败。er"
...
let const_rateFormat = (String(appSnapUrl) + kObtainUniversalPath.replacingOccurrences(of: "track", with: "e"))
//: final class WCDBUserInfoTable: TableCodable {
final class FairHangInfoTable: TableCodable {
    /// 用户Id【主键】
    //: var uid: String = ""
    var uid: String = ""
    /// 昵称
    ...
    enum CodingKeys: String, CodingTableKey {
        ...
        typealias Root = FairHangInfoTable
        //: case uid
        case uid
        //: case nickname
        case nickname
...
// MARK: - ComprehendReactiveCompatible和WCDBUserInfoTable转换

//: extension WCDBUserInfoTable {
extension FairHangInfoTable {
    /// ComprehendReactiveCompatible  =>  FairHangInfoTable
    //: private class func mapToDBUserInfoData(_ userInfo: ComprehendReactiveCompatible) -> WCDBUserInfoTable {
    private class func exist(_ userInfo: ComprehendReactiveCompatible) -> FairHangInfoTable {
        //: let dbData = WCDBUserInfoTable()
        let dbData = FairHangInfoTable()
    ...
    /// [FairHangInfoTable] => [ComprehendReactiveCompatible]
    //: private class func mapToUserInfoModels(_ dbDatas: [WCDBUserInfoTable]) -> [ComprehendReactiveCompatible] {
    private class func thinAcross(_ dbDatas: [FairHangInfoTable]) -> [ComprehendReactiveCompatible] {
        //: return dbDatas.map { data in
        return dbDatas.map { data in
            //: let userInfo = ComprehendReactiveCompatible()
            let userInfo = ComprehendReactiveCompatible()
...
extension FairHangInfoTable {
    ...
    class func drop(with userId: String) {
        ...
        try? HimManager.shared.database?.run(transaction: { _ in
            ...
            let condition = FairHangInfoTable.Properties.uid.in(userId)
            //: try? WCDBManager.shared.database?.delete(fromTable: WCDBUserInfoTableName,
            try? HimManager.shared.database?.delete(fromTable: const_rateFormat,
                                                     //: where: condition)
                                                     where: condition)
            //: })
        })
    ...
    /// - Parameter userId: 用户id
    /// - Returns: 模型
    //: class func db_getUserInfo(with userId: String) -> ComprehendReactiveCompatible? {
    class func comeUponMergeStreetwise(with userId: String) -> ComprehendReactiveCompatible? {
        //: let userInfos = WCDBUserInfoTable.db_getUserInfos(with: [userId])
        let userInfos = FairHangInfoTable.putBold(with: [userId])
    ...
    /// - Returns: 数据数组
    //: class func db_getUserInfos(with userIds: [String]) -> [ComprehendReactiveCompatible] {
    class func putBold(with userIds: [String]) -> [ComprehendReactiveCompatible] {
        //: guard userIds.count > 0 else { return [] }
        guard userIds.count > 0 else { return [] }
        //: do {
        do {
            //: let condition = WCDBUserInfoTable.Properties.uid.in(userIds)
            let condition = FairHangInfoTable.Properties.uid.in(userIds)
            ...
            let userInfos: [FairHangInfoTable]? = try HimManager.shared.database?.getObjects(on: FairHangInfoTable.Properties.all, fromTable: const_rateFormat, where: condition)
            //: if userInfos?.isEmpty == false {
            if userInfos?.isEmpty == false {
                //: let models = WCDBUserInfoTable.mapToUserInfoModels(userInfos!)
                let models = FairHangInfoTable.thinAcross(userInfos!)
                //: return models
                return models
            }
            //: } catch {
    ...
    //: class func db_updateUserInfo(_ userInfo: ComprehendReactiveCompatible) {
    class func commensurate(_ userInfo: ComprehendReactiveCompatible) {
        //: let dbData = WCDBUserInfoTable.mapToDBUserInfoData(userInfo)
        let dbData = FairHangInfoTable.exist(userInfo)
        //: try? WCDBManager.shared.database?.run(transaction: { _ in
        try? HimManager.shared.database?.run(transaction: { _ in
            // 数据库中已存在，则更新数据
            //: if WCDBUserInfoTable.db_getUserInfo(with: dbData.uid) != nil {
            if FairHangInfoTable.comeUponMergeStreetwise(with: dbData.uid) != nil {
                //: do {
                do {
                    //: let condition = WCDBUserInfoTable.Properties.uid == dbData.uid
                    let condition = FairHangInfoTable.Properties.uid == dbData.uid
                    //: try WCDBManager.shared.database?.update(table: WCDBUserInfoTableName,
                    try HimManager.shared.database?.update(table: const_rateFormat,
                                                            //: on: WCDBUserInfoTable.Properties.all,
                                                            on: FairHangInfoTable.Properties.all,
                                                            //: with: dbData,
                                                            with: dbData,
                                                            //: where: condition)
                                                            where: condition)
                ...
                FairHangInfoTable.noFrom(dbData)
            }
            //: })
        })
    }
