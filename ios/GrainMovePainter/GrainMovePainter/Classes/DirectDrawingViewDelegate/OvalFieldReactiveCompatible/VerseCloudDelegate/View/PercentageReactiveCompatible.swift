...
fileprivate let noti_writePortionData:[UInt8] = [0xf3,0xf8,0xf3,0xfe,0xb2,0xed,0xf9,0xee,0xef,0xfc,0xc4,0xb3,0xaa,0xf2,0xeb,0xfd,0xaa,0xf8,0xf9,0xfe,0xaa,0xec,0xef,0xef,0xf8,0xaa,0xf3,0xf7,0xfa,0xf6,0xef,0xf7,0xef,0xf8,0xfe,0xef,0xee]

fileprivate func rugClub(rank num: UInt8) -> UInt8 {
...
// __CLOSE_PRINT__
//
//  PercentageReactiveCompatible.swift
//  AbroadTalking
//
...
enum ConversionTextConvertible: Int {
    //: case TimeGap = 0
...
@objc protocol JoinViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func earlyGetMoving()

    //: @objc optional func endScroll()
    @objc optional func scroll()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class PercentageReactiveCompatible: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(sharedAction))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_writePortionData.map{rugClub(rank: $0)}, encoding: .utf8)!)
    }
...
extension PercentageReactiveCompatible {
    //: func startTimer() {
    func closed() {
        //: stopTimer()
        pendingCount()
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(tingHumanAction), userInfo: nil, repeats: true)
    }
...
extension PercentageReactiveCompatible {
    ...
    func motivationModel(msgModel: MpDanmuModel?) {
    ...
    func incomeBottom() {
        ...
        if AllHeight > notiGroupData {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < notiGroupData {
                ...
                self.msgTableView.height = notiGroupData
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = notiGroupData - AllHeight
        ...
        gapSend(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func gapSend(animated: Bool) {
...
extension PercentageReactiveCompatible: UITableViewDelegate, UITableViewDataSource {
    ...
    func preponderance(_ scrollView: UIScrollView) {
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(notiAssetData)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = SatelliteThen.byPath(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension PercentageReactiveCompatible {
    ...
    func evaluate() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: app_buildPath, height: notiGroupData)
    }
}
