...
fileprivate let constLengthPath:[UInt8] = [0x9f,0xa4,0x9f,0xaa,0x5e,0x99,0xa5,0x9a,0x9b,0xa8,0x70,0x5f,0x56,0x9e,0x97,0xa9,0x56,0xa4,0xa5,0xaa,0x56,0x98,0x9b,0x9b,0xa4,0x56,0x9f,0xa3,0xa6,0xa2,0x9b,0xa3,0x9b,0xa4,0xaa,0x9b,0x9a]

fileprivate func squareVisualRegion(coordinator num: UInt8) -> UInt8 {
...
/*: "#FFD13A" :*/
fileprivate let noti_justMessage:[Character] = ["#","F","F","D","1","3","A"]

/*: "gift_gb_umnad_1" :*/
fileprivate let showOmitTitle:[Character] = ["g","i","f","t","_"]
fileprivate let data_appealStr:[Character] = ["g","b","_","u","m","n","a","d","_","1"]

/*: "icon_circle_pic_default" :*/
fileprivate let data_componentPath:[Character] = ["i","c"]
fileprivate let noti_flatStr:[Character] = ["o","n","_","c","i","r","c","l","e","_","p","i","c","_","d","e","f","a","u","l","t"]

/*: "Open lucky blind-box" :*/
fileprivate let showAdvancedName:String = "builder unknownOpen l"
fileprivate let k_frontMsg:String = " blindguard past honey giving"

/*: "Sent " :*/
fileprivate let appExploreContent:String = "play imaginationSent "

/*: "PingFangSC-Semibold" :*/
fileprivate let dataMinFormat:[Character] = ["P","i","n","g","F","a","n","g","S"]
fileprivate let data_overPath:String = "guidance weight giftC-Sem"

/*: "Bombard" :*/
fileprivate let dataRiverText:[Character] = ["B","o","m","b","a","r","d"]
...
class CrushThen: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = RepresentModelType()
    //: var giftSendItem = [TalkingGiftNumArrModel()]
    var giftSendItem = [HrArrModel()]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constLengthPath.map{squareVisualRegion(coordinator: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickLb: UILabel = {
        ...
        lb.font = .promptReload(fontSize: 14)
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(noti_justMessage)))
        //: lb.font = UIFont.pingfangRugularFont(fontSize: 13)
        lb.font = UIFont.promptReload(fontSize: 13)
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var pluralTip: PingLieItemLabel = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = PingLieItemLabel()
        //: lab.isHidden = true
        lab.isHidden = true
    ...
    private lazy var countLb: PingLieItemLabel = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = PingLieItemLabel()
...
extension CrushThen {
    ...
    func giftHorse(model: RepresentModelType) {
        ...
        self.bgImage.image = UIImage.hr(name: (String(showOmitTitle) + String(data_appealStr)))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.hr(name: (String(data_componentPath) + String(noti_flatStr)))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == ResourceTotalLiteral.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(showAdvancedName.suffix(6)) + "ucky" + String(k_frontMsg.prefix(6)) + "-box").localized
            //: }else {
        } else {
        ...
        self.avatarImage.salute(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.salute(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(appExploreContent.suffix(5))).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(dataMinFormat) + String(data_overPath.suffix(5)) + "ibold"), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = moveTitle(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(dataMinFormat) + String(data_overPath.suffix(5)) + "ibold"), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = moveTitle(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.eliteTip(texts: strs, colors: colors, fonts: fonts)
        ...
        firePan(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func firePan(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(dataMinFormat) + String(data_overPath.suffix(5)) + "ibold"), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = moveTitle(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(dataMinFormat) + String(data_overPath.suffix(5)) + "ibold"), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = moveTitle(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.detergentSaveer(150, 77, 20, 1), UIColor.white, UIColor.detergentSaveer(150, 77, 20, 1)]
        ...
        attText = attText.eliteTip(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.shareSolar(attributedText: attText)
        ...
        countLb.pillowLengthFrame(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func medicament(giftMsgModel: RepresentModelType) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(underOrigin), object: nil)
        ...
        self.perform(#selector(underOrigin), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func modelBodyAnimate(model _: RepresentModelType, animate _: Bool) {
    ...
    func animationThroughFramework(frame: CGRect) {
        //: UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
        UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
            //: self.frame = frame
            self.frame = frame
    ...
    @objc func underOrigin() {
        //: NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(animateHide), object: nil)
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(underOrigin), object: nil)
        //: giftMsgModel = TalkingGiftMsgAnimatModel()
        giftMsgModel = RepresentModelType()
...
extension CrushThen {
    ...
    func moveTitle(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension CrushThen {
    ...
    private func underForSubviews() {
        ...
        font = moveTitle(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.eliteTip(texts: [(String(dataRiverText)).localized], colors: [UIColor.detergentSaveer(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.pillowLengthFrame(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.shareSolar(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.pillowLengthFrame(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
