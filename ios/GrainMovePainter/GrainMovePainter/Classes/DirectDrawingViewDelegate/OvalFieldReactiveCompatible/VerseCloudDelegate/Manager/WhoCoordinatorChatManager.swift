...
/*: "Speaker On" :*/
fileprivate let const_lockStr:[Character] = ["S","p","e","a","k","e","r"," ","O"]
fileprivate let kCorrectTitle:String = "feature"

/*: "Speaker Off" :*/
fileprivate let constLearnStr:String = "Speaker spark sign none column"
fileprivate let notiServerText:String = "Offopposite force"

/*: "TRTC 音频通话销毁. logId： :*/
fileprivate let constOutputShotKey:[Character] = ["T","R","T","C"," ","音","\u{9891}","通","话"]
fileprivate let app_addedData:[Character] = ["销","毁","."," ","l","o","g","I","d","\u{ff1a}"]

/*: "TRTC 音频通话错误. errCode: :*/
fileprivate let show_unableFormat:String = "lie contain must pinTRTC 音"
fileprivate let show_curveCosName:String = "误. enobody medal"

/*: , errMsg: :*/
fileprivate let data_designMsg:[Character] = [","," ","e","r","r","M"]
fileprivate let userCorruptUrl:String = "right hold mobile constitutional sciencesg:"

/*: "TRTC 音频通话SDK收到警告. warningCode: :*/
fileprivate let notiAwakeSearchVoiceStr:[Character] = ["T","R","T","C"," ","音","\u{9891}","通","话","S"]
fileprivate let constWeightPhysicsReasonMsg:[Character] = ["D","K","收","到","\u{8b66}","告","."," ","w","a","r","n","i","n","g","C","o","d","e",":"]

/*: , warningMsg: :*/
fileprivate let app_programOffAdjustmentStr:[Character] = [","," ","w","a","r","n","i","n","g","M"]
fileprivate let show_audienceData:String = "sg:pi beat hem"

/*: "TRTC 音频通话进入房间. result: :*/
fileprivate let const_readerWillPath:String = "TRTC pan limit para seek successfully"
fileprivate let main_strengthUrl:String = "\u{8fdb}入房\u{95f4}. r"
fileprivate let notiMiddleFormat:String = "pass visible beyond removalesult:"

/*: , logId： :*/
fileprivate let user_eyeMessage:String = ", "
fileprivate let mainPanMessage:String = "selection broad cell fall branchlogId："

/*:  加入了房间" :*/
fileprivate let appRecoverContent:String = " 加入了country"

/*:  离开了房间" :*/
fileprivate let k_specialId:String = " 离level了房间"
...
// __CLOSE_PRINT__
//
//  WhoCoordinatorChatManager.swift
//  AbroadTalking
//
...
private class JoinReactiveCompatible: NSObject {
...
class WhoCoordinatorChatManager: NSObject {
    ...
    private var config = JoinReactiveCompatible() // 音频渲染配置类
    //: private var logId = 0
    ...
    override init() {
        ...
        self.notificationCounterruptSettingsAnd()
    }
...
extension WhoCoordinatorChatManager {
    /// 挂断来电后，需重恢复音频设备
    //: func enableAudioModule(enable: Bool) {
    func modifyFire(enable: Bool) {
    ...
    func realised(roomId: Int) {
        //: self.handsfreeBtn?.isEnabled = !hasHeadSet()
        self.handsfreeBtn?.isEnabled = !minimumScript()
    ...
    func expectedRefuse(isMute: Bool) {
    ...
    func worker(isHandsfree: Bool) {
        //: guard hasHeadSet() == false else { return }
        guard minimumScript() == false else { return }
        ...
        self.handsfreeDesLab?.text = isHandsfree ? (String(const_lockStr) + kCorrectTitle.replacingOccurrences(of: "feature", with: "n")).localized : (String(constLearnStr.prefix(8)) + String(notiServerText.prefix(3))).localized
        //: updateWhenPlayDeviceChanged(isHandsfree: isHandsfree)
        nearHandsfree(isHandsfree: isHandsfree)
    }
}
...
extension WhoCoordinatorChatManager {
    ...
    private func minimumScript() -> Bool {
...
extension WhoCoordinatorChatManager {
    ...
    func notificationCounterruptSettingsAnd() {
        // 监听 App 切后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(notification_stopLocalAudio(notif:)),
                                               selector: #selector(gameNotif(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(creativePerson(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(screenNotif(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(direct(notif:)),
    ...
    @objc private func gameNotif(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func creativePerson(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func screenNotif(notif: Notification) {
    ...
    @objc func direct(notif: Notification) {
        ...
        case .newDeviceAvailable: // 听筒
            ...
            nearHandsfree(isHandsfree: false)
        ...
        case .oldDeviceUnavailable: // 免提
            ...
            self.handsfreeDesLab?.text = (String(const_lockStr) + kCorrectTitle.replacingOccurrences(of: "feature", with: "n")).localized
            //: updateWhenPlayDeviceChanged(isHandsfree: self.config.isHandsfree)
            nearHandsfree(isHandsfree: self.config.isHandsfree)
...
extension WhoCoordinatorChatManager {
    ...
    private func nearHandsfree(isHandsfree: Bool) {
    ...
    private func layerResistance() {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话销毁. logId：\(logId).")
        ReckonLogTool.commentAdvertising(msg: (String(constOutputShotKey) + String(app_addedData)) + "\(logId).")
        //: guard self.config.isPage else { return }
        guard self.config.isPage else { return }

        //: if self.config.isHandsfree == false {
        if self.config.isHandsfree == false { // 恢复系统扬声器播放声音
            //: updateWhenPlayDeviceChanged(isHandsfree: true)
            nearHandsfree(isHandsfree: true)
        }
...
extension WhoCoordinatorChatManager: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        ReckonLogTool.commentAdvertising(msg: (String(show_unableFormat.suffix(6)) + "\u{9891}\u{901a}话\u{9519}" + String(show_curveCosName.prefix(4)) + "rrCode:") + "\(errCode)" + (String(data_designMsg) + String(userCorruptUrl.suffix(3))) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        ReckonLogTool.commentAdvertising(msg: (String(notiAwakeSearchVoiceStr) + String(constWeightPhysicsReasonMsg)) + "\(warningCode)" + (String(app_programOffAdjustmentStr) + String(show_audienceData.prefix(3))) + "\(String(describing: warningMsg)).")
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话进入房间. result:\(result), logId：\(logId).")
        ReckonLogTool.commentAdvertising(msg: (String(const_readerWillPath.prefix(5)) + "\u{97f3}频\u{901a}\u{8bdd}" + main_strengthUrl + String(notiMiddleFormat.suffix(6))) + "\(result)" + (user_eyeMessage.capitalized + String(mainPanMessage.suffix(6))) + "\(logId).")
        //: if result < 0 {
        if result < 0 { // 加入失败
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (appRecoverContent.replacingOccurrences(of: "country", with: "房间")))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason _: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (k_specialId.replacingOccurrences(of: "level", with: "开")))
    }
}
