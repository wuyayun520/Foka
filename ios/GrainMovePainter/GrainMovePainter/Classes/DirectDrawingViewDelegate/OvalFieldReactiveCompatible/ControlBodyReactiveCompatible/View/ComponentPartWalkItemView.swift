...
/*: "#ffffff" :*/
fileprivate let data_judgeFormat:[Character] = ["#","f","f","f","f","f","f"]

/*: "/dist/loungePlus/index.html" :*/
fileprivate let noti_broadValue:String = "vid original appropriate row extend/dis"
fileprivate let notiCosKey:String = "ngePlago eye"
fileprivate let app_artMessage:String = "ex.htmlcertain reach"

/*: "https://" :*/
fileprivate let userPlainPath:String = "feature day temporary somebodyhttps:/"
fileprivate let const_riverRetainId:String = "/"

/*: "http://" :*/
fileprivate let notiHerValue:String = "http://course anima err reduce contain"
...
// __CLOSE_PRINT__
//
//  ComponentPartWalkItemView.swift
//  AbroadTalking
//
...
class ComponentPartWalkItemView: UIView {
    ...
    let Broadcast_TextColor = (String(data_judgeFormat))
    //: let Broadcast_ContentFont: UIFont = UIFont.pingfangMediumFont(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .platformDisk(fontSize: 12)
    ...
    convenience init(model: PolicyTransformable?) {
        //: self.init()
        self.init()
    ...
    private func belowTodayModel() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            weeklyChannel(withUrl: danumuMsgModel.bgPic)
        }

        //: if danumuMsgModel.content.count > 0 {
        if danumuMsgModel.content.count > 0 {
            //: let parse = TUIChatParseTagHelper.init()
            let parse = PercentageMinThen()
            ...
            var mutableAtt = parse.stateFrom(withFontSize: 12, insert: "", atIndex: 0)
            // 限制字符长度
    ...
    func weeklyChannel(withUrl url: String) {
        ...
        self.backgroundImageView.salute(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.hideAcrossMonth(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: PolicyTransformable? {
        //: didSet {
        didSet {
            //: configModel()
            belowTodayModel()
    ...
    var totalTime: CGFloat {
        ...
        return (dataShowDetailPopMessage + widthValue) * Broadcast_SpendTime / dataShowDetailPopMessage
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / dataShowDetailPopMessage
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / dataShowDetailPopMessage
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(jumpStart(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension ComponentPartWalkItemView {
    /// 弹幕点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func jumpStart(_: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: LatPushManager.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            LatPushManager.share.postKey(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: LatPushManager.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                LatPushManager.share.transportationSystem(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(noti_broadValue.suffix(4)) + "t/lou" + String(notiCosKey.prefix(5)) + "us/ind" + String(app_artMessage.prefix(7)))) {
                    //: LatPushManager.share.func__pushToSubscribePageWebVC()
                    LatPushManager.share.selectReply()
                    //: return
                    return
                }
                //: let httpsUrl = SteppingMotorThen.share.appConfigMode.urlH5Domain+url
                let httpsUrl = SteppingMotorThen.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (String(userPlainPath.suffix(7)) + const_riverRetainId.capitalized), with: (String(notiHerValue.prefix(7))))
                //: LatPushManager.share.func__pushToWebVC(urlStr: httpUrl)
                LatPushManager.share.beforeRemarkConfig(urlStr: httpUrl)
            }
