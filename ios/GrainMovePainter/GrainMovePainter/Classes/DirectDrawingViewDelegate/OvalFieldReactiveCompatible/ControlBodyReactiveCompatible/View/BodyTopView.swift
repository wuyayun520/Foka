...
fileprivate let app_ficeTitle:[UInt8] = [0x59,0x5e,0x59,0x44,0x18,0x53,0x5f,0x54,0x55,0x42,0xa,0x19,0x10,0x58,0x51,0x43,0x10,0x5e,0x5f,0x44,0x10,0x52,0x55,0x55,0x5e,0x10,0x59,0x5d,0x40,0x5c,0x55,0x5d,0x55,0x5e,0x44,0x55,0x54]

private func spanPrivate(take num: UInt8) -> UInt8 {
    return num ^ 48
}

/*: "bth_live_follow_pre" :*/
fileprivate let show_playTitle:String = "level plat installationbth_li"
fileprivate let const_referId:String = "yes time destination towardve_fol"

/*: "btn_live_mini_nor" :*/
fileprivate let data_freshMessage:[Character] = ["b","t","n","_","l","i","v","e","_","m","i","n","i","_","n"]
fileprivate let constRateValue:String = "orecommendation"

/*: "#2DF2FF" :*/
fileprivate let userDrawKey:[Character] = ["#","2"]
fileprivate let notiMutualId:[Character] = ["D","F","2","F","F"]

/*: "Live" :*/
fileprivate let data_productFormat:[Character] = ["L","i","v","e"]

/*: "#FF506D" :*/
fileprivate let kHoneyValue:String = "#FF50forward operate"
fileprivate let dataArriveViolationKey:String = "people comment assist trigger never6D"

/*: "removeAttentionUid" :*/
fileprivate let noti_hiddenSituationMessage:String = "rlifemovlife"
fileprivate let dataViaStopTrackTitle:String = "ntiabout"

/*: "attentionUid" :*/
fileprivate let app_tailKey:String = "atteranget"
fileprivate let user_memoryName:String = "para"

/*: "source" :*/
fileprivate let k_spaceKey:[Character] = ["s","o","u","r","c"]
fileprivate let notiUsManageName:String = "E"

/*: "num" :*/
fileprivate let userPerId:String = "NUM"

/*: "UID: :*/
fileprivate let const_holdValue:[Character] = ["U","I","D",":"]

/*: "btn_live_closed_nor" :*/
fileprivate let user_theData:[Character] = ["b","t","n","_","l","i","v","e","_","c","l","o"]
fileprivate let const_thinkName:[Character] = ["s"]
fileprivate let notiEqualFormat:[Character] = ["e","d","_","n","o","r"]
...
class BodyTopView: UIView {
    //: weak var weakVC: TalkingBaseViewController?
    weak var weakVC: BodyLatViewController?
    // 右侧按钮点击Block
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= SteppingMotorThen.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                self.countdownLab.text = "\(main_tabTitle - noFaceTime)s"
                // svga
                //: self.svgaView.isHidden = false
                self.svgaView.isHidden = false
                //: self.svgaView.refreshView(countdown: "\(MAX_NOFACETIME-noFaceTime)")
                self.svgaView.locationCap(countdown: "\(main_tabTitle - noFaceTime)")
                //: } else {
            } else {
    ...
    private var liveModel = FluModelType()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_ficeTitle.map{spanPrivate(take: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(location), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nickNameLab: UILabel = {
        ...
        lab.font = UIFont.platformDisk(fontSize: 11)
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.hr(name: (String(show_playTitle.suffix(6)) + String(const_referId.suffix(6)) + "low_pre")), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(foreClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.hr(name: (String(data_freshMessage) + constRateValue.replacingOccurrences(of: "recommendation", with: "r"))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(miniPlace), for: .touchUpInside)
    ...
    private lazy var liveStatusImgV: MagnitudeercalateTalkingButton = {
        ...
        let image = UIImage.toReject(color: UIColor(hex: (String(userDrawKey) + String(notiMutualId)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.outPush(), for: .normal)
        img.setImage(image.outPush(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(data_productFormat)).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var countdownView: UIView = {
        ...
        icon.backgroundColor = UIColor(hex: (String(kHoneyValue.prefix(5)) + String(dataArriveViolationKey.suffix(2))))
    ...
    private lazy var countdownLab: UILabel = {
        ...
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.text = " \(SteppingMotorThen.share.appConfigMode.countdown2)s"
        lab.text = " \(SteppingMotorThen.share.appConfigMode.countdown2)s"
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.micState(color: UIColor.black.withAlphaComponent(0.25), forState: .normal)
        //: btn.addTarget(self, action: #selector(onlineNumBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(diversion), for: .touchUpInside)
    ...
    private lazy var onlineListView: ReloadlyReactiveCompatible = {
        //: let v = TalkingLiveRoomOnlineListView()
        let v = ReloadlyReactiveCompatible()
...
extension BodyTopView {
    ...
    @objc func page(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(noti_hiddenSituationMessage.replacingOccurrences(of: "life", with: "e") + "Atte" + dataViaStopTrackTitle.replacingOccurrences(of: "about", with: "o") + "nUid")] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(app_tailKey.replacingOccurrences(of: "range", with: "n") + "ionUi" + user_memoryName.replacingOccurrences(of: "para", with: "d"))] = uid
            //: dict["source"] = "1"
            dict[(String(k_spaceKey) + notiUsManageName.lowercased())] = "1"
        }
        //: TalkingChatRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
        TimeTestedRequestTool.displayTemp(isAttention: !isAttention, params: dict) { succeed, _, _ in
            //: completion?(succeed)
            completion?(succeed)
        }
    }
...
extension BodyTopView {
    ...
    @objc private func diversion() {
        //: self.onlineListView.showView(chatGroupId: liveModel.chatGroupId)
        self.onlineListView.checkChild(chatGroupId: liveModel.chatGroupId)
    }
    ...
    @objc private func agree() {
        ...
        ControlBodyReactiveCompatible().section(chatGroupId: liveModel.chatGroupId, completion: { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: let num = json["num"].string
            let num = json[(userPerId.lowercased())].string
    ...
    private func minimize() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(agree), userInfo: nil, repeats: true)
    }
...
extension BodyTopView {
    ...
    func createDelay(_ liveModel: FluModelType) {
        ...
        uidLab.text = (String(const_holdValue)) + "\(liveModel.streamerInfo.uid)"
        ...
        icon.salute(urlStr: liveModel.streamerInfo.headPic, placeImg: UIImage.contentTip(sex: liveModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        ...
        if isAnchor {
            ...
            miniBtn.setImage(UIImage.hr(name: (String(data_freshMessage) + constRateValue.replacingOccurrences(of: "recommendation", with: "r"))), for: .normal)
            //: refreshFollow(isAttention: true)
            forepartSumeraction(isAttention: true)
            //: } else {
        } else {
            //: miniBtn.setImage(UIImage.BundleImageNamed(name: "btn_live_closed_nor"), for: .normal)
            miniBtn.setImage(UIImage.hr(name: (String(user_theData) + String(const_thinkName) + String(notiEqualFormat))), for: .normal)
            //: refreshFollow(isAttention: self.liveModel.streamerInfo.isAttention)
            forepartSumeraction(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
    ...
    @objc func miniPlace() {
        ...
        if isAnchor { // 改为小窗口
            ...
            MaterialLiveManager.threadContext().task(smallWindow: true)
            //: currentViewController()?.navigationController?.popViewController(animated: false)
            strokeController()?.navigationController?.popViewController(animated: false)
        }
    }
    ...
    @objc private func location() {
        ...
        heel(uid: String(MaterialLiveManager.threadContext().liveRoomModel.streamerInfo.uid))
    }

    /// 展示个人资料卡
    //: private func showUserCard(uid: String) {
    private func heel(uid: String) {
        //: let userCard = TalkingLiveUserCardView(frame: .zero, uid: uid)
        let userCard = AllyPlusViewDelegate(frame: .zero, uid: uid)
        //: userCard.delegate = weakVC as? any TalkingLiveUserCardViewDelegate
        userCard.delegate = weakVC as? any MilitaryObjectProtocol
        //: userCard.show()
        userCard.accountFrameInner()
    }

    /// 刷新关注按钮
    //: private func refreshFollow(isAttention: Bool) {
    private func forepartSumeraction(isAttention: Bool) {
    ...
    private func render() {
        ...
        self.liveStatusImgV.isHidden = !MaterialLiveManager.threadContext().isLive
    ...
    @objc private func foreClick() {
        //: req_followRequest(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
        page(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.forepartSumeraction(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
}
...
extension BodyTopView {
    ...
    private func produce() {
        ...
        self.frame = CGRect(x: 0, y: 0, width: dataShowDetailPopMessage, height: dataEnvironmentId + actualWidth(w: 68))
