...
fileprivate let constNowFormat:[UInt8] = [0x97,0x90,0x97,0x8a,0xd6,0x9d,0x91,0x9a,0x9b,0x8c,0xc4,0xd7,0xde,0x96,0x9f,0x8d,0xde,0x90,0x91,0x8a,0xde,0x9c,0x9b,0x9b,0x90,0xde,0x97,0x93,0x8e,0x92,0x9b,0x93,0x9b,0x90,0x8a,0x9b,0x9a]

private func sustainLicense(scale num: UInt8) -> UInt8 {
    return num ^ 254
}

/*: "icon_coinxxx" :*/
fileprivate let appBushMessage:[Character] = ["i","c","o","n","_","c","o","i"]
fileprivate let kAlterValue:[Character] = ["n","x","x","x"]

/*: "icon_gift_num_bg" :*/
fileprivate let app_resumePath:[Character] = ["i","c","o","n","_","g","i","f","t"]
fileprivate let notiModifyData:String = "while natural pattern universal resent_num_bg"
...
// __CLOSE_PRINT__
//
//  AttributeRequestReactiveCompatible.swift
//  AbroadTalking
//
...
class AttributeRequestReactiveCompatible: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = MyGiftModel()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constNowFormat.map{sustainLicense(scale: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.opPoint(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: MagnitudeercalateTalkingButton = {
        //: let btn = TalkingButton.init()
        let btn = MagnitudeercalateTalkingButton()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.opPoint(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.hr(name: (String(appBushMessage) + String(kAlterValue))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.hr(name: (String(app_resumePath) + String(notiModifyData.suffix(7))))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.opPoint(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension AttributeRequestReactiveCompatible {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func rid(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.nameure().cgColor
    ...
    func clip(currenmodel: MyGiftModel, giftType: DoweryTermConvertible) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.underStack(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.underStack(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.opPoint(type: .Regular, fontSize: 11),
...
extension AttributeRequestReactiveCompatible {
    ...
    func the() {
        ...
        bgView.backgroundColor = UIColor.colorRemark()
