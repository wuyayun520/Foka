...
fileprivate let constNaturalData:[UInt8] = [0xf2,0xf5,0xf2,0xef,0xb3,0xf8,0xf4,0xff,0xfe,0xe9,0xa1,0xb2,0xbb,0xf3,0xfa,0xe8,0xbb,0xf5,0xf4,0xef,0xbb,0xf9,0xfe,0xfe,0xf5,0xbb,0xf2,0xf6,0xeb,0xf7,0xfe,0xf6,0xfe,0xf5,0xef,0xfe,0xff]

/*: "icon_userdetail_header_shadow" :*/
fileprivate let const_goodMatchName:String = "iselectedon"
fileprivate let mainAttentionTitle:String = "still change right exhibitail_"
fileprivate let main_claimSegmentMsg:String = "message pocket volumeer_sh"

/*: "icon_lounge" :*/
fileprivate let const_exampleFormat:[Character] = ["i","c","o","n","_","l"]
fileprivate let data_modifyFormat:String = "OUNGE"

/*: "Official" :*/
fileprivate let appExclusiveData:[Character] = ["O","f","f","i","c","i"]
fileprivate let data_sameYouOverlookTitle:String = "AL"

/*: "Reviewing" :*/
fileprivate let data_appearanceFormat:String = "Reviewiapproval portion"
fileprivate let main_builderTitle:[Character] = ["n","g"]

/*: "#FF506D" :*/
fileprivate let show_attractiveId:String = "behind max ready am#FF506D"

/*: "icon_user_detail_intimate_header" :*/
fileprivate let main_galleryKey:[UInt8] = [0x9d,0x97,0xa3,0xa2,0x93,0xa9,0xa7,0x99,0xa6,0x93,0x98,0x99,0xa8,0x95,0x9d,0xa0,0x93,0x9d,0xa2,0xa8,0x9d,0xa1,0x95,0xa8,0x99,0x93,0x9c,0x99,0x95,0x98,0x99,0xa6]

fileprivate func outlineProcedure(official num: UInt8) -> UInt8 {
...
/*: "?toUid= :*/
fileprivate let dataLoseMessage:String = "multi to?toUid="

/*: "UserMyCellThen" :*/
fileprivate let constMovieData:String = "until ally gender mentionTUserD"
fileprivate let showDailyMsg:[Character] = ["e","a","d","e","r"]
fileprivate let kResultHePath:[Character] = ["I","t","e","m","C","e","l","l"]

/*: "#FFFFFF" :*/
fileprivate let const_describeId:String = "#Fpreserve entry score modest creation"
fileprivate let noti_flatPath:[Character] = ["F","F","F","F","F"]

/*: "#BDB8B5" :*/
fileprivate let app_rawMessage:[Character] = ["#","B","D","B","8","B","5"]

/*: "head_detail_bottom" :*/
fileprivate let user_rowRevenueUrl:String = "heh"
fileprivate let main_areaPath:String = "past toward bullet ban chinad_d"
fileprivate let mainBubbleFlagOnMsg:String = "l_bottomrank until arc"

/*: "#2DF2FF" :*/
fileprivate let kWhetherStr:String = "#2DF2FFhello picture pub"

/*: "Live" :*/
fileprivate let mainSecretName:String = "thanks anniversaryLive"

/*: "#48FF1E" :*/
fileprivate let app_threadCurFormat:[Character] = ["#","4","8","F","F","1","E"]

/*: "Online" :*/
fileprivate let k_branchHarassmentPath:String = "Onlinemiddle square"

/*: "#FF4B7F" :*/
fileprivate let mainInputValue:String = "#FF4B7Fteam destination"

/*: "busy" :*/
fileprivate let mainCastUrl:[Character] = ["b","u","s","y"]
...
let dataReasonTitle = (302.0 + dataEnvironmentId)

//: class TalkingUserDetailHeaderView: UIView {
class SpecialBannerDelegate: UIView {
    //: var VideoCallBlock: (() -> Void)?
    var VideoCallBlock: (() -> Void)?
    //: private var userInfoModel = TalkingUserInfoModel()
    private var userInfoModel = ParrotTransformable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constNaturalData.map{$0^155}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerView: JXBanner = {
        ...
        let frame = CGRect(x: 0, y: 0, width: dataShowDetailPopMessage, height: dataReasonTitle)
        ...
        view.placeholderImgView.image = UIImage.bringWant()
    ...
    private lazy var shadowBgView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.hr(name: (const_goodMatchName.replacingOccurrences(of: "selected", with: "c") + "_userdet" + String(mainAttentionTitle.suffix(4)) + "head" + String(main_claimSegmentMsg.suffix(5)) + "adow")))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: dataReasonTitle - 15, width: dataShowDetailPopMessage, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.hr(name: (String(const_exampleFormat) + data_modifyFormat.lowercased()))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.micState(color: UIColor.databaseIn(), forState: .normal)
        //: img.setTitle("Official".localized, for: .normal)
        img.setTitle((String(appExclusiveData) + data_sameYouOverlookTitle.lowercased()).localized, for: .normal)
        //: img.setTitleColor(.white, for: .normal)
        img.setTitleColor(.white, for: .normal)
        //: img.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 11)
        img.titleLabel?.font = UIFont.promptReload(fontSize: 11)
    ...
    private lazy var onlineImgV: MagnitudeercalateTalkingButton = {
    ...
    private lazy var liveView: DiscussMiniView = {
        //: let v = UserLivePullStreamsView()
        let v = DiscussMiniView()
        //: v.isHidden = true
        v.isHidden = true
        //: v.playEndBlock = {
        v.playEndBlock = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            guard let self = self else { return }
            //: LatPushManager.share.func_audiencePushToLiveRoomVC(uid: self.userInfoModel.uid, enterType: .userDetail)
            LatPushManager.share.transportationSystem(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(data_appearanceFormat.prefix(7)) + String(main_builderTitle)).localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(show_attractiveId.suffix(7))))
    ...
    lazy var lastLoginLab: UILabel = {
        ...
        lb.font = .opPoint(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
        return lb
    ...
    private lazy var intimateBtn: MagnitudeercalateTalkingButton = {
        ...
        btn.setImage(UIImage.hr(name: String(bytes: main_galleryKey.map{outlineProcedure(official: $0)}, encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(midByClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension SpecialBannerDelegate {
    /// 查看视频
    //: private func goToVideo() {
    private func three() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: self.userInfoModel.videoUrl)
        let vc = DialogCoordinatorViewController(videoPath: self.userInfoModel.videoUrl)
        //: if SteppingMotorThen.share.loginUid != self.userInfoModel.uid {
        if SteppingMotorThen.share.loginUid != self.userInfoModel.uid {
    ...
    private func custom(_ index: Int) {
        ...
        let vc = ControlDataSource(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.strokeController()?.navigationController?.pushViewController(vc, animated: true)
    }

    //: @objc func intimateBtnClick() {
    @objc func midByClick() {
        //: let url = TalkingWebManager.getFullUrl(urlType: .Intimate) + "?toUid=\(self.userInfoModel.uid)"
        let url = SolarBatteryManager.alleged(urlType: .Intimate) + (String(dataLoseMessage.suffix(7))) + "\(self.userInfoModel.uid)"
...
extension SpecialBannerDelegate: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: UserMyCellThen.self, reuseIdentifier: (String(constMovieData.suffix(6)) + "etailH" + String(showDailyMsg) + String(kResultHePath)))
    }

    //: func jxBanner(_ banner: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        //: let bannerCell = cell as! TUserDetailHeaderItemCell
        let bannerCell = cell as! UserMyCellThen
        //: let urlStr = self.dataArray[index]
        let urlStr = self.dataArray[index]

        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == WalkMilitaryOutputStreamable.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            //: bannerCell.setVideoView()
            bannerCell.questionableNor()
            //: bannerCell.configModel(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            bannerCell.config(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.bgImgView.salute(urlStr: urlStr, placeImg: UIImage.cry())
        }
    ...
    func jxBanner(_: JXBannerType, centerIndex: Int, centerCell _: UICollectionViewCell) {
        //: guard userInfoModel.uid == SteppingMotorThen.share.loginUid else {
        guard userInfoModel.uid == SteppingMotorThen.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(const_describeId.prefix(2)) + String(noti_flatPath)))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(app_rawMessage)))!
...
extension SpecialBannerDelegate: JXBannerDelegate {
    ...
    public func jxBanner(_: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == WalkMilitaryOutputStreamable.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            three()
            ...
            if userInfoModel.sex == WalkMilitaryOutputStreamable.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
...
extension SpecialBannerDelegate {
    ...
    func fitView(_ userModel: ParrotTransformable) {
        ...
        if userModel.sex == WalkMilitaryOutputStreamable.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        let normalImg = UIImage.hr(name: (user_rowRevenueUrl.replacingOccurrences(of: "eh", with: "e") + String(main_areaPath.suffix(4)) + "etai" + String(mainBubbleFlagOnMsg.prefix(8)))).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = SteppingMotorThen.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {
            if let vipModel = SteppingMotorThen.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.salute(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        self.nicknameLab.textColor = userModel.loungePlus ? .colorObserver() : .white
        ...
        if userModel.intimateUrl.count > 0 {
            ...
            intimateImgV.underStack(urlStr: userModel.intimateUrl)
        }
        // 在线状态
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.toReject(color: UIColor(hex: (String(kWhetherStr.prefix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.outPush(), for: .normal)
            onlineImgV.setImage(image.outPush(), for: .normal)
            //: onlineImgV.setTitle("Live".localized, for: .normal)
            onlineImgV.setTitle((String(mainSecretName.suffix(4))).localized, for: .normal)
            ...
            let image = UIImage.toReject(color: UIColor(hex: (String(app_threadCurFormat)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.outPush(), for: .normal)
            onlineImgV.setImage(image.outPush(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((String(k_branchHarassmentPath.prefix(6))).localized, for: .normal)
            ...
            let image = UIImage.toReject(color: UIColor(hex: (String(mainInputValue.prefix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.outPush(), for: .normal)
            onlineImgV.setImage(image.outPush(), for: .normal)
            //: onlineImgV.setTitle("busy".localized, for: .normal)
            onlineImgV.setTitle((String(mainCastUrl)).localized, for: .normal)
            //: } else {
        } else {
        ...
        // 直播
        //: if SteppingMotorThen.share.loginUid != userModel.uid &&
        if SteppingMotorThen.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.belowFlagMini(liveUrl: userModel.livePlayUrl)
        }
    }

    /// 添加视图
    //: private func setupSubviews() {
    private func lateSubviews() {
    ...
    private func conversation() {
        ...
        liveView.snp.makeConstraints { make in
            ...
            make.top.equalTo(dataEnvironmentId + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(dataEnvironmentId + 15)
