...
fileprivate let k_assertValue:[UInt8] = [0x46,0x4b,0x46,0x51,0x5,0x40,0x4c,0x41,0x42,0x4f,0x17,0x6,0xfd,0x45,0x3e,0x50,0xfd,0x4b,0x4c,0x51,0xfd,0x3f,0x42,0x42,0x4b,0xfd,0x46,0x4a,0x4d,0x49,0x42,0x4a,0x42,0x4b,0x51,0x42,0x41]

fileprivate func feedbackReload(during num: UInt8) -> UInt8 {
...
/*: "#333333" :*/
fileprivate let k_coverText:String = "#publishpublish3"
fileprivate let main_fitFormat:String = "3"

/*: "#999999" :*/
fileprivate let showGuideDiskValue:[Character] = ["#","9","9","9","9","9"]
fileprivate let noti_patronMsg:String = "9"

/*: "isCupid" :*/
fileprivate let appLieId:String = "isCupiddramatic only target appropriate operate"

/*: "autoGreetAuth" :*/
fileprivate let noti_dropValue:[Character] = ["a","u","t","o","G","r","e"]
fileprivate let app_identifyUrl:String = "etAuthsuccessfully hung area"

/*: "isLeaderboardAnonymous" :*/
fileprivate let kFiftyContent:String = "isLeaderbmutual center tail everyone"
fileprivate let dataAdvanceMemoryName:String = "oardAevaluate river outlet challenge auto"

/*: "isSameGendered" :*/
fileprivate let main_helpId:[Character] = ["i","s","S","a","m","e","G","e","n","d","e","r","e","d"]

/*: "type" :*/
fileprivate let appProtectionWarnText:[Character] = ["t","y","p","e"]

/*: "value" :*/
fileprivate let appStandardValue:String = "VALUE"
...
// __CLOSE_PRINT__
//
//  ShadowNoticeReactiveCompatible.swift
//  AbroadTalking
//
...
class ShadowNoticeReactiveCompatible: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_assertValue.map{feedbackReload(during: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleBLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (k_coverText.replacingOccurrences(of: "publish", with: "33") + main_fitFormat.capitalized))
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var switchView: UISwitch = {
        ...
        swit.addTarget(self, action: #selector(nativity), for: .valueChanged)
        //: return swit
        return swit
        //: }()
    }()
    ...
    lazy var detailLB: CigaretteReactiveCompatible = {
        //: let lb = TextAlignLb.init()
        let lb = CigaretteReactiveCompatible()
        //: lb.setIsTop(isTop: true)
        lb.medalTop(isTop: true)
        //: lb.font = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        lb.font = UIFont.opPoint(type: .Regular, fontSize: 15)
        //: lb.textColor = UIColor.init(hex: "#999999")
        lb.textColor = UIColor(hex: (String(showGuideDiskValue) + noti_patronMsg.capitalized))
        //: lb.isHidden = true
        lb.isHidden = true
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
...
extension ShadowNoticeReactiveCompatible {
    //: func setCellMessage(titile: String, row: Int, isLast: Bool) {
    func messageCell(titile: String, row: Int, isLast: Bool) {
...
//        if titile == ConversionUnderReplaceableCollection.General.rawValue {
//            LabelMpThen.checkPushNotification { isShow in
...
//        }
    ...
    @objc func nativity() {
        ...
        if titleBLB.text == WalkTitleConvertible.Turnon_Notif.rawValue.localized {
        ...
        } else if titleBLB.text == WalkTitleConvertible.Cupid.rawValue.localized {
            //: setConfig(type: "isCupid", value: self.switchView.isOn)
            listener(type: (String(appLieId.prefix(7))), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Automatic.rawValue.localized {
        } else if titleBLB.text == WalkTitleConvertible.Automatic.rawValue.localized {
            //: setConfig(type: "autoGreetAuth", value: self.switchView.isOn)
            listener(type: (String(noti_dropValue) + String(app_identifyUrl.prefix(6))), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Ranking.rawValue.localized {
        } else if titleBLB.text == WalkTitleConvertible.Ranking.rawValue.localized {
            //: setConfig(type: "isLeaderboardAnonymous", value: self.switchView.isOn)
            listener(type: (String(kFiftyContent.prefix(9)) + String(dataAdvanceMemoryName.prefix(5)) + "nonymous"), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Same_gendered.rawValue.localized {
        } else if titleBLB.text == WalkTitleConvertible.Same_gendered.rawValue.localized {
            //: setConfig(type: "isSameGendered", value: self.switchView.isOn)
            listener(type: (String(main_helpId)), value: self.switchView.isOn)
        }
    }

    //: func setConfig(type: String, value: Bool) {
    func listener(type: String, value: Bool) {
        //: let va = value ? 1 : -1
        let va = value ? 1 : -1
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["type"] = type
        dict[(String(appProtectionWarnText))] = type
        //: dict["value"] = va
        dict[(appStandardValue.lowercased())] = va

        //: ProgressHUD.show()
        DirectProgressHUD.discountShow()
        //: TalkingMeRequestTool.req_SettingChange(params: dict) { succeed, result, errorModel in
        GenRequestTool.theme(params: dict) { succeed, _, _ in
            //: ProgressHUD.dismiss()
...
//        if titleBLB.text == ConversionUnderReplaceableCollection.General.rawValue {
//            LabelMpThen.checkPushNotification { isShow in
//                DispatchQueue.main.sync {
//                    self.switchView.isOn = isShow
//                }
