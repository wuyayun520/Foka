...
fileprivate let kBurnText:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Edit Title" :*/
fileprivate let userOperatePath:String = "representative send eyeEdit"

/*: "Finish" :*/
fileprivate let k_makeupData:String = "approximately speaker rescueFinish"

/*: "btn_delete" :*/
fileprivate let notiAspectFinishContent:String = "highlight read boldbtn_"
...
class EchoViewDelegate: UIView {
    ...
    override init(frame: CGRect) {
        ...
        super.init(frame: frame)
        //: self.setupSubviews()
        self.transistoriseSubviews()
        //: self.setupSubViewsConstraint()
        self.narrow()
        //: let tapGesture = UITapGestureRecognizer(target: self, action: #selector(dismissView))
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(containTexture))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kBurnText.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(userOperatePath.suffix(4)) + " Title").localized
    ...
    lazy var inputTView: DialogAddedReactiveCompatible = {
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(k_makeupData.suffix(6))).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.originInstance(colors: UIColor.o(), size: CGSize(width: 267, height: 50)), for: .normal)
        //: btn.layer.cornerRadius = 25
        btn.layer.cornerRadius = 25
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(finishBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(plop), for: .touchUpInside)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.hr(name: (String(notiAspectFinishContent.suffix(4)) + "delete")), for: .normal)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(someSegment), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension EchoViewDelegate {
    //: @objc func dismissView() {
    @objc func containTexture() {}
...
extension EchoViewDelegate: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = packthreadReadingSuite(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func packthreadReadingSuite(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
...
extension EchoViewDelegate {
    ...
    private func transistoriseSubviews() {
