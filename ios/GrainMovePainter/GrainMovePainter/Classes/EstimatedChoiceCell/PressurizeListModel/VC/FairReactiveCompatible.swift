...
/*: "Face verification" :*/
fileprivate let data_officialFormat:[Character] = ["F","a"]
fileprivate let constTitleId:String = "bag bridge reminderce ver"
fileprivate let user_animalExampleBetterUrl:String = "tidemonstrate"
...
class FairReactiveCompatible: BodyLatViewController {
    //: var isRegisterPush = false
    var isRegisterPush = false /// 注册页进行真人认证
    //: var isNoCodePush = false
    var isNoCodePush = false /// 没有填写邀请码进入
    ...
    override func viewDidLoad() {
        ...
        title = (String(data_officialFormat) + String(constTitleId.suffix(6)) + "ifica" + user_animalExampleBetterUrl.replacingOccurrences(of: "demonstrate", with: "on")).localized
        //: self.createUI()
        self.remain()
...
extension FairReactiveCompatible: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    //: private func takePhoto() {
    private func crushPriority() {
        ...
        self.dismiss(animated: true) {
            ...
            if let photo = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
                ...
                if self.isRegisterPush {
                    ...
                    let vc = CaptiveThereStereoscopicPictureRecognizerDelegate(photoImage: photo)
...
extension FairReactiveCompatible {
    ...
    func remain() {
        ...
        mainView.btnClickBlock = { [weak self] () in
            ...
            LabelMpThen.commitBlock(true) { isOpen in
