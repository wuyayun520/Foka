...
fileprivate let user_withoutId:String = "icon_kowhen approval daily mix disappear"
fileprivate let main_positiveEffFormat:String = "pill"
fileprivate let dataMemberFormat:[Character] = ["g","_","k","o","n","g","_","d","e","f","a","u","l","t"]

/*: "page" :*/
fileprivate let user_robotMessage:[UInt8] = [0x40,0x51,0x57,0x55]

private func calculationCarryRegular(filter num: UInt8) -> UInt8 {
    return num ^ 48
}

/*: "uid" :*/
fileprivate let noti_boundName:[UInt8] = [0x64,0x69,0x75]

/*: "list" :*/
fileprivate let constAnswerUrl:[Character] = ["l","i","s","t"]
...
class BeatViewDelegate: BodyLatViewController {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [HemTransformable]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: constQuantityervalId, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(TakeViewCell.self, forCellWithReuseIdentifier: TakeViewCell.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.hr(name: (String(user_withoutId.prefix(7)) + main_positiveEffFormat.replacingOccurrences(of: "pill", with: "n") + String(dataMemberFormat)))
...
extension BeatViewDelegate {
    //: private func headerRefresh() {
    private func indicatorTwo() {
        //: pageIndex = 0
        pageIndex = 0
        //: load_listData()
        list()
    ...
    private func list() {
        ...
        let params = [String(bytes: user_robotMessage.map{calculationCarryRegular(filter: $0)}, encoding: .utf8)!: pageIndex, String(bytes: noti_boundName.reversed(), encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        DrawingRequestTool.detailCompletion(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(String(constAnswerUrl))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = ContentTheoryDialogMeasurable.deserialize(from: dict as? Dictionary) {
...
extension BeatViewDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: TakeViewCell.className(), for: indexPath) as! TakeViewCell
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = TakeThen(model: detailModel)
...
extension BeatViewDelegate: JXPagingViewListViewDelegate {
...
extension BeatViewDelegate {
    ...
    func flush(_ userModel: ParrotTransformable) {
