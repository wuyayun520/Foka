...
fileprivate let const_slipName:[UInt8] = [0xd5,0xd2,0xd5,0xc8,0x94,0xdf,0xd3,0xd8,0xd9,0xce,0x86,0x95,0x9c,0xd4,0xdd,0xcf,0x9c,0xd2,0xd3,0xc8,0x9c,0xde,0xd9,0xd9,0xd2,0x9c,0xd5,0xd1,0xcc,0xd0,0xd9,0xd1,0xd9,0xd2,0xc8,0xd9,0xd8]

private func clearlyWord(except num: UInt8) -> UInt8 {
    return num ^ 188
}

/*: "Post" :*/
fileprivate let userAdvicePath:String = "Posterror solid exactly increase conduct"

/*: "Profile" :*/
fileprivate let main_missName:String = "Profileready scramble develop"

/*: "bth_me_follow_nor" :*/
fileprivate let userCorrelationMsg:String = "document instance engagementbth_m"
fileprivate let kCurrencyStr:String = "package sune_fol"
fileprivate let data_writtenMessage:String = "low_norback motivation"

/*: "removeAttentionUid" :*/
fileprivate let constCompositionStr:[Character] = ["r","e","m","o","v","e","A","t","t","e"]
fileprivate let appBrainPath:String = "NT"
fileprivate let user_offerUrl:[Character] = ["i","o","n","U","i","d"]

/*: "attentionUid" :*/
fileprivate let userSeekTitle:String = "aretainretainenretain"
fileprivate let dataSinceId:[Character] = ["d"]

/*: "bth_me_follow_pre" :*/
fileprivate let kKingName:[UInt8] = [0x31,0x43,0x37,0x2e,0x3c,0x34,0x2e,0x35,0x3e,0x3b,0x3b,0x3e,0x46,0x2e,0x3f,0x41,0x34]

fileprivate func duringPublicly(maximum num: UInt8) -> UInt8 {
...
class SpringViewController: BodyLatViewController {
    //: var uid: String = ""
    var uid: String = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_slipName.map{clearlyWord(except: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var navView: ExpressionPowBarView = { // 导航
        //: let v = TalkingUserDetailNaviBarView()
        let v = ExpressionPowBarView()
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var headerView: SpecialBannerDelegate = { // banner
        ...
        let v = SpecialBannerDelegate(frame: frame)
    ...
    private lazy var bottomView: VascularSystemView = { // bottom
        //: let v = TalkingUserDetailBottomView(frame: .zero)
        let v = VascularSystemView(frame: .zero)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var segmentedView: JXSegmentedView = {
        ...
        let segmentedView = JXSegmentedView(frame: CGRect(x: 0, y: dataEnvironmentId, width: dataShowDetailPopMessage, height: 44))
        ...
        segmentedDataSource.titleNormalColor = UIColor.uniformColor()
        //: segmentedDataSource.titleSelectedColor = UIColor.appTitleColor()
        segmentedDataSource.titleSelectedColor = UIColor.quitTo()
        ...
        indicator.indicatorColor = UIColor.quitTo()
    ...
    private lazy var pagerView: JXPagingListRefreshView = {
        ...
        view.pinSectionHeaderVerticalOffset = Int(show_turnStr)
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var titleArray: [String] = //: return ["Post".localized, "Profile".localized]
        [(String(userAdvicePath.prefix(4))).localized, (String(main_missName.prefix(7))).localized]
    //: }()

    //: private lazy var dataVC: [TalkingBaseViewController] = {
    private lazy var dataVC: [BodyLatViewController] = //: return [postVC, profileVC]
        [postVC, profileVC]
    //: }()

    //: private lazy var postVC: TalkingUserDetailPostViewController = {
    private lazy var postVC: BeatViewDelegate = {
        //: let vc = TalkingUserDetailPostViewController()
        let vc = BeatViewDelegate()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var profileVC: WhoObjectProtocol = {
        //: let vc = TalkingUserDetailProfileViewController()
        let vc = WhoObjectProtocol()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var followBtn: UIImageView = {
        ...
        img.image = UIImage.hr(name: (String(userCorrelationMsg.suffix(5)) + String(kCurrencyStr.suffix(5)) + String(data_writtenMessage.prefix(7))))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(followButtonClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(tailgateBanClick))
...
extension SpringViewController {
    ...
    private func dot() {
        ...
        if userInfoModel.isAttention {
            ...
            dict[(String(constCompositionStr) + appBrainPath.lowercased() + String(user_offerUrl))] = userInfoModel.uid
            //: isattion = false
            isattion = false
            //: } else {
        } else {
            //: dict["attentionUid"] = userInfoModel.uid
            dict[(userSeekTitle.replacingOccurrences(of: "retain", with: "t") + "ionUi" + String(dataSinceId))] = userInfoModel.uid
        }
        //: TalkingChatRequestTool.req_atationTool(isAttention: isattion, params: dict) { succeed, result, errorModel in
        TimeTestedRequestTool.displayTemp(isAttention: isattion, params: dict) { succeed, _, _ in
    ...
    func searched() {
        ...
        queue.async(group: group) {
            ...
            RequestThen.tidings(uid: self.uid) { succeed, result, _ in
                //: self.setNoremarlView()
                self.filter()
                //: if self.uid == SteppingMotorThen.share.loginUid {
                if self.uid == SteppingMotorThen.share.loginUid {
                    //: self.bottomView.showEditBtn()
                    self.bottomView.showPersonBracket()
                }
                //: if succeed == true {
                if succeed == true {
                    ...
                    if let userModel = JSONDeserializer<ParrotTransformable>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                        ...
                        self.navView.narrowView(userModel)
                        //: self.headerView.refreshView(userModel)
                        self.headerView.fitView(userModel)
                        //: self.bottomView.refreshView(userModel)
                        self.bottomView.saveResolutionView(userModel)
                        //: self.postVC.refreshPostViewController(userModel)
                        self.postVC.flush(userModel)
                        // 场景三：男性用户邀请通话弹窗监听
                        ...
                        (VersePowKingfisherCompatible.shared as! VersePowKingfisherCompatible).provostMarshal(uid: userModel.uid, videoUrl: userModel.videoUrl, headPic: userModel.headPic)
                    }
                }
                //: group.leave()
                group.leave()
        ...
        queue.async(group: group) {
            ...
            RequestThen.smooth(uid: self.uid) { succeed, result, _ in
                //: if succeed == true {
                if succeed == true {
                    ...
                    for dic in result as! [[String: Any]] {
                        ...
                        if let model = TakeTransformable.deserialize(from: dic as? Dictionary) {
    ...
    private func reloadOkNotify() {
        //: refreshfollowButtonStatus()
        pairDiskStatus()

        // profile分区数据
        //: var profileSectionArr = [TUserDetailCellType.profileType]
        var profileSectionArr = [CompartmentCellType.profileType]
        ...
        self.profileVC.isSequence(giftsArr, profileSectionArr, userInfoModel)
    }
}
...
extension SpringViewController {
    ...
    @objc private func tailgateBanClick() {
        //: req_followRequest()
        dot()
    }
    ...
    private func pairDiskStatus() {
        //: self.followBtn.isHidden = (userInfoModel.uid == SteppingMotorThen.share.loginUid)
        self.followBtn.isHidden = (userInfoModel.uid == SteppingMotorThen.share.loginUid)
        //: let followImgStr = self.userInfoModel.isAttention ? "bth_me_follow_pre":"bth_me_follow_nor"
        let followImgStr = self.userInfoModel.isAttention ? String(bytes: kKingName.map{duringPublicly(maximum: $0)}, encoding: .utf8)! : (String(userCorrelationMsg.suffix(5)) + String(kCurrencyStr.suffix(5)) + String(data_writtenMessage.prefix(7)))
        //: self.followBtn.image = UIImage.BundleImageNamed(name: followImgStr)
        self.followBtn.image = UIImage.hr(name: followImgStr)
    }
    ...
    private func cuttingEdge() {
        ...
        guard userInfoModel.loungePlus else { return }
        //: guard let vipModel = SteppingMotorThen.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userInfoModel.vipSkinId}).first else { return }
        guard let vipModel = SteppingMotorThen.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userInfoModel.vipSkinId }).first else { return }
        ...
        vipCenterView.underStack(urlStr: vipModel.vipSkinCenter, placeImg: nil)
        //: vipBottomView.setUrlImage(urlStr: vipModel.vipSkinBottom, placeImg: nil)
        vipBottomView.underStack(urlStr: vipModel.vipSkinBottom, placeImg: nil)
    }
}
...
extension SpringViewController: JXSegmentedViewDelegate {
...
extension SpringViewController: JXPagingViewDelegate {
...
extension SpringViewController {
    ...
    private func filter() {
        ...
        bottomView.snp.makeConstraints { make in
            ...
            make.height.equalTo(constQuantityervalId)
        }
    }
    ...
    private func block() {
        ...
        pagerView.frame = CGRect(x: 0, y: 0, width: dataShowDetailPopMessage, height: dataViewPath)
