...
fileprivate let noti_walkPath:[UInt8] = [0x78,0x7f,0x78,0x65,0x39,0x72,0x7e,0x75,0x74,0x63,0x2b,0x38,0x31,0x79,0x70,0x62,0x31,0x7f,0x7e,0x65,0x31,0x73,0x74,0x74,0x7f,0x31,0x78,0x7c,0x61,0x7d,0x74,0x7c,0x74,0x7f,0x65,0x74,0x75]

/*: "8075F5" :*/
fileprivate let noti_transferPath:String = "advice fall8075F5"

/*: "9A9B9C" :*/
fileprivate let const_connectPath:String = "title cell car9A9B9C"

/*: "kOpacityAnimation" :*/
fileprivate let constMDialogData:String = "chart requestkOpac"
fileprivate let constNumbValue:String = "ityAnicamera refer starting merely choose"

/*: "opacity" :*/
fileprivate let dataLockKey:String = "oret"
fileprivate let data_pubStr:String = "acitcomputer"

/*: "forwards" :*/
fileprivate let user_dignityMessage:[Character] = ["f","o","r","w","a","r"]
fileprivate let app_motivationUrl:[Character] = ["d","s"]

/*: "easeIn" :*/
fileprivate let app_dimensionFormat:[Character] = ["e","a","s","e","I","n"]

/*: "8C7AF8" :*/
fileprivate let data_arcTitle:String = "lift how para loop8C7AF"
fileprivate let dataGeneralNearKey:String = "8"
...
class FunView: UIView {
    ...
    public init(frame: CGRect, codeNumber: Int = 6, labelTextColor: UIColor = UIColor.black, mainColor: UIColor = UIColor.orange, normalColor: UIColor = UIColor.gray, margin: CGFloat = 12.0) {
        ...
        self.adhereTodayReciprocation()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: noti_walkPath.map{$0^17}, encoding: .utf8)!)
    }
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(pressOrientation(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension FunView {
    ...
    func allow() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (String(noti_transferPath.suffix(6))))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (String(const_connectPath.suffix(6))))!.withAlphaComponent(0.25).cgColor
    ...
    fileprivate func afterLayer(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(constMDialogData.suffix(5)) + String(constNumbValue.prefix(6)) + "mation"))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(gravityLibrary(), forKey: (String(constMDialogData.suffix(5)) + String(constNumbValue.prefix(6)) + "mation"))
        }
    ...
    fileprivate func gravityLibrary() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (dataLockKey.replacingOccurrences(of: "ret", with: "p") + data_pubStr.replacingOccurrences(of: "computer", with: "y")))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (String(user_dignityMessage) + String(app_motivationUrl)))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (String(app_dimensionFormat))))
        //: return animation
        return animation
    }
}
...
extension FunView {
    ...
    private func panelSubviews() {
        ...
        for index in 0 ..< codeNumber {
            ...
            line.fillColor = UIColor(hex: (String(data_arcTitle.suffix(5)) + dataGeneralNearKey.capitalized))?.cgColor
    ...
    private func adhereTodayReciprocation() {}
}
