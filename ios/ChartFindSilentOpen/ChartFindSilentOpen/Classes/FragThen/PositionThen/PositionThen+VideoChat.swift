...
/*: "type" :*/
fileprivate let layoutObjectPlatform:[Character] = ["t","y","p","e"]

/*: "You are currently on a call, please try again later" :*/
fileprivate let screenOverlookData:[UInt8] = [0x60,0x76,0x7c,0x27,0x68,0x79,0x6c,0x27,0x6a,0x7c,0x79,0x79,0x6c,0x75,0x7b,0x73,0x80,0x27,0x76,0x75,0x27,0x68,0x27,0x6a,0x68,0x73,0x73,0x33,0x27,0x77,0x73,0x6c,0x68,0x7a,0x6c,0x27,0x7b,0x79,0x80,0x27,0x68,0x6e,0x68,0x70,0x75,0x27,0x73,0x68,0x7b,0x6c,0x79]

fileprivate func goodUpon(little num: UInt8) -> UInt8 {
    let value = Int(num) - 7
    if value < 0 {
...
/*: "onCallResume" :*/
fileprivate let colorBrakeValue:String = "onCallReflow bass house examine base"
fileprivate let layoutPaperTitle:String = "SUME"
...
extension PositionThen {
    ...
    func thresholdSituation(info: [String: Any], situation: FrameworkChatSituation = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
        //: let type = json["type"].intValue
        let type = json[(String(layoutObjectPlatform))].intValue
...
extension PositionThen {
    ...
    private func fallibleTerrible(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.isFamous {
        if !resumeTalk && TopThen.shared.isFamous {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.colouration(showMsg: String(bytes: screenOverlookData.map{goodUpon(little: $0)}, encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if famousTick()!.isKind(of: TelecastingHiObjectProtocol.self) ||
            //: currentViewController()!.isKind(of: TalkingVoiceChatViewController.self) ||
            famousTick()!.isKind(of: FrameworkViewController.self) ||
            //: currentViewController()!.isKind(of: TalkingVideoInitivCallVC.self) {
            famousTick()!.isKind(of: InstantiatePlayerDelegate.self)
        {
            //: return
            return
        }
        ...
        if (SensitiveFamousApplication.shared as! SensitiveFamousApplication).inviteCallView != nil {
            //: (TalkingApplication.shared as! TalkingApplication).inviteCallView?.closeView()
            (SensitiveFamousApplication.shared as! SensitiveFamousApplication).inviteCallView?.totalerrupt()
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! TelecastingHiObjectProtocol).chatModel = GrantChatModel.middle(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = FrameworkViewController()
            ...
            (vc as! FrameworkViewController).chatModel = GrantChatModel.middle(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(String(colorBrakeValue.prefix(8)) + layoutPaperTitle.lowercased())].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.articleData(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func articleData(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = famousTick() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            //: let rootVc = FragThen.getWindow().rootViewController as! UITabBarController
            let rootVc = FragThen.termsSource().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(FrameworkViewController.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(TelecastingHiObjectProtocol.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            var currentChatVc: TelecastingHiObjectProtocol?
            //: let pushChatVc = roomVC as! TalkingVideoChatViewController
            let pushChatVc = roomVC as! TelecastingHiObjectProtocol

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVideoChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? TelecastingHiObjectProtocol
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.recordToo(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: TamFamousControllerDelegate.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! TamFamousControllerDelegate
            ...
            var currentChatVc: FrameworkViewController?
            //: let pushChatVc = roomVC as! TalkingVoiceChatViewController
            let pushChatVc = roomVC as! FrameworkViewController

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVoiceChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? FrameworkViewController
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.detail(resumeData: resumeData)
                }
                //: return
                return
            }
