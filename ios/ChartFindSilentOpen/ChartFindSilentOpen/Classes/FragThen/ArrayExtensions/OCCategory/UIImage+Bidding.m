...
@implementation UIImage (Bidding)

//: - (NSData *)compressedImageData {
- (NSData *)velleity {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.distanceModelCompress;
...
- (UIImage *)harsh {
    ...
    return [UIImage imageWithData:[self velleity]];
}


//: - (UIImage *)wcSessionCompress {
- (UIImage *)hideDeviceDisappear {
    //: return [self wcCompres:YES];
    return [self totalerval:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)distanceModelCompress {
    //: return [self wcCompres:NO];
    return [self totalerval:NO];
}
...
- (UIImage *)totalerval:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self size:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self animal:size];
...
+ (NSData *)enter:(UIImage *)image read:(NSInteger)fileSize{
    ...
    if (shortSize>1080) {//压缩到最短边不大于1080,也避免了shortSize==0的情况
        ...
        image = [image gesture:targetSize];
    }

    //: UIImage *thumIMG = image;
    UIImage *thumIMG = image;
    //: NSData *outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
    NSData *outIMGData = [thumIMG nameureDistance:(fileSize*1024)];
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG center:scalesize flourish:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG nameureDistance:(fileSize*1024)];
    }
...
- (UIImage*)gesture:(CGSize)targetSize{
...
- (UIImage *)center:(CGSize)targetSize flourish:(BOOL)isScale{
...
- (NSData *)nameureDistance:(NSInteger)fileSize{
...
+ (UIImage *)fragment:(UIImage*)orginImage putUp:(NSUInteger)level {
    // 获取BitmapData
