...
/*: "add_header_cookies" :*/
fileprivate let kInsideStretchDevice:String = "add_htail upper recall original system"
fileprivate let appSunModeLogger:String = "_"
fileprivate let colorCollectionValue:[Character] = ["c","o","o","k","i","e","s"]
...
public extension UIImageView {
    ...
    @objc func imitation(urlStr: String,
    ...
    internal func pup(urlStr: String,
                      //: placeImg: UIImage = UIImage.placeImgSquare(),
                      placeImg: UIImage = UIImage.rimePecCorporate(),
    ...
    @objc func belowMax(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: layoutStatusSession.wander(), with: componentFormalPath)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.historicPeriod())
    }
    ...
    @objc func rereadMorro(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: HarvestThen.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(kInsideStretchDevice.prefix(5)) + "eader" + appSunModeLogger.capitalized + String(colorCollectionValue)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.rimePecCorporate(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
