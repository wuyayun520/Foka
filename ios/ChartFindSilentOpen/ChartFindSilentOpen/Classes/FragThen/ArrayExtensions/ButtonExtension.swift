...
/*: "recharge" :*/
fileprivate let screenFormTitle:String = "reweeklya"
fileprivate let displayTheoreticAlert:[Character] = ["r","g","e"]

/*: "recharge_ :*/
fileprivate let displayPremiumSystem:String = "refighterha"
fileprivate let coreContainerSession:[Character] = ["r","g","e","_"]

/*: ".png" :*/
fileprivate let spacingResignText:[Character] = [".","p","n","g"]

/*: .png" :*/
fileprivate let viewBroadcastSession:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let appFastEyeCleanUtility:String = ".jpgrush moon ad"

/*: .jpg" :*/
fileprivate let coreMeanwhilePreference:String = "filter sleeve.jpg"
...
enum RationalImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class GrantRushTalkingButton: UIButton {
    ...
    var imageAlignment: RationalImageAlignment = .left
...
extension UIButton {
    ...
    func singleFileTitle(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        blue(interval: interval, topOffset: 0)
    }
    ...
    func blue(interval: CGFloat, topOffset: CGFloat) {
    ...
    func comfort(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func innerDoing(color: UIColor, forState: UIControl.State) {
    ...
    func riskyOf(urlStr: String,
                 //: placeImg: UIImage? = UIImage.placeImgSquare(),
                 placeImg: UIImage? = UIImage.rimePecCorporate(),
                 ...
                 loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: layoutStatusSession.wander(), with: componentFormalPath)
    ...
    func imaginationImage(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((screenFormTitle.replacingOccurrences(of: "weekly", with: "ch") + String(displayTheoreticAlert))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (screenFormTitle.replacingOccurrences(of: "weekly", with: "ch") + String(displayTheoreticAlert)), with: (displayPremiumSystem.replacingOccurrences(of: "fighter", with: "c") + String(coreContainerSession)) + "\(screenSaltConfig)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.riskyOf(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(spacingResignText))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(spacingResignText)), with: "_\(screenSaltConfig)" + (String(viewBroadcastSession)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(appFastEyeCleanUtility.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(appFastEyeCleanUtility.prefix(4))), with: "_\(screenSaltConfig)" + (String(coreMeanwhilePreference.suffix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.riskyOf(urlStr: newUrl, placeImg: nil)
    }
