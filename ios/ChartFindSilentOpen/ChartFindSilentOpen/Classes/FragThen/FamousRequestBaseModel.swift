...
/*: "http://stat. :*/
fileprivate let componentPasseHopMetrics:String = "htthold"

/*: .com" :*/
fileprivate let kGuidePath:String = ".comletter situation tern"

/*: "http://" :*/
fileprivate let componentCharacterPage:String = "spot"
fileprivate let themeSectionPreference:[Character] = ["t","t","p",":","/","/"]

/*: "https://" :*/
fileprivate let spacingRollingHelper:String = "range estimated turn ball ophttps://"

/*: "http://m. :*/
fileprivate let spacingSplitTimer:String = "off rank load perhttp:/"
fileprivate let colorAmUrl:[Character] = ["/","m","."]

/*: "http:// :*/
fileprivate let layoutRepresentationContent:String = "HT"
fileprivate let componentRetSystem:[Character] = ["t","p",":","/","/"]

/*: "3,90" :*/
fileprivate let sessionReplyRequestPreference:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let spacingNoticeSizePlatform:String = "1990-01"
fileprivate let moduleFreshClothingMetrics:[Character] = ["-","0","1"]

/*: "MM-dd-yyyy" :*/
fileprivate let componentReachForwardAlert:String = "MM-ddaccept fatal gender previous logical"
fileprivate let widgetGoingAliveYouthPath:[Character] = ["-","y","y","y","y"]
...
class WorkplaceQueryRequestModel: NSObject {
    ...
    var requestType: VersionRequestType?
    ...
    override init() {
        ...
        self.requestServer = HarvestThen.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func potpourriObject() -> Bool {
...
struct HarvestModelType: HandyJSON {
    //: var errno: Int!
...
public struct FluErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct SizeTransformable: HandyJSON {
    ...
    var CSConfig = PreferenceTransformable() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (componentPasseHopMetrics.replacingOccurrences(of: "hold", with: "p") + "://stat.") + "\(componentFormalPath)" + (String(kGuidePath.prefix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((componentCharacterPage.replacingOccurrences(of: "spot", with: "h") + String(themeSectionPreference))) && !statDomain.hasPrefix((String(spacingRollingHelper.suffix(8)))) {
                //: return "http://".appending(statDomain)
                return (componentCharacterPage.replacingOccurrences(of: "spot", with: "h") + String(themeSectionPreference)).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(spacingSplitTimer.suffix(6)) + String(colorAmUrl)) + "\(componentFormalPath)" + (String(kGuidePath.prefix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (layoutRepresentationContent.lowercased() + String(componentRetSystem)) + "\(urlDomain)"
        }
    }
...
public class IndividualHandyJSON: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [ReachMeasurable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [ReachMeasurable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [ReachMeasurable] = [] // 语音收费设置
    ...
    var vipSetting = [PromiseMTheorySettingModel]() // vip皮肤
    //: var gameShowBit = 0
    ...
    var tabPageBanner = [EveryMeasurable]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = NumbererventionTransformable() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class LanguageThen: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [SensitiveTagModel]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [SensitiveTagModel]?
    ...
    var gallery: [FrameworkHandyJSON]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (sessionReplyRequestPreference.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.park(self.birthday ?? (spacingNoticeSizePlatform.capitalized + String(moduleFreshClothingMetrics)), dateFormat: (String(componentReachForwardAlert.prefix(5)) + String(widgetGoingAliveYouthPath)))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.runUniversal().year else { return "0" }
...
struct FrameworkHandyJSON: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct SensitiveTagModel: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func restore(compareTo: SensitiveTagModel) -> Bool {
        //: return
        return
...
struct TravelerRushMeanSunModelType: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [AuthorisationHandyJSON]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [AuthorisationHandyJSON]!
}

//: struct UserTagTypeModel: HandyJSON {
struct AuthorisationHandyJSON: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [SensitiveTagModel]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct PreferenceTransformable: HandyJSON {
...
struct PromiseMTheorySettingModel: HandyJSON {
...
struct NumbererventionTransformable: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
