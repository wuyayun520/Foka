...
protocol SparkNavigationDelegate: AnyObject {
    /// current selected information.
    /// - Parameters:
    ///   - viewController: selected VC.
    ///   - index: current
    //: func selectedViewController(_ viewController: UIViewController, index: Int)
    func lock(_ viewController: UIViewController, index: Int)
}

//: class QYSlideNavigationViewController: UIViewController {
class EnhanceAcidViewDelegate: UIViewController {
    ...
    weak var delegate: SparkNavigationDelegate?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = FragThen.getScreenHeight() - (topDistance + kTitle_H)
        var height = FragThen.sendHeight() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= coreTitlePlatform
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: FragThen.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: FragThen.funGet(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(FragThen.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(FragThen.funGet()), height: 0)
        ...
        scrollView.semanticContentAttribute = PushThen.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: FragThen.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: FragThen.funGet()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: FragThen.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: FragThen.funGet()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = PushThen.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(equip), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = FragThen.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = FragThen.funGet() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(FragThen.getScreenWidth()-originX-titleWidth)
            let x = PushThen.shared.direction == .leftToRight ? originX:(FragThen.funGet()-originX-titleWidth)
...
extension EnhanceAcidViewDelegate {
    //: @objc private func titleButtonClick(_ sender: UIButton) {
    @objc private func equip(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        asSelectedButton(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * FragThen.getScreenWidth()
        let offset_x = CGFloat(index) * FragThen.funGet()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        pointSuccessfullyPrep(withIndex: index)
        //: seletedViewController(withIndex: index)
        rootIndex(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func pointSuccessfullyPrep(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * FragThen.getScreenWidth()
        let origin_x = CGFloat(index) * FragThen.funGet()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: FragThen.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: FragThen.funGet(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func asSelectedButton(_ button: UIButton) {
    ...
    func hub(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= FragThen.getScreenWidth() {
        if titleScrollView.contentSize.width <= FragThen.funGet() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - FragThen.getScreenWidth() * 0.5
        var offset_x = sender.center.x - FragThen.funGet() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - FragThen.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - FragThen.funGet()
...
extension EnhanceAcidViewDelegate {
    ...
    func rootIndex(withIndex index: Int) {
        //: guard index < children.count else { return }
        guard index < children.count else { return }
        //: delegate?.selectedViewController(children[index], index: index)
        delegate?.lock(children[index], index: index)
    }
}

// MARK: - UIScrollViewDelegate
//: extension QYSlideNavigationViewController: UIScrollViewDelegate {
extension EnhanceAcidViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / FragThen.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / FragThen.funGet())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            asSelectedButton(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        pointSuccessfullyPrep(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / FragThen.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / FragThen.funGet())
        ...
        //: let scaleR = scrollView.contentOffset.x / FragThen.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / FragThen.funGet() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
