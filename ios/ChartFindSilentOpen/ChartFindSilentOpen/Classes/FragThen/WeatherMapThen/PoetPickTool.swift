...
/*: "Finish" :*/
fileprivate let sessionBoldUtility:[Character] = ["F","i","n","i","s","h"]

/*: "Cancel" :*/
fileprivate let moduleMiddleHelper:[Character] = ["C","a","n","c","e","l"]

/*: "btn_photo_choice_pre" :*/
fileprivate let coreIgnoreLogger:[Character] = ["b","t","n","_","p","h"]
fileprivate let colorBindKey:[Character] = ["o"]
fileprivate let themeAliveSystem:String = "difference beyondto_c"
fileprivate let featureSatisfyUrl:String = "fill birth actor maybe expression_pre"

/*: "btn_photo_add_nor" :*/
fileprivate let sessionJusText:[Character] = ["b","t","n","_","p","h","o","t","o","_","a","d","d","_","n","o","r"]

/*: "/tmp" :*/
fileprivate let viewWanderKey:String = "/tmpworld find catch drag"

/*: "Export session failed" :*/
fileprivate let colorSoonPath:String = "mine choice flag fruit lureExport"
fileprivate let commonObserveKey:String = "ion faileambit greet warn one install"
fileprivate let themeWorkUtility:[Character] = ["d"]

/*: "Export canceled" :*/
fileprivate let serviceTempPanPreference:[Character] = ["E","x","p","o","r","t"," ","c","a","n"]
fileprivate let kAtTimer:[Character] = ["c","e","l","e","d"]

/*: "Successful!" :*/
fileprivate let featureFinitePlatform:String = "property rag where trap sleeveSucces"
fileprivate let themeThirtyConfig:String = "!"

/*: "Unsupported video formats" :*/
fileprivate let screenAsideId:[Character] = ["U","n","s","u","p","p","o","r","t","e","d"," ","v","i","d","e","o"," ","f","o","r","m","a","t","s"]

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let appFieldPath:String = "seekseekseekseek"
fileprivate let layoutAggressionPlatform:String = "d-HH:minfrastructure military ask occasion tin"
fileprivate let kLayerDevice:[Character] = ["-","S","S","S"]

/*: "/tmp/video- :*/
fileprivate let featureButterPage:[Character] = ["/","t","m","p","/"]
fileprivate let displaySecuritySession:[Character] = ["v","i","d","e","o","-"]

/*: .mp4" :*/
fileprivate let coreBeyondMessage:String = "condition added how.mp4"
...
class PoetPickTool: NSObject {
    ...
    class func assetActual(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = moduleAdjustPath * 488 / 375
            //: if needCircleCrop == true {
            if needCircleCrop == true { // 切圆图
                //: height = ScreenWidth
                height = moduleAdjustPath
            }
            //: imagePicker?.cropRect = CGRect(x: 0, y: (ScreenHeight-height)/2, width: ScreenWidth, height: height)
            imagePicker?.cropRect = CGRect(x: 0, y: (componentMeEvent - height) / 2, width: moduleAdjustPath, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        imagePicker?.doneBtnTitleStr = (String(sessionBoldUtility)).localized
        //: imagePicker?.cancelBtnTitleStr = "Cancel".localized
        imagePicker?.cancelBtnTitleStr = (String(moduleMiddleHelper)).localized

        //: imagePicker?.barItemTextFont = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.modelSize(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.valueColor()
        ...
        imagePicker?.naviTitleFont = UIFont.modelSize(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.restoreAudience()
        ...
        imagePicker?.iconThemeColor = UIColor.tributeChromaticColour()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.BundleImageNamed(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.titled(name: (String(coreIgnoreLogger) + String(colorBindKey) + String(themeAliveSystem.suffix(4)) + "hoice" + String(featureSatisfyUrl.suffix(4))))
        //: imagePicker?.takePictureImage = UIImage.BundleImageNamed(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.titled(name: (String(sessionJusText)))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                doneButton?.titleLabel?.font = UIFont.modelSize(type: .Medium, fontSize: 14)
                ...
                if text.count > 0 && Int(text)! > 0 {
                    ...
                    doneButton?.setTitle((String(sessionBoldUtility)).localized + "(\(text))", for: .normal)
                    //: } else {
                } else {
                    //: doneButton?.setTitle("Finish".localized, for: .normal)
                    doneButton?.setTitle((String(sessionBoldUtility)).localized, for: .normal)
                }
                //: return
        }
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.tributeChromaticColour()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: moduleAdjustPath - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func primrosePath(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: asset!, options: options) { (asset: AVAsset?, _: AVAudioMix?, _) in
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                if !FileManager.default.fileExists(atPath: NSHomeDirectory().appending((String(viewWanderKey.prefix(4))))) {
                    //: do {
                    do {
                        //: try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending("/tmp"), withIntermediateDirectories: true)
                        try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending((String(viewWanderKey.prefix(4)))), withIntermediateDirectories: true)
                        //: } catch {
                    } catch {}
                }
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (String(colorSoonPath.suffix(6)) + " sess" + String(commonObserveKey.prefix(9)) + String(themeWorkUtility)))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(serviceTempPanPreference) + String(kAtTimer)))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(featureFinitePlatform.suffix(6)) + "sful" + themeThirtyConfig.capitalized))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(screenAsideId)))
            }
        }
    }
    ...
    private class func searchion() -> String {
        //: let formater = DateFormatter()
        let formater = DateFormatter()
        //: formater.dateFormat = "yyyy-MM-dd-HH:mm:ss-SSS"
        formater.dateFormat = (appFieldPath.replacingOccurrences(of: "seek", with: "y") + "-MM-d" + String(layoutAggressionPlatform.prefix(6)) + "m:ss" + String(kLayerDevice))
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(featureButterPage) + String(displaySecuritySession)) + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000))" + (String(coreBeyondMessage.suffix(4))))
        //: return outputPath
        return outputPath
    }
}
