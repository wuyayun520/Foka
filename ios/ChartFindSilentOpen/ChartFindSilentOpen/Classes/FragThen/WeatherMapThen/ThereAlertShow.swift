...
fileprivate let componentBoldMetrics:[Character] = ["b","t","n","_","s","h","o","w","_","a","l","e","r","t","B","a","c"]
fileprivate let displayNeedError:String = "khandound"

/*: "btn_alertBackground_false" :*/
fileprivate let colorOutputHelper:String = "mic closed coordinate abstract advancedbtn_al"
fileprivate let featureAmpleSustainPage:String = "kgraudience"
...
public class ThereAlertShow: NSObject {
    /// 通用回调
...
public extension ThereAlertShow {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: FeatureAlertConfig) -> Void
    ...
    internal class func tillLaunch(title: String? = nil,
                                   //: message: String?  = nil,
                                   message: String? = nil,
    ...
    @objc internal class func tapStyleBlock(title: String? = nil,
    ...
    class func regarding(title: String? = nil,
                         ...
                         config: FeatureAlertConfig? = nil)
    {
        ...
        let alertView = AcidReactiveCompatible(title: title,
        ...
        //: FragThen.getWindow().addSubview(alertView)
        FragThen.termsSource().addSubview(alertView)
    ...
    class func outsideGold() {
        //: FragThen.getWindow().subviews.forEach { (view) in
        FragThen.termsSource().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: AcidReactiveCompatible.self) {
    ...
    class func separation(title: String, isTouch: Bool) {
        //: FragThen.getWindow().subviews.forEach { (view) in
        FragThen.termsSource().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: AcidReactiveCompatible.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: AcidReactiveCompatible = view as! AcidReactiveCompatible
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.headingOccur(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class FeatureAlertConfig {
    /// 执行动画时间
    ...
    public var maskType: PromiseMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .modelSize(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.restoreAudience()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .modelSize(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.restoreAudience()
    ...
    public var buttonFont: UIFont = .modelSize(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.tributeChromaticColour()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(componentBoldMetrics) + displayNeedError.replacingOccurrences(of: "hand", with: "gr"))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(colorOutputHelper.suffix(6)) + "ertBac" + featureAmpleSustainPage.replacingOccurrences(of: "audience", with: "o") + "und_false")
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .modelSize(type: .Medium, fontSize: 16)
