...
fileprivate let sessionAdTellMagnituderoSettings:[UInt8] = [0x4a,0x4d,0x4a,0x57,0xb,0x40,0x4c,0x47,0x46,0x51,0x19,0xa,0x3,0x4b,0x42,0x50,0x3,0x4d,0x4c,0x57,0x3,0x41,0x46,0x46,0x4d,0x3,0x4a,0x4e,0x53,0x4f,0x46,0x4e,0x46,0x4d,0x57,0x46,0x47]

private func readySend(gift num: UInt8) -> UInt8 {
    return num ^ 35
}
...
class HiViewCell: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: sessionAdTellMagnituderoSettings.map{readySend(gift: $0)}, encoding: .utf8)!)
    }
...
extension HiViewCell {
    //: func setFeatureItem(featureItem: SpringModel) {
    func unseeablePut(featureItem: SpringModel) {
        ...
        eliteSmart(iconName: featureItem.norImageName, selectedIconName: featureItem.selImageName, title: featureItem.title, isSelected: featureItem.isSelected)
    }

    //: func configIcon(iconName: String, selectedIconName: String, title: String, isSelected: Bool) {
    func eliteSmart(iconName: String, selectedIconName _: String, title: String, isSelected: Bool) {
        //: iconImage.image = UIImage.BundleImageNamed(name: iconName)
        iconImage.image = UIImage.titled(name: iconName)
        ...
        if isSelected == true {
            ...
            let color = UIColor.tributeChromaticColour()
