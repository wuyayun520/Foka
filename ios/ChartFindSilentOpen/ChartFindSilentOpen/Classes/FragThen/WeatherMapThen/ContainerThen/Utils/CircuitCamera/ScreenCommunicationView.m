...
@interface PreferPartyData : NSObject

+ (instancetype)sharedInstance;

//: M_SenseME_Face_Video_5.3.4
@property (nonatomic, copy) NSString *featureMinimumEvent;

//: camera_focus_red
@property (nonatomic, copy) NSString *serviceTempCurrentContent;

//: model
@property (nonatomic, copy) NSString *themeInviteSystem;

//: FaceActionEvent_Interval_12_frame
@property (nonatomic, copy) NSString *screenSmoothSensitiveName;

//: hasFace
@property (nonatomic, copy) NSString *viewSignContent;

//: key.Notification.Tap.To.Modify.Exposure.Point
@property (nonatomic, copy) NSString *featureDirtyPreference;

//: FaceActionEvent_Interval_1_second
@property (nonatomic, copy) NSString *featurePadTimer;

@end

@implementation PreferPartyData

+ (instancetype)sharedInstance {
    static PreferPartyData *instance = nil;
...
+ (NSData *)PreferPartyDataToData:(NSString *)value {
...
- (Byte *)PreferPartyDataToCache:(Byte *)data {
    int female = data[0];
    Byte alterCurrent = data[1];
    int switchce = data[2];
    for (int i = switchce; i < switchce + female; i++) {
        int value = data[i] + alterCurrent;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[switchce + female] = 0;
    return data + switchce;
}

- (NSString *)StringFromPreferPartyData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self PreferPartyDataToCache:data]];
}

//: camera_focus_red
- (NSString *)serviceTempCurrentContent {
    if (!_serviceTempCurrentContent) {
		NSString *origin = @"106209E5EFE50FB9D901FF0B0310FFFD040D011311FD10030234";
		NSData *data = [PreferPartyData PreferPartyDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _serviceTempCurrentContent = [self StringFromPreferPartyData:value];
    }
    return _serviceTempCurrentContent;
}

//: hasFace
- (NSString *)viewSignContent {
    if (!_viewSignContent) {
		NSString *origin = @"0761068503EF070012E5000204BE";
		NSData *data = [PreferPartyData PreferPartyDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _viewSignContent = [self StringFromPreferPartyData:value];
    }
    return _viewSignContent;
}

//: FaceActionEvent_Interval_1_second
- (NSString *)featurePadTimer {
    if (!_featurePadTimer) {
		NSString *origin = @"21580444EE090B0DE90B1C111716ED1E0D161C07F1161C0D1A1E091407D9071B0D0B17160CD9";
		NSData *data = [PreferPartyData PreferPartyDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _featurePadTimer = [self StringFromPreferPartyData:value];
    }
    return _featurePadTimer;
}

//: M_SenseME_Face_Video_5.3.4
- (NSString *)featureMinimumEvent {
    if (!_featureMinimumEvent) {
		NSString *origin = @"1A2F05B4091E3024363F44361E16301732343630273A3536403006FF04FF05AE";
		NSData *data = [PreferPartyData PreferPartyDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _featureMinimumEvent = [self StringFromPreferPartyData:value];
    }
    return _featureMinimumEvent;
}

//: model
- (NSString *)themeInviteSystem {
    if (!_themeInviteSystem) {
		NSString *origin = @"050A0363655A5B620D";
		NSData *data = [PreferPartyData PreferPartyDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _themeInviteSystem = [self StringFromPreferPartyData:value];
    }
    return _themeInviteSystem;
}

//: FaceActionEvent_Interval_12_frame
- (NSString *)screenSmoothSensitiveName {
    if (!_screenSmoothSensitiveName) {
		NSString *origin = @"212E0CFF7CF058DA6FBCA578183335371335463B41401748374046311B4046374448333E310304313844333F370F";
		NSData *data = [PreferPartyData PreferPartyDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _screenSmoothSensitiveName = [self StringFromPreferPartyData:value];
    }
    return _screenSmoothSensitiveName;
}

//: key.Notification.Tap.To.Modify.Exposure.Point
- (NSString *)featureDirtyPreference {
    if (!_featureDirtyPreference) {
		NSString *origin = @"2D0D051C885E586C214162675C595C5654675C626121475463214762214062575C596C21386B6362666865582143625C6167D8";
		NSData *data = [PreferPartyData PreferPartyDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _featureDirtyPreference = [self StringFromPreferPartyData:value];
    }
    return _featureDirtyPreference;
}
...
@interface ScreenCommunicationView ()<LoseDelegate,UIGestureRecognizerDelegate>
{
...
//: @property (nonatomic, strong) DistanceLicHelper * lichelper;
@property (nonatomic, strong) DistanceLicHelper * lichelper;
//: @property (nonatomic, strong) MapReactiveCompatible * ParamUtil;
@property (nonatomic, strong) MapReactiveCompatible * ParamUtil;
...
@implementation ScreenCommunicationView
...
- (instancetype)init {
    ...
    if (self) {
        ...
        //: self.lichelper = [DistanceLicHelper share];
        self.lichelper = [DistanceLicHelper share];
        //: self.ParamUtil = [[MapReactiveCompatible alloc]init];
        self.ParamUtil = [[MapReactiveCompatible alloc]init];
        //: _currentDropCount = 5;
        ...
        if ([self.lichelper licence]) {
            ...
            //: [[MapCacheCost share] getLocalModel];
            [[MapCacheCost share] outConnect];
            ...
            [NSNotificationCenter.defaultCenter addObserver:self
                                                   ...
                                                   selector:@selector(screenQuery:)
                                                       //: name:@"key.Notification.Tap.To.Modify.Exposure.Point"
                                                       name:[PreferPartyData sharedInstance].featureDirtyPreference
                                                     //: object:nil];
                                                     object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(noed:) name:self.ParamUtil.kNotificationSelectBeautyFilterStr object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hairsed:) name:self.ParamUtil.kNotificationBeautyFilterValueChangeStr object:nil];
...
- (instancetype)initManager:(NSString*)sessionPreset {
    ...
    if (self = [super init]) {
        ...
        if ([self.lichelper licence]) {
            ...
            [self initCirclet:sessionPreset];

        //: } else {
        } else {
            //: [DistanceLicHelper.share checkRemoteLicInfoWithCompletion:^(BOOL x) {
            [DistanceLicHelper.share ideaOldCompletionWithCompletion:^(BOOL x) {
                //: if (x) {
                if (x) {
                    //: [self initFilterView:sessionPreset];
                    [self initCirclet:sessionPreset];
...
- (void)initCirclet:(NSString*)sessionPreset {
    //: self.layer.masksToBounds = YES;
    self.layer.masksToBounds = YES;
    //: self.clipsToBounds = YES;
    self.clipsToBounds = YES;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(noed:)
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hairsed:)
...
- (void)head {
    ...
    //: if ([MapCacheCost share].filterSuite.filterModelPath.length > 0) {
    if ([MapCacheCost share].filterSuite.filterModelPath.length > 0) {
        //: SpringModel* model = [[MapCacheCost share] getFilterDataModel];
        SpringModel* model = [[MapCacheCost share] chapter];
        //: if (model) {
        if (model) {
...
- (void)setCurrentSessionPreset:(NSString *)currentSessionPreset {
    ...
    [self.stCamera smoothTapBar:self.previewCenter plus:self.previewFrame];
}
...
- (void)property {
    ...
    _stCamera = [[CircuitCamera alloc] initWithAll:AVCaptureDevicePositionFront
                                          //: sessionPresset:self.currentSessionPreset
                                          communication:self.currentSessionPreset
                                                     //: fps:fps
                                                     color:fps
                                           //: needYuvOutput:NO];
                                           resign:NO];
...
- (void)work:(CGFloat)width burn:(CGFloat)height {
    //: CGRect previewRect = [self.stCamera getScaleRect:CGRectMake(0 , 0 , width, height)
    CGRect previewRect = [self.stCamera remote:CGRectMake(0 , 0 , width, height)
                                                   //: scaleToFit:NO];
                                                   rect:NO];
    ...
    if (self.glPreview.superview == nil) {
        ...
        @synchronized (self) {
           ...
           _glPreview = [[FourCoreView alloc] initWithLock:previewRect appearFrame:previewContext];
        }
...
- (void)jaw {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(motionEnd) name:UIApplicationWillResignActiveNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidEnterBackground) name:UIApplicationDidEnterBackgroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(actionOwner) name:UIApplicationDidEnterBackgroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appWillEnterForeground) name:UIApplicationWillEnterForegroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(frameGroupDrop) name:UIApplicationWillEnterForegroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidBecomeActive) name:UIApplicationDidBecomeActiveNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(programNumber) name:UIApplicationDidBecomeActiveNotification object:nil];
}
...
- (void)pageOf:(CMAccelerometerData *)accelerometerData {
...
- (st_rotate_type)still {
    ...
    [self pageOf:self.motionManager.accelerometerData];
...
- (void)component {
    ...
    //初始化检测模块句柄
    //: NSString *strModelPath = [FragThen.getModelFileName stringByAppendingPathComponent:@"M_SenseME_Face_Video_5.3.4"];
    NSString *strModelPath = [FragThen.finis stringByAppendingPathComponent:[PreferPartyData sharedInstance].featureMinimumEvent];
    //: strModelPath = [strModelPath stringByAppendingPathExtension:@"model"];
    strModelPath = [strModelPath stringByAppendingPathExtension:[PreferPartyData sharedInstance].themeInviteSystem];
...
- (BOOL)oculus:(CVPixelBufferRef *)pixelBufferOut
                          ...
                          dataTexture:(CVOpenGLESTextureRef *)cvTexture {
...
- (BOOL)noseProfile:(CVPixelBufferRef)pixelBuffer {
...
- (void)screenQuery:(NSNotification *)sender {
    ...
    if ([tapGesture isKindOfClass:UITapGestureRecognizer.class]) {
        ...
        [self tinScreen:tapGesture];
    }

}

//: - (void)tapScreen:(UITapGestureRecognizer *)tapGesture {
- (void)tinScreen:(UITapGestureRecognizer *)tapGesture {
    ...
    [self.stCamera smoothTapBar:point plus:self.frame];
...
- (UIImageView *)focusImageView {
    ...
    if (!_focusImageView) {
        ...
        _focusImageView.image = [UIImage imageNamed:[PreferPartyData sharedInstance].serviceTempCurrentContent];
...
- (void)noed:(NSNotification*) notice {
    //: if ([notice.object isKindOfClass:[SpringModel class]]) {
    if ([notice.object isKindOfClass:[SpringModel class]]) {
        //: SpringModel* model = (SpringModel*)notice.object;
        SpringModel* model = (SpringModel*)notice.object;
        //: [self handleFilterChanged:model];
        [self pageIn:model];
    }
...
//: - (void)handleFilterChanged:(SpringModel *)model {
- (void)pageIn:(SpringModel *)model {
...
- (void)hairsed:(NSNotification*) notice {
    //: SpringModel* model = (SpringModel*)notice.object;
    SpringModel* model = (SpringModel*)notice.object;
    //: if (_hFilter) {
    if (_hFilter) {
...
- (void)direction {
    ...
    //: STFilterEntiretycostModel *beautyValueCache = MapCacheCost.share.currentBeautySuite;
    ContainerThen *beautyValueCache = MapCacheCost.share.shouldSuite;

    // 设置美白参数
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    // 设置默认红润参数
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    // 设置默认磨皮参数
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    // 设置默认大眼参数
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    // 设置默认瘦脸参数
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    // 设置小脸参数
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];

    //瘦脸型
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    //窄脸
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    //圆眼
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    //下巴
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    //额头
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    //瘦鼻翼
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    //长鼻
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    //嘴形
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fMouthStrength]];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self key:beautyValueCache.fMouthStrength]];
    //缩人中
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fPhiltrumStrength]];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self key:beautyValueCache.fPhiltrumStrength]];

    //设置对比度参数
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fContrastStrength]];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self key:beautyValueCache.fContrastStrength]];

    //设置饱和度参数
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSaturationStrength]];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self key:beautyValueCache.fSaturationStrength]];

    //锐化参数
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSharpenStrength]];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self key:beautyValueCache.fSharpenStrength]];

    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fAppleMusleStrength]];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self key:beautyValueCache.fAppleMusleStrength]];

    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];

    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];

    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];

    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];

    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    //: [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];
    [self.ParamUtil scaleTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];

}

//: - (float)rangeMappedValueFrom:(float) rawValue {
- (float)key:(float) rawValue {
...
void gestureFactor(st_mobile_animal_face_t *src, int faceCount, st_mobile_animal_face_t *dst) {
...
void fadeWriting(st_mobile_animal_face_t *src, int faceCount) {
    //: if (faceCount > 0) {
    if (faceCount > 0) {
        //: for (int i = 0; i < faceCount; ++i) {
        for (int i = 0; i < faceCount; ++i) {
...
- (void)butterflyEffect:(CMSampleBufferRef)originalSampleBuffer
                       //: resultPixelBuffer:(CVPixelBufferRef)resultPixelBufffer {
                       separateOut:(CVPixelBufferRef)resultPixelBufffer {
    ...
    if ( originalSampleBuffer && resultPixelBufffer ) {
        ...
        if (tmpDelegate && [tmpDelegate respondsToSelector:@selector(quantityerest:kind:)]) {
            ...
            if (sampleBuffer) {
                ...
                [tmpDelegate quantityerest:self kind:sampleBuffer];
            }
...
- (CVPixelBufferRef)read:(CVPixelBufferRef)pixelBuffer {
...
- (void)estimated:(AVCaptureOutput *)captureOutput more:(CMSampleBufferRef)sampleBuffer gravity:(AVCaptureConnection *)connection {
    ...
    st_rotate_type stMobileRotate = [self still];
    ...
    if (_hDetector) {
        ...
        if (self.iCurrentAction > 0) {
            ...
            if (_faceFrameCount == 12) { // 第12帧时
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:[PreferPartyData sharedInstance].screenSmoothSensitiveName object:nil userInfo: @{[PreferPartyData sharedInstance].viewSignContent: @(hasFace)}];
            }
            ...
            if (_faceFrameCount%18 == 0) { // 每秒发送一次
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:[PreferPartyData sharedInstance].featurePadTimer object:nil userInfo: @{[PreferPartyData sharedInstance].viewSignContent: @(hasFace)}];
            }
        }
    }
    ...
    if (faceCount > 0) {
        ...
        gestureFactor(_detectResult1, faceCount, newDetectResult1);
    }
    ...
    dispatch_async(self.renderQueue, ^{
       ...
       BOOL isTextureOriginReady = [self noseProfile:pixelBuffer];
       ...
       // ST_MOBILE 以下为滤镜部分
        ...
        if (callBackFlag) [self butterflyEffect:sampleBuffer separateOut:resultPixelBufffer];
        ...
        if (faceCount > 0) {
            ...
            fadeWriting(newDetectResult1, faceCount);
        }
        //: if (callBackFlag) {
        if (callBackFlag) {
            //: @synchronized (self) {
            @synchronized (self) {
                ...
                [self.glPreview saveCustom:textureResult];
            }
        }
