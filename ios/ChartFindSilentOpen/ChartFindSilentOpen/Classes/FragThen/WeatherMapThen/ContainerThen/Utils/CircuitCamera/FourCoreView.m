...
@interface WireVisualData : NSObject

@end

@implementation WireVisualData

+ (NSData *)WireVisualDataToData:(NSString *)value {
...
+ (Byte *)WireVisualDataToCache:(Byte *)data {
    int result = data[0];
    Byte originalGain = data[1];
    int miniCloud = data[2];
    for (int i = miniCloud; i < miniCloud + result; i++) {
        int value = data[i] - originalGain;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[miniCloud + result] = 0;
    return data + miniCloud;
}

+ (NSString *)StringFromWireVisualData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self WireVisualDataToCache:data]];
}

//: STDisplayShader
+ (NSString *)commonWantTitle {
    /* static */ NSString *commonWantTitle = nil;
    if (!commonWantTitle) {
		NSString *origin = @"0F0D0B38A12F8A6E87EBE160615176807D796E8660756E71727F33";
		NSData *data = [WireVisualData WireVisualDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        commonWantTitle = [self StringFromWireVisualData:value];
    }
    return commonWantTitle;
}
...
- (instancetype)initWithLock:(CGRect)frame appearFrame:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self sample:[WireVisualData commonWantTitle]
                //: fragmentShader:@"STDisplayShader"
                coverEstimated:[WireVisualData commonWantTitle]
                    //: forProgram:&stDisplayProgram];
                    suite:&stDisplayProgram];
...
- (BOOL)sample:(NSString *)vertexShaderName
          //: fragmentShader:(NSString *)fragmentShaderName
          coverEstimated:(NSString *)fragmentShaderName
              //: forProgram:(GLuint *)programPointer;
              suite:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self modelRound:&vertexShader animalContent:0x8B31 root:serviceLinkReportLogger]) {
    ...
    if (![self modelRound:&fragShader animalContent:0x8B30 root:commonMediaPage]) {
...
- (BOOL)modelRound:(GLuint *)shader animalContent:(GLenum)type root:(const char *)str
{
    //: GLint status;
    GLint status;
    //: const GLchar *source;
...
- (void)saveCustom:(GLuint)texture
{
    ...
    if ([EAGLContext setCurrentContext:self.glContext]) {
        ...
        [self glasses:texture];
    }
}

//: - (BOOL)drawFrameWithTexture:(GLuint)texture
- (BOOL)glasses:(GLuint)texture
{
