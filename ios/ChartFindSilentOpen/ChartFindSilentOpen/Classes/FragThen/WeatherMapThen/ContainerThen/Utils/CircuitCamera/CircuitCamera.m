...
typedef struct {
    ...
    Byte *look;
    unsigned int mode;
	int hintEnableceInvitation;
	int lifeStyle;
} StructBecauseData;

@interface BecauseData : NSObject

+ (instancetype)sharedInstance;

@end

@implementation BecauseData

+ (instancetype)sharedInstance {
    static BecauseData *instance = nil;
...
+ (NSData *)BecauseDataToData:(NSString *)value {
...
- (Byte *)BecauseDataToByte:(StructBecauseData *)data {
    for (int i = 0; i < data->mode; i++) {
        data->look[i] ^= data->fashion;
    }
    data->look[data->mode] = 0;
	if (data->mode >= 2) {
		data->hintEnableceInvitation = data->look[0];
		data->lifeStyle = data->look[1];
	}
    return data->look;
}

- (NSString *)StringFromBecauseData:(StructBecauseData *)data {
    return [NSString stringWithUTF8String:(char *)[self BecauseDataToByte:data]];
}

//: Width
- (NSString *)spacingWantKeyPath {
    /* static */ NSString *spacingWantKeyPath = nil;
    if (!spacingWantKeyPath) {
		NSString *origin = @"fbc5c8d8c43e";
		NSData *data = [BecauseData BecauseDataToData:origin];
        StructBecauseData value = (StructBecauseData){172, (Byte *)data.bytes, 5, 165, 29};
        spacingWantKeyPath = [self StringFromBecauseData:&value];
    }
    return spacingWantKeyPath;
}

//: 请打开相机权限
- (NSString *)appPresenceId {
    /* static */ NSString *appPresenceId = nil;
    if (!appPresenceId) {
		NSString *origin = @"51160e5f302a5c05395e22015f25035f243a5020291d";
		NSData *data = [BecauseData BecauseDataToData:origin];
        StructBecauseData value = (StructBecauseData){185, (Byte *)data.bytes, 21, 72, 116};
        appPresenceId = [self StringFromBecauseData:&value];
    }
    return appPresenceId;
}

//: Height
- (NSString *)colorAppearSettings {
    /* static */ NSString *colorAppearSettings = nil;
    if (!colorAppearSettings) {
		NSString *origin = @"d7faf6f8f7eb80";
		NSData *data = [BecauseData BecauseDataToData:origin];
        StructBecauseData value = (StructBecauseData){159, (Byte *)data.bytes, 6, 83, 220};
        colorAppearSettings = [self StringFromBecauseData:&value];
    }
    return colorAppearSettings;
}
...
@interface CircuitCamera () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
...
@implementation CircuitCamera

//: - (instancetype)initWithDevicePosition:(AVCaptureDevicePosition)iDevicePosition
- (instancetype)initWithAll:(AVCaptureDevicePosition)iDevicePosition
                        //: sessionPresset:(AVCaptureSessionPreset)sessionPreset
                        communication:(AVCaptureSessionPreset)sessionPreset
                                   //: fps:(int)iFPS
                                   color:(int)iFPS
                         //: needYuvOutput:(BOOL)needYuvOutput
                         resign:(BOOL)needYuvOutput
{
    //: self = [super init];
    self = [super init];
    //: if (self) {
    if (self) {
        ...
        self.videoDevice = [self disappearWill:iDevicePosition];
...
- (void)smoothTapBar:(CGPoint)point plus:(CGRect)frame
{
    ...
    [self hemAndHaw:AVCaptureFocusModeAutoFocus
           //: exposureMode:AVCaptureExposureModeContinuousAutoExposure
           late:AVCaptureExposureModeContinuousAutoExposure
                //: atPoint:CGPointMake(fX, fY)];
                focusBottomPoint:CGPointMake(fX, fY)];

}

//: - (void)focusWithMode:(AVCaptureFocusMode)focusMode exposureMode:(AVCaptureExposureMode)exposureMode atPoint:(CGPoint)point
- (void)hemAndHaw:(AVCaptureFocusMode)focusMode late:(AVCaptureExposureMode)exposureMode focusBottomPoint:(CGPoint)point
{

    //: [self changeDeviceProperty:^(AVCaptureDevice *captureDevice) {
    [self input:^(AVCaptureDevice *captureDevice) {
...
- (void)input:(void(^)(AVCaptureDevice *))propertyChange
{
...
- (void)setHandleQuote:(float)value
{
...
- (void)setDevicePosition:(AVCaptureDevicePosition)devicePosition
{
    ...
    if (_devicePosition != devicePosition && devicePosition != AVCaptureDevicePositionUnspecified) {
        ...
        if (_session) {
            ...
            AVCaptureDevice *targetDevice = [self disappearWill:devicePosition];

            //: if (targetDevice && [self judgeCameraAuthorization]) {
            if (targetDevice && [self white]) {
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:[[BecauseData sharedInstance] spacingWantKeyPath]] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:[[BecauseData sharedInstance] colorAppearSettings]] floatValue];
...
- (CGRect)remote:(CGRect)rect rect:(BOOL)bScaleToFit
{
    //: CGRect rectRet = rect;
    CGRect rectRet = rect;
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:[[BecauseData sharedInstance] spacingWantKeyPath]] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:[[BecauseData sharedInstance] colorAppearSettings]] floatValue];
...
- (BOOL)white
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:[[BecauseData sharedInstance] appPresenceId]
...
- (AVCaptureDevice *)disappearWill:(AVCaptureDevicePosition)position
{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (!self.bSessionPause) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(estimated:more:gravity:)]) {
            //: [self.delegate captureOutput:captureOutput didOutputSampleBuffer:sampleBuffer fromConnection:connection];
            [self.delegate estimated:captureOutput more:sampleBuffer gravity:connection];
        }
    }
