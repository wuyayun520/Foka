...
/*: "Select a date" :*/
fileprivate let kTributeDevice:String = "Select aflag new"
fileprivate let coreIconHelper:String = " dateaspect dimension wait"
...
protocol CrashResumeObjectProtocol: NSObjectProtocol {
...
class DateView: BrandPickView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: CrashResumeObjectProtocol?

    //: override func initPickView() {
    override func noView() {
        //: super.initPickView()
        super.noView()

        //: titleString = "Select a date".localized
        titleString = (String(kTributeDevice.prefix(8)) + String(coreIconHelper.prefix(5))).localized
        ...
        distinguishingCharacteristic(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func distinguishingCharacteristic(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func shirtButtonPhoto(button: UIButton) {
        //: super.confirmClick(button: button)
        super.shirtButtonPhoto(button: button)
        //: refreshPickViewData()
        dataTag()
        ...
        if let day = selectDay {
            ...
            delegate?.bot(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.bot(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return swineInfluenza(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func swineInfluenza(year: Int, month: Int) -> Int {
