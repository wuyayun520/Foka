...
fileprivate let commonImaginationMessage:[UInt8] = [0xfa,0xff,0xfa,0x5,0xb9,0xf4,0x0,0xf5,0xf6,0x3,0xcb,0xba,0xb1,0xf9,0xf2,0x4,0xb1,0xff,0x0,0x5,0xb1,0xf3,0xf6,0xf6,0xff,0xb1,0xfa,0xfe,0x1,0xfd,0xf6,0xfe,0xf6,0xff,0x5,0xf6,0xf5]

fileprivate func systemGreen(appropriate num: UInt8) -> UInt8 {
...
fileprivate let coreMergeSlideLogger:[Character] = ["b","t","n","J","Q","b","E","A","B","_","e","t","i","h","w"]
fileprivate let styleBirthError:String = "tone missing remind_kcab_"

/*: "Edit" :*/
fileprivate let k_weekMinimizeFormat:String = "phase bag toxic promotionEdit"

/*: "Finish" :*/
fileprivate let k_gapPath:String = "historic minimize weight confirmFinis"
fileprivate let displaySquareMetrics:String = "H"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let kInvitationStandHarshSession:String = "whole height featurebtnY"
fileprivate let screenHearPath:String = "pot char quit_oediv"

/*: "Cancel" :*/
fileprivate let displayAgainstExcuseAlert:String = "Cancefeedback big"
fileprivate let sessionIncreaseSocialPlatform:[Character] = ["l"]

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let widgetAgreePreference:String = "processing referencebgQk"
fileprivate let viewTapSettings:String = "viol"
fileprivate let sessionPatentEvent:[Character] = ["v","_","t","o","a","s","t"]

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let layoutFaultData:[UInt8] = [0x66,0x7a,0x77,0x32,0x88,0x7b,0x76,0x77,0x81,0x32,0x7e,0x77,0x80,0x79,0x86,0x7a,0x32,0x85,0x7a,0x81,0x87,0x7e,0x76,0x32,0x74,0x77,0x32,0x37,0x52,0x3f,0x37,0x52,0x32,0x85,0x77,0x75,0x81,0x80,0x76,0x85,0x40,0x32,0x62,0x7e,0x77,0x73,0x85,0x77,0x32,0x7b,0x80,0x86,0x77,0x84,0x75,0x77,0x82,0x86,0x32,0x7b,0x86]

fileprivate func outputHuman(quit num: UInt8) -> UInt8 {
...
// __CLOSE_PRINT__
//
//  ThatNothingRecognizerDelegate.swift
//  AbroadTalking
//
...
typealias VideoEditBlock = (_ videoInfo: WorkplaceModelType?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let serviceStatusUtility: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class ThatNothingRecognizerDelegate: GroupThen {
    ...
    private var slider: TamEnhanceView?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = WorkplaceModelType() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = RationalEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.isFamous == true {
        if TopThen.shared.isFamous == true { // 正在音视频通话，静音播放
    ...
    init(style: RationalEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (moduleAdjustPath - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: commonImaginationMessage.map{systemGreen(appropriate: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.titled(name: (String(coreMergeSlideLogger) + String(styleBirthError.suffix(6)) + "shadow")), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = PushThen.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(saveAchievement), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(k_weekMinimizeFormat.suffix(4))).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.tributeChromaticColour(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.attachSize(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = PushThen.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(k_gapPath.suffix(5)) + displaySquareMetrics.lowercased()).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.celluloidSize(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.tributeChromaticColour()
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.titled(name: (String(kInvitationStandHarshSession.suffix(4)) + "Do2AS" + String(screenHearPath.suffix(6)) + "_play")), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(performSender(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(displayAgainstExcuseAlert.prefix(5)) + String(sessionIncreaseSocialPlatform)).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(ticChannel), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.titled(name: (String(widgetAgreePreference.suffix(4)) + "RTbF_oe" + viewTapSettings.replacingOccurrences(of: "viol", with: "di") + String(sessionPatentEvent))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: layoutFaultData.map{outputHuman(quit: $0)}, encoding: .utf8)!.meanwhile(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
        //: lab.font = UIFont.pingfangRugularFont(fontSize: 13)
        lab.font = UIFont.attachSize(fontSize: 13)
        //: v.addSubview(lab)
        v.addSubview(lab)
    ...
    lazy var useVideoView: SauceThen = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = SauceThen(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension ThatNothingRecognizerDelegate {
    /// 返回按钮事件
    //: @objc func backAction() {
    @objc func saveAchievement() {
    ...
    private func prevalence(animated _: Bool = true) {
    ...
    @objc func humanAction() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: moduleAdjustPath - 2 * 26, height: componentMeEvent - 46 - 127 - featureDailyHelper)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = moduleAdjustPath / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: componentMeEvent, width: moduleAdjustPath, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = TamEnhanceView(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: serviceStatusUtility)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - featureDailyHelper)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: MassDefectSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func nameAction() {
        ...
        if self.videoEditHandler != nil {
            ...
            MedalVideoManager.shared.roughnessHandler(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.prevalence()
    ...
    @objc func performSender(sender: UIButton) {
...
extension ThatNothingRecognizerDelegate {
    ...
    private func environsStyle() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(availablePlay), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.subGiving(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func fileConstraint() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + featureDailyHelper)
        }
