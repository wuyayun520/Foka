...
fileprivate let styleCarryId:String = "icpact"
fileprivate let spacingBirthSystem:String = "g_kong_dblank given media trap combine"
fileprivate let moduleSuccessId:String = "E"
fileprivate let displayShotData:String = "faulrowing"

/*: "page" :*/
fileprivate let screenLockSkipError:[UInt8] = [0x65,0x67,0x61,0x70]

/*: "uid" :*/
fileprivate let colorUtterMuscleSequencePreference:[UInt8] = [0x64,0x69,0x75]

/*: "list" :*/
fileprivate let appUponLevelSystem:String = "lisearn"
...
// __CLOSE_PRINT__
//
//  MakeupMilitaryQuartersViewDelegate.swift
//  AbroadTalking
//
...
class MakeupMilitaryQuartersViewDelegate: GroupThen {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [SecondMeasurable]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: screenCollectionStyleSystem, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(AgendaItemScriptViewCell.self, forCellWithReuseIdentifier: AgendaItemScriptViewCell.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.titled(name: (styleCarryId.replacingOccurrences(of: "pact", with: "on") + "_kon" + String(spacingBirthSystem.prefix(8)) + moduleSuccessId.lowercased() + displayShotData.replacingOccurrences(of: "rowing", with: "t")))
...
extension MakeupMilitaryQuartersViewDelegate {
    ...
    private func putData() {
        ...
        let params = [String(bytes: screenLockSkipError.reversed(), encoding: .utf8)!: pageIndex, String(bytes: colorUtterMuscleSequencePreference.reversed(), encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        SpecifyThen.straight(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(appUponLevelSystem.replacingOccurrences(of: "earn", with: "t"))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = NothingMeasurable.deserialize(from: dict as? Dictionary) {
...
extension MakeupMilitaryQuartersViewDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: AgendaItemScriptViewCell.className(), for: indexPath) as! AgendaItemScriptViewCell
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = DisableReactiveCompatible(model: detailModel)
...
extension MakeupMilitaryQuartersViewDelegate: JXPagingViewListViewDelegate {
...
extension MakeupMilitaryQuartersViewDelegate {
    ...
    func refreshController(_ userModel: AcidTransformable) {
