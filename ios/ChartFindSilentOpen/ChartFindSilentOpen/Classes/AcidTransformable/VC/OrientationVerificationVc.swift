...
// __CLOSE_PRINT__
//
//  OrientationVerificationVc.swift
//  AbroadTalking
//
...
class OrientationVerificationVc: GroupThen {
...
extension OrientationVerificationVc {
    ...
    override func perPopback() {
        ...
        for index in 0..<(self.navigationController?.viewControllers.count)! {
            ...
            if self.navigationController?.viewControllers[index] .isKind(of: HiThen.self) == true {
                //: let vc = (self.navigationController?.viewControllers[index] as? TalkingLoginEditAuthAndVideoVC)!
                let vc = (self.navigationController?.viewControllers[index] as? HiThen)!
