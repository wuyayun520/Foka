...
fileprivate let componentStatSharedPage:[UInt8] = [0xc6,0xc1,0xc6,0xdb,0x87,0xcc,0xc0,0xcb,0xca,0xdd,0x95,0x86,0x8f,0xc7,0xce,0xdc,0x8f,0xc1,0xc0,0xdb,0x8f,0xcd,0xca,0xca,0xc1,0x8f,0xc6,0xc2,0xdf,0xc3,0xca,0xc2,0xca,0xc1,0xdb,0xca,0xcb]

private func remReply(wed num: UInt8) -> UInt8 {
    return num ^ 175
}

/*: "icon_male_default" :*/
fileprivate let featureDescriptionPreference:[UInt8] = [0x38,0x32,0x3e,0x3f,0xe,0x3c,0x30,0x3d,0x34,0xe,0x35,0x34,0x37,0x30,0x24,0x3d,0x25]

private func moveDifference(resent num: UInt8) -> UInt8 {
    return num ^ 81
}

/*: "icon_female_default" :*/
fileprivate let coreProcessorPage:[Character] = ["i","c","o","n","_","f","e","m","a","l","e","_","d"]
fileprivate let themePendingUrl:String = "ecommunicationult"

/*: "level_%@" :*/
fileprivate let modulePressureSystem:String = "plate"
fileprivate let viewReceiveSession:String = "as layervel_%@"
...
class DeadlineThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: componentStatSharedPage.map{remReply(wed: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var headerPic = UIImageView().then {
        ...
        $0.image = UIImage.rimePecCorporate()
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .modelSize(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .modelSize(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension DeadlineThen {
    //: func updateContentWithModel(model: TPreviewInfoModel) {
    func thumbCrush(model: OffRushModelType) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.rimePecCorporate())
        ...
        let imgStr = model.sex == PanelScalarLiteral.male.rawValue ? String(bytes: featureDescriptionPreference.map{moveDifference(resent: $0)}, encoding: .utf8)! : (String(coreProcessorPage) + themePendingUrl.replacingOccurrences(of: "communication", with: "fa"))
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (modulePressureSystem.replacingOccurrences(of: "plate", with: "le") + String(viewReceiveSession.suffix(6))), model.level)
