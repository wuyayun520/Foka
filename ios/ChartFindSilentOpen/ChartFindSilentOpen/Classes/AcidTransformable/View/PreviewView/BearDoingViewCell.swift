...
fileprivate let sessionThematicName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_loading_lose" :*/
fileprivate let sessionAgencyId:[Character] = ["i","m","g","_","l","o","a","d","i","n","g","_","l","o","s","e"]
...
class BearDoingViewCell: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: sessionThematicName.reversed(), encoding: .utf8)!)
    }
...
extension BearDoingViewCell {
    //: private func bindInteraction() {
    private func guidance() {
        //: let singleTap = UITapGestureRecognizer(target: self, action: #selector(singleTapAction))
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(titleEventSkip))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(pastAction)).then {
...
extension BearDoingViewCell {
    ...
    func thatSwitcher(imgModel: MapMeasurable) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(sessionAgencyId))), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.time()
            }
        }
    ...
    func calculateResume(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(sessionAgencyId))), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.time()
        }
    }
...
extension BearDoingViewCell: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension BearDoingViewCell {
    ...
    func time() {
        ...
        var newW = moduleAdjustPath
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = moduleAdjustPath * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > componentMeEvent {
            //: newH = ScreenHeight
            newH = componentMeEvent
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * componentMeEvent / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (moduleAdjustPath - newW) / 2.0, y: (componentMeEvent - newH) / 2)
