...
/*: "#AAAAAA" :*/
fileprivate let spacingUnderTimer:String = "#AAAAclick lose"
fileprivate let k_lowerExploreHelper:String = "amber"

/*: " No more than 10 characters" :*/
fileprivate let serviceGiftEvent:String = "skirt mouth secret left which No "
fileprivate let sessionSpotValue:String = "tbeautifuln"
fileprivate let moduleFileRoundSystem:String = "chhistoricrhistoric"

/*: "init(coder:) has not been implemented" :*/
fileprivate let displayHisName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class PromiseThen: UIView {
    ...
    private lazy var inputTView: CircleMapTextView = {
        //: let input = InputTextView()
        let input = CircleMapTextView()
        //: input.font = UIFont.pingfangFont(type: .Regular, fontSize: 16)
        input.font = UIFont.modelSize(type: .Regular, fontSize: 16)
        //: input.placeholderColor = .init(hex: "#AAAAAA") ?? UIColor.gray
        input.placeholderColor = .init(hex: (String(spacingUnderTimer.prefix(5)) + k_lowerExploreHelper.replacingOccurrences(of: "amber", with: "AA"))) ?? UIColor.gray
        //: input.placeholder = " No more than 10 characters".localized
        input.placeholder = (String(serviceGiftEvent.suffix(4)) + "more " + sessionSpotValue.replacingOccurrences(of: "beautiful", with: "ha") + " 10 " + moduleFileRoundSystem.replacingOccurrences(of: "historic", with: "a") + "cters").localized
        ...
        input.backgroundColor = .layerColor()
        ...
        let attributes = [NSAttributedString.Key.foregroundColor: UIColor.restoreAudience(), .font: UIFont.modelSize(type: .Regular, fontSize: 16)]
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.tributeChromaticColour().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(appearanceClean), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        rightBtn.setBackgroundImage(UIImage.totalelMention(colors: UIColor.motion(), size: CGSize(width: 267, height: 50)), for: .normal)
        ...
        rightBtn.setBackgroundImage(UIImage.totalelMention(colors: UIColor.bestSelling(), size: CGSize(width: 267, height: 50)), for: .disabled)
        ...
        rightBtn.addTarget(self, action: #selector(conclusion), for: .touchUpInside)
    ...
    init(title: String?,
         ...
         config: FeatureAlertConfig?)
    {
        //: let model = config ?? ShowAlertConfig.init()
        let model = config ?? FeatureAlertConfig()
        //: alertConfig = model
        alertConfig = model
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: displayHisName.reversed(), encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func editor(title: String, isTouch: Bool) {
        ...
        if !isTouch {
            ...
            rightBtn.setBackgroundImage(UIImage.titled(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.BundleImageNamed(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
            let image = UIImage.titled(name: alertConfig.rightBackgroundImage).withTintColor(.tributeChromaticColour())
...
extension PromiseThen: UITextViewDelegate {
    //: func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
    func menu(_ textField: UITextField, shouldChangeCharactersIn _: NSRange, replacementString string: String) -> Bool {
    ...
    func oldMic(_ textField: UITextField) -> Bool {
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > alertConfig.lengthLimit {
            ...
            if textNum > alertConfig.lengthLimit && alertConfig.lengthLimit > 0 {
                ...
                textView.text = jumpProperty(index: alertConfig.lengthLimit, text: textContent)
            }
        }
    }

    //: private func string_prefix(index: Int, text: String) -> String {
    private func jumpProperty(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
