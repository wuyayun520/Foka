...
fileprivate let appPaperUrl:[UInt8] = [0x7c,0x7b,0x7c,0x61,0x3d,0x76,0x7a,0x71,0x70,0x67,0x2f,0x3c,0x35,0x7d,0x74,0x66,0x35,0x7b,0x7a,0x61,0x35,0x77,0x70,0x70,0x7b,0x35,0x7c,0x78,0x65,0x79,0x70,0x78,0x70,0x7b,0x61,0x70,0x71]

private func rowDown(tern num: UInt8) -> UInt8 {
    return num ^ 21
}

/*: "More" :*/
fileprivate let k_networkAlert:[Character] = ["M","o","r","e"]

/*: "icon_details_arrow" :*/
fileprivate let layoutDelayError:String = "exposure doticon_"
fileprivate let commonTradeHelper:String = "rroparticle"

/*: "Post" :*/
fileprivate let moduleAroundRingUrl:[Character] = ["P","o","s","t"]

/*: "btn_dynamic_stop_nor" :*/
fileprivate let componentPsychologicalFaceSession:[Character] = ["b","t","n","_","d"]
fileprivate let sessionGiveLeastPlatform:[Character] = ["y"]
fileprivate let screenFamilyMessage:[Character] = ["n","a","m","i","c","_","s","t","o","p","_","n","o","r"]
...
class PackageThen: UITableViewCell {
    //: var userId: String = ""
    var userId: String = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appPaperUrl.map{rowDown(tern: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .childMagnitude(0, 0, 0, 0.4)
    ...
    lazy var moreButton: GrantRushTalkingButton = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .modelSize(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(k_networkAlert)).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (String(layoutDelayError.suffix(5)) + "details_a" + commonTradeHelper.replacingOccurrences(of: "particle", with: "w"))), for: .normal)
...
extension PackageThen {
    //: private func bindInteraction() {
    private func menuTotaleraction() {}
}
...
extension PackageThen {
    ...
    func userLast(userModel: AcidTransformable) {
        ...
        strA = [(String(moduleAroundRingUrl)).localized, str]
        //: colorA = [UIColor.appTitleColor(), UIColor.appValueColor()]
        colorA = [UIColor.restoreAudience(), UIColor.valueColor()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.celluloidSize(fontSize: 18), UIFont.celluloidSize(fontSize: 16)]
        ...
        attributedString = attributedString.withFonts(texts: strA, colors: colorA, fonts: fontA)
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: SecondMeasurable = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.rimePecCorporate())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    running(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.rimePecCorporate())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    running(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.rimePecCorporate())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    running(btn: thirdImgView)
                }
            }
        }
    }
    ...
    func running(btn: UIImageView) {
        ...
        let imag = UIImageView(image: UIImage(named: (String(componentPsychologicalFaceSession) + String(sessionGiveLeastPlatform) + String(screenFamilyMessage))))
...
extension PackageThen {
    ...
    private func agent() {
        ...
        let height = (moduleAdjustPath - actualWidth(w: 15 * 4)) / 3.0
