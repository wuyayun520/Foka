...
/*: "uid" :*/
fileprivate let commonThereData:String = "portraitid"

/*: "onlineStatus" :*/
fileprivate let appBadPage:String = "onprocessn"
fileprivate let colorPreventionUrl:String = "spread lowly popular typeeStatus"

/*: "isNewUser" :*/
fileprivate let moduleAwakeEvent:String = "igiven"
fileprivate let k_workplaceKey:String = "board company quantityNewUser"

/*: "isTPAuth" :*/
fileprivate let commonAutomaticallySystem:String = "sibling minute apartment care lowerisTPAuth"

/*: "totalIntimate" :*/
fileprivate let commonSmoothTimer:String = "tdocka"
fileprivate let widgetObserverValue:String = "imatbridge"

/*: "userStatus" :*/
fileprivate let widgetWalkingPreference:String = "userStick parent depth related"
fileprivate let layoutLitAgainMessage:String = "TATUS"

/*: "intimate" :*/
fileprivate let k_nearbyHelper:[Character] = ["i"]
fileprivate let componentWrittenData:String = "ntimatsecret"
...
enum ChatCrashAcquireQuantityLiteral: Int {
    //: case top  = 0
    case top = 0 // 置顶
    //: case normal = 1
    case normal = 1 // 默认
...
class EvolutionListManager {
    ...
    private var moreMsgModel: SensitiveConversationModel = //: return SensitiveConversationModel.getPlaceholderModel(chatType: .moreMsg)
        .variety(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func settle() {
        //: self.officialArr = GroupConversationListener.shared.getOfficialList()
        self.officialArr = GroupConversationListener.shared.fullAppear()
        //: self.topArr = GroupConversationListener.shared.topConvList
        self.topArr = GroupConversationListener.shared.topConvList
        //: self.normalArr = GroupConversationListener.shared.norConvList
        self.normalArr = GroupConversationListener.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        necessaryData()
    ...
    func receiveCompletion(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: GroupConversationListener.shared.getChatRoomData { succeed in
        GroupConversationListener.shared.quantitativeRelation { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = GroupConversationListener.shared.chatRoomList
                self.chatRoomArr = GroupConversationListener.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func objectExamine(indexPath: IndexPath) -> SensitiveConversationModel? {
    ...
    func clearUp(model: SensitiveConversationModel?) -> SensitiveConversationModel? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: SubstanceHandyJSON? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: SubstanceHandyJSON? = UsufructuaryThen.befitting(targetId: targetId)
    ...
    func databaseFlag(conversationList: [SensitiveConversationModel], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        UsufructuaryThen.cache(with: tempArr) { _, error in
    ...
    func deleteAppear(conversationModel: SensitiveConversationModel) {
    ...
    func turnOver(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: GroupConversationListener.shared.func__getConversationList { arr, leftMore in
        GroupConversationListener.shared.address { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [SensitiveConversationModel] = //: return Array<SensitiveConversationModel>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [SensitiveConversationModel] = //: return Array<SensitiveConversationModel>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [SensitiveConversationModel] = //: return Array<SensitiveConversationModel>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [SensitiveConversationModel] = //: return Array<SensitiveConversationModel>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [SensitiveConversationModel] = //: return Array<SensitiveConversationModel>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [SensitiveConversationModel] = //: return Array<SensitiveConversationModel>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [SensitiveConversationModel] = //: return Array<SensitiveConversationModel>()
        .init()
    //: }()
}
...
extension EvolutionListManager {
    ...
    func necessaryData() {
        //: self.moreMsgArr = GroupConversationListener.shared.moreMsgList
        self.moreMsgArr = GroupConversationListener.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func numberryMediaPath(indexPath: IndexPath) -> SensitiveConversationModel? {
    ...
    func transition() -> SensitiveConversationModel {
        // 更新更多消息最新数据
        //: req_moreMsgInitData()
        necessaryData()
...
extension EvolutionListManager {
    ...
    func favourite(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        UsufructuaryThen.serviceRecognize(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(commonThereData.replacingOccurrences(of: "portrait", with: "u"))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(appBadPage.replacingOccurrences(of: "process", with: "li") + String(colorPreventionUrl.suffix(7)))].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(moduleAwakeEvent.replacingOccurrences(of: "given", with: "s") + String(k_workplaceKey.suffix(7)))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(commonAutomaticallySystem.suffix(8)))].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(commonSmoothTimer.replacingOccurrences(of: "dock", with: "ot") + "lInt" + widgetObserverValue.replacingOccurrences(of: "bridge", with: "e"))].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(String(widgetWalkingPreference.prefix(5)) + layoutLitAgainMessage.lowercased())].int ?? 1
                    ...
                    self.essential(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func jumpSomebody(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(commonThereData.replacingOccurrences(of: "portrait", with: "u"))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(appBadPage.replacingOccurrences(of: "process", with: "li") + String(colorPreventionUrl.suffix(7)))] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(moduleAwakeEvent.replacingOccurrences(of: "given", with: "s") + String(k_workplaceKey.suffix(7)))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(String(widgetWalkingPreference.prefix(5)) + layoutLitAgainMessage.lowercased())] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((String(widgetWalkingPreference.prefix(5)) + layoutLitAgainMessage.lowercased())) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension EvolutionListManager {
    ...
    func jawBlack() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in GroupConversationListener.shared.norConvList {
        for model in GroupConversationListener.shared.norConvList {
            //: if model.totalIntimate >= PositionThen.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= PositionThen.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.clearUp(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in GroupConversationListener.shared.topConvList {
        for model in GroupConversationListener.shared.topConvList {
            //: if model.totalIntimate >= PositionThen.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= PositionThen.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func thinZone(convModel: SensitiveConversationModel, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func conversationDoing(indexPath: IndexPath) -> SensitiveConversationModel? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func essential(uid: String, intimate: Int) {
        ...
        UsufructuaryThen.followingOf(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func brasil(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(commonThereData.replacingOccurrences(of: "portrait", with: "u"))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(String(k_nearbyHelper) + componentWrittenData.replacingOccurrences(of: "secret", with: "e"))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
