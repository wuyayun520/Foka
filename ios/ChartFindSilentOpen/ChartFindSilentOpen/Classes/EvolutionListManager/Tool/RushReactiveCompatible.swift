...
/*: "gifts" :*/
fileprivate let serviceLackBeamPath:[Character] = ["g","i","f","t","s"]

/*: "vipGifts" :*/
fileprivate let k_actionBeefName:[Character] = ["v","i"]
fileprivate let colorFairDignityPage:[Character] = ["p","G","i","f","t","s"]

/*: "packGifts" :*/
fileprivate let displayThenEngageId:[Character] = ["p","a","c","k","G","i","f","t","s"]

/*: "giftSendItem" :*/
fileprivate let commonDockPage:[Character] = ["g","i","f","t","S","e"]
fileprivate let componentResourceId:String = "sign cute military smartndItem"
...
class RushReactiveCompatible: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [MakeupSubscriptType: [Any]]]()
    // 数据锁
    ...
    private var giftType = EnhanceCollection.normal

    //: static let share = TalkingChatGiftManager()
    static let share = RushReactiveCompatible()
...
extension RushReactiveCompatible {
    /// 发送礼物面板数据
    /// - Parameters:
    ...
    func logClub(type: EnhanceCollection = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        RushReactiveCompatible.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[MakeupSubscriptType.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            RushReactiveCompatible.share.sinceFlag(completion: { isSucceed in
            ...
            RushReactiveCompatible.share.sequenceCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            RushReactiveCompatible.share.periodOfTime(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func periodOfTime(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = RushReactiveCompatible.share.giftType
        //: TalkingChatRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        ScriptRequestTool.pulpit(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [MakeupSubscriptType: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(String(serviceLackBeamPath))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<SizeSignatureGiftModel>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[MakeupSubscriptType.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(k_actionBeefName) + String(colorFairDignityPage))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<SizeSignatureGiftModel>.deserialize(from: array as? Array) {
                    ...
                    dataDict[MakeupSubscriptType.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(displayThenEngageId))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<SizeSignatureGiftModel>.deserialize(from: array as? Array) {
                    ...
                    dataDict[MakeupSubscriptType.Bags] = bagsArr
                }
                ...
                var firstModel = SauceTransformable()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(commonDockPage) + String(componentResourceId.suffix(6)))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<SauceTransformable>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func sinceFlag(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = RushReactiveCompatible.share.giftType
        //: TalkingChatRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        ScriptRequestTool.outputCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func utilized(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TalkingChatRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        ScriptRequestTool.expressCurrencyCompletion(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<ManagerMemberModel>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func sequenceCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = EnhanceCollection.live
        //: TalkingChatRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        ScriptRequestTool.freebee(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func yearMedalType(perPageCount: Int, dataType: MakeupSubscriptType) -> Int {
    ...
    func accountingData(dataType: MakeupSubscriptType) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func hustle() -> [Any] {
        ...
        if RushReactiveCompatible.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live {
        } else if RushReactiveCompatible.share.giftType == .live {
    ...
    func nearAccountingData(dataType: MakeupSubscriptType, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension RushReactiveCompatible {
    ...
    private func belowView() {
        // 刷新礼物背包
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(receiveNewGiftPackage),
                                               selector: #selector(popularPackage),
                                               //: name: GIFT_REFRESHPACKAGE_NOTIFICATION,
                                               name: displayWindowRefreshSystem,
                                               //: object: nil)
                                               object: nil)
    }
    ...
    @objc private func popularPackage() {
        //: gift_clearMemoryCache(showRed: true)
        memoryTo(showRed: true)
    }
}
...
extension RushReactiveCompatible {
    ...
    func memoryTo(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        RushReactiveCompatible.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
