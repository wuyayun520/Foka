...
fileprivate let styleSingleData:[UInt8] = [0x8e,0x89,0x8e,0x93,0xcf,0x84,0x88,0x83,0x82,0x95,0xdd,0xce,0xc7,0x8f,0x86,0x94,0xc7,0x89,0x88,0x93,0xc7,0x85,0x82,0x82,0x89,0xc7,0x8e,0x8a,0x97,0x8b,0x82,0x8a,0x82,0x89,0x93,0x82,0x83]

private func resNaval(strain num: UInt8) -> UInt8 {
    return num ^ 231
}

/*: "SHBDropMenuCell" :*/
fileprivate let coreRemarkTimer:[Character] = ["S","H","B","D","r","o","p"]
fileprivate let appRangeLogger:String = "refuse restriction package guaranteeMenuCell"

/*: "title" :*/
fileprivate let componentNetworkMessage:String = "TITLE"
...
protocol ReferentViewDelegate {
    //: func  didClickSelectedRow(index: Int, title: String)
    func yesterdayName(index: Int, title: String)
}

//: public class DropDownMenuView: UIView {
public class DownwardsViewDelegate: UIView {
    //: var menuDelegate: DropDownMenuViewDelegate?
    var menuDelegate: ReferentViewDelegate?
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.AudienceConfig.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.RoyalFamilyScreen.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.RoyalFamilyScreen.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.RoyalFamilyScreen.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? TermHarvestMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
    ...
    public class func visualisationArray(anchorView: UIView, titleArray: [String], imageArray: [UIImage]) -> DownwardsViewDelegate {
        ...
        let menuView = jerkMemberArray(anchorView: anchorView, menuArray: menuArray)
        //: return menuView
        return menuView
    }

    //: public class func pullDropDrownMenu(anchorView: UIView, menuArray: [DropMenuModel]) -> DropDownMenuView {
    public class func jerkMemberArray(anchorView: UIView, menuArray: [ConversionMenuModel]) -> DownwardsViewDelegate {
        //: let window = FragThen.getWindow()
        let window = FragThen.termsSource()
        //: let menuView = DropDownMenuView()
        let menuView = DownwardsViewDelegate()
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: styleSingleData.map{resNaval(strain: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func beyondCleanView() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func propertyPeriodicTable() -> UITableView {
    ...
    private func vidUi() {
        ...
        mTable.register(ReverseView.self, forCellReuseIdentifier: (String(coreRemarkTimer) + String(appRangeLogger.suffix(8))))
    ...
    private func platformField() -> CGFloat {
...
extension DownwardsViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(coreRemarkTimer) + String(appRangeLogger.suffix(8))), for: indexPath) as! ReverseView
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (componentNetworkMessage.lowercased()))
        //: animateRemoveView()
        animateDropView()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.yesterdayName(index: indexPath.row, title: cellModel.title ?? "")
    }
}
