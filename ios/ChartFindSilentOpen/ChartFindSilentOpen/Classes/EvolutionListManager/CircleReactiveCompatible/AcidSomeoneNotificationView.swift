...
fileprivate let coreBrightAIgnoreTimer:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Reply" :*/
fileprivate let screenPriorityPreference:[Character] = ["R","e","p","l","y"]

/*: "#F95151" :*/
fileprivate let themeLiveUtility:String = "shirt less limit tot#F9515"
fileprivate let screenBelowMessage:String = "1"
...
// __CLOSE_PRINT__
//
//  AcidSomeoneNotificationView.swift
//  AbroadTalking
//
...
let spacingColorConfig = (appSceneUrl + 80.0)
//: class TalkingMessageNotificationView: UIView {
class AcidSomeoneNotificationView: UIView {
    //: var startDismissBlock: ((_ model: TalkingConversationModel) -> Void)?
    var startDismissBlock: ((_ model: SensitiveConversationModel) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: AcidSomeoneNotificationView) -> Void)? // 完成移除视图Block
    //: private var currModel = TalkingConversationModel()
    private var currModel = SensitiveConversationModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: coreBrightAIgnoreTimer.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .modelSize(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .modelSize(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(screenPriorityPreference)).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(themeLiveUtility.suffix(6)) + screenBelowMessage.capitalized)), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension AcidSomeoneNotificationView {
    /// 展示视图，5s后自动移除
    //: func show(_ model: TalkingConversationModel?) {
    func familyDiskShow(_ model: SensitiveConversationModel?) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -spacingColorConfig
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func anima() {
        ...
        if currModel.chatType == .service {
            ...
            ProdPushManager.share.monthCompletion(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func quote(gesture: UISwipeGestureRecognizer) {
...
extension AcidSomeoneNotificationView {
    ...
    func factorReload() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.image = UIImage.towardPrepare()
            //: iconBorder.isHidden = true
            iconBorder.isHidden = true
        }

        //: msgShowLabel.attributedText = TalkingChatListTableCell.handleShowC2CDetailTextOfMessage(model: currModel)
        msgShowLabel.attributedText = BardReactiveCompatible.gravityRes(model: currModel)
    }
    ...
    private func scoff() {
        ...
        self.frame = CGRect(x: 0, y: -spacingColorConfig, width: moduleAdjustPath, height: spacingColorConfig)
