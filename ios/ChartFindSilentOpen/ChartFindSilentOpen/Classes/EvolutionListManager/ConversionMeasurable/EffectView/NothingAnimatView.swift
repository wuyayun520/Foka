...
fileprivate let coreHardwareFormat:[UInt8] = [0x8a,0x8d,0x8a,0x97,0xcb,0x80,0x8c,0x87,0x86,0x91,0xd9,0xca,0xc3,0x8b,0x82,0x90,0xc3,0x8d,0x8c,0x97,0xc3,0x81,0x86,0x86,0x8d,0xc3,0x8a,0x8e,0x93,0x8f,0x86,0x8e,0x86,0x8d,0x97,0x86,0x87]

private func awakeField(unit num: UInt8) -> UInt8 {
    return num ^ 227
}

/*: "iosSvgaEffect" :*/
fileprivate let themeCurrencyData:[Character] = ["i","o","s","S","v","g","a","E","f","f"]
fileprivate let viewThirtyMetrics:[Character] = ["e","c","t"]

/*: "iosEffect" :*/
fileprivate let displayCompanyValue:String = "residence hung restoreiosE"

/*: "entityId" :*/
fileprivate let screenTapMetrics:String = "straightntity"
fileprivate let styleAnimalPlatform:String = "Idpaper icon park"
...
class NothingAnimatView: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: RushModelType?
    ...
    var animatedView: SpecifySizeThen?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: GroupStepProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: coreHardwareFormat.map{awakeField(unit: $0)}, encoding: .utf8)!)
    }
...
extension NothingAnimatView {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func ammunition(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(themeCurrencyData) + String(viewThirtyMetrics))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(displayCompanyValue.suffix(4)) + "ffect")] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(screenTapMetrics.replacingOccurrences(of: "straight", with: "e") + String(styleAnimalPlatform.prefix(2)))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = ageTake(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func ageTake(msgModel: Any?) -> Bool {
        ...
        if msgModel is RushModelType {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? RushModelType
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is ExternalAnimatModel {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! ExternalAnimatModel
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.contributionNum(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = RushModelType()
            ...
            if giftMsgModel.showType == BetterPriorityTarget.myStery.rawValue || giftMsgModel.showType == BetterPriorityTarget.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: RushModelType = self.effectMsgModelArray[i] as! RushModelType
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? RushModelType())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? RushModelType(), at: index)
                }
            ...
            each(effectMsgModel: effectMsgModel!)
        }
...
extension NothingAnimatView {
    ...
    func render() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = erase() else { return }
                //: firstShow(effectMsgModel: removeModel)
                each(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.uphold()
        }
    ...
    func erase() -> RushModelType? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return RushModelType()
        }
        ...
        if effectMsgModel is RushModelType {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! RushModelType
        ...
        return effectMsgModel as? RushModelType
    }
}
...
extension NothingAnimatView {
    ...
    func each(effectMsgModel: RushModelType) {
        ...
        self.delegate?.uncontaminatedMin(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        SinceAnimatTool.shared.pushTag(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.fishingLicenceChange(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? ConversionMeasurable())
            //: })
        })
    }
    ...
    func fishingLicenceChange(effectMsgModel: RushModelType, success: Bool, effectModel: ConversionMeasurable) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.transfer(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    breath(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func submitAvailable(effectMsgModel: RushModelType, effectModel: ConversionMeasurable) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.full()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.fun()
        ...
        className = SinceAnimatTool.shared.commercialEnterprise(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! SpecifySizeThen.Type
...
extension NothingAnimatView: ScriptViewDelegate {
    ...
    func rollAround(effectItemView: SpecifySizeThen, success: Bool) {
        ...
        self.delegate?.spendManager(effectView: self, obj: effectItemView.effectMsgModel ?? RushModelType(), success: success)
    }
    ...
    func dataResultCounteraction(effectItemView: SpecifySizeThen) {
        ...
        self.delegate?.uS(effectView: self, obj: effectMsgModel!)
    }
}
...
extension NothingAnimatView {
    ...
    func tapeMotivation() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.full() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.fun() // clean animation data
