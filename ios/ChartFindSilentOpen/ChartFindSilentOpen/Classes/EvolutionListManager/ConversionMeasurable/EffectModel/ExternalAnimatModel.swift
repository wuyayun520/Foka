...
/*: "num" :*/
fileprivate let layoutExactlySystem:String = "NUM"
...
@objc public enum BetterPriorityTarget: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct ExternalAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// BetterPriorityTarget
    ...
    mutating func contributionNum(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct ModelType: HandyJSON {
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (layoutExactlySystem.lowercased())
    }
}
