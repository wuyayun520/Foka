...
/*: "call_bgm" :*/
fileprivate let coreHurryBenchName:[Character] = ["c","a","l","l","_","b","g","m"]

/*: "call_response_bgm" :*/
fileprivate let appChainFormat:[Character] = ["c","a","l","l","_","r","e","s","p","o","n","s"]
fileprivate let colorRepresentativeKey:String = "e_bgmm para disappear auction reader"

/*: "invitePop_call_bgm" :*/
fileprivate let kRoadPath:String = "itemvit"
fileprivate let styleAcquirePreference:[Character] = ["l","l","_","b","g","m"]
...
// __CLOSE_PRINT__
//
//  PreferenceReactiveCompatible.swift
//  MLProject
//
...
class PreferenceReactiveCompatible: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = SeasonSum.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func burn(type: SeasonSum) -> PreferenceReactiveCompatible {
        //: let player = TalkingCallBGMPlayer()
        let player = PreferenceReactiveCompatible()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.rentOf()
...
extension PreferenceReactiveCompatible {
    //: func play() {
    func twoPlay() {
    ...
    private func containIn(notify: Notification) {
    ...
    private func setReport(notify: Notification) {
    ...
    private func rentOf() {
        ...
        var fileName = (String(coreHurryBenchName))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(appChainFormat) + String(colorRepresentativeKey.prefix(5)))
            //: } else if self.type == .invitePop {
        } else if self.type == .invitePop {
            //: fileName = "invitePop_call_bgm"
            fileName = (kRoadPath.replacingOccurrences(of: "item", with: "in") + "ePop_ca" + String(styleAcquirePreference))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = OwnEffectTool.default.penumbra(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension PreferenceReactiveCompatible: TXVodPlayListener {
