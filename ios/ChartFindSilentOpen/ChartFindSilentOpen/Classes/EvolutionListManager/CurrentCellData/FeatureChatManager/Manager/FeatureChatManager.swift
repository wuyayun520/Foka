...
/*: "mf/call/GetUserInfo" :*/
fileprivate let viewArticleHelper:String = "mf/corientation pitch ting"
fileprivate let themeSumerestedPreference:String = "GetUsessential patent"
fileprivate let featureNetValue:[Character] = ["e","r","I","n","f","o"]

/*: "uid" :*/
fileprivate let themeRoughUtility:[UInt8] = [0xa9,0xb5,0xb8]

private func layerEvery(family num: UInt8) -> UInt8 {
    return num ^ 220
}

/*: "TRTC 随机视频匹配中." :*/
fileprivate let moduleTotalerruptLogger:String = "nose tongueTRTC 随机视"
fileprivate let widgetAdditionPlatform:String = "频\u{5339}配中."

/*: "TRTC  :*/
fileprivate let appSmallTimer:[Character] = ["T","R","T","C"," "]

/*: 视频通话. logId: :*/
fileprivate let moduleNaturalConfig:[Character] = ["视","\u{9891}","通","话","."," ","l","o","g","I","d",":"]

/*: "TRTC 视频通话销毁. logId： :*/
fileprivate let spacingAboutConfig:String = "TRTC \u{89c6}"
fileprivate let coreThemTimer:String = "closed position aim too walking毁. logId："

/*: "TRTC 视频通话错误. errCode: :*/
fileprivate let k_expectedName:String = "TRTC 视resist right mouth"
fileprivate let sessionProductionHelper:String = "material off errC"

/*: , errMsg: :*/
fileprivate let featureMarkLogger:String = "self forget operation, errM"
fileprivate let widgetCareEventId:String = "sg:corner dominant"

/*: "TRTC 视频通话SDK收到警告. warningCode: :*/
fileprivate let viewRollingMarginMessage:[Character] = ["T","R","T","C"," ","视","频","通","话","S","D","K","收","到","警","告","."]
fileprivate let colorRecordingSettings:String = " wadirect common"
fileprivate let themeOriginalName:String = "rningCat aspect yes population"

/*: , warningMsg: :*/
fileprivate let displayReasonUtterMessage:String = "butterfly rack laboratory, wa"
fileprivate let k_innerTitle:String = ":"

/*: "TRTC 视频通话进入房间. result: :*/
fileprivate let serviceSingleTimer:String = "sampleTC"
fileprivate let widgetPicMessage:String = "话进\u{5165}房\u{95f4}."
fileprivate let layoutUtilityConfig:[Character] = [" "]
fileprivate let styleTermsLiveSession:[Character] = ["r","e","s","u","l","t",":"]

/*: , logId： :*/
fileprivate let appEffSideError:String = "available currently strike box form, logI"
fileprivate let viewTrustError:String = "constraint argumentd："

/*:  加入了房间" :*/
fileprivate let coreMayEvent:[Character] = [" ","加","入","\u{4e86}","\u{623f}"]
fileprivate let styleSharePairName:String = "sou"

/*:  离开了房间" :*/
fileprivate let componentPerformTimer:[Character] = [" ","离","开","了","房","间"]
...
class EarnChatConfig: NSObject {
    ...
    var remoteCameraStatus = RationalCameraStatus.Normal // 对方摄像头状态
}

//: class TalkingVideoChatManager: NSObject {
class FeatureChatManager: NSObject {
    //: var infoModel: TalkingVideoChatModel?
    var infoModel: GrantChatModel? // 视频通话model
    //: var config = TalkingVideoChatConfig()
    var config = EarnChatConfig() // 视频渲染配置类
    ...
    private var trtcView: BrobdingnagianThen? // 预览view（使用商汤美颜）
...
extension FeatureChatManager {
    /// 开始视频速配（男）
    /// - Parameters:
    ///   - uid: 接收者的id
    ///   - completion: 回调
    //: class func req_callGetUserInfo(uid: String, completion: @escaping FinishBlock) {
    class func usedCompletion(uid: String, completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = WorkplaceQueryRequestModel()
        //: reqModel.requestPath = "mf/call/GetUserInfo"
        reqModel.requestPath = (String(viewArticleHelper.prefix(4)) + "all/" + String(themeSumerestedPreference.prefix(5)) + String(featureNetValue))
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["uid": uid]
        reqModel.params = [String(bytes: themeRoughUtility.map{layerEvery(family: $0)}, encoding: .utf8)!: uid]
...
extension FeatureChatManager {
    ...
    func frontIn(_ frontCamera: Bool) {
        ...
        self.overTin(front: frontCamera)
    }

    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func headRank(_ openCamera: Bool) {
    ...
    func segmentView() {
        ...
        DispatchQueue.main.async {
            ...
            self.option(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
            //: self.reloadTRTCRemotView()
            self.propositionItem()
        }
    }
...
extension FeatureChatManager {
    ...
    private func overTin(front: Bool) {
        //: guard SenseTime_Use == true else {
        guard kVoiceMicPathContent == true else {
        ...
        self.trtcView?.supremeFront(isFront: front)
    }

    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func bowler(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard kVoiceMicPathContent == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.towardGesture(isOpen: isOpen)
...
extension FeatureChatManager {
    ...
    func brush() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(openicial(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(immobilize(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(partNotif(notif:)),
    ...
    @objc private func openicial(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func immobilize(notif: Notification?) {
        //: if notif == nil {
    ...
    @objc private func partNotif(notif: Notification) {
    ...
    @objc func offAd(notif: Notification) {
...
extension FeatureChatManager {
    ...
    func sumeraction(isMatch: Bool = false) {
        //: if isMatch {
        if isMatch {
            //: UploadLogTool.writeLog(msg: "TRTC 随机视频匹配中.")
            CheckThen.placeLoad(msg: (String(moduleTotalerruptLogger.suffix(8)) + widgetAdditionPlatform))
            //: } else {
        } else {
            //: UploadLogTool.writeLog(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            CheckThen.placeLoad(msg: (String(appSmallTimer)) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (String(moduleNaturalConfig)) + "\(infoModel?.logId ?? 0).")
        }
        //: if self.infoModel?.videoStage == .Waiting {
        if self.infoModel?.videoStage == .Waiting {
            //: startTRTCPreview(smallWindow: false)
            option(smallWindow: false)
    ...
    func option(smallWindow: Bool) {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = BrobdingnagianThen()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.eyePusher(isSmallWindow: smallWindow)
        //: if smallWindow {
    ...
    private func quantityercommunicate() {
    ...
    private func anyToo() {
        ...
        CheckThen.placeLoad(msg: (spacingAboutConfig + "频\u{901a}话销" + String(coreThemTimer.suffix(9))) + "\(infoModel?.logId ?? 0).")
...
extension FeatureChatManager: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        CheckThen.placeLoad(msg: (String(k_expectedName.prefix(6)) + "频通话错误." + String(sessionProductionHelper.suffix(5)) + "ode:") + "\(errCode)" + (String(featureMarkLogger.suffix(6)) + String(widgetCareEventId.prefix(3))) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        CheckThen.placeLoad(msg: (String(viewRollingMarginMessage) + String(colorRecordingSettings.prefix(3)) + String(themeOriginalName.prefix(6)) + "ode:") + "\(warningCode)" + (String(displayReasonUtterMessage.suffix(4)) + "rningMsg" + k_innerTitle.capitalized) + "\(String(describing: warningMsg)).")
        ...
        if self.config.remoteCameraStatus == .Normal {
            ...
            NotificationCenter.default.post(name: sessionEnableBottomTitle, object: self)
        }
    }
    ...
    func onEnterRoom(_ result: Int) {
        ...
        CheckThen.placeLoad(msg: (serviceSingleTimer.replacingOccurrences(of: "sample", with: "TR") + " 视频通" + widgetPicMessage + String(layoutUtilityConfig) + String(styleTermsLiveSession)) + "\(result)" + (String(appEffSideError.suffix(6)) + String(viewTrustError.suffix(2))) + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (String(coreMayEvent) + styleSharePairName.replacingOccurrences(of: "sou", with: "间")))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason _: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (String(componentPerformTimer)))
    }
    ...
    func onUserVideoAvailable(_ userId: String, available: Bool) {
        //: guard userId != PositionThen.share.loginUserMode.userID else { return }
        guard userId != PositionThen.share.loginUserMode.userID else { return }
