...
/*: "#BAFBFF" :*/
fileprivate let sessionDailyError:String = "process add heavy black index#BAFBFF"
...
class ThereThen: NSObject {
    ...
    var model = FamousEditTheoreticalAccountModelType()

    //: func caculateMsgHeight(model: TalkingVideoCallDanmuModel) -> TalkingVideoCallDanmuModel {
    func createModel(model: FamousEditTheoreticalAccountModelType) -> FamousEditTheoreticalAccountModelType {
        //: self.model = model
        self.model = model
        /// 普通文本消息
    ...
    func heightModel(model: FamousEditTheoreticalAccountModelType) -> FamousEditTheoreticalAccountModelType {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func contenture() {
        ...
        let name = signReply(text: firstStr, font: UIFont.celluloidSize(fontSize: 14), color: UIColor(hex: (String(sessionDailyError.suffix(7))))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = signReply(text: self.model.content, font: UIFont.celluloidSize(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        capricorn(attribText: self.model.msgAttribText)
    }
    ...
    func asPage() {
        ...
        let content = signReply(text: self.model.transContent, font: UIFont.celluloidSize(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func balanceFragment() -> NSMutableParagraphStyle {
    ...
    func signReply(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = balanceFragment()
        //: return attribute
        return attribute
    }
    ...
    func capricorn(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = k_sinceTimer - 36
