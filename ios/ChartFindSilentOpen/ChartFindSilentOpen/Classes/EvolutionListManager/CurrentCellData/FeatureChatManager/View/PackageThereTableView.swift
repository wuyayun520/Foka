...
fileprivate let screenVaryRoundTimer:[UInt8] = [0xba,0xbd,0xba,0xa7,0xfb,0xb0,0xbc,0xb7,0xb6,0xa1,0xe9,0xfa,0xf3,0xbb,0xb2,0xa0,0xf3,0xbd,0xbc,0xa7,0xf3,0xb1,0xb6,0xb6,0xbd,0xf3,0xba,0xbe,0xa3,0xbf,0xb6,0xbe,0xb6,0xbd,0xa7,0xb6,0xb7]
...
@objc protocol HiObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func regain()

    //: @objc optional func endScroll()
    @objc optional func detectScroll()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class PackageThereTableView: UIView {
    ...
    var msgArray = [FamousEditTheoreticalAccountModelType]() /// 消息数组(数据源)
    //: var  tempMsgArray = Array<TalkingVideoCallDanmuModel>()
    var tempMsgArray = [FamousEditTheoreticalAccountModelType]() /// 用于存储消息还未刷新到tableView的时候接收到的消息
    //: var inPending = false
    var inPending = false /// 是否处于爬楼状态
    //: var refreshTimer: Timer?
    var refreshTimer: Timer? /// 刷新定时器
    //: var reloadType: VideoReloadLiveMsgRoomType?
    var reloadType: BivouacMirrorPath?
    //: weak var delegate: TalkingVideoDanmuMsgListTableViewDelegate?
    weak var delegate: HiObjectProtocol?
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(scriptEqual))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: screenVaryRoundTimer.map{$0^211}, encoding: .utf8)!)
    }
...
extension PackageThereTableView {
    //: func startTimer() {
    func signifier() {
        //: stopTimer()
        anNightTimer()
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(networkEvent), userInfo: nil, repeats: true)
    }
    ...
    @objc func networkEvent() {
        //: tryToappendAndScrollToBottom()
        roadMap()
    }

    //: @objc func tapAction() {
    @objc func scriptEqual() {
...
extension PackageThereTableView {
    ...
    func micAcross(msgModel: FamousEditTheoreticalAccountModelType?) {
    ...
    func cornerUnless() {
        ...
        if AllHeight > sessionObjectEventAlert {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < sessionObjectEventAlert {
                ...
                self.msgTableView.height = sessionObjectEventAlert
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = sessionObjectEventAlert - AllHeight
        ...
        revenue(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func revenue(animated: Bool) {
    ...
    func loseInUnfinishedPending(inPending: Bool) {
...
extension PackageThereTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func mpAttach(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.detectScroll?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(themeQualityValue)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = CurrentReactiveCompatible.appealingness(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension PackageThereTableView {
    ...
    func image() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: k_sinceTimer, height: sessionObjectEventAlert)
    }
}
