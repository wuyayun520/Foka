...
fileprivate let moduleMentionName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "UITableViewCell" :*/
fileprivate let widgetQuickTriggerSettings:[Character] = ["U","I","T","a","b","l","e","V","i","e","w","C","e","l","l"]
...
class CurrentReactiveCompatible: UITableViewCell {
    //: var msgModel = TalkingVideoCallDanmuModel.init()
    var msgModel = FamousEditTheoreticalAccountModelType()
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.msgLabel.snp.makeConstraints { make in
            ...
            make.top.equalTo(themeQualityValue + 2)
...
//            make.trailing.lessThanOrEqualTo(-32)
        ...
        self.bgLb.snp.makeConstraints { make in
            ...
            make.top.equalTo(themeQualityValue)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: moduleMentionName.reversed(), encoding: .utf8)!)
    }
...
extension CurrentReactiveCompatible {
    ...
    class func appealingness(tableView: UITableView, msg: FamousEditTheoreticalAccountModelType, indexPath _: IndexPath) -> CurrentReactiveCompatible {
        //: var cell: TalkingVideoDanmuMsgBassCell?
        var cell: CurrentReactiveCompatible?

        //: if msg.messageType == 1 {
        if msg.messageType == 1 {
            //: let identifier = "\(TalkingVideoDanmuMsgTextCell.className())_\(msg.messageType)"
            let identifier = "\(DisableNameCell.className())_\(msg.messageType)"
            //: tableView.register(TalkingVideoDanmuMsgTextCell.self, forCellReuseIdentifier: identifier)
            tableView.register(DisableNameCell.self, forCellReuseIdentifier: identifier)
            //: cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingVideoDanmuMsgTextCell
            cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? DisableNameCell
            //: if cell == nil {
            if cell == nil {
                //: cell = TalkingVideoDanmuMsgTextCell(style: .default, reuseIdentifier: identifier)
                cell = DisableNameCell(style: .default, reuseIdentifier: identifier)
            }
            //: }else {
        } else {
            //: let identifier = "UITableViewCell"
            let identifier = (String(widgetQuickTriggerSettings))
            //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier)
            var cell = tableView.dequeueReusableCell(withIdentifier: identifier)
            //: if cell == nil {
            if cell == nil {
                //: cell = TalkingVideoDanmuMsgBassCell(style: .default, reuseIdentifier: identifier)
                cell = CurrentReactiveCompatible(style: .default, reuseIdentifier: identifier)
            }
            //: return cell as! TalkingVideoDanmuMsgBassCell
            return cell as! CurrentReactiveCompatible
        }
