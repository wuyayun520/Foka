...
fileprivate let colorContainHelper:[UInt8] = [0xd1,0xd6,0xd1,0xcc,0x90,0xdb,0xd7,0xdc,0xdd,0xca,0x82,0x91,0x98,0xd0,0xd9,0xcb,0x98,0xd6,0xd7,0xcc,0x98,0xda,0xdd,0xdd,0xd6,0x98,0xd1,0xd5,0xc8,0xd4,0xdd,0xd5,0xdd,0xd6,0xcc,0xdd,0xdc]

private func timeReadyResign(common num: UInt8) -> UInt8 {
    return num ^ 184
}

/*: "#FFD13A" :*/
fileprivate let commonEffectRingDevice:[Character] = ["#","F","F","D","1","3"]
fileprivate let moduleTranslateMessage:String = "originally"

/*: "gift_gb_umnad_1" :*/
fileprivate let styleReplyConfig:[Character] = ["g","i","f","t","_","g","b","_","u","m","n","a","d","_","1"]

/*: "icon_circle_pic_default" :*/
fileprivate let featureSufficientSettings:String = "ICON"
fileprivate let spacingLocalSystem:String = "contain pressure purchasele_p"
fileprivate let displayPointOperatePlatformLogger:String = "efauend"
fileprivate let themeShouldBurnMetrics:String = "achievement"

/*: "Open lucky blind-box" :*/
fileprivate let themeTransformPreference:[Character] = ["O","p","e","n"," ","l","u","c","k","y"," ","b","l","i","n","d","-","b","o","x"]

/*: "Sent " :*/
fileprivate let viewHemId:[Character] = ["S","e","n","t"," "]

/*: "PingFangSC-Semibold" :*/
fileprivate let themeHidePlatform:[Character] = ["P","i","n","g","F"]
fileprivate let kToPreference:[Character] = ["a","n","g","S","C","-","S","e","m","i","b","o","l","d"]

/*: "Bombard" :*/
fileprivate let spacingNavalId:String = "Bombardcommercial gator"
...
class SinceDisableThen: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = ExternalAnimatModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: colorContainHelper.map{timeReadyResign(common: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickLb: UILabel = {
        ...
        lb.font = .attachSize(fontSize: 14)
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(commonEffectRingDevice) + moduleTranslateMessage.replacingOccurrences(of: "originally", with: "A")))
        //: lb.font = UIFont.pingfangRugularFont(fontSize: 13)
        lb.font = UIFont.attachSize(fontSize: 13)
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var pluralTip: PointView = {
    ...
    private lazy var countLb: PointView = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = PointView()
...
extension SinceDisableThen {
    //: func setGiftMsgModel(model: TalkingGiftMsgAnimatModel) {
    func contrastBean(model: ExternalAnimatModel) {
        ...
        self.bgImage.image = UIImage.titled(name: (String(styleReplyConfig)))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.titled(name: (featureSufficientSettings.lowercased() + "_circ" + String(spacingLocalSystem.suffix(4)) + "ic_d" + displayPointOperatePlatformLogger.replacingOccurrences(of: "end", with: "l") + themeShouldBurnMetrics.replacingOccurrences(of: "achievement", with: "t")))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == BetterPriorityTarget.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(themeTransformPreference)).localized
            //: }else {
        } else {
        ...
        self.avatarImage.pup(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.pup(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(viewHemId)).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(themeHidePlatform) + String(kToPreference)), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = overRejectHead(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(themeHidePlatform) + String(kToPreference)), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = overRejectHead(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.withFonts(texts: strs, colors: colors, fonts: fonts)
        ...
        exIcon(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func exIcon(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(themeHidePlatform) + String(kToPreference)), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = overRejectHead(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(themeHidePlatform) + String(kToPreference)), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = overRejectHead(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.childMagnitude(150, 77, 20, 1), UIColor.white, UIColor.childMagnitude(150, 77, 20, 1)]
        ...
        attText = attText.withFonts(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.identityText(attributedText: attText)
        ...
        countLb.changeover(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func partyFrame(giftMsgModel: ExternalAnimatModel) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(daily), object: nil)
        ...
        self.perform(#selector(daily), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func quantityo(model _: ExternalAnimatModel, animate _: Bool) {
    ...
    @objc func daily() {
        ...
        giftMsgModel = ExternalAnimatModel()
...
extension SinceDisableThen {
    ...
    func overRejectHead(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension SinceDisableThen {
    ...
    private func everyMargin() {
        ...
        font = overRejectHead(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.withFonts(texts: [(String(spacingNavalId.prefix(7))).localized], colors: [UIColor.childMagnitude(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.changeover(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.identityText(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.changeover(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
