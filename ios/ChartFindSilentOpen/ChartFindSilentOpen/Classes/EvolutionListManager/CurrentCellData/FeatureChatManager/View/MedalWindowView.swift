...
fileprivate let displayMeanName:[UInt8] = [0xde,0xd9,0xde,0xc3,0x9f,0xd4,0xd8,0xd3,0xd2,0xc5,0x8d,0x9e,0x97,0xdf,0xd6,0xc4,0x97,0xd9,0xd8,0xc3,0x97,0xd5,0xd2,0xd2,0xd9,0x97,0xde,0xda,0xc7,0xdb,0xd2,0xda,0xd2,0xd9,0xc3,0xd2,0xd3]

/*: "#171717" :*/
fileprivate let widgetToxicFormat:String = "#171717"

/*: "icon_cameraoff" :*/
fileprivate let coreNoId:String = "icodraw"
fileprivate let sessionQuantityryPreference:[Character] = ["o","f","f"]
...
class MedalWindowView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.childMagnitude(51, 51, 51, 1)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: displayMeanName.map{$0^183}, encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (widgetToxicFormat.capitalized))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        v.image = UIImage.titled(name: (coreNoId.replacingOccurrences(of: "draw", with: "n") + "_camera" + String(sessionQuantityryPreference)))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension MedalWindowView {
    /// 刷新视频通话背景
    /// - Parameters:
    ///   - urlStr: 背景图片
    ///   - isOpenCamera: 摄像头状态
    //: func refreshBgView(urlStr: String?, isOpenCamera: VideoCameraStatus) {
    func host(urlStr: String?, isOpenCamera: RationalCameraStatus) {
...
extension MedalWindowView: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func the(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, moduleAdjustPath - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, componentMeEvent - self.frame.size.height * 0.5)
    ...
    @objc private func applicationEvent() {
        //: if self.block != nil {
        if self.block != nil {
            //: self.block!()
            self.block!()
...
extension MedalWindowView {
    ...
    private func anyPan() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(applicationEvent))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(the(pan:)))
        //: pan.delegate = self
        pan.delegate = self
