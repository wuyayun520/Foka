...
fileprivate let themeDrySession:[UInt8] = [0x35,0x3a,0x35,0x40,0xf4,0x2f,0x3b,0x30,0x31,0x3e,0x6,0xf5,0xec,0x34,0x2d,0x3f,0xec,0x3a,0x3b,0x40,0xec,0x2e,0x31,0x31,0x3a,0xec,0x35,0x39,0x3c,0x38,0x31,0x39,0x31,0x3a,0x40,0x31,0x30]

fileprivate func calculateBehind(found num: UInt8) -> UInt8 {
...
/*: "icon_chat_miss" :*/
fileprivate let layoutDoingeText:[Character] = ["i","c","o","n","_","c","h","a","t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let coreBraceName:[Character] = ["i","c","o","n","_","c"]
fileprivate let appProcedureButtonMaterialTimer:String = "blank res connect logical fatalhat_yes"
...
class ForagingThen: OccurView {
//    var transBlock: (() -> Void)?

    //: var textData: CompartmentReactiveCompatible?
    var textData: CompartmentReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: themeDrySession.map{calculateBehind(found: $0)}, encoding: .utf8)!)
    }
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != FragThen.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.indexMenu() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.titled(name: (String(layoutDoingeText)))
                ...
                self.transStatusImagV.image = UIImage.titled(name: (String(coreBraceName) + String(appProcedureButtonMaterialTimer.suffix(7))))
            }
    ...
    func palpate() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(preserve(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == FragThen.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == FragThen.ringRowingVertical())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            linkUp()
...
//            }
    ...
    func linkUp() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(communal(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func TouchCellMessageAction(tap: UITapGestureRecognizer?) {
    @objc func preserve(tap _: UITapGestureRecognizer?) {
    ...
    @objc func communal(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
...
//    }
    ...
    @objc func beam() {
        ...
        self.delegate?.answerCell(cell: self)
    }
}
