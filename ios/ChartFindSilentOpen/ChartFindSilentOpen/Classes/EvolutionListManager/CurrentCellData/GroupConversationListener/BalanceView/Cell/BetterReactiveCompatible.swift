...
/*: "#ffffff" :*/
fileprivate let sessionEssentialPreference:String = "creature capture extension note fighter#ffffff"

/*: "earned" :*/
fileprivate let themeHighwayPopulationConfig:String = "lot"
fileprivate let componentLuckWallRegularDevice:String = "arneround"

/*: "Coins in" :*/
fileprivate let appResultEvent:String = "recommend plain routeCoins i"
fileprivate let componentDarkError:String = "family"

/*: "Error:  :*/
fileprivate let screenWaterGoodPreference:[Character] = ["E","r","r","o","r"]
fileprivate let colorEffectQuickTitle:[Character] = [":"," "]

/*: "#C946D2" :*/
fileprivate let k_combinedEvent:String = "#C946D2above earn suitable plat"

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let appBetweenKey:String = "btn_chat_luck appreciate commercial"
fileprivate let componentGreenDevice:[Character] = ["f","l","o","a","t","S","_"]
fileprivate let displayFilterPath:String = "modify upper areauserB"
...
class BetterReactiveCompatible: UIView {
    ...
    private let TextColor = (String(sessionEssentialPreference.suffix(7)))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: ScreenReachTransformable?) {
        //: self.init()
        self.init()
    ...
    private func justAbout() {
        ...
        if danumuMsgModel.headPic.count > 0 {
            ...
            headAnimatedImageView.imitation(urlStr: danumuMsgModel.headPic)
        }
        //: if danumuMsgModel.bgPic.count > 0 {
        if danumuMsgModel.bgPic.count > 0 {
            //: updateBackground(withUrl: danumuMsgModel.bgPic)
            reserveLess(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (themeHighwayPopulationConfig.replacingOccurrences(of: "lot", with: "e") + componentLuckWallRegularDevice.replacingOccurrences(of: "round", with: "d"))) {
                ...
                if let range = attrStr.string.range(of: (String(appResultEvent.suffix(7)) + componentDarkError.replacingOccurrences(of: "family", with: "n"))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.modelSize(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (String(screenWaterGoodPreference) + String(colorEffectQuickTitle)) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func reserveLess(withUrl url: String) {
        //: self.backgroundImageView.setUrlImage(urlStr: url) {[weak self] result, image in
        self.backgroundImageView.pup(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.databaseQuality(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: ScreenReachTransformable? {
        //: didSet {
        didSet {
            //: configModel()
            justAbout()
    ...
    var totalTime: CGFloat {
        ...
        return (moduleAdjustPath + widthValue) * SpendTime / moduleAdjustPath
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * LineSpendTime / ScreenWidth
        return widthValue * LineSpendTime / moduleAdjustPath
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * SpendTime / ScreenWidth
        return widthValue * SpendTime / moduleAdjustPath
    }
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(k_combinedEvent.prefix(7))))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.titled(name: (String(appBetweenKey.prefix(9)) + String(componentGreenDevice) + String(displayFilterPath.suffix(5)) + "K_nor"))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(promptChoose(_:)))
        //: return t
        return t
        //: }()
    }()
    ...
    lazy var contentLabel: UILabel = {
        ...
        l.font = UIFont.celluloidSize(fontSize: 12)
        //: l.textColor = .white
        l.textColor = .white
        //: l.numberOfLines = 1
        l.numberOfLines = 1
...
extension BetterReactiveCompatible {
    ...
    @objc func promptChoose(_: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = PositionThen.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = PositionThen.share.appConfigMode.urlH5Domain + gameUrl
        ...
        ProdPushManager.share.rangeGalleryThirty(urlStr: httpsUrl, webConfig: config)
    }
}
...
extension BetterReactiveCompatible {
    ...
    private func beyondSetupConstraint() {
