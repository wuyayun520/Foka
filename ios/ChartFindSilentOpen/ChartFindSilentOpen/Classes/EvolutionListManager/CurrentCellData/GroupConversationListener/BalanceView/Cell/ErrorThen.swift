...
fileprivate let featureFemaleSystem:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_start" :*/
fileprivate let componentScheduleValue:String = "import copyicon_c"
fileprivate let componentStreamThoughData:String = "T"

/*: "icon_chat_lock" :*/
fileprivate let serviceIdentifyTimer:[Character] = ["i","c","o","n","_","c","h","a","t"]
fileprivate let k_exhibitMetrics:String = "_lockorient sweet circle listen interaction"

/*: "icon_chat_burned" :*/
fileprivate let commonTransitionPlatform:[Character] = ["i","c","o","n","_","c","h","a","t","_","b","u"]
fileprivate let widgetAdvancedName:String = "RNED"

/*: "icon_chat_burned_shan" :*/
fileprivate let appLiveMetrics:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let sessionDefinePage:[Character] = ["t","_","b","u"]
fileprivate let featureAbstractError:String = "rned_shanpill light"

/*: "Already burned" :*/
fileprivate let screenTernPreference:String = "plain reserve communicateAlre"
fileprivate let featureHairCollectError:String = "burntraded"
...
class ErrorThen: OccurView {
    //: var videoData: RationalCellData?
    var videoData: RationalCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: featureFemaleSystem.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var playImgView: UIImageView = {
        ...
        imgV.image = UIImage.titled(name: (String(componentScheduleValue.suffix(6)) + "hat_star" + componentStreamThoughData.lowercased()))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var lockImgView: UIImageView = {
        ...
        imgV.image = UIImage.titled(name: (String(serviceIdentifyTimer) + String(k_exhibitMetrics.prefix(5))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var burnedImgView: GrantRushTalkingButton = {
        ...
        imgV.setBackgroundImage(UIImage.titled(name: (String(commonTransitionPlatform) + widgetAdvancedName.lowercased())), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.titled(name: (String(appLiveMetrics) + String(sessionDefinePage) + String(featureAbstractError.prefix(9)))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(screenTernPreference.suffix(4)) + "ady " + featureHairCollectError.replacingOccurrences(of: "trade", with: "e")).localized, for: .normal)
...
extension ErrorThen {
    ...
    override func minority(with data: ThatReactiveCompatible) {
        ...
        //: self.videoData = data as? RationalCellData
        self.videoData = data as? RationalCellData
        ...
        self.dutationLab.text = NSDate.rimeSocial(totalTime: duration)
        ...
        self.soundMosaic(urlStr: videoModel.coverImg, isMosaic: imgIsMosaic)
    }
    ...
    func soundMosaic(urlStr: String, isMosaic: Bool) {
        //: guard let newData = self.videoData else { return }
        guard let newData = self.videoData else { return }
        ...
        if !coverImgPath.isEmptyString, isExit { // 加载本地
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.picImgView.image = UIImage.fragment(image, putUp: 90)
                self.picImgView.image = UIImage.fragment(image, putUp: 90)
                //: } else {
            } else {
            ...
            self.picImgView.rereadMorro(urlStr: urlStr) { [weak self] image in
