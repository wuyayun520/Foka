...
/*: "title" :*/
fileprivate let appConclusionPlatform:[Character] = ["t","i","t","l","e"]

/*: "content" :*/
fileprivate let componentWritingTimer:String = "contagree"
fileprivate let featureCanName:String = "NT"

/*: "init(coder:) has not been implemented" :*/
fileprivate let styleDigitalEquityLogger:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img" :*/
fileprivate let styleDifferenceId:[Character] = ["i","m","g"]

/*: "jumpKey" :*/
fileprivate let layoutPsychologicalMessage:[Character] = ["j","u","m","p","K"]
fileprivate let sessionRankPreference:String = "efavor"
...
@objcMembers public class PingCrashCellData: CrashCellData {
    // 数据
    //: var extraJson: JSON {
    var extraJson: JSON {
    ...
    public lazy var titleAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.restoreAudience(), .font: UIFont.modelSize(type: .Medium, fontSize: 17)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["title"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(String(appConclusionPlatform))].stringValue)
    ...
    public lazy var contentAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.valueColor(), .font: UIFont.modelSize(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["content"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(componentWritingTimer.replacingOccurrences(of: "agree", with: "e") + featureCanName.lowercased())].stringValue)
        //: attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        //: return attributeStr
        return attributeStr
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: styleDigitalEquityLogger.reversed(), encoding: .utf8)!)
    }
    ...
    override public func graphic(_: CGFloat) -> CGFloat {
        ...
        if extraJson[(String(styleDifferenceId))].stringValue.count > 0 {
        ...
        if extraJson[(String(appConclusionPlatform))].stringValue.count > 0 {
        ...
        if extraJson[(componentWritingTimer.replacingOccurrences(of: "agree", with: "e") + featureCanName.lowercased())].stringValue.count > 0 {
            //: height += 4
            height += 4
        ...
        if extraJson[(String(layoutPsychologicalMessage) + sessionRankPreference.replacingOccurrences(of: "favor", with: "y"))].stringValue.count > 0 {
            //: jumpHeight = 35
            jumpHeight = 35
            //: height += jumpHeight
            height += jumpHeight
