...
fileprivate let sessionCryGreatestEvent:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "[未知消息]" :*/
fileprivate let layoutRecommendationMobileUtility:String = "["
fileprivate let coreHungWorldName:String = "未origin消息]"

/*: "extra" :*/
fileprivate let displayTransformTimer:String = "EXTRA"

/*: "#0084FF" :*/
fileprivate let sessionAppealError:String = "mortgage information shrink#0084FF"

/*: "#FFEE4B" :*/
fileprivate let layoutThenName:String = "#FFEE4Bprovider security server up out"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let componentShotSettings:[Character] = ["<","a","t",">","@","[","\\","S","\\","s","]","+","?"]
fileprivate let viewAnimaName:String = "official pad</at>"

/*: "jumpKey" :*/
fileprivate let appLeanUtility:String = "jumpKphone dangerous"
fileprivate let layoutFormalOccurName:[Character] = ["e","y"]

/*: "user" :*/
fileprivate let viewAccessEnterpriseError:String = "usefuture"

/*: "uid" :*/
fileprivate let spacingMergeSettings:[UInt8] = [0xce,0xc2,0xbd]

fileprivate func castWait(invite num: UInt8) -> UInt8 {
...
/*: "#16D073" :*/
fileprivate let kDeviceId:String = "#16D073once matter voice"

/*: "#F95644" :*/
fileprivate let widgetProductAlert:[Character] = ["#","F","9","5","6","4"]
fileprivate let sessionLovelyDevice:[Character] = ["4"]
...
//: @objcMembers public class FluThen: TalkingChatMsgBaseCellData {
@objcMembers public class FluThen: CrashCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: sessionCryGreatestEvent.reversed(), encoding: .utf8)!)
    }
    ...
    override public var msgModel: StallModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (layoutRecommendationMobileUtility.capitalized + coreHungWorldName.replacingOccurrences(of: "origin", with: "知"))
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = aboutLip(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func aboutLip(msgInfoModel: TalkingStepTheoreticalAccountModel) -> NSMutableAttributedString {
        ...
        guard dic[(displayTransformTimer.lowercased())] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.restoreAudience() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(sessionAppealError.suffix(7))))! : UIColor(hex: (String(layoutThenName.prefix(7))))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(componentShotSettings) + String(viewAnimaName.suffix(5))), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(appLeanUtility.prefix(5)) + String(layoutFormalOccurName))].stringValue == (viewAccessEnterpriseError.replacingOccurrences(of: "future", with: "r")) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: spacingMergeSettings.map{castWait(invite: $0)}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: spacingMergeSettings.map{castWait(invite: $0)}, encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func bottom() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(kDeviceId.prefix(7))))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(widgetProductAlert) + String(sessionLovelyDevice)))!
        }
