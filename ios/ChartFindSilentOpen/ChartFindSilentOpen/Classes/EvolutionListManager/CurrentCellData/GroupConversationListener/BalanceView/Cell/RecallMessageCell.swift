...
fileprivate let moduleMentionFormat:[UInt8] = [0x42,0x45,0x42,0x5f,0x3,0x48,0x44,0x4f,0x4e,0x59,0x11,0x2,0xb,0x43,0x4a,0x58,0xb,0x45,0x44,0x5f,0xb,0x49,0x4e,0x4e,0x45,0xb,0x42,0x46,0x5b,0x47,0x4e,0x46,0x4e,0x45,0x5f,0x4e,0x4f]

/*: "icon_chat_translate_nor" :*/
fileprivate let themeIgnoreBackConfig:[Character] = ["i","c","o","n","_","c","h","a","t","_"]
fileprivate let moduleTurnDigUrl:[Character] = ["t","r","a","n","s","l","a","t","e","_","n","o","r"]

/*: "icon_chat_translate_pre" :*/
fileprivate let moduleSuggestMessage:String = "more letter oneicon_"
fileprivate let featureWeAlert:[Character] = ["c","h","a","t","_","t","r","a","n","s","l","a","t"]
fileprivate let coreChoosePlatform:String = "e_preyou tell value businessman pattern"

/*: "icon_chat_miss" :*/
fileprivate let widgetExampleCorePath:String = "rough reason text playicon_c"
fileprivate let componentRidSession:String = "quality"
fileprivate let featureMyTitle:[Character] = ["a","t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let spacingMoonError:[Character] = ["i","c","o","n","_","c","h","a","t","_","y","e","s"]
...
class RecallMessageCell: OccurView {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: FluThen?
    var textData: FluThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: moduleMentionFormat.map{$0^43}, encoding: .utf8)!)
    }
    ...
    lazy var transLB: YYLabel = {
        ...
        label.font = UIFont.attachSize(fontSize: 16)
        //: return label
        return label
        //: }()
    }()
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.titled(name: (String(themeIgnoreBackConfig) + String(moduleTurnDigUrl))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.titled(name: (String(moduleSuggestMessage.suffix(5)) + String(featureWeAlert) + String(coreChoosePlatform.prefix(5)))), for: .highlighted)
...
extension RecallMessageCell {
    ...
    override func minority(with data: ThatReactiveCompatible) {
        ...
        if imgPreview.count > 0, textData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
            ...
            giftImagV.imitation(urlStr: imgPreview)
        }
        ...
        //: let maxWidth = FragThen.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = FragThen.funGet() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.titled(name: (String(widgetExampleCorePath.suffix(6)) + componentRidSession.replacingOccurrences(of: "quality", with: "h") + String(featureMyTitle)))
            ...
            self.transStatusImagV.image = UIImage.titled(name: (String(spacingMoonError)))
        }
    ...
    func paraGes() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(cell(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func cell(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
