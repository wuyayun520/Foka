...
fileprivate let moduleGoFormat:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "CCCCCC" :*/
fileprivate let displayHandHelper:String = "cccccc"

/*: "icon_lounge" :*/
fileprivate let displayBusyLogger:[Character] = ["i","c","o","n","_","l"]
fileprivate let viewRemTwentyHelper:[Character] = ["o","u","n","g","e"]

/*: "#FFEE4B" :*/
fileprivate let moduleResistRejectError:String = "someone who complaint responsibility called#FFEE4B"

/*: "View read receipts?" :*/
fileprivate let componentStyleKey:String = "View mate letter joint something"
fileprivate let viewExceptFormat:[Character] = ["r","e","a","d"," ","r","e","c","e","i","p","t","s","?"]

/*: "#FBE3FE" :*/
fileprivate let layoutEntityHelper:String = "#FBE3regain target near else visual"
fileprivate let widgetStallUrl:String = "Foutdo"

/*: "#D8DDFF" :*/
fileprivate let sessionMinorityName:[Character] = ["#","D","8","D","D","F","F"]
...
class EnhanceView: OccurView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: moduleGoFormat.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var messageLB: YYLabel = {
        ...
        label.font = UIFont.attachSize(fontSize: 15)
        //: label.textColor = UIColor.init(hex: "CCCCCC")
        label.textColor = UIColor(hex: (displayHandHelper.uppercased()))
    ...
    private lazy var vipReadReceiptBtn: GrantRushTalkingButton = {
        //: let btn = TalkingButton()
        let btn = GrantRushTalkingButton()
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_lounge"), for: .normal)
        btn.setImage(UIImage.titled(name: (String(displayBusyLogger) + String(viewRemTwentyHelper))), for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        btn.titleLabel?.font = UIFont.modelSize(type: .Regular, fontSize: 15)
        //: btn.setTitleColor(UIColor(hex: "#FFEE4B"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(moduleResistRejectError.suffix(7)))), for: .normal)
        //: btn.setTitle("View read receipts?".localized, for: .normal)
        btn.setTitle((String(componentStyleKey.prefix(5)) + String(viewExceptFormat)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(columnEnable), for: .touchUpInside)
        ...
        let gColor = [UIColor(hex: (String(layoutEntityHelper.prefix(5)) + widgetStallUrl.replacingOccurrences(of: "outdo", with: "E")))!.withAlphaComponent(0.6).cgColor,
                      //: UIColor.init(hex: "#D8DDFF")!.withAlphaComponent(0.6).cgColor]
                      UIColor(hex: (String(sessionMinorityName)))!.withAlphaComponent(0.6).cgColor]
        //: btn.setBackgroundImage(UIImage.imageGradientColor(colors: gColor, size: CGSize(width: btnWidth, height: btnHeight)), for: .normal)
        btn.setBackgroundImage(UIImage.totalelMention(colors: gColor, size: CGSize(width: btnWidth, height: btnHeight)), for: .normal)
...
extension EnhanceView {
    ...
    @objc private func columnEnable() {
...
extension EnhanceView {
    ...
    override func minority(with data: ThatReactiveCompatible) {
        ...
        guard let tipCellData = data as? MinimizeCellData else { return }
        ...
        if dataLayout != nil {
            ...
            //: let layout = YYTextLayout.init(containerSize: CGSize.init(width: FragThen.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), text: tipCellData.attributedString)
            let layout = YYTextLayout(containerSize: CGSize(width: FragThen.funGet() - 50, height: CGFloat(MAXFLOAT)), text: tipCellData.attributedString)
        ...
        if tipCellData.msgModel.tips.content == (String(componentStyleKey.prefix(5)) + String(viewExceptFormat)).localized {
    ...
    override func updateConstraints() {
        ...
        self.container.snp.remakeConstraints { make in
            ...
            make.width.lessThanOrEqualTo(moduleAdjustPath - 40)
            //: make.top.equalTo(10)
            make.top.equalTo(10)
    ...
    func suite() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(smallTap(tap:)))
    ...
    @objc func smallTap(tap _: UITapGestureRecognizer) {
