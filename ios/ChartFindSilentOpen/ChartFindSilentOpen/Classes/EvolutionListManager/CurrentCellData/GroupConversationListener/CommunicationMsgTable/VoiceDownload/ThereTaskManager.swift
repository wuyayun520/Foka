...
fileprivate let spacingDignityPreference:String = "Docuopen lock trade let valid"
fileprivate let displayChoiceError:String = "/Useour third"

/*: "Documents/User/record/" :*/
fileprivate let appScaleConfig:String = "Docucrush drop global poo originally"
fileprivate let kLotSession:[Character] = ["m","e","n","t","s","/","U","s","e","r","/","r","e","c","o","r","d","/"]

/*: "cancelled" :*/
fileprivate let modulePartMayUrl:String = "due"
fileprivate let screenLetterFormat:String = "andockelled"

/*: .wav" :*/
fileprivate let k_eticVisibleMetrics:String = "sou on note sum.wav"
...
typealias OneKeyGreetDownloadBlock = (_ model: ExpectSensitiveReactiveCompatible) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol VoiceOverTermEyebrowPencilManagerDelegate: NSObjectProtocol {
    /// 下载完成
    ...
    func profile(model: ExpectSensitiveReactiveCompatible)
    /// 过期语音消息，服务器缓存时间为7天
...
extension ThereTaskManager {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func reasonStream(_ tasksArray: [ExpectSensitiveReactiveCompatible]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.doinge()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(spacingDignityPreference.prefix(4)) + "ments" + String(displayChoiceError.prefix(4)) + "r/voice/")) {
                //: model.zipFilePath = "\(MedalReactiveCompatible.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(MedalReactiveCompatible.bundle())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(appScaleConfig.prefix(4)) + String(kLotSession))) {
                //: model.zipFilePath = "\(MedalReactiveCompatible.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(MedalReactiveCompatible.forget())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            CommunicationMsgTable.sprechgesangPath(msgId: model.taskId, status: "\(MedalBinaryMagnitude.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func sort(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = ExpectSensitiveReactiveCompatible.pickUpInSimulation(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? ExpectSensitiveReactiveCompatible()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(spacingDignityPreference.prefix(4)) + "ments" + String(displayChoiceError.prefix(4)) + "r/voice/")) {
            //: taskModel.zipFilePath = "\(MedalReactiveCompatible.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(MedalReactiveCompatible.bundle())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(appScaleConfig.prefix(4)) + String(kLotSession))) {
            //: taskModel.zipFilePath = "\(MedalReactiveCompatible.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(MedalReactiveCompatible.forget())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension ThereTaskManager: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive _: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = ExpectSensitiveReactiveCompatible.usPath(path: taskModel.zipFilePath)
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.profile(model: taskModel)
    }
    ...
    func urlSession(_: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (modulePartMayUrl.replacingOccurrences(of: "due", with: "c") + screenLetterFormat.replacingOccurrences(of: "dock", with: "c")) {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    errOpposite(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.birthWindow(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                errOpposite(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.instanceIt(url: task.taskDescription ?? "")
        }
...
extension ThereTaskManager {
    ...
    func errOpposite(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(MedalBinaryMagnitude.pause.rawValue)" : "\(MedalBinaryMagnitude.error.rawValue)"
            //: WCDBVoiceMsgTable.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            CommunicationMsgTable.sprechgesangPath(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.builderModel(model: taskModel)
            }
    ...
    func birthWindow(url: String) {
        ...
        case .chatRecord:
            ...
            CommunicationMsgTable.job(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.black(model: taskModel)
    ...
    func instanceIt(url: String) {
        ...
        let size = ExpectSensitiveReactiveCompatible.usPath(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            ...
            if taskModel.taskTypeId == .chatRecord {
                ...
                CommunicationMsgTable.sprechgesangPath(msgId: taskModel.taskId, status: "\(MedalBinaryMagnitude.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(k_eticVisibleMetrics.suffix(4)))
            ...
            self.bubble(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.naturalEventCommon(model: taskModel)
            ...
            case .chatRecord:
                ...
                CommunicationMsgTable.sprechgesangPath(msgId: taskModel.taskId, status: "\(MedalBinaryMagnitude.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.random(model: taskModel)
    ...
    func bubble(at path: String, withKeyId keyId: String, withTypeId typeId: TamHashable, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(k_eticVisibleMetrics.suffix(4)))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    if typeId == .chatRecord {
                        ...
                        CommunicationMsgTable.sprechgesangPath(msgId: keyId, filePath: newPath)
                    }
