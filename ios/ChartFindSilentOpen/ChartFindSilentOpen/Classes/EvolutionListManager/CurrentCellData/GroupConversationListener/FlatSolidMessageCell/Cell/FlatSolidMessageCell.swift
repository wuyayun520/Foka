...
fileprivate let styleTheoryHelper:String = "exposure scanTSys"
fileprivate let colorWhitePerformPage:String = "ageCfind demand jaw var"
fileprivate let moduleLittleReportAlert:String = "best scope bucketReuseId"

/*: "init(coder:) has not been implemented" :*/
fileprivate let coreWayId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_talk_notic" :*/
fileprivate let displayWomanCommercialText:[Character] = ["i","c","o","n","_","t","a","l","k"]
fileprivate let commonSatisfyEvent:String = "position gesture fine second_notic"
...
@objc public protocol ReachHarvest: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: FlatSolidMessageCell)
    func social(cell: FlatSolidMessageCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: FlatSolidMessageCell)
    func managerOwn(cell: FlatSolidMessageCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: FlatSolidMessageCell)
    func clickCell(cell: FlatSolidMessageCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: FlatSolidMessageCell)
    func cur(cell: FlatSolidMessageCell)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: FlatSolidMessageCell)
    func answerCell(cell: FlatSolidMessageCell)
}

//: @objcMembers public class FlatSolidMessageCell: TCommonTableViewCell {
@objcMembers public class FlatSolidMessageCell: ThinViewCell {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\ThereMessageReactiveCompatible.h
     */
    //: public var messageData: ThereMessageReactiveCompatible?
    public var messageData: ThereMessageReactiveCompatible?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(styleTheoryHelper.suffix(4)) + "temMess" + String(colorWhitePerformPage.prefix(4)) + "ell_" + String(moduleLittleReportAlert.suffix(7)))

    /**
     *  协议委托
     *  负责实现 ReachHarvest 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: ReachHarvest?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: coreWayId.reversed(), encoding: .utf8)!)
    }
    ...
    override public func minority(with data: ThatReactiveCompatible) {
        //: super.fill(with: data)
        super.minority(with: data)
        //: let newData: ThereMessageReactiveCompatible = data as! ThereMessageReactiveCompatible
        let newData: ThereMessageReactiveCompatible = data as! ThereMessageReactiveCompatible
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.skipTown()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(combineRecognizer(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.celluloidSize(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(house(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(birdSEye(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.titled(name: (String(displayWomanCommercialText) + String(commonSatisfyEvent.suffix(6))))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(minimum(recognizer:)))
...
//: extension FlatSolidMessageCell {
extension FlatSolidMessageCell {
    //: @objc func onSelectMessageAvatar(recognizer: UIGestureRecognizer) {
    @objc func combineRecognizer(recognizer _: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.cur(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func birdSEye(recognizer: UIGestureRecognizer) {
