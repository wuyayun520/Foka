...
/*: "#ffffff" :*/
fileprivate let sessionBoundKey:[Character] = ["#","f","f","f","f","f","f"]

/*: "/dist/loungePlus/index.html" :*/
fileprivate let commonRecordWanderApproveId:String = "cry female yellow which/dist/l"
fileprivate let spacingBlackStallMetrics:String = "OUN"
fileprivate let k_retainFormat:String = "pose pitch wire smoothgePlus"
fileprivate let widgetHuntSettings:String = "contend usex.html"

/*: "https://" :*/
fileprivate let themeOrientPreference:[Character] = ["h"]
fileprivate let appEquallyCanSystem:[Character] = ["t","t","p","s",":","/","/"]

/*: "http://" :*/
fileprivate let displayGameTickSession:[Character] = ["h","t","t","p",":","/","/"]
...
class SightReactiveCompatible: UIView {
    ...
    let Broadcast_TextColor = (String(sessionBoundKey))
    //: let Broadcast_ContentFont: UIFont = UIFont.pingfangMediumFont(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .celluloidSize(fontSize: 12)
    ...
    convenience init(model: CapHandyJSON?) {
        //: self.init()
        self.init()
    ...
    private func minScope() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            crushView(withUrl: danumuMsgModel.bgPic)
        }

        //: if danumuMsgModel.content.count > 0 {
        if danumuMsgModel.content.count > 0 {
            //: let parse = TUIChatParseTagHelper.init()
            let parse = ParseReactiveCompatible()
            ...
            var mutableAtt = parse.audienceLoad(withFontSize: 12, insert: "", atIndex: 0)
            // 限制字符长度
    ...
    func crushView(withUrl url: String) {
        ...
        self.backgroundImageView.pup(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.databaseQuality(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: CapHandyJSON? {
        //: didSet {
        didSet {
            //: configModel()
            minScope()
    ...
    var totalTime: CGFloat {
        ...
        return (moduleAdjustPath + widthValue) * Broadcast_SpendTime / moduleAdjustPath
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / moduleAdjustPath
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / moduleAdjustPath
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(computerSimulation(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension SightReactiveCompatible {
    ...
    @objc func computerSimulation(_: UITapGestureRecognizer) {
        ...
        case "1": // 个人中心
            ...
            ProdPushManager.share.untilBuild(uid: self.danumuMsgModel?.jumpKey)
        ...
        case "2": // 直播间
            ...
            if let uid = self.danumuMsgModel?.jumpKey {
                ...
                ProdPushManager.share.vidType(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(commonRecordWanderApproveId.suffix(7)) + spacingBlackStallMetrics.lowercased() + String(k_retainFormat.suffix(6)) + "/ind" + String(widgetHuntSettings.suffix(7)))) {
                    //: TalkingAppPushManager.share.func__pushToSubscribePageWebVC()
                    ProdPushManager.share.feather()
                    //: return
                    return
                }
                //: let httpsUrl = PositionThen.share.appConfigMode.urlH5Domain+url
                let httpsUrl = PositionThen.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (String(themeOrientPreference) + String(appEquallyCanSystem)), with: (String(displayGameTickSession)))
                //: TalkingAppPushManager.share.func__pushToWebVC(urlStr: httpUrl)
                ProdPushManager.share.rangeGalleryThirty(urlStr: httpUrl)
            }
...
extension SightReactiveCompatible {
    ...
    private func getCrackingIdentity() {
