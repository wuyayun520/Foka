...
/*: "MF:LiveChatWelMsg" :*/
fileprivate let screenSurePersonLogger:String = "m"
fileprivate let k_seaName:[Character] = ["F",":","L","i","v","e","C","h","a","t","W","e","l","M","s","g"]

/*: "MF:LiveChatPrizeMsg" :*/
fileprivate let serviceWillHelper:[Character] = ["M","F",":","L","i","v","e","C","h","a"]
fileprivate let colorYellowSession:[Character] = ["t","P"]
fileprivate let colorFitKey:[Character] = ["r","i","z","e","M","s","g"]

/*: "MF:LiveChatAttentionMsg" :*/
fileprivate let k_behindPath:String = "anima pretty putMF:L"
fileprivate let sessionDrawingUrl:String = "tAttenwritten arch"
fileprivate let spacingMinimizeUtility:String = "display"

/*: "#BAFBFF" :*/
fileprivate let colorCastAlert:String = "chart movie ingredient#BAFBF"
fileprivate let styleHurryHelper:[Character] = ["F"]

/*: "uid" :*/
fileprivate let displayUpperPath:String = "UID"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let moduleJoinKey:[Character] = ["<","a","t",">","@","[","\\","S","\\","s","]"]
fileprivate let colorBindEvent:[Character] = ["+","?","<","/","a","t",">"]

/*: "Sent " :*/
fileprivate let moduleThemeSession:String = "Sent street mutual aspect"

/*: "#FFA4CF" :*/
fileprivate let corePreparePreference:[Character] = ["#","F","F","A","4"]
fileprivate let styleStreetFormat:String = "abstractF"

/*: "Here comes " :*/
fileprivate let viewGatorMetrics:[Character] = ["H","e","r","e"," ","c"]
fileprivate let layoutVarietyData:[Character] = ["o","m","e","s"," "]

/*: "#FFE386" :*/
fileprivate let widgetPurpleMessage:[Character] = ["#","F","F","E","3","8","6"]

/*: "followed the anchor" :*/
fileprivate let coreBarrelFactorSettings:[Character] = ["f","o","l","l","o","w","e","d"," ","t","h","e"]
fileprivate let k_validPage:String = " anchorposition conversation cut"

/*: "level_ :*/
fileprivate let viewSunFormat:String = "level_app capacity form king warning"

/*: "icon_lounge" :*/
fileprivate let k_knownPresentationPage:String = "place external dynamic offericon_"
fileprivate let kThirtyYaSocialUtility:[Character] = ["l","o","u","n","g","e"]
...
class DecisionMakerThen: NSObject {
    ...
    func decennary(model: ClangoringHandyJSON) -> ClangoringHandyJSON {
        //: self.model = model
        self.model = model
        /// 普通文本消息,礼物消息
        ...
        } else if self.model.MsgExtension == (screenSurePersonLogger.uppercased() + String(k_seaName)) {
            //: caculateWelMsg()
            ain()
            /// 中奖消息
            //: } else if self.model.MsgExtension == "MF:LiveChatPrizeMsg" {
        } else if self.model.MsgExtension == (String(serviceWillHelper) + String(colorYellowSession) + String(colorFitKey)) {
        ...
        } else if self.model.MsgExtension == (String(k_behindPath.suffix(4)) + "iveCha" + String(sessionDrawingUrl.prefix(6)) + "tionMs" + spacingMinimizeUtility.replacingOccurrences(of: "display", with: "g")) {
            //: caculAttionMsg()
            grandstand()
        }
        //: return self.model
    ...
    func nonePic() {
        ...
        let nameAttr = draft(text: firstStr, font: UIFont.celluloidSize(fontSize: 14), color: UIColor(hex: (String(colorCastAlert.suffix(6)) + String(styleHurryHelper)))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
        ...
        if self.model.msgInfo?.messageType == 1 {
            ...
            let content = draft(text: self.model.msgInfo?.content ?? "", font: UIFont.celluloidSize(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            let atUid = jumps?[(displayUpperPath.lowercased())] as? Int ?? 0
            //: let text = self.model.msgInfo?.content ?? ""
            let text = self.model.msgInfo?.content ?? ""
            //: let regex = try! NSRegularExpression(pattern: "<at>@[\\S\\s]+?</at>", options: [])
            let regex = try! NSRegularExpression(pattern: (String(moduleJoinKey) + String(colorBindEvent)), options: [])
            ...
            if matchRange?.length ?? 0 > 9 {
                ...
                let subStr = String.angleFire(str: text, from: (matchRange?.location ?? 0) + 4, to: (matchRange?.length ?? 0) - 5)
                //: let contentStr = String.substringByUnicodeIndex(str: text, from: (matchRange?.length ?? 0), to: text.utf16.count )
                let contentStr = String.angleFire(str: text, from: (matchRange?.length ?? 0), to: text.utf16.count)
                ...
                let atName = draft(text: subStr, font: UIFont.celluloidSize(fontSize: 14), color: UIColor(hex: (String(colorCastAlert.suffix(6)) + String(styleHurryHelper)))!, isTap: true, atUid: "\(atUid)")
                ...
                let content = draft(text: contentStr, font: UIFont.celluloidSize(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            let sentAttr = draft(text: (String(moduleThemeSession.prefix(5))).localized, font: UIFont.celluloidSize(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            if let toUser = self.model.toUser { // 送礼接收人
                ...
                let nickNameAttr = draft(text: "\(toUser.nickname) ", font: UIFont.modelSize(type: .Medium, fontSize: 14), color: UIColor(hex: (String(corePreparePreference) + styleStreetFormat.replacingOccurrences(of: "abstract", with: "C")))!, isTap: true, atUid: String(toUser.uid))
                //: nameAttr.append(nickNameAttr)
                nameAttr.append(nickNameAttr)
            }
            //: let giftnameStr = changeGiftName(giftmodel: self.model.gift!)
            let giftnameStr = switcheBy(giftmodel: self.model.gift!)
            ...
            let giftAttr = draft(text: giftnameStr, font: UIFont.modelSize(type: .Medium, fontSize: 14), color: UIColor(hex: (String(corePreparePreference) + styleStreetFormat.replacingOccurrences(of: "abstract", with: "C")))!, isTap: false, atUid: nil)
            ...
            let imag = self.model.gift?.giftImg?.databaseQuality(with: size)
            ...
            let giftIcon = NSMutableAttributedString.yy_attachmentString(withContent: imag, contentMode: .center, attachmentSize: size, alignTo: UIFont.celluloidSize(fontSize: 14), alignment: .center)
            //: giftIcon.yy_paragraphStyle = paragraphStyle()
            giftIcon.yy_paragraphStyle = appear()
            ...
            let numAttr = self.draft(text: "*\(self.model.gift?.num ?? 0)", font: UIFont.celluloidSize(fontSize: 14), color: UIColor(hex: (String(corePreparePreference) + styleStreetFormat.replacingOccurrences(of: "abstract", with: "C")))!, isTap: false, atUid: nil)
        ...
        notPoint(attribText: self.model.msgAttribText)
    }

    /// 公告富文本
    //: func showAnnouncementMsg(model: TalkingLiveRoomDanmuModel) -> TalkingLiveRoomDanmuModel {
    func misdirect(model: ClangoringHandyJSON) -> ClangoringHandyJSON {
        //: self.model = model
        self.model = model
        // 内容
        //: let paraStyle = paragraphStyle()
        ...
        let content = draft(text: self.model.msgInfo?.content ?? "", font: UIFont.celluloidSize(fontSize: 14), color: UIColor(hex: (String(colorCastAlert.suffix(6)) + String(styleHurryHelper)))!, isTap: false, atUid: nil)
    ...
    func ain() {
        ...
        let strnor = (String(viewGatorMetrics) + String(layoutVarietyData)).localized
        ...
        let content = draft(text: strnor, font: UIFont.celluloidSize(fontSize: 14), color: UIColor(hex: (String(widgetPurpleMessage)))!, isTap: false, atUid: nil)
        ...
        let attachText = draft(text: strc, font: UIFont.celluloidSize(fontSize: 14), color: UIColor(hex: (String(colorCastAlert.suffix(6)) + String(styleHurryHelper)))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
    ...
    func signQuantityerval() {
        ...
        let content = draft(text: firstStr, font: UIFont.celluloidSize(fontSize: 14), color: UIColor.white, isTap: true, atUid: String(self.model.user?.uid ?? 0))
        //: textView.append(content)
        textView.append(content)
        //: self.model.msgAttribText = textView
        self.model.msgAttribText = textView
    ...
    func grandstand() {
        ...
        let strnor = (String(coreBarrelFactorSettings) + String(k_validPage.prefix(7))).localized
        //: let strc =  "\(self.model.user?.nickname ?? "") "
        let strc = "\(self.model.user?.nickname ?? "") "
        ...
        let content = draft(text: strnor, font: UIFont.celluloidSize(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
        ...
        let nameText = draft(text: strc, font: UIFont.celluloidSize(fontSize: 14), color: UIColor(hex: (String(colorCastAlert.suffix(6)) + String(styleHurryHelper)))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
    ...
    func appear() -> NSMutableParagraphStyle {
    ...
    func draft(text: String, font: UIFont, color: UIColor, isTap: Bool, atUid: String?) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = appear()
        ...
        if isTap == true {
            ...
            attribute.yy_setTextHighlight(range, color: color, backgroundColor: .clear, userInfo: nil) { _, _, _, _ in
                ...
                /// 不能@自己
                //: if atUid != PositionThen.share.loginUserMode.userID {
                if atUid != PositionThen.share.loginUserMode.userID {
    ...
    func notPoint(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = MsgTableViewWidth - 16
        let maxWidth = layoutPlayPreference - 16
    ...
    private func switcheBy(giftmodel: ExternalAnimatModel) -> String {
...
extension DecisionMakerThen {
    /// 获取弹幕富文本图片
    //: func getImagesAttributedString() -> NSMutableAttributedString {
    func dryMin() -> NSMutableAttributedString {
        ...
        levelImgV.image = UIImage.titled(name: (String(viewSunFormat.prefix(6))) + "\(self.model.user?.level ?? 1)")
        //: let levelAttr = getAttachText(imageView: levelImgV)
        let levelAttr = getToday(imageView: levelImgV)
        ...
        if self.model.user?.loungePlus == true {
            ...
            vipImgV.image = UIImage.titled(name: (String(k_knownPresentationPage.suffix(5)) + String(kThirtyYaSocialUtility)))
            //: let vipAttr = getAttachText(imageView: vipImgV)
            let vipAttr = getToday(imageView: vipImgV)
    ...
    func getToday(imageView: UIImageView) -> NSMutableAttributedString {
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView.image, contentMode: .scaleToFill, attachmentSize: imageView.size, alignTo: UIFont.attachSize(fontSize: 12), alignment: .center)
        //: attachText.yy_paragraphStyle = paragraphStyle()
        attachText.yy_paragraphStyle = appear()
        //: return attachText
        return attachText
    }
}
