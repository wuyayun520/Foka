...
fileprivate let colorPlainFormat:[Character] = ["N","e","e","d"," ","a"," ","g","i","f","t"," ","t","o"," ","u","n","l","o","c","k"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let styleContactUtility:[UInt8] = [0xa3,0xa4,0xa3,0xbe,0xe2,0xa9,0xa5,0xae,0xaf,0xb8,0xf0,0xe3,0xea,0xa2,0xab,0xb9,0xea,0xa4,0xa5,0xbe,0xea,0xa8,0xaf,0xaf,0xa4,0xea,0xa3,0xa7,0xba,0xa6,0xaf,0xa7,0xaf,0xa4,0xbe,0xaf,0xae]

/*: "intimatephoto_btn_no" :*/
fileprivate let serviceOrangeHighlightName:[Character] = ["i","n","t","i"]
fileprivate let commonAlreadyPlayerPath:[Character] = ["m","a","t","e","p","h","o","t","o","_","b","t","n","_","n","o"]

/*: "Burn After Reading" :*/
fileprivate let commonAbstractValue:[Character] = ["B","u","r","n"," ","A","f","t","e","r"," "]
fileprivate let styleAttributeMetrics:String = "dialog storage connect shape numberReading"

/*: "Use Photo" :*/
fileprivate let commonAssessmentName:String = "Use Photmodify dialog limited during stay"
fileprivate let componentLiveLogger:[Character] = ["o"]

/*: "Use Video" :*/
fileprivate let displayFormId:[Character] = ["U","s","e"," ","V","i","d"]
fileprivate let commonContributionError:[Character] = ["e","o"]

/*: "intimatephoto_btn_chose" :*/
fileprivate let featureManualDevice:[Character] = ["i","n","t","i","m","a","t","e","p","h","o","t","o"]
fileprivate let featureDenyPage:String = "_btnemotion column cooper track"

/*: "%@ (%@x1  %@ Gold coins)" :*/
fileprivate let themeHorribleValue:String = "%survivor (%survivor"
fileprivate let featurePecPath:[Character] = ["l","d"]
fileprivate let spacingOperationUrl:String = "portrait local coins)"

/*: "#7E6AFF" :*/
fileprivate let sessionConnectionDevice:String = "formal following selection shared closed#7E6AFF"
...
class SauceThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: styleContactUtility.map{$0^202}, encoding: .utf8)!)
    }
    ...
    private lazy var burnAfterReadingImgV: UIImageView = {
        ...
        let img = UIImageView(image: UIImage.titled(name: (String(serviceOrangeHighlightName) + String(commonAlreadyPlayerPath))))
    ...
    private lazy var burnAfterReadingLab: UILabel = {
        ...
        lab.text = (String(commonAbstractValue) + String(styleAttributeMetrics.suffix(7))).localized
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var burnAfterReadingBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(offceBefore), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var needGiftBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(result), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var useBtn: UIButton = {
        ...
        btn.setTitle(self.type == .intimatePhoto ? (String(commonAssessmentName.prefix(8)) + String(componentLiveLogger)).localized : (String(displayFormId) + String(commonContributionError)).localized, for: .normal)
...
extension SauceThen {
    ...
    @objc private func quantityerval() {
    ...
    @objc private func offceBefore() {
        ...
        let imageName = burnAfterReadingBtn.isSelected ? (String(featureManualDevice) + String(featureDenyPage.prefix(4)) + "_chose") : (String(serviceOrangeHighlightName) + String(commonAlreadyPlayerPath))
        //: burnAfterReadingImgV.image = UIImage.BundleImageNamed(name: imageName)
        burnAfterReadingImgV.image = UIImage.titled(name: imageName)
    }
}
...
extension SauceThen {
    ...
    @objc private func result() {
        ...
        guard needGiftBtn.isSelected == false else { // 还原状态
            ...
            needGiftImgeV.image = UIImage.titled(name: (String(serviceOrangeHighlightName) + String(commonAlreadyPlayerPath)))
        ...
        let type = (self.type == .intimatePhoto ? EnhanceCollection.intimatePhoto : EnhanceCollection.intimateVideo)
        //: TalkingChatGiftManager.share.func__sendGiftEvent(type: type) {
        RushReactiveCompatible.share.logClub(type: type) {
    ...
    private func lastView() {
        ...
        giftView.okActionBlock = { [weak self] giftModel in
            ...
            self.needGiftImgeV.image = UIImage.titled(name: (String(featureManualDevice) + String(featureDenyPage.prefix(4)) + "_chose"))
            ...
            let giftAttrStr = NSMutableAttributedString(string: (themeHorribleValue.replacingOccurrences(of: "survivor", with: "@") + "x1  %@ Go" + String(featurePecPath) + String(spacingOperationUrl.suffix(7))).meanwhile(self.normalGiftText, giftModel.name, giftModel.price))
            ...
            let attributes = [.foregroundColor: UIColor(hex: (String(sessionConnectionDevice.suffix(7))))!, .font: UIFont.modelSize(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
...
extension SauceThen {
    ...
    private func cinemaOrigin() {
        ...
        whiteBgV.addSubview(burnAfterReadingBtn)
        //: if PositionThen.share.loginUserMode.status != 1 &&
        if PositionThen.share.loginUserMode.status != 1,
    ...
    private func substitute() {
        ...
        useBtn.snp.makeConstraints { make in
            ...
            make.bottom.equalToSuperview().offset(-20 - featureDailyHelper)
