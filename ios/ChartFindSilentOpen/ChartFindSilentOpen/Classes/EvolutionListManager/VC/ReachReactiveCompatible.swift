...
fileprivate let themeTranslationSession:[UInt8] = [0xce,0xd3,0xce,0xd9,0x8d,0xc8,0xd4,0xc9,0xca,0xd7,0x9f,0x8e,0x85,0xcd,0xc6,0xd8,0x85,0xd3,0xd4,0xd9,0x85,0xc7,0xca,0xca,0xd3,0x85,0xce,0xd2,0xd5,0xd1,0xca,0xd2,0xca,0xd3,0xd9,0xca,0xc9]

fileprivate func inklingPeople(pad num: UInt8) -> UInt8 {
...
/*: "icon_photo_giftbg" :*/
fileprivate let viewBecomeChangeMessage:[Character] = ["i","c","o","n","_","p","h","o","t","o","_","g"]
fileprivate let moduleFormalUtility:String = "standftbg"

/*: "Send" :*/
fileprivate let sessionStateFormat:[Character] = ["S","e","n","d"]

/*: "nav_back_black_nor" :*/
fileprivate let screenPlanWaterMaximConfig:String = "highway visible until overnav_ba"
fileprivate let spacingPoContent:String = "arrow family text writck_b"
fileprivate let spacingPingMetrics:String = "ask pan_nor"

/*: "toUid" :*/
fileprivate let componentTellVisiblePage:String = "toUida want fragment"

/*: "giftId" :*/
fileprivate let componentAlreadyEvent:[Character] = ["g","i","f","t","I"]
fileprivate let widgetSomeoneConPage:[Character] = ["d"]

/*: "giftNum" :*/
fileprivate let sessionFamilyError:String = "giftNapproval week dot"
fileprivate let widgetCreatureValue:String = "lessm"

/*: "unlockMsgId" :*/
fileprivate let layoutFlagId:[Character] = ["u","n","l","o","c","k","M","s","g","I","d"]

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let commonExtraUtility:[UInt8] = [0x78,0x53,0x53,0x52,0x16,0x57,0x16,0x13,0x76,0x16,0x1e,0x13,0x76,0x16,0x71,0x59,0x5a,0x52,0x16,0x55,0x59,0x5f,0x58,0x45,0x1f,0x16,0x42,0x59,0x16,0x43,0x58,0x5a,0x59,0x55,0x5d]

private func meatLimit(tolerance num: UInt8) -> UInt8 {
    return num ^ 54
}

/*: "icon_photo_lock" :*/
fileprivate let kThenAlert:String = "icon_pdefine extension capture difference magnetic"
fileprivate let layoutBirthRandomDevice:String = "_lockbook presentation depress reward silver"

/*: "Click on the screen to view 15s" :*/
fileprivate let layoutBelowName:[UInt8] = [0x73,0x35,0x31,0x20,0x77,0x65,0x69,0x76,0x20,0x6f,0x74,0x20,0x6e,0x65,0x65,0x72,0x63,0x73,0x20,0x65,0x68,0x74,0x20,0x6e,0x6f,0x20,0x6b,0x63,0x69,0x6c,0x43]

/*: "icon_photo_burn" :*/
fileprivate let k_likeText:String = "icoaddition"
fileprivate let displayTwelvePreference:[Character] = ["_","b","u","r","n"]

/*: "Already burned" :*/
fileprivate let commonReadyPage:String = "brace meet rumen runAlrea"
fileprivate let spacingGreenNetworkValue:String = "RNED"
...
typealias IntimatePhotoBlock = (_ model: HighwayModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class ReachReactiveCompatible: GroupThen {
    ...
    //: private var imageTotalData = SinceFamousReactiveCompatible(direction: .MsgDirectionOutgoing)
    private var imageTotalData = SinceFamousReactiveCompatible(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = AbTalkingChatMsgIntimatePhotoModel()
    private var photoModel = HighwayModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: SinceFamousReactiveCompatible, toUid: String) {
    init(imageCellData: SinceFamousReactiveCompatible, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (imageTotalData.msgModel.user.uid == Int(PositionThen.share.loginUserMode.userID))
        isMySelf = (imageTotalData.msgModel.user.uid == Int(PositionThen.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: themeTranslationSession.map{inklingPeople(pad: $0)}, encoding: .utf8)!)
    }
    ...
    override func loadView() {
        ...
        if #available(iOS 13.2, *) {
            ...
            if PositionThen.share.appConfigMode.disableShootScreen {
                ...
                let bgView = ShieldView(frame: CGRect(x: 0, y: 0, width: moduleAdjustPath, height: componentMeEvent - sessionTableUrl))
                //: self.view = bgView
                self.view = bgView
            }
            //: } else {
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.titled(name: (String(viewBecomeChangeMessage) + moduleFormalUtility.replacingOccurrences(of: "stand", with: "i")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.imitation(urlStr: photoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var desLab: UILabel = {
        ...
        lab.font = UIFont.celluloidSize(fontSize: 19)
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(sessionStateFormat)).localized, for: .normal)
        //: btn.setTitleColor(.white, for: .normal)
        btn.setTitleColor(.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Medium, fontSize: 19)
        btn.titleLabel?.font = UIFont.modelSize(type: .Medium, fontSize: 19)
        //: btn.setBackgroundImage(UIImage.imageGradientColor(colors: UIColor.appGradientColor(), size: CGSize(width: 220, height: 44)), for: .normal)
        btn.setBackgroundImage(UIImage.totalelMention(colors: UIColor.motion(), size: CGSize(width: 220, height: 44)), for: .normal)
        //: btn.layer.cornerRadius = 22
        btn.layer.cornerRadius = 22
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.addTarget(self, action: #selector(giftSendButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(harshStage), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var progressView: ScriptView = {
        //: let frame = CGRect(x: ScreenWidth-42-15, y: StatusBarHeight+10, width: 42, height: 42)
        let frame = CGRect(x: moduleAdjustPath - 42 - 15, y: appSceneUrl + 10, width: 42, height: 42)
        //: let v = CircleCountdownView(frame: frame)
        let v = ScriptView(frame: frame)
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.titled(name: (String(screenPlanWaterMaximConfig.suffix(6)) + String(spacingPoContent.suffix(4)) + "lack" + String(spacingPingMetrics.suffix(4)))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
        //: btn.addTarget(self, action: #selector(backButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(premature), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ReachReactiveCompatible {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func psychologicalFeature(giftId: String, giftNum: String) {
        ...
        params[(String(componentTellVisiblePage.prefix(5)))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(componentAlreadyEvent) + String(widgetSomeoneConPage))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(sessionFamilyError.prefix(5)) + widgetCreatureValue.replacingOccurrences(of: "less", with: "u"))] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(String(layoutFlagId))] = self.imageTotalData.msgModel.msgInfo.msgId

        // 送礼接口
        //: TalkingChatRequestTool.req_SendGiftPrivete(param: params, completion: { succeed, result, errorModel in
        ScriptRequestTool.afterGift(param: params, completion: { succeed, result, errorModel in
            //: if succeed {
...
extension ReachReactiveCompatible {
    ...
    @objc private func harshStage() {
        //: req_sendGiftMsg(giftId: photoModel.giftId, giftNum: "1")
        psychologicalFeature(giftId: photoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnPhotoTapGesture() {
    @objc private func bound() {
        //: guard PositionThen.share.networkStatus != .Unavailable else {
        guard PositionThen.share.networkStatus != .Unavailable else {
            //: func__showStatusBarErrorMsg(showMsg: kNetErrorMsg)
            colouration(showMsg: serviceDetailFoundValue)
            //: return
        ...
        WeatherMapThen.sinceRisky(superView: self.view)
        //: self.loadIntimatePhoto(urlStr: photoModel.originUrl, isMosaic: false) {[weak self] isSucceed in
        self.genderTemp(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            //: ProgressHUD.dismiss()
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.thirdTime(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension ReachReactiveCompatible {
    ...
    private func statFor() {
        ...
        desLab.snp.makeConstraints { make in
            ...
            make.centerY.equalToSuperview().offset(sessionTableUrl / 2)
        }
        ...
        progressView.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(moduleAdjustPath - 42 - 15)
    ...
    private func noseUi() {
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: commonExtraUtility.map{meatLimit(tolerance: $0)}, encoding: .utf8)!.meanwhile(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.titled(name: (String(kThenAlert.prefix(6)) + "hoto" + String(layoutBirthRandomDevice.prefix(5))))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: layoutBelowName.reversed(), encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnPhotoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(bound))
                ...
                self.intimateStatusImgV.image = UIImage.titled(name: (k_likeText.replacingOccurrences(of: "addition", with: "n") + "_photo" + String(displayTwelvePreference)))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(commonReadyPage.suffix(5)) + "dy bu" + spacingGreenNetworkValue.lowercased()).localized
            }
        ...
        self.genderTemp(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func genderTemp(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
        ...
        guard isMySelf == false else { // 本人发的图片，展示本地缓存资源
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.imageView.image = UIImage.fragment(image, putUp: 90)
                self.imageView.image = UIImage.fragment(image, putUp: 90)
                //: } else {
            } else {
        ...
        imageView.rereadMorro(urlStr: urlStr) { [weak self] image in
            //: guard let self = self else { return }
            guard let self = self else { return }
