...
fileprivate let widgetBorderPopularSettings:[UInt8] = [0x57,0x50,0x57,0x4a,0x16,0x5d,0x51,0x5a,0x5b,0x4c,0x4,0x17,0x1e,0x56,0x5f,0x4d,0x1e,0x50,0x51,0x4a,0x1e,0x5c,0x5b,0x5b,0x50,0x1e,0x57,0x53,0x4e,0x52,0x5b,0x53,0x5b,0x50,0x4a,0x5b,0x5a]

/*: "icon_photo_giftbg" :*/
fileprivate let appRetailerKitKey:String = "icon_powner less icon detailed prompt"
fileprivate let spacingInfoFormat:String = "hoto_gboth name deliver fire outer"

/*: "Send" :*/
fileprivate let spacingSouthwestUtility:String = "pro signature scanSend"

/*: "nav_back_black_nor" :*/
fileprivate let kTotLogger:[Character] = ["n","a","v","_","b","a","c","k","_","b","l","a"]
fileprivate let styleAcrossLogger:String = "target"
fileprivate let layoutRemUtility:String = "k_norfloat trip"

/*: "toUid" :*/
fileprivate let featureExhibitSession:String = "some across structure method mentiontoUid"

/*: "giftId" :*/
fileprivate let spacingYourselfMetrics:String = "permission requiregiftId"

/*: "giftNum" :*/
fileprivate let kCarrierPage:String = "manage tot forget clothes gogiftNum"

/*: "unlockMsgId" :*/
fileprivate let serviceClearlySession:String = "landnlo"
fileprivate let commonIonSettings:String = "start attractive found corner maskId"

/*: "Load failed, return to retry" :*/
fileprivate let k_shText:String = "Load fsuggest favor possible worker source"
fileprivate let themeTabError:String = "look stop mirror beginailed,"
fileprivate let viewTicFullVideoSession:String = "n totone ignore explain save"

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let layoutShakeValue:[UInt8] = [0x29,0x2,0x2,0x3,0x47,0x6,0x47,0x42,0x27,0x47,0x4f,0x42,0x27,0x47,0x20,0x8,0xb,0x3,0x47,0x4,0x8,0xe,0x9,0x14,0x4e,0x47,0x13,0x8,0x47,0x12,0x9,0xb,0x8,0x4,0xc]

/*: "icon_photo_lock" :*/
fileprivate let themeEndLogger:String = "icon_change normal"
fileprivate let serviceLightMessage:String = "_locklaunch miss bond"

/*: "Click on the screen to play this video(Burn After Reading)" :*/
fileprivate let layoutAnonymousTimer:[UInt8] = [0xc4,0xed,0xea,0xe4,0xec,0xa1,0xf0,0xef,0xa1,0xf5,0xe9,0xe6,0xa1,0xf4,0xe4,0xf3,0xe6,0xe6,0xef,0xa1,0xf5,0xf0,0xa1,0xf1,0xed,0xe2,0xfa,0xa1,0xf5,0xe9,0xea,0xf4,0xa1,0xf7,0xea,0xe5,0xe6,0xf0,0xa9,0xc3,0xf6,0xf3,0xef,0xa1,0xc2,0xe7,0xf5,0xe6,0xf3,0xa1,0xd3,0xe6,0xe2,0xe5,0xea,0xef,0xe8,0xaa]

fileprivate func backgroundDock(still num: UInt8) -> UInt8 {
...
/*: "icon_photo_burn" :*/
fileprivate let coreStrongName:String = "icon_phtrip slim tolerance"
fileprivate let moduleCanError:[Character] = ["o","t","o","_","b","u","r","n"]

/*: "Already burned" :*/
fileprivate let displayFoodPlatform:String = "Alreadypretty certain lay"
fileprivate let k_rawError:String = " burnedbird a arch together project"
...
// __CLOSE_PRINT__
//
//  MakeupTermRecognizerDelegate.swift
//  AbroadTalking
//
...
typealias IntimateVideoBlock = (_ model: ResignModel) -> Void
//: typealias VideoStatusBlock = (_ status: VideoPlayerStatus) -> Void
typealias VideoStatusBlock = (_ status: ThinPlayerStatus) -> Void

//: class TalkingIntimateVideoPreviewVC: TalkingBaseViewController {
class MakeupTermRecognizerDelegate: GroupThen {
    ...
    //: private var videoTotalData = RationalCellData(direction: .MsgDirectionIncoming)
    private var videoTotalData = RationalCellData(direction: .MsgDirectionIncoming) // 视频消息数据
    //: private var videoModel = AbTalkingChatMsgVideoModel()
    private var videoModel = ResignModel() // 当前私密视频模型
    ...
    ///   - videoCellData: 视频模型
    ///   - toUid: 对方Id
    //: init(videoCellData: RationalCellData, toUid: String) {
    init(videoCellData: RationalCellData, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (videoTotalData.msgModel.user.uid == Int(PositionThen.share.loginUserMode.userID))
        isMySelf = (videoTotalData.msgModel.user.uid == Int(PositionThen.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: widgetBorderPopularSettings.map{$0^62}, encoding: .utf8)!)
    }
    ...
    private lazy var videoProgressView: PushQuantityeractionView = {
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.titled(name: (String(appRetailerKitKey.prefix(6)) + String(spacingInfoFormat.prefix(6)) + "iftbg"))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.imitation(urlStr: videoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(spacingSouthwestUtility.suffix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(channel), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.titled(name: (String(kTotLogger) + styleAcrossLogger.replacingOccurrences(of: "target", with: "c") + String(layoutRemUtility.prefix(5)))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
        //: btn.addTarget(self, action: #selector(backButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(prepareList), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension MakeupTermRecognizerDelegate {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func answerTalk(giftId: String, giftNum: String) {
        ...
        params[(String(featureExhibitSession.suffix(5)))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(spacingYourselfMetrics.suffix(6)))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(kCarrierPage.suffix(7)))] = giftNum
        //: params["unlockMsgId"] = self.videoTotalData.msgModel.msgInfo.msgId
        params[(serviceClearlySession.replacingOccurrences(of: "land", with: "u") + "ckMsg" + String(commonIonSettings.suffix(2)))] = self.videoTotalData.msgModel.msgInfo.msgId
...
extension MakeupTermRecognizerDelegate {
    ...
    @objc private func channel() {
        //: req_sendGiftMsg(giftId: videoModel.giftId, giftNum: "1")
        answerTalk(giftId: videoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnVideoTapGesture() {
    @objc private func liveSet() {
        ...
        self.videoStatusBlock = { [weak self] status in
            ...
            guard status == .Playing else {
                ...
                if status == .Failed {
                    ...
                    self.colouration(showMsg: (String(k_shText.prefix(6)) + String(themeTabError.suffix(6)) + " retur" + String(viewTicFullVideoSession.prefix(4)) + " retry").localized)
                }
                //: return
                return
            }
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.thirdTime(totalTime: 600, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension MakeupTermRecognizerDelegate {
    ...
    @objc private func familyGetWeavingObserver() {
...
extension MakeupTermRecognizerDelegate {
    ...
    private func opIn(urlStr: String, isMosaic: Bool) {
    ...
    private func directionFocus() {
        ...
        self.player.recognizeView(url: videoPath, view: self.videoView)
    }
}

// MARK: - RecallReactiveCompatible, GiveAndTakeThen

//: extension TalkingIntimateVideoPreviewVC: TalkingVideoPlayerDelegate, InteractionViewDelegate {
extension MakeupTermRecognizerDelegate: RecallReactiveCompatible, GiveAndTakeThen {
    ...
    func aboutSourceStatus(player _: EarnThen, status: ThinPlayerStatus) {
        ...
        self.videoProgressView.feedStatus(status: status)
        //: if status == .Playing {
        if status == .Playing {
    ...
    func example(player _: EarnThen, duration: Int, currentTime: Int) {
        ...
        self.videoProgressView.invite(duration: duration, currentTime: currentTime)
    }

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat) {
    func live(player _: EarnThen, progress: CGFloat) {
        //: self.videoProgressView.updateProgress(value: progress)
        self.videoProgressView.flushTo(value: progress)
    }

    // MARK: - GiveAndTakeThen
...
extension MakeupTermRecognizerDelegate {
    ...
    private func usLoad() {
        ...
        videoProgressView.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(sessionTableUrl + 30)
            //: make.bottom.equalToSuperview().offset(-(kDeviceSafeBottomHeight+64))
            make.bottom.equalToSuperview().offset(-(featureDailyHelper + 64))
        }
    ...
    private func fillChoice() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(familyGetWeavingObserver),
    ...
    private func enableicialUi() {
        ...
        if videoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.opIn(urlStr: videoModel.coverImg, isMosaic: true)
            ...
            self.videoProgressView.invite(duration: videoModel.duration, currentTime: 0)
            //: self.maskView.isHidden = false
            self.maskView.isHidden = false
            //: self.desLab.text = "Need a %@ (%@ Gold coins) to unlock".localizedArguments(videoModel.giftName, videoModel.giftPrice)
            self.desLab.text = String(bytes: layoutShakeValue.map{$0^103}, encoding: .utf8)!.meanwhile(videoModel.giftName, videoModel.giftPrice)
            ...
            if videoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.titled(name: (String(themeEndLogger.prefix(5)) + "photo" + String(serviceLightMessage.prefix(5))))
                //: self.desLab.text = "Click on the screen to play this video(Burn After Reading)".localized
                self.desLab.text = String(bytes: layoutAnonymousTimer.map{backgroundDock(still: $0)}, encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnVideoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(liveSet))
                ...
                self.intimateStatusImgV.image = UIImage.titled(name: (String(coreStrongName.prefix(7)) + String(moduleCanError)))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(displayFoodPlatform.prefix(7)) + String(k_rawError.prefix(7))).localized
            }
