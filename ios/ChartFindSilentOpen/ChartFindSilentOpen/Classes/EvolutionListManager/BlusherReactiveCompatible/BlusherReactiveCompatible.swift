...
fileprivate let kSortPlatform:String = "\u{8bbe}置普通状态"
fileprivate let kPlatId:String = "时 请\u{4f7f}\u{7528}R"
fileprivate let coreAdditionalPage:[Character] = ["G","B"]
fileprivate let sessionPicturePage:[Character] = ["空","间","\u{7684}","颜","色","值"]

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let widgetRemCurrentMetrics:[Character] = ["设","置","选","\u{4e2d}","状","态","的"]
fileprivate let commonGameMetrics:[Character] = ["\u{6587}","\u{5b57}","颜","\u{8272}","时"," ","请"]
fileprivate let screenEveryoneData:String = "使\u{7528}RGB空"

/*: "init(coder:) has not been implemented" :*/
fileprivate let appSupportSolarTunTitle:[UInt8] = [0x9a,0x9d,0x9a,0x87,0xdb,0x90,0x9c,0x97,0x96,0x81,0xc9,0xda,0xd3,0x9b,0x92,0x80,0xd3,0x9d,0x9c,0x87,0xd3,0x91,0x96,0x96,0x9d,0xd3,0x9a,0x9e,0x83,0x9f,0x96,0x9e,0x96,0x9d,0x87,0x96,0x97]

private func maleArgument(manual num: UInt8) -> UInt8 {
    return num ^ 243
}

/*: "设置的下标不合法!!" :*/
fileprivate let widgetIncreaseSettings:[Character] = ["设","置","的","\u{4e0b}","标","不"]
fileprivate let styleRankSession:[Character] = ["\u{5408}","法","!","!"]

/*: "Bags" :*/
fileprivate let spacingReloadlyRejectLogger:[Character] = ["B","a","g","s"]
...
// __CLOSE_PRINT__
//
//  BlusherReactiveCompatible.swift
//  ZEScollViewPage
//
...
open class BlusherReactiveCompatible: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: PingSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.turn(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((kSortPlatform + "\u{7684}文字\u{989c}色" + kPlatId + String(coreAdditionalPage) + String(sessionPicturePage)))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.turn(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(widgetRemCurrentMetrics) + String(commonGameMetrics) + screenEveryoneData + "间的\u{989c}色值"))
        }
    ...
    fileprivate func turn(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: PingSegmentStyle, titles: [String]) {
    ...
    @objc func disable(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? ExternalView else { return }
        //: currentIndex = currentLabel.tag
        currentIndex = currentLabel.tag
        //: print(currentLabel.tag)
        //: adjustUIWhenBtnOnClickWithAnimate(true)
        adjustFromEnableWork(true)
    }

    //: required public init?(coder aDecoder: NSCoder) {
    public required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: appSupportSolarTunTitle.map{maleArgument(manual: $0)}, encoding: .utf8)!)
    }
}
...
public extension BlusherReactiveCompatible {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func quantityervalCon(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (String(widgetIncreaseSettings) + String(styleRankSession)))
        ...
        adjustFromEnableWork(animated)
    }

    // 暴露给外界来刷新标题的显示
    //: public func reloadTitlesWithNewTitles(_ titles: [String]) {
    func versionForNames(_ titles: [String]) {
        // 移除所有的scrollView子视图
        ...
        quantityervalCon(0, animated: true)
    }
}
...
//: extension ScrollSegmentView {
extension BlusherReactiveCompatible {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func inmate() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = ExternalView(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.disable(_:)))
            ...
            if title == (String(spacingReloadlyRejectLogger)).localized {
    ...
    fileprivate func shape() {
        ...
        if let firstLabel = labelsArray[0] as? ExternalView {
    ...
    public func shitworkWeight(_ currentIndex: Int) {
...
open class ExternalView: UILabel {
    /// 用来记录当前label的缩放比例
