...
// __CLOSE_PRINT__
//
//  TamFamousControllerDelegate.swift
//  AbroadTalking
//
...
class TamFamousControllerDelegate: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: SignatureHashableRepresentation?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.hemAndHaw()
            }
        }
    ...
    func popularBubbleToo() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.primaryColour(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.layerColor()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.restoreAudience(), NSAttributedString.Key.font: UIFont.modelSize(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.primaryColour(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.restoreAudience(), NSAttributedString.Key.font: UIFont.modelSize(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.restoreAudience()
...
extension TamFamousControllerDelegate: UINavigationControllerDelegate {
    ...
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        ...
        if (viewController.hidesBottomBarWhenPushed) {
            ...
            BillThen.shared.advSuperView?.isHidden = true
        //: } else {
        } else {
...
extension TamFamousControllerDelegate: UIGestureRecognizerDelegate {
