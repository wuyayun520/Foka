...
fileprivate let sessionMovieConfig:[UInt8] = [0x7b,0x7c,0x7b,0x66,0x3a,0x71,0x7d,0x76,0x77,0x60,0x28,0x3b,0x32,0x7a,0x73,0x61,0x32,0x7c,0x7d,0x66,0x32,0x70,0x77,0x77,0x7c,0x32,0x7b,0x7f,0x62,0x7e,0x77,0x7f,0x77,0x7c,0x66,0x77,0x76]

private func thenThem(read num: UInt8) -> UInt8 {
    return num ^ 18
}

/*: "btn_live_toast_bg" :*/
fileprivate let coreDirectHeadSettings:String = "btn_lfail chain southwest who appear"
fileprivate let moduleThresholdKey:String = "ast_bgappear town"

/*: "btn_live_toast_arrow" :*/
fileprivate let widgetPackageRetPlatform:String = "show wait tone strongbtn_li"
fileprivate let commonEstimatedBurnScaleData:String = "arrfrom"
...
var featureEnableTitleTimer = false

//: class TalkingLiveTabToast: UIView {
class RecallTabToast: UIView {
    // 气泡间距
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: sessionMovieConfig.map{thenThem(read: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bubbleImgView: UIImageView = {
        ...
        var img = UIImage.titled(name: (String(coreDirectHeadSettings.prefix(5)) + "ive_to" + String(moduleThresholdKey.prefix(6)))).resizableImage(withCapInsets: bubbleInsets, resizingMode: .stretch)
        //: imgV.image = img
        imgV.image = img
        //: return imgV
        return imgV
    ...
    private lazy var arrowImgView: UIImageView = {
        ...
        imgV.image = UIImage.titled(name: (String(widgetPackageRetPlatform.suffix(6)) + "ve_toast_" + commonEstimatedBurnScaleData.replacingOccurrences(of: "from", with: "ow")))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension RecallTabToast {
    /// 展示视图，5s后自动移除
    //: func show(superView: UIView) {
    func resultThirdView(superView: UIView) {
        //: guard LIVETABTOASTISSHOW == false else { return }
        guard featureEnableTitleTimer == false else { return }
        ...
        //: guard PositionThen.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        guard PositionThen.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        //: LIVETABTOASTISSHOW = true
        featureEnableTitleTimer = true

        //: textLab.text = PositionThen.share.appUserConfigMode.liveTabTips
        textLab.text = PositionThen.share.appUserConfigMode.liveTabTips
        //: let maxWidth = ScreenWidth - 48
        let maxWidth = moduleAdjustPath - 48
