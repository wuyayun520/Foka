...
fileprivate let coreFoodPreference:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
// __CLOSE_PRINT__
//
//  OriginalKingViewController.swift
//  AbroadTalking
//
...
class OriginalKingViewController: GroupThen {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: ResumeTransitioningDelegate!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = ResumeTransitioningDelegate(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: coreFoodPreference.reversed(), encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class ResumeTransitioningDelegate: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: moduleAdjustPath, height: componentMeEvent * 0.6)
    ...
    func birthShadow(corners: UIRectCorner, radius: CGFloat) {
...
extension ResumeTransitioningDelegate: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = SaucePresentationController(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension ResumeTransitioningDelegate: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            substantiate(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            popularContext(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func substantiate(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func popularContext(transitionContext: UIViewControllerContextTransitioning) {
...
class SaucePresentationController: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: ResumeTransitioningDelegate!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: ResumeTransitioningDelegate, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func cutRunning() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(personSample))
        //: tap.delegate = self
        tap.delegate = self
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: moduleAdjustPath, height: componentMeEvent)
