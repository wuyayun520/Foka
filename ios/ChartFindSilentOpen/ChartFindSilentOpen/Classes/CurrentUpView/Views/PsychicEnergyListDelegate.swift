...
fileprivate let appMeetingPurpleId:[UInt8] = [0xf4,0xf3,0xf4,0xe9,0xb5,0xfe,0xf2,0xf9,0xf8,0xef,0xa7,0xb4,0xbd,0xf5,0xfc,0xee,0xbd,0xf3,0xf2,0xe9,0xbd,0xff,0xf8,0xf8,0xf3,0xbd,0xf4,0xf0,0xed,0xf1,0xf8,0xf0,0xf8,0xf3,0xe9,0xf8,0xf9]

private func securityStaff(breath num: UInt8) -> UInt8 {
    return num ^ 157
}

/*: "Not yet~" :*/
fileprivate let commonTranslationError:String = "calendar dialog party anti renderNot yet~"

/*: "#8C7AFF" :*/
fileprivate let colorToneCropTitle:[Character] = ["#","8","C","7","A","F","F"]
...
// __CLOSE_PRINT__
//
//  PsychicEnergyListDelegate.swift
//  AbroadTalking
//
...
class PsychicEnergyListDelegate: UIView {
    //: var type = 0
    var type = 0
    ...
    override init(frame: CGRect) {
        ...
        super.init(frame: frame)
        //: self.setupSubviews()
        self.carteCounterest()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: appMeetingPurpleId.map{securityStaff(breath: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var dataArray: [SatellitePingTransformable] = [] {
        //: didSet {
        didSet {
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(WorkplaceThen.self, forCellReuseIdentifier: WorkplaceThen.className())
        //: $0.register(SocialRankListViewCell.self, forCellReuseIdentifier: SocialRankListViewCell.className())
        $0.register(SumermediationThen.self, forCellReuseIdentifier: SumermediationThen.className())
    }
    ...
    lazy var tableHeaderView = HeaderThen(frame: CGRect(x: 0, y: 0, width: moduleAdjustPath, height: actualHeight(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = EmptyFluReactiveCompatible().then {
        //: $0.backgroundColor = .white
        $0.backgroundColor = .white
        //: $0.desLab.text = "Not yet~".localized
        $0.desLab.text = (String(commonTranslationError.suffix(8))).localized
        //: $0.desLab.textColor = UIColor(hex: "#8C7AFF")
        $0.desLab.textColor = UIColor(hex: (String(colorToneCropTitle)))
...
extension PsychicEnergyListDelegate: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: WorkplaceThen.className(), for: indexPath) as! WorkplaceThen
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: SumermediationThen.className(), for: indexPath) as! SumermediationThen
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.ignoreAcross(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.install(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: WorkplaceThen.className()) as! WorkplaceThen
...
extension PsychicEnergyListDelegate: JXSegmentedListContainerViewListDelegate {
...
extension PsychicEnergyListDelegate {
    //: private func setupSubviews() {
    private func carteCounterest() {
