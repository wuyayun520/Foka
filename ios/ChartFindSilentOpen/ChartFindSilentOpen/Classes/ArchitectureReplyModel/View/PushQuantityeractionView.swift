...
fileprivate let commonFailureIngredientMetrics:[UInt8] = [0xec,0xeb,0xec,0xf1,0xad,0xe6,0xea,0xe1,0xe0,0xf7,0xbf,0xac,0xa5,0xed,0xe4,0xf6,0xa5,0xeb,0xea,0xf1,0xa5,0xe7,0xe0,0xe0,0xeb,0xa5,0xec,0xe8,0xf5,0xe9,0xe0,0xe8,0xe0,0xeb,0xf1,0xe0,0xe1]

/*: "btn_dynamic_stop_nor" :*/
fileprivate let widgetSomeoneUtility:[Character] = ["b","t","n","_","d","y","n","a","m"]
fileprivate let widgetProjectData:String = "irent"
fileprivate let screenShowMessage:String = "_stosnap enjoy position connect past"

/*: "btnwOblEh_tdj_oediv_stop" :*/
fileprivate let componentLicensePreference:[Character] = ["b","t","n","w","O","b","l","E","h","_","t","d","j","_","o","e","d","i","v"]
fileprivate let spacingRemContent:[Character] = ["_","s","t","o","p"]

/*: "btnCmjoS7_tdj_oediv_play" :*/
fileprivate let layoutDitSpotKey:String = "bag mustbtnCm"
fileprivate let k_timeName:String = "laboratory mark contend injure towardj_oed"
...
// __CLOSE_PRINT__
//
//  PushQuantityeractionView.swift
//  AbroadTalking
//
...
protocol GiveAndTakeThen: NSObject {
    //: func func__interactionViewStatus(view: TalkingVideoPlayerInteractionView, status: InteractionViewStatus)
    func tillTing(view: PushQuantityeractionView, status: EveryViewStatus)

    //: func func__interactionViewSlideValue(view: TalkingVideoPlayerInteractionView, value: CGFloat)
    func choice(view: PushQuantityeractionView, value: CGFloat)
}

//: class TalkingVideoPlayerInteractionView: UIView {
class PushQuantityeractionView: UIView {
    //: open weak var delegate: InteractionViewDelegate?
    open weak var delegate: GiveAndTakeThen?
    ...
    override init(frame: CGRect) {
        ...
        self.stopQuantityeraction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: commonFailureIngredientMetrics.map{$0^133}, encoding: .utf8)!)
    }
    ...
    private lazy var centerPlayButton: UIButton = {
        ...
        btn.setImage(UIImage.titled(name: (String(widgetSomeoneUtility) + widgetProjectData.replacingOccurrences(of: "rent", with: "c") + String(screenShowMessage.prefix(4)) + "p_nor")), for: .normal)
        //: btn.addTarget(self, action: #selector(centerButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(notice(sender:)), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var playButton: UIButton = {
        ...
        btn.setImage(UIImage.titled(name: (String(componentLicensePreference) + String(spacingRemContent))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btnCmjoS7_tdj_oediv_play"), for: .selected)
        btn.setImage(UIImage.titled(name: (String(layoutDitSpotKey.suffix(5)) + "joS7_td" + String(k_timeName.suffix(5)) + "iv_play")), for: .selected)
        //: btn.addTarget(self, action: #selector(playButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(nearCharmSender(sender:)), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var failImageView: UIImageView = {
        ...
        imag.image = UIImage.titled(name: "")
        //: imag.isHidden = true
        imag.isHidden = true
    ...
    private lazy var slider: FamousResumePlayerSlider = {
        ...
        let slid = FamousResumePlayerSlider(frame: CGRect(x: 100, y: 0, width: moduleAdjustPath - 167, height: 40), sliderWidth: 12, sliderColor: UIColor.white, progressHeight: 2, progressBgColor: UIColor(red: 255 / 255.0, green: 255 / 255.0, blue: 255 / 255.0, alpha: 0.3), progressPlayedColor: UIColor.white, progressCachedColor: UIColor.clear, isShowCorner: true)
...
extension PushQuantityeractionView {
    ...
    private func stopQuantityeraction() {
        ...
        self.slider.getValueBlock = { [weak self] value in
            ...
            self.delegate?.choice(view: self, value: value)
        }
        ...
        self.slider.panEndHandler = { [weak self] _ in
            ...
            self.unconscious(status: .StopSeek)
        }
    }
}
