...
fileprivate let viewLotMetrics:[UInt8] = [0x64,0x63,0x64,0x79,0x25,0x6e,0x62,0x69,0x68,0x7f,0x37,0x24,0x2d,0x65,0x6c,0x7e,0x2d,0x63,0x62,0x79,0x2d,0x6f,0x68,0x68,0x63,0x2d,0x64,0x60,0x7d,0x61,0x68,0x60,0x68,0x63,0x79,0x68,0x69]

/*: "#4FAAFF" :*/
fileprivate let colorAutomaticallyData:String = "cap hem population ample range#4FAAF"
fileprivate let k_residentTimer:String = "modify"

/*: "reply" :*/
fileprivate let moduleCancelValue:[Character] = ["r","e","p","l","y"]

/*: "F5F5F5" :*/
fileprivate let moduleFormalConfig:String = "editor"
fileprivate let appPackageUrl:[Character] = ["5","F","5","F","5"]
...
class DisableFailView: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: viewLotMetrics.map{$0^13}, encoding: .utf8)!)
    }
...
extension DisableFailView {
    //: @objc func onLongTapCell(sender: UIGestureRecognizer) {
    @objc func weightTurnPost(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension DisableFailView {
    //: func configCell(model: TalkingCommentReplyModel) {
    func modelAccess(model: ArchitectureReplyModel) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(colorAutomaticallyData.suffix(6)) + k_residentTimer.replacingOccurrences(of: "modify", with: "F")))
            ...
            let font = UIFont.attachSize(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (String(moduleCancelValue)).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.appValueDetailColor(), UIColor.appTitleColor(), UIColor.appValueDetailColor(), UIColor.appTitleColor()]
                colorA = [UIColor.vary(), UIColor.restoreAudience(), UIColor.vary(), UIColor.restoreAudience()]
                ...
                colorA = [UIColor.vary(), UIColor.restoreAudience()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func modifyHeight(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height _: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: moduleAdjustPath - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (moduleFormalConfig.replacingOccurrences(of: "editor", with: "F") + String(appPackageUrl)))?.cgColor
...
extension DisableFailView {
    ...
    private func setupInsideFirst() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(weightTurnPost(sender:)))
