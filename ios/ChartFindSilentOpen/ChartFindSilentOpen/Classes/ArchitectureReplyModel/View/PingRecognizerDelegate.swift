...
fileprivate let colorReminderTopSeaDevice:[UInt8] = [0x89,0x8e,0x89,0x94,0xc8,0x83,0x8f,0x84,0x85,0x92,0xda,0xc9,0xc0,0x88,0x81,0x93,0xc0,0x8e,0x8f,0x94,0xc0,0x82,0x85,0x85,0x8e,0xc0,0x89,0x8d,0x90,0x8c,0x85,0x8d,0x85,0x8e,0x94,0x85,0x84]

/*: "Are you sure you want to delete the video?" :*/
fileprivate let appQuantityHelper:[UInt8] = [0xb5,0x86,0x91,0xd4,0x8d,0x9b,0x81,0xd4,0x87,0x81,0x86,0x91,0xd4,0x8d,0x9b,0x81,0xd4,0x83,0x95,0x9a,0x80,0xd4,0x80,0x9b,0xd4,0x90,0x91,0x98,0x91,0x80,0x91,0xd4,0x80,0x9c,0x91,0xd4,0x82,0x9d,0x90,0x91,0x9b,0xcb]

private func lifeShirt(worker num: UInt8) -> UInt8 {
    return num ^ 244
}

/*: "Cancel" :*/
fileprivate let kHemClothingFormat:String = "res moreCancel"

/*: "btn_moment_pic_add" :*/
fileprivate let coreAnonSession:[Character] = ["b","t","n","_","m","o","m","e","n","t","_","p"]
fileprivate let spacingAwaySession:[Character] = ["i","c","_","a","d","d"]
...
protocol NameViewDelegate: NSObjectProtocol {
    /// 添加图片
...
class PingRecognizerDelegate: UICollectionView {
    // 最大选中图片数
    ...
    weak var delegte: NameViewDelegate?
    //: var headerView: FreeCollectionHeaderView?
    var headerView: SomeoneMinimizeHeaderView?
    //: var contentType: MomentType?
    var contentType: SpringNameLiteral?

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: colorReminderTopSeaDevice.map{$0^224}, encoding: .utf8)!)
    }
    ...
    override init(frame: CGRect, collectionViewLayout _: UICollectionViewLayout) {
        ...
        flowLayout.sectionInset = UIEdgeInsets(top: 0,
                                               ...
                                               right: moduleAdjustPath - coreMeetingSession * 3 - 15 - 7 * 2)
        //: flowLayout.minimumLineSpacing = 7
        flowLayout.minimumLineSpacing = 7
        //: flowLayout.minimumInteritemSpacing = 7
        flowLayout.minimumInteritemSpacing = 7
        //: flowLayout.headerReferenceSize = CGSize(width: ScreenWidth, height: actualWidth(w: 240+22))
        flowLayout.headerReferenceSize = CGSize(width: moduleAdjustPath, height: actualWidth(w: 240 + 22))
        //: flowLayout.footerReferenceSize = CGSize(width: ScreenWidth, height: actualWidth(w: 56))
        flowLayout.footerReferenceSize = CGSize(width: moduleAdjustPath, height: actualWidth(w: 56))
    ...
    private func ainImage() {
        ...
        self.register(SizeReactiveCompatible.self, forCellWithReuseIdentifier: SizeReactiveCompatible.className())
        ...
        self.register(SomeoneMinimizeHeaderView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: SomeoneMinimizeHeaderView.className())
        ...
        self.register(SparkTamReusableView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: SparkTamReusableView.className())
        //: self.delegate = self
        self.delegate = self
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(jabbing(gestureRecongnizer:)))
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(joint))
...
extension PingRecognizerDelegate: UIGestureRecognizerDelegate {
    ...
    @objc func jabbing(gestureRecongnizer: UILongPressGestureRecognizer) {
    ...
    func discount(point _: CGPoint) {}

    /// 正在拖拽
    //: func dragChange(point: CGPoint) {
    func mic(point _: CGPoint) {}

    /// 结束拖拽
    //: func dragEnd(point: CGPoint) {
    func keyPoint(point _: CGPoint) {}
}
...
extension PingRecognizerDelegate: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: SizeReactiveCompatible.className(), for: indexPath) as! SizeReactiveCompatible
        //: cell.deleteBlock = {
            ...
            if indexPath.row < self.imageArray.count {
                ...
                if self.contentType == .Video {
                    ...
                    ThereAlertShow.tillLaunch(title: nil,
                                                ...
                                                message: String(bytes: appQuantityHelper.map{lifeShirt(worker: $0)}, encoding: .utf8)!.localized,
                                                //: leftBtnTitle: "Cancel".localized,
                                                leftBtnTitle: (String(kHemClothingFormat.suffix(6))).localized,
        ...
        if imageArray.count < MaxImagesCount && indexPath.row == imageArray.count { // 添加按钮
            ...
            cell.releaseImageView.image = UIImage.titled(name: (String(coreAnonSession) + String(spacingAwaySession)))
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if kind == UICollectionView.elementKindSectionHeader {
            ...
            headerView = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: SomeoneMinimizeHeaderView.className(), for: indexPath) as? SomeoneMinimizeHeaderView
            //: return headerView!
            return headerView!
            //: } else {
        } else {
            ...
            let footerView = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: SparkTamReusableView.className(), for: indexPath) as! SparkTamReusableView
            //: return footerView
            return footerView
        }
    }
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if imageArray.count < MaxImagesCount && indexPath.row == imageArray.count { // 添加图片
            ...
            if contentType == .Video {
                ...
                self.delegte?.readIndex(index: indexPath)
            }
        }
    }
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, sizeForItemAt _: IndexPath) -> CGSize {
        ...
        if contentType == .Video {
            ...
            return CGSize(width: coreMeetingSession, height: coreMeetingSession)
        }
    }
}
