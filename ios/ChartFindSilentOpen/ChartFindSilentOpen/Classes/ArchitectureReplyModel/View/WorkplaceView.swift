...
fileprivate let featureInmatePage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class WorkplaceView: UICollectionView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: featureInmatePage.reversed(), encoding: .utf8)!)
    }

    //: func configModel(model: TalkingMomentModel) {
    func configErrModel(model: NothingMeasurable) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: self.reloadData()
        self.reloadData()
...
extension WorkplaceView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, insetForSectionAt _: Int) -> UIEdgeInsets {
        ...
        if cunrrenModel.pic.count == 4 {
            ...
            return UIEdgeInsets(top: 0, left: 0, bottom: 0, right: moduleAdjustPath - 200 - 30)
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, sizeForItemAt _: IndexPath) -> CGSize {
        ...
        if cunrrenModel.pic.count == 1 {
            ...
            var width = isMyhost ? (moduleAdjustPath - 30 - 24 - 30) / 3 : (moduleAdjustPath - 30 - 24) / 3
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, referenceSizeForHeaderInSection _: Int) -> CGSize {
        ...
        return CGSize(width: moduleAdjustPath, height: 1)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: PhotosReactiveCompatible = collectionView.dequeueReusableCell(withReuseIdentifier: PhotosReactiveCompatible.className(), for: indexPath) as! PhotosReactiveCompatible
        //: let model: TalkingMomentPhotoModel = cunrrenModel.pic[indexPath.row]
        let model: PackageHandyJSON = cunrrenModel.pic[indexPath.row]
        //: cell.configUrl(url: model.thumbUrl ?? "")
        cell.voiceGame(url: model.thumbUrl ?? "")
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = RushTermPhotosVc(momentModel: cunrrenModel, index: indexPath.row, type: .normal)
...
extension WorkplaceView {
    ...
    private func ban() {
        ...
        register(PhotosReactiveCompatible.self, forCellWithReuseIdentifier: PhotosReactiveCompatible.className())
        //: dataSource = self
        dataSource = self
        //: delegate = self
        delegate = self
