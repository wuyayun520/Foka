...
fileprivate let kProcessMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_home_v" :*/
fileprivate let screenAtSession:String = "harvest"
fileprivate let moduleCalendarPreference:[Character] = ["c","o","n","_","h","o","m","e","_","v"]

/*: "icon_lounge" :*/
fileprivate let k_helloName:[Character] = ["i","c","o","n","_","l"]
fileprivate let widgetMorroError:[Character] = ["o","u","n","g","e"]

/*: "#AAAAAA" :*/
fileprivate let spacingPinTicProcessingName:String = "#AAAAAAprevention examine coordinate"

/*: "icon_moment_nor_report" :*/
fileprivate let k_multipleMessage:[Character] = ["i"]
fileprivate let viewRatingBoxPath:String = "con_basic already"
fileprivate let displayCommunicateTimer:String = "nt_nsuccessful chin mobile"

/*: "img_me_edit_photo_default" :*/
fileprivate let k_padName:[Character] = ["i","m","g","_","m","e","_","e","d","i","t","_","p","h","o"]
fileprivate let coreResidenceUrl:String = "to_defrecall product"

/*: "icon_home_boy" :*/
fileprivate let moduleNotePath:[Character] = ["i","c","o","n","_","h","o","m","e","_","b"]
fileprivate let serviceExactlyDevice:[Character] = ["o","y"]

/*: "icon_home_girl" :*/
fileprivate let sessionRawUrl:[Character] = ["i","c","o","n","_","h","o","m"]
fileprivate let sessionAdFormat:String = "required space reply animal lande_girl"

/*: "level_" :*/
fileprivate let spacingGatorPicTimer:String = "level_ring escape ok scene"
...
class BrasilView: UITableViewCell {
    //: var cid = ""
    var cid = ""
    //: var uid = ""
    var uid = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kProcessMessage.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(someoneReading), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
        //: return btn
        return btn
    ...
    lazy var cardImgView: UIImageView = {
        ...
        img.image = UIImage.titled(name: (screenAtSession.replacingOccurrences(of: "harvest", with: "i") + String(moduleCalendarPreference)))
    ...
    lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.titled(name: (String(k_helloName) + String(widgetMorroError)))
        //: self.addSubview(img)
        self.addSubview(img)
        //: return img
        return img
    ...
    lazy var timeLB: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(spacingPinTicProcessingName.prefix(7))))
        //: self.addSubview(label)
        self.addSubview(label)
        //: return label
        return label
    ...
    lazy var BlockBtn: UIButton = {
        ...
        btn.setImage(UIImage.titled(name: (String(k_multipleMessage) + String(viewRatingBoxPath.prefix(4)) + "mome" + String(displayCommunicateTimer.prefix(4)) + "or_report")), for: .normal)
        //: btn.imageView?.contentMode = .scaleAspectFill
        btn.imageView?.contentMode = .scaleAspectFill
        //: btn.addTarget(self, action: #selector(BlockBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(soundWith), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
        //: return btn
        return btn
...
extension BrasilView {
    ...
    @objc func disk(gestureRecongnizer: UIGestureRecognizer) {
    ...
    @objc func soundWith() {
        ...
        let reportView = FeatureDataSource(frame: UIScreen.main.bounds, type: .reportCommentType, rId: cid)
        //: reportView.showReportViewIn(view: FragThen.getWindow())
        reportView.three(view: FragThen.termsSource())
    }
}
...
extension BrasilView {
    ...
    func compartment(model: GrantSearchMeasurable) {
        //: cid = model.cid ?? ""
        cid = model.cid ?? ""
        //: uid = model.uid ?? ""
        uid = model.uid ?? ""
        //: iconBtn.setUrlImage(urlStr: model.headPic ?? "", placeImg: UIImage.BundleImageNamed(name: "img_me_edit_photo_default"))
        iconBtn.riskyOf(urlStr: model.headPic ?? "", placeImg: UIImage.titled(name: (String(k_padName) + String(coreResidenceUrl.prefix(6)) + "ault")))
        ...
        nameLabel.textColor = model.loungePlus ? .birthControl() : .valueColor()
        ...
        case PanelScalarLiteral.male.rawValue:
            ...
            sexBtn.setBackgroundImage(UIImage.titled(name: (String(moduleNotePath) + String(serviceExactlyDevice))), for: .normal)
        //: break
        ...
        case PanelScalarLiteral.female.rawValue:
            ...
            sexBtn.setBackgroundImage(UIImage.titled(name: (String(sessionRawUrl) + String(sessionAdFormat.suffix(6)))), for: .normal)
        //: break
        //: default:
        default:
            //: break
        ...
        levelBtn.setBackgroundImage(UIImage.titled(name: (String(spacingGatorPicTimer.prefix(6))) + String(model.level ?? 1)), for: .normal)
        ...
        contentLB.text = model.content
        //: BlockBtn.isHidden = PositionThen.share.loginUserMode.userID == model.uid
        BlockBtn.isHidden = PositionThen.share.loginUserMode.userID == model.uid
    }
}
...
extension BrasilView {
    ...
    private func destroyCharacter() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(process))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let longPress = UILongPressGestureRecognizer(target: self, action: #selector(onLongTapCell(gestureRecongnizer:)))
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(disk(gestureRecongnizer:)))
