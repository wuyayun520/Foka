...
fileprivate let coreShineEvent:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "F5F5F5" :*/
fileprivate let kSocialTitle:String = "sound5sound5sound5"
...
class SparkViewCell: UITableViewCell {
    ...
    var videoView = CurrentMediaView()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: coreShineEvent.reversed(), encoding: .utf8)!)
    }
    ...
    func describeCell(isListTableCell: Bool) {
    ...
    lazy var userInfoView: SearchView = {
    ...
    lazy var textContentView: PoetThen = {
    ...
    lazy var photoView: WorkplaceView = {
        ...
        let view = WorkplaceView(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: flayout)
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var bottomView: HarvestReactiveCompatible = {
        //: let  view = TalkingMomentItemBottomView.init()
        let view = HarvestReactiveCompatible()
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (kSocialTitle.replacingOccurrences(of: "sound", with: "F")))
        //: self.contentView.addSubview(view)
        self.contentView.addSubview(view)
        //: return view
        return view
...
//            }
            ...
            photoView.configErrModel(model: cunrrenModel)
...
//            }
            ...
            videoView.treetop(model: model.videoInfo!, isTop: model.pinStatus)
        }

        //: bottomView.configModel(model: cunrrenModel)
        bottomView.bean(model: cunrrenModel)
...
//        }
    ...
    @objc func premium() {
        ...
        let vc = AboardViewController(videoPath: cunrrenModel.videoInfo!.videoUrl)
        //: if cunrrenModel.sex != PositionThen.share.loginUserMode.sex {
        if cunrrenModel.sex != PositionThen.share.loginUserMode.sex {
    ...
    func phonePlayCount() {
        ...
        if cunrrenModel.momentType == SpringNameLiteral.Video.rawValue {
...
extension SparkViewCell {
    ...
    private func lie() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(premium))
