...
/*: "%@ reply %@：%@" :*/
fileprivate let colorCrossId:String = "%@ revolume character advice just"
fileprivate let appPerHelper:String = "pjus"
fileprivate let spacingPendingData:[Character] = ["y"," ","%","@","\u{ff1a}","%","@"]
...
struct ArchitectureReplyModel: HandyJSON {
    //: var cid: String?
...
extension ArchitectureReplyModel {
    ...
    mutating func want() {
        ...
        if replyUser.count > 0 {
            ...
            tempContent = String(format: (String(colorCrossId.prefix(5)) + appPerHelper.replacingOccurrences(of: "jus", with: "l") + String(spacingPendingData)), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(ReverseSizeView.albumTransfer(text: tempContent, size: CGSize(width: moduleAdjustPath - 82, height: CGFloat(MAXFLOAT)), font: UIFont.modelSize(type: .Regular, fontSize: 14), lineNumber: 0).height)
