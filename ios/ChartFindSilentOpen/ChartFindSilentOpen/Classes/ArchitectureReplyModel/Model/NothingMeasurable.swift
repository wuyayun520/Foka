...
struct NothingMeasurable: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
    ...
    var pic = [PackageHandyJSON]()
    //: var momentType: String?
    var momentType: String?
    //: var videoInfo: TalkingMomentVideoModel?
    var videoInfo: SecondOriginalModelType?
...
struct PackageHandyJSON: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
struct SecondOriginalModelType: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: FixedPromiseScalarLiteral?
}

///
//: enum MomentVideoStatus: String {
enum FixedPromiseScalarLiteral: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension NothingMeasurable {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func memory(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(ReverseSizeView.albumTransfer(text: content!, size: CGSize(width: moduleAdjustPath - 54, height: CGFloat(MAXFLOAT)), font: UIFont.modelSize(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == SpringNameLiteral.Photo.rawValue || momentType == SpringNameLiteral.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
