...
/*: "reply" :*/
fileprivate let displayFairPath:String = "repsuperiory"
...
struct GrantSearchMeasurable: HandyJSON {
    //: var cid: String?
    ...
    var comment = [ArchitectureReplyModel]()
...
extension GrantSearchMeasurable {
    //: mutating func caculateCommentItemHeight() {
    mutating func grownup() {
        //: var tempContent = ""
        var tempContent = ""
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if PushThen.shared.direction == .leftToRight {
                ...
                tempContent = (displayFairPath.replacingOccurrences(of: "superior", with: "l")).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (displayFairPath.replacingOccurrences(of: "superior", with: "l")).localized
            }
        ...
        let sizeheight = Int(ReverseSizeView.albumTransfer(text: tempContent, size: CGSize(width: moduleAdjustPath - 70, height: CGFloat(MAXFLOAT)), font: UIFont.modelSize(type: .Regular, fontSize: 16), lineNumber: 0).height)
