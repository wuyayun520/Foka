...
/*: "/TXCache" :*/
fileprivate let themeTargetPreference:[Character] = ["/","T","X","C","a"]
fileprivate let moduleMaleRushTailError:String = "CHE"
...
protocol RecallReactiveCompatible: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func aboutSourceStatus(player: EarnThen, status: ThinPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func example(player: EarnThen, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func live(player: EarnThen, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class EarnThen: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: RecallReactiveCompatible?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: ThinPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = EarnThen()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(themeTargetPreference) + moduleMaleRushTailError.lowercased())) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(photomicrographNight), userInfo: nil, repeats: true)
...
extension EarnThen {
    ...
    func recognizeView(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: layoutStatusSession.wander(), with: componentFormalPath)
    ...
    //: @objc func timerRun() {
    @objc func photomicrographNight() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.live(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func warning(renderMode: BalanceRenderMode) {
    ...
    func telegraphicSignal(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isFamous == true {
        if TopThen.shared.isFamous == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func sessionIn() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.aboutSourceStatus(player: self, status: self.playStatus!)
        }
    }
...
extension EarnThen: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.example(player: self, duration: Int(duration), currentTime: Int(currentTime))
