...
/*: "Cancel" :*/
fileprivate let sessionDitUtility:String = "Cancelwhy cost side"

/*: "Post" :*/
fileprivate let displayAssetEvent:String = "Postping artifact temp"

/*: "Upload failed" :*/
fileprivate let spacingMentionOpportunityConfig:String = "automatic competition street directUplo"
fileprivate let coreInfrastructureId:[Character] = ["a","d"," ","f","a","i","l","e","d"]

/*: "imgUrl" :*/
fileprivate let componentPathUtility:String = "act pack agencyimgUrl"

/*: "videoUrl" :*/
fileprivate let displayTheoryPreference:String = "ventere"

/*: "content" :*/
fileprivate let screenUsedSession:[Character] = ["c","o","n","t","e","n"]
fileprivate let widgetCenterToolText:String = "side"

/*: "location" :*/
fileprivate let commonReduceMotionValue:[Character] = ["l","o","c","a","t","i","o","n"]

/*: "Video review in progress" :*/
fileprivate let colorPecAlert:[Character] = ["V","i","d","e","o"," ","r","e","v","i","e","w"," ","i","n"," ","p","r","o","g","r","e","s"]
fileprivate let appVersionPlatform:[Character] = ["s"]

/*: "Shoot video at least 5 seconds" :*/
fileprivate let colorNoAllowFormat:[Character] = ["S","h","o","o","t"," "]
fileprivate let kJawTimer:[Character] = ["v","i","d","e","o"," ","a","t"," ","l"]
fileprivate let themeStarDivisionId:[Character] = ["e","a","s","t"," ","5"," ","s","e","c","o","n","d","s"]

/*: "Camera" :*/
fileprivate let colorEaseMetrics:String = "Camerasoftly title link gallery hi"

/*: "Photo" :*/
fileprivate let layoutLiveContent:String = "Photofloat early spark take tolerance"

/*: "Video" :*/
fileprivate let k_cutUtility:String = "focus map unionVideo"
...
protocol BrandObjectProtocol: NSObjectProtocol {
    // 发布成功
    //: func ShareSucceed()
    func commandSucceed()
}
...
class MapObjectProtocol: GroupThen {
    //: weak var delegate: TalkingFreeViewControllerDelegate?
    weak var delegate: BrandObjectProtocol?
    ...
    private lazy var reqManager: SpecifyThen = //: return SpecifyThen()
        .init()
    //: }()
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(sessionDitUtility.prefix(6))).localized, for: .normal)
    ...
    private lazy var postBtn: UIButton = {
        ...
        btn.setTitle((String(displayAssetEvent.prefix(4))).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.totalelMention(colors: UIColor.motion(), size: CGSize(width: 70, height: 30)), for: .normal)
        //: btn.layer.cornerRadius = 15
        btn.layer.cornerRadius = 15
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(postBtnClickEvent), for: .touchUpInside)
        btn.addTarget(self, action: #selector(militaryQuartersToHumanActivity), for: .touchUpInside)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
    ...
    private lazy var collectionView: PingRecognizerDelegate = {
        //: let collectionView = FreeCollectionView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        let collectionView = PingRecognizerDelegate(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        ...
        collectionView.deleteActionBlock = { [weak self] in
            ...
            if self.collectionView.imageArray.count == 0 {
                ...
                self.collectionView.contentType = SpringNameLiteral.nones
                //: self.videoPath = ""
                self.videoPath = ""
                //: self.coverPath = ""
                self.coverPath = ""
...
extension MapObjectProtocol {
    ...
    func aboard() {
        ...
        reqManager.letGoOfResolutionMomentPhotosContentCompletion(photos: self.collectionView.imageArray, content: content) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            ...
            NotificationCenter.default.post(name: widgetCreateListUtility, object: self, userInfo: nil)
        }
    }
    ...
    func otherVideo() {
        ...
        MedalVideoManager.shared.slide(type: 1, coverPath: self.coverPath, videoPath: self.videoPath) { coverUrl, videoUrl, succeed in
            //: guard succeed else {
            guard succeed else {
                //: ProgressHUD.dismiss()
                WeatherMapThen.evolution()
                //: self.func__showStatusBarErrorMsg(showMsg: "Upload failed".localized)
                self.colouration(showMsg: (String(spacingMentionOpportunityConfig.suffix(4)) + String(coreInfrastructureId)).localized)
                //: return
                return
            }
            ...
            dict[(String(componentPathUtility.suffix(6)))] = coverUrl
            //: dict["videoUrl"] = videoUrl
            dict[(displayTheoryPreference.replacingOccurrences(of: "enter", with: "id") + "oUrl")] = videoUrl
            //: dict["content"] = content
            dict[(String(screenUsedSession) + widgetCenterToolText.replacingOccurrences(of: "side", with: "t"))] = content
            //: dict["location"] = ""
            dict[(String(commonReduceMotionValue))] = ""

            //: TalkingMomentRequestTool.releaseVideoMoment(params: dict) {[weak self] succeed, result, errorModel in
            SpecifyThen.library(params: dict) { [weak self] succeed, _, _ in
                //: ProgressHUD.dismiss()
                WeatherMapThen.evolution()
                //: guard let self = self else { return }
                guard let self = self else { return }
                ...
                self.quantityerproposal(showMsg: (String(colorPecAlert) + String(appVersionPlatform)).localized)
                //: self.navigationController?.popViewController(animated: true)
                self.navigationController?.popViewController(animated: true)
                //: self.delegate?.ShareSucceed()
                self.delegate?.commandSucceed()
                //: NotificationCenter.default.post(name: POST_EXPLORE_SUCCESS_NOTIFICATION, object: self, userInfo: nil)
                NotificationCenter.default.post(name: widgetCreateListUtility, object: self, userInfo: nil)
            }
        }
...
extension MapObjectProtocol {
    ...
    @objc func militaryQuartersToHumanActivity() { // 发布
    ...
    func backUsed() {
        ...
        MagnitudeReactiveCompatible.pry(true) { (isOpen: Bool) in
    ...
    func beyondDisk() {
        ...
        MagnitudeReactiveCompatible.contendBrightEquity(true) { (isOpen: Bool) in
            ...
            let vc = PoetPickTool.assetActual(maxCount: count, allowPhoto: true, allowVideo: false)
            ...
            vc.didFinishPickingVideoHandle = { [weak self] (_: UIImage?,
                    ...
                    PoetPickTool.primrosePath(asset: assets) { filePath in
                        ...
                        if seconds >= 5 {
                            ...
                            self.sh(url: filePath!)
                            //: } else {
                        } else {
                            //: DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                                //: self.func__showStatusBarErrorMsg(showMsg: "Shoot video at least 5 seconds".localized)
                                self.colouration(showMsg: (String(colorNoAllowFormat) + String(kJawTimer) + String(themeStarDivisionId)).localized)
                            }
                        }
                    }
            }
    ...
    func sh(url: URL) {
        ...
        let videoVC = ThatNothingRecognizerDelegate(maxDuration: 60.0, videoURL: url) { videoInfo in
...
extension MapObjectProtocol: UIImagePickerControllerDelegate & UINavigationControllerDelegate {
        ...
        self.dismiss(animated: true) {
            ...
            if mediaType == kUTTypeMovie as String {
                ...
                if seconds >= 5 {
                    ...
                    self.sh(url: info[.mediaURL]! as! URL)
...
// MARK: - NameViewDelegate

//: extension TalkingFreeViewController: FreeCollectionViewDelegate {
extension MapObjectProtocol: NameViewDelegate {
    /// 选择图片
    ...
    func mini() {
        ...
        if self.collectionView.contentType == .Photo {
            ...
            vc.shareList(cellTitleList: [(String(colorEaseMetrics.prefix(6))).localized, (String(layoutLiveContent.prefix(5))).localized])
            //: }else {
        } else {
            //: vc.initwithList(cellTitleList: ["Camera".localized, "Photo".localized, "Video".localized])
            vc.shareList(cellTitleList: [(String(colorEaseMetrics.prefix(6))).localized, (String(layoutLiveContent.prefix(5))).localized, (String(k_cutUtility.suffix(5))).localized])
        }
    ...
    func readIndex(index: IndexPath) {
        //: let vc = TalkingMomentPhotosVC.init(images: self.collectionView.imageArray, index: index.row, type: .publish)
        let vc = RushTermPhotosVc(images: self.collectionView.imageArray, index: index.row, type: .publish)
        // 删除按钮block
    ...
    func asset() {
        ...
        let vc = AboardViewController(videoPath: self.videoPath)
        //: vc.modalPresentationStyle = .fullScreen
        vc.modalPresentationStyle = .fullScreen
        //: self.present(vc, animated: true)
        self.present(vc, animated: true)
...
extension MapObjectProtocol {
    ...
    func focus() {
        ...
        cancelBtn.snp.makeConstraints { make in
            ...
            make.top.equalTo(appSceneUrl + 7)
        ...
        collectionView.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(sessionTableUrl)
