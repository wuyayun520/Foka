...
fileprivate let viewTamPage:[UInt8] = [0xac,0xab,0xac,0xb1,0xed,0xa6,0xaa,0xa1,0xa0,0xb7,0xff,0xec,0xe5,0xad,0xa4,0xb6,0xe5,0xab,0xaa,0xb1,0xe5,0xa7,0xa0,0xa0,0xab,0xe5,0xac,0xa8,0xb5,0xa9,0xa0,0xa8,0xa0,0xab,0xb1,0xa0,0xa1]

private func frameIts(method num: UInt8) -> UInt8 {
    return num ^ 197
}

/*: "Save successfully" :*/
fileprivate let layoutBeanSystem:String = "recording nthSave s"
fileprivate let k_rimeValue:String = "fullbeauty"
...
class RushTermPhotosVc: PreferenceFullReactiveCompatible {
    ...
    init(momentModel: NothingMeasurable, index: Int, type: ScreenPhotoType) {
        ...
        if PushThen.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(moduleAdjustPath), y: 0)
                //: self.pageIndex = index
                self.pageIndex = index
                //: self.updataPageLabel()
                self.theContrastLabel()
            }
        }
    }
    ...
    init(images: [UIImage], index: Int, type: ScreenPhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: viewTamPage.map{frameIts(method: $0)}, encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        doDisableDirect(modle: currenModel)
//        if myType == .normal {
//            setDownloadView()
//        }
    }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: moduleAdjustPath, height: componentMeEvent)
        ...
        view.frame = CGRect(x: 0, y: 0, width: moduleAdjustPath, height: componentMeEvent)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(moduleAdjustPath), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(moduleAdjustPath), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(moduleAdjustPath), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(MarchEquinoxViewCell.self, forCellWithReuseIdentifier: MarchEquinoxViewCell.className())
...
//    lazy var downloadBtn: GrantRushTalkingButton = {
//        let btn = GrantRushTalkingButton.init()
...
//        MagnitudeReactiveCompatible.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: PackageHandyJSON = self.currenModel.pic[self.pageIndex]
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.quantityerproposal(showMsg: (String(layoutBeanSystem.suffix(6)) + "uccess" + k_rimeValue.replacingOccurrences(of: "beauty", with: "y")).localized)
                }
                //: } else {
            } else {
...
extension RushTermPhotosVc: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func feedback() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func same() {
        //: if  currenModel.sex==PositionThen.share.loginUserMode.sex {
        if currenModel.sex == PositionThen.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: MarchEquinoxViewCell = collectionView.dequeueReusableCell(withReuseIdentifier: MarchEquinoxViewCell.className(), for: indexPath) as! MarchEquinoxViewCell
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: PackageHandyJSON = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.tapering(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.male(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.male(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: MarchEquinoxViewCell = cell as! MarchEquinoxViewCell
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / moduleAdjustPath)
        //: if myType == .normal {
        if myType == .normal {
