...
class BalanceViewController: GroupThen {
    //: var type = LoginType.PhoneLogin
    var type = ExpectUTF8Initializable.PhoneLogin // 修改类型
    //: var content = ""                // 手机号
    var content = "" // 手机号
    //: var isNewPwd = false
    ...
    private lazy var pwdView: GleanThen = {
        //: let v = TalkingSetNewPwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
        let v = GleanThen(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
...
extension BalanceViewController {
    ...
    private func replacement(_ pwd: String, _ vcode: String?) {
        //: ProgressHUD.show()
        ...
        ScriptReactiveCompatible.mark(pwd: pwd, vcode: vcode) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
    ...
    private func linguisticContext() {
        ...
        ScriptReactiveCompatible.calculateStyleCompletion { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            WeatherMapThen.evolution()
            //: guard let self = self else { return }
            guard let self = self else { return }
...
extension BalanceViewController {
    ...
    private func speedReadingSubviews() {
