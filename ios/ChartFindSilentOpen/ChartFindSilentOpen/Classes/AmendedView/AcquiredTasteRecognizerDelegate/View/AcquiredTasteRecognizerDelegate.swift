...
fileprivate let sessionKeepRefRollValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
// __CLOSE_PRINT__
//
//  AcquiredTasteRecognizerDelegate.swift
//  AbroadTalking
//
...
enum GroupCaseIterable: CaseIterable {
    //: case Left
    case Left
    //: case Right
    case Right
...
protocol SoftballBigLeagueObjectProtocol: NSObjectProtocol {
    ///
...
class AcquiredTasteRecognizerDelegate: UIView {
    ...
    var cell_delegate: SoftballBigLeagueObjectProtocol?

    //: var model: TalkingMatchResultModel?
    var model: FirstModelType?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: sessionKeepRefRollValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(panAdmin(pan:)))
        //: v.delegate = self
        v.delegate = self
        //: return v
        return v
...
extension AcquiredTasteRecognizerDelegate: UIGestureRecognizerDelegate {
    ...
    @objc func panAdmin(pan: UIPanGestureRecognizer) {
        //: switch pan.state {
        switch pan.state {
        //: case .began:
        case .began:
        ...
        case .changed:
            ...
            if PushThen.shared.direction == .rightToLeft {
                ...
                if movePoint.x > moduleAdjustPath / 4 {
                    ...
                    self.cell_delegate?.cartePoint(cell: self, forMovePoint: self.currentPoint)
    ...
    func partnerFore() {
        ...
        self.cell_delegate?.goSub(cell: self)
    }
    ...
    func direction(direction: GroupCaseIterable) {
        //: switch (direction) {
        switch direction {
        //: case .Left:
        case .Left:
...
extension AcquiredTasteRecognizerDelegate {
    //: func setupView() {
    func soapBubble() {
