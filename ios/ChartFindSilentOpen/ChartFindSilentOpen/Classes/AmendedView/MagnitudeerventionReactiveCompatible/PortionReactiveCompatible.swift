...
fileprivate let k_pointKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "gift_bg_dm" :*/
fileprivate let widgetDifferenceUrl:String = "gift_bgregister production correct assert"
fileprivate let themeCornerPage:String = "_dmselected called policeman"

/*: "#FFF4B5" :*/
fileprivate let widgetGuideValue:String = "#FFF4B5application country background"

/*: "won " :*/
fileprivate let themeOverlookHelper:String = "red opposite inside threewon "

/*: " coins in " :*/
fileprivate let viewSoftlyLogger:[Character] = [" ","c","o","i","n","s"," ","i"]
fileprivate let commonTapeColumnHelper:String = "n interested trap carry maxim"

/*: "just sent" :*/
fileprivate let kMateContent:String = "just problem bit guard tick"
fileprivate let displayLuckTitle:String = "senleading"

/*: "#4120C1" :*/
fileprivate let widgetSunComputerName:[Character] = ["#","4","1","2","0","C","1"]
...
// __CLOSE_PRINT__
//
//  PortionReactiveCompatible.swift
//  ChartFindSilentOpen
//
...
class PortionReactiveCompatible: UIView {
    /// 礼物model
    //: private var giftModel = SocialBroadcastModel()
    private var giftModel = TotalendChromosphereTransformable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_pointKey.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        ...
        v.image = UIImage.titled(name: (String(widgetDifferenceUrl.prefix(7)) + String(themeCornerPage.prefix(3))))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(widgetGuideValue.prefix(7))))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(widgetGuideValue.prefix(7))))
...
extension PortionReactiveCompatible {
    /// 刷新视图
    //: func refreshGift(_ model: SocialBroadcastModel) {
    func pec(_ model: TotalendChromosphereTransformable) {
        //: giftModel = model
        giftModel = model
        //: icon.setUrlImage(urlStr: giftModel.headPic)
        icon.imitation(urlStr: giftModel.headPic)
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(themeOverlookHelper.suffix(4))).localized + "\(giftModel.totalCoin)" + (String(viewSoftlyLogger) + String(commonTapeColumnHelper.prefix(2))).localized + giftModel.gameName
            ...
            giftImgV.imitation(urlStr: giftModel.img)
            //: giftNumLab.isHidden = false
            giftNumLab.isHidden = false
            //: giftNumLab.text = " x\(giftModel.num)"
            giftNumLab.text = " x\(giftModel.num)"
            //: let rangeText = "just sent".localized
            let rangeText = (String(kMateContent.prefix(5)) + displayLuckTitle.replacingOccurrences(of: "leading", with: "t")).localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = fluctuationFromGiftmodel(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (String(widgetSunComputerName)))!, range: nsRange)
            }
    ...
    private func fluctuationFromGiftmodel(giftmodel: TotalendChromosphereTransformable) -> String {
    ...
    private func collectionDecision() {
...
extension PortionReactiveCompatible {
    ...
    private func outsideSubviews() {
        //: self.frame = CGRect(x: 0, y: StatusBarNavigationBarHeight, width: ScreenWidth, height: 60)
        self.frame = CGRect(x: 0, y: sessionTableUrl, width: moduleAdjustPath, height: 60)
