...
/*: "position.x" :*/
fileprivate let kExhibitMetrics:String = "POSITI"

/*: "slideIn" :*/
fileprivate let appReplyMessage:String = "bucket"
fileprivate let coreChangeLogger:[Character] = ["l","i","d","e","I","n"]

/*: "slideOut" :*/
fileprivate let themeFirstConfig:[Character] = ["s","l","i","d","e","O","u","t"]
...
// __CLOSE_PRINT__
//
//  MagnitudeerventionReactiveCompatible.swift
//  ChartFindSilentOpen
//
...
class MagnitudeerventionReactiveCompatible: NSObject {
    //: static let shared = GiftFloatViewManager()
    static let shared = MagnitudeerventionReactiveCompatible()
    //: private var giftArr = [SocialBroadcastModel]()
    private var giftArr = [TotalendChromosphereTransformable]()
    ...
    private lazy var giftView: PortionReactiveCompatible = {
        //: let v = GiftFloatView()
        let v = PortionReactiveCompatible()
        //: FragThen.getWindow().addSubview(v)
        FragThen.termsSource().addSubview(v)
        //: return v
        return v
        //: }()
    }()
...
extension MagnitudeerventionReactiveCompatible {
    /// 添加礼物
    //: private func addGift(gift: SocialBroadcastModel) {
    private func enrich(gift: TotalendChromosphereTransformable) {
        //: self.synchronized(self) {
        self.plate(self) {
            //: self.giftArr.append(gift)
            self.giftArr.append(gift)
            //: self.showNextGift(index: 0)
            self.titleure(index: 0)
        }
    }

    //: public func synchronized<T>(_ lock: AnyObject, _ body: () throws -> T) rethrows -> T {
    public func plate<T>(_ lock: AnyObject, _ body: () throws -> T) rethrows -> T {
    ...
    private func genusViperaAnimation(_ model: TotalendChromosphereTransformable) {
        ...
        let isRTL = PushThen.shared.direction == .rightToLeft
        // 起始位置：屏幕外
        //: let startCenterX = isRTL ? -centerX : ScreenWidth + centerX
        let startCenterX = isRTL ? -centerX : moduleAdjustPath + centerX
        // 目标位置：屏幕中间
        //: let targetCenterX = ScreenWidth / 2
        let targetCenterX = moduleAdjustPath / 2
        // 结束位置：屏幕另一侧
        //: let endCenterX = isRTL ? ScreenWidth + centerX : -centerX
        let endCenterX = isRTL ? moduleAdjustPath + centerX : -centerX
        ...
        let slideIn = CABasicAnimation(keyPath: (kExhibitMetrics.lowercased() + "on.x"))
        ...
        self.giftView.layer.add(slideIn, forKey: (appReplyMessage.replacingOccurrences(of: "bucket", with: "s") + String(coreChangeLogger)))
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            ...
            let slideOut = CABasicAnimation(keyPath: (kExhibitMetrics.lowercased() + "on.x"))
            ...
            self.giftView.layer.add(slideOut, forKey: (String(themeFirstConfig)))
...
extension MagnitudeerventionReactiveCompatible {
    ...
    func consolidate() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.randomNoti(noti:)),
                                               //: name: USER_RECIEVED_GIFT_NOTIFICATION,
                                               name: componentPurchaseMessage,
                                               //: object: nil)
                                               object: nil)
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.approach),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.nomadic),
    ...
    @objc private func randomNoti(noti: Notification) {
        ...
        guard let data = noti.object,
              ...
              let gift = JSONDeserializer<TotalendChromosphereTransformable>.deserializeFrom(dict: data as? [String: Any], designatedPath: nil),
              //: gift.broadType != BroadcastType.Unknow.rawValue
              gift.broadType != FailNameConvertible.Unknow.rawValue
        //: else {
        else {
            //: return
            return
        }
        //: let currUserType = PositionThen.share.appUserConfigMode.userType
        let currUserType = PositionThen.share.appUserConfigMode.userType
        // 过滤接口异常
        ...
        // 非特殊用户 不展示 不同地区用户礼物
        //: if currUserType != 0, gift.userCountryType != PositionThen.share.appUserConfigMode.userCountryType {
        if currUserType != 0, gift.userCountryType != PositionThen.share.appUserConfigMode.userCountryType {
            //: return
            return
        }
        ...
        self.enrich(gift: gift)
    }

    /// 应用退到后台
    //: @objc private func enterBackground() {
    @objc private func approach() {
